...
def get_qty_in_stock(item_code, item_warehouse_field, warehouse=None):
    ...
    if not warehouse:
        warehouse = frappe.db.get_value(
            "Item", item_code, item_warehouse_field)

    if not warehouse and template_item_code and template_item_code != item_code:
        warehouse = frappe.db.get_value(
            "Item", template_item_code, item_warehouse_field)
    ...
    if warehouse:
        ...
        if stock_qty:
            stock_qty = adjust_qty_for_expired_items(
                item_code, stock_qty, warehouse)
...
def adjust_qty_for_expired_items(item_code, stock_qty, warehouse):
    batches = frappe.get_all('Batch', filters=[{'item': item_code}], fields=[
                             'expiry_date', 'name'])
    ...
    for batch in expired_batches:
        if warehouse:
            stock_qty[0][0] = max(0, stock_qty[0][0] -
                                  get_batch_qty(batch, warehouse))
        else:
            stock_qty[0][0] = max(
