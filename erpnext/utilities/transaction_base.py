    ...
    def delete_events(self):
        ...
        if participations:
            for participation in participations:
                total_participants = frappe.get_all("Event Participants", filters={
                                                    "parenttype": "Event", "parent": participation.parent})

                if len(total_participants) <= 1:
                    frappe.db.sql(
                        "delete from `tabEvent` where name='%s'" % participation.parent)

                frappe.db.sql(
                    "delete from `tabEvent Participants` where name='%s'" % participation.name)
            ...
            for d in self.get("items"):
                if d.get(ref_link_field):
                    ref_rate = frappe.db.get_value(
                        ref_dt + " Item", d.get(ref_link_field), "rate")
...
def validate_uom_is_integer(doc, uom_field, qty_fields, child_dt=None):
    ...
    distinct_uoms = list(set([d.get(uom_field)
