...
class update_entries_after(object):
    ...
    def get_precision(self):
        company_base_currency = frappe.get_cached_value(
    ...
    def get_dependent_entries_to_fix(self, entries_to_fix, sle):
        ...
        if not dependant_sle:
            ...
            if (dependant_sle.item_code, dependant_sle.warehouse) not in self.new_items:
                self.new_items[(dependant_sle.item_code,
                                dependant_sle.warehouse)] = dependant_sle
            return entries_to_fix
    ...
    def process_sle(self, sle):
        ...
        if sle.serial_no:
            ...
            self.wh_data.stock_value = flt(
                self.wh_data.qty_after_transaction) * flt(self.wh_data.valuation_rate)
        else:
        ...
        self.wh_data.stock_value = flt(
            self.wh_data.stock_value, self.precision)
    ...
    def get_incoming_outgoing_rate_from_transaction(self, sle):
        ...
        # Sales and Purchase Return
            ...
            if frappe.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
                # don't move this import to top
                from erpnext.controllers.sales_and_purchase_return import get_rate_for_return
                rate = get_rate_for_return(
                    sle.voucher_type, sle.voucher_no, sle.item_code, voucher_detail_no=sle.voucher_detail_no)
            else:
    ...
    def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
        ...
        if item_code == sle.item_code:
            frappe.db.set_value(sle.voucher_type + " Item",
                                sle.voucher_detail_no, "incoming_rate", outgoing_rate)
        else:
            # packed item
    ...
    def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
        if frappe.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
            frappe.db.set_value(sle.voucher_type + " Item",
                                sle.voucher_detail_no, "base_net_rate", outgoing_rate)
        else:
            frappe.db.set_value("Purchase Receipt Item Supplied",
    ...
    def get_serialized_values(self, sle):
        ...
        if new_stock_qty > 0:
            new_stock_value = (self.wh_data.qty_after_transaction *
    ...
    def get_fifo_values(self, sle):
        ...
        if actual_qty > 0:
            ...
            while qty_to_pop:
                ...
                if outgoing_rate > 0:
                    ...
                    if index == None:
                        new_stock_value = sum(
                            (d[0]*d[1] for d in self.wh_data.stock_queue)) - qty_to_pop*outgoing_rate
                        new_stock_qty = sum(
                            (d[0] for d in self.wh_data.stock_queue)) - qty_to_pop
                        self.wh_data.stock_queue = [
        ...
        stock_value = sum((flt(batch[0]) * flt(batch[1])
                           for batch in self.wh_data.stock_queue))
        ...
        for warehouse, exceptions in iteritems(self.exceptions):
            ...
            if ((exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]) in
                ...
                msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
                    abs(deficiency), frappe.get_desk_link(
                        'Item', exceptions[0]["item_code"]),
...
def get_previous_sle(args, for_update=False):
    ...
    sle = get_stock_ledger_entries(
        args, "<=", "desc", "limit 1", for_update=for_update)
...
def get_valuation_rate(item_code, warehouse, voucher_type, voucher_no,
    ...
    if not allow_zero_rate and not valuation_rate and raise_error_if_no_rate \
        ...
        message = _("Valuation Rate for the Item {0}, is required to do accounting entries for {1} {2}.").format(
            form_link, voucher_type, voucher_no)
        message += "<br><br>" + _(" Here are the options to proceed:")
        solutions = "<li>" + \
            _("If the item is transacting as a Zero Valuation Rate item in this entry, please enable 'Allow Zero Valuation Rate' in the {0} Item table.").format(
                voucher_type) + "</li>"
        solutions += "<li>" + _("If not, you can Cancel / Submit this entry ") + _(
            "{0}").format(frappe.bold("after")) + _(" performing either one below:") + "</li>"
        sub_solutions = "<ul><li>" + \
