...
sales_doctypes = ['Quotation', 'Sales Order',
                  'Delivery Note', 'Sales Invoice', 'POS Invoice']
purchase_doctypes = ['Material Request', 'Supplier Quotation',
...
def update_stock(args, out):
    if (args.get("doctype") == "Delivery Note" or
        ...
        if out.has_batch_no and not args.get("batch_no"):
            ...
            actual_batch_qty = get_batch_qty(
                out.batch_no, out.warehouse, out.item_code)
            if actual_batch_qty:
        ...
        if out.has_serial_no and args.get('batch_no'):
            ...
            out.serial_no = get_serial_no(
                out, args.serial_no, sales_order=reserved_so)
...
def set_valuation_rate(out, args):
    if frappe.db.exists("Product Bundle", args.item_code, cache=True):
        ...
        out.update(get_valuation_rate(args.item_code,
                                      args.company, out.get("warehouse")))
...
def process_args(args):
    ...
    if not args.get("price_list"):
        args.price_list = args.get(
            "selling_price_list") or args.get("buying_price_list")
...
def validate_item_details(args, item):
    ...
    if args.transaction_type == "selling" and cint(item.has_variants):
        ...
        if args.get("is_subcontracted") == "Yes" and item.is_sub_contracted_item != 1:
            throw(
                _("Item {0} must be a Sub-contracted Item").format(item.name))
...
def get_basic_details(args, item, overwrite_warehouse=True):
    ...
    for d in [
            ...
            ["Warehouse", "warehouse", ""]]:
        if not out[d[1]]:
            out[d[1]] = frappe.get_cached_value(
                'Company',  args.company,  d[2]) if d[2] else None
...
def get_item_tax_info(company, tax_category, item_codes):
    ...
    for item_code in item_codes:
        ...
        get_item_tax_template(
            {"company": company, "tax_category": tax_category}, item, out[item_code])
        out[item_code]["item_tax_rate"] = get_item_tax_map(
            company, out[item_code].get("item_tax_template"), as_json=True)
...
def _get_item_tax_template(args, taxes, out={}, for_validate=False):
    ...
    if taxes_with_validity:
        taxes = sorted(taxes_with_validity,
                       key=lambda i: i.valid_from, reverse=True)
    else:
    ...
    if not cost_center and (item and item_group and brand):
        if args.get('customer'):
            cost_center = item.get('selling_cost_center') or item_group.get(
                'selling_cost_center') or brand.get('selling_cost_center')
        else:
            cost_center = item.get('buying_cost_center') or item_group.get(
                'buying_cost_center') or brand.get('buying_cost_center')
        ...
        if frappe.has_permission("Item Price", "write"):
            ...
            item_price = frappe.db.get_value('Item Price',
                                             {'item_code': args.item_code,
                                                 'price_list': args.price_list, 'currency': args.currency},
...
def get_party_item_code(args, item_doc, out):
    ...
    if args.transaction_type == "buying" and args.supplier:
        item_supplier = item_doc.get(
            "supplier_items", {"supplier": args.supplier})
...
def get_conversion_factor(item_code, uom):
    variant_of = frappe.db.get_value(
        "Item", item_code, "variant_of", cache=True)
...
def get_serial_no_details(item_code, warehouse, stock_qty, serial_no):
    args = frappe._dict({"item_code": item_code, "warehouse": warehouse,
                         "stock_qty": stock_qty, "serial_no": serial_no})
...
def get_bin_details_and_serial_nos(item_code, warehouse, has_batch_no=None, stock_qty=None, serial_no=None):
    ...
    if flt(stock_qty) > 0:
        if has_batch_no:
            args = frappe._dict(
                {"item_code": item_code, "warehouse": warehouse, "stock_qty": stock_qty})
            serial_no = get_serial_no(args)
            bin_details_and_serial_nos.update({'serial_no': serial_no})
            return bin_details_and_serial_nos

        bin_details_and_serial_nos.update(get_serial_no_details(
            item_code, warehouse, stock_qty, serial_no))
    return bin_details_and_serial_nos
...
def get_batch_qty_and_serial_no(batch_no, stock_qty, warehouse, item_code, has_serial_no):
    ...
    batch_qty_and_serial_no.update(
    ...
    if (flt(batch_qty_and_serial_no.get('actual_batch_qty')) >= flt(stock_qty)) and has_serial_no:
        args = frappe._dict({"item_code": item_code, "warehouse": warehouse,
                             "stock_qty": stock_qty, "batch_no": batch_no})
        serial_no = get_serial_no(args)
...
def apply_price_list_on_item(args):
    ...
    item_details.update(get_pricing_rule_for_item(
        args, item_details.price_list_rate))
...
def get_price_list_currency_and_exchange_rate(args):
    ...
    price_list_uom_dependant = price_list_details.get(
...
def get_serial_no(args, serial_nos=None, sales_order=None):
    ...
    if args.get('warehouse') and args.get('stock_qty') and args.get('item_code'):
        has_serial_no = frappe.get_value(
            'Item', {'item_code': args.item_code}, "has_serial_no")
        if args.get('batch_no') and has_serial_no == 1:
            return get_serial_no_batchwise(args, sales_order)
        elif has_serial_no == 1:
            args = json.dumps({"item_code": args.get('item_code'), "warehouse": args.get(
                'warehouse'), "stock_qty": args.get('stock_qty')})
            args = process_args(args)
