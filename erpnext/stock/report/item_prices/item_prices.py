...
def get_columns(filters):
    ...
    columns = [_("Item") + ":Link/Item:100", _("Item Name") + "::150", _("Item Group") + ":Link/Item Group:125",
               _("Brand") + "::100", _("Description") +
               "::150", _("UOM") + ":Link/UOM:80",
               _("Last Purchase Rate") + ":Currency:90", _("Valuation Rate") +
               ":Currency:80",	_("Sales Price List") + "::180",
...
def get_price_list():
    ...
    for j in price_list:
        if j.price:
            rate.setdefault(j.item_code, {}).setdefault(
                "Buying" if j.buying else "Selling", []).append(j.price)
    item_rate_map = {}
