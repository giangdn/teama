...
def get_data(item):
    ...
    if not variant_results:
        frappe.msgprint(
            _("There isn't any item variant for the selected item"))
        return []
    else:
        variants = ", ".join([frappe.db.escape(variant['name'])
    ...
    for item_dict in variant_dicts:
        ...
        if stock_details_map.get(name):
            item_dict["Inventory"] = stock_details_map.get(name)[
                "Inventory"] or 0
            item_dict["In Production"] = stock_details_map.get(name)[
                "In Production"] or 0
            item_dict["Available Selling"] = stock_details_map.get(name)[
                "Available Selling"] or 0
        else:
            item_dict["Inventory"] = item_dict["In Production"] = item_dict["Available Selling"] = 0

        item_dict["Avg. Buying Price List Rate"] = buying_price_map.get(
            name) or 0
        item_dict["Avg. Selling Price List Rate"] = selling_price_map.get(
            name) or 0
...
def get_columns(item):
    ...
    columns += [_("Avg. Buying Price List Rate") + ":Currency:110", _("Avg. Selling Price List Rate") + ":Currency:110",
                ...
                _("Open Orders") +
                ":Float:100", _("Available Selling") + ":Float:100"
                ]
