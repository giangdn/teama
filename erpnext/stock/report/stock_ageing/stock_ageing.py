...
def get_fifo_queue(filters, sle=None):
    ...
    for d in sle:
        key = (d.name, d.warehouse) if filters.get(
            'show_warehouse_wise_stock') else d.name
        ...
        if d.voucher_type == "Stock Reconciliation":
            d.actual_qty = flt(d.qty_after_transaction) - \
                flt(item_details[key].get("qty_after_transaction", 0))
                ...
                if serial_no_list:
                    for serial_no in serial_no_list:
                        if serial_no_batch_purchase_details.get(serial_no):
                            fifo_queue.append(
                                [serial_no, serial_no_batch_purchase_details.get(serial_no)])
                        else:
                            serial_no_batch_purchase_details.setdefault(
                                serial_no, d.posting_date)
            ...
            if serial_no_list:
                ...
                while qty_to_pop:
                    ...
                    if 0 < flt(batch[0]) <= qty_to_pop:
                        ...
                        transferred_item_details[transferred_item_key].append(
                            fifo_queue.pop(0))
                    else:
...
def setup_ageing_columns(filters, range_columns):
    for i, label in enumerate(["0-{range1}".format(range1=filters["range1"]),
                               "{range1}-{range2}".format(range1=cint(
                                   filters["range1"]) + 1, range2=filters["range2"]),
                               "{range2}-{range3}".format(range2=cint(
                                   filters["range2"]) + 1, range3=filters["range3"]),
                               "{range3}-{above}".format(range3=cint(filters["range3"]) + 1, above=_("Above"))]):
        add_column(range_columns, label="Age (" + label + ")",
