...
class TestLandedCostVoucher(unittest.TestCase):
    def test_landed_cost_voucher(self):
        ...
        pr_lc_value = frappe.db.get_value("Purchase Receipt Item", {
                                          "parent": pr.name}, "landed_cost_voucher_amount")
    ...
    def test_landed_cost_voucher_for_serialized_item(self):
        ...
        serial_no_rate = frappe.db.get_value(
            "Serial No", "SN001", "purchase_rate")
    ...
    def test_landed_cost_voucher_for_odd_numbers(self):
        pr = make_purchase_receipt(company="_Test Company with perpetual inventory",
                                   warehouse="Stores - TCP1", supplier_warehouse="Work in Progress - TCP1", do_not_save=True)
...
def distribute_landed_cost_on_items(lcv):
    ...
    for item in lcv.get("items"):
        item.applicable_charges = flt(
            item.get(based_on)) * flt(lcv.total_taxes_and_charges) / flt(total)
        item.applicable_charges = flt(
