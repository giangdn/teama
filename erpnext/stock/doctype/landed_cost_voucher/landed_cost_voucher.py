...
class LandedCostVoucher(Document):
    ...
    def validate_purchase_receipts(self):
        ...
        for item in self.get("items"):
            if not item.receipt_document:
                frappe.throw(
                    _("Item must be added using 'Get Items from Purchase Receipts' button"))
    ...
    def set_total_taxes_and_charges(self):
        self.total_taxes_and_charges = sum(
            [flt(d.base_amount) for d in self.get("taxes")])
    ...
    def validate_applicable_charges_for_item(self):
        ...
        if based_on != 'distribute manually':
            ...
            total = sum([flt(d.get('applicable_charges'))
                         for d in self.get("items")])

        if not total:
            frappe.throw(
                _("Total {0} for all items is zero, may be you should change 'Distribute Charges Based On'").format(based_on))
        ...
        diff = flt(self.total_taxes_and_charges) - \
            flt(total_applicable_charges)
    ...
    def update_landed_cost(self):
        ...
        for d in self.get("purchase_receipts"):
            ...
            doc.update_stock_ledger(
                allow_negative_stock=True, via_landed_cost_voucher=True)
