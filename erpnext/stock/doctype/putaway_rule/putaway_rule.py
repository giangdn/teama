...
class PutawayRule(Document):
    ...
    def validate_priority(self):
        if self.priority < 1:
            frappe.throw(_("Priority cannot be lesser than 1."),
    ...
    def validate_capacity(self):
        ...
        balance_qty = get_stock_balance(
            self.item_code, self.warehouse, nowdate())
    ...
    def set_stock_capacity(self):
        self.stock_capacity = (flt(self.conversion_factor)
                               or 1) * flt(self.capacity)
...
def apply_putaway_rule(doctype, items, company, sync=None, purpose=None):
    ...
    for item in items:
        ...
        pending_stock_qty = flt(
            item.transfer_qty) if doctype == "Stock Entry" else flt(item.stock_qty)
        uom_must_be_whole_number = frappe.db.get_value(
            'UOM', item.uom, 'must_be_whole_number')

        if not pending_qty or not item_code:
            updated_table = add_row(
                item, pending_qty, source_warehouse or item.warehouse, updated_table)
            continue
        ...
        if not rules:
            ...
            if at_capacity:
                ...
                updated_table = add_row(
                    item, pending_qty, warehouse, updated_table)
            continue
        ...
        for rule in item_wise_rules[key]:
            if pending_stock_qty > 0 and rule.free_space:
                stock_qty_to_allocate = flt(rule.free_space) if pending_stock_qty >= flt(
...
def get_ordered_putaway_rules(item_code, company, source_warehouse=None):
    ...
    rules = frappe.get_all("Putaway Rule",
                           fields=["name", "item_code",
                                   "stock_capacity", "priority", "warehouse"],
                           filters=filters,
    ...
    for rule in rules:
        balance_qty = get_stock_balance(
            rule.item_code, rule.warehouse, nowdate())
    ...
    vacant_rules = sorted(vacant_rules, key=lambda i: (
        i['priority'], -i['free_space']))
...
def add_row(item, to_allocate, warehouse, updated_table, rule=None, serial_nos=None):
    ...
    new_updated_table_row.idx = 1 if not updated_table else cint(
        updated_table[-1].idx) + 1
    ...
    if item.doctype == "Stock Entry Detail":
        ...
        new_updated_table_row.transfer_qty = flt(
            to_allocate) * flt(new_updated_table_row.conversion_factor)
    else:
        new_updated_table_row.stock_qty = flt(
            to_allocate) * flt(new_updated_table_row.conversion_factor)
    ...
    if serial_nos:
        new_updated_table_row.serial_no = get_serial_nos_to_allocate(
            serial_nos, to_allocate)
...
def show_unassigned_items_message(items_not_accomodated):
    ...
    frappe.msgprint(msg, title=_("Insufficient Capacity"),
...
def get_serial_nos_to_allocate(serial_nos, to_allocate):
    if serial_nos:
        ...
        # pop out allocated serial nos and modify list
        serial_nos[:] = serial_nos[cint(to_allocate):]
