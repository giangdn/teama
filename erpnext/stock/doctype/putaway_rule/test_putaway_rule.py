...
class TestPutawayRule(unittest.TestCase):
    def setUp(self):
        ...
        self.warehouse_1 = frappe.db.get_value(
            "Warehouse", {"warehouse_name": "Rack 1"})
        self.warehouse_2 = frappe.db.get_value(
            "Warehouse", {"warehouse_name": "Rack 2"})
    ...
    def test_putaway_rules_multi_uom_whole_uom(self):
        ...
        rule_2 = create_putaway_rule(
            item_code="_Rice", warehouse=self.warehouse_2, capacity=500)
    ...
    def test_putaway_rules_with_reoccurring_item(self):
        ...
        # 100 unassigned in second row from 200
        self.assertEqual(pr.items[1].qty, 100)
        self.assertEqual(pr.items[1].warehouse, self.warehouse_1)
    ...
    def test_putaway_rule_on_stock_entry_material_transfer_batch_serial_item(self):
        ...
        pr = make_purchase_receipt(
            item_code="Water Bottle", qty=5, do_not_submit=1)
...
def create_putaway_rule(**args):
    ...
    putaway.stock_uom = frappe.db.get_value(
        "Item", putaway.item_code, "stock_uom")
    putaway.uom = args.uom or putaway.stock_uom
    putaway.conversion_factor = get_conversion_factor(
        putaway.item_code, putaway.uom)['conversion_factor']
