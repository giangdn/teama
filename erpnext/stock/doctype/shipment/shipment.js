...
frappe.ui.form.on("Shipment", {
	...
	onload: function(frm) {
		frm.set_query("delivery_address_name", () => {
			let delivery_to = `delivery_${frappe.model.scrub(
		...
		frm.set_query("pickup_address_name", () => {
			let pickup_from = `pickup_${frappe.model.scrub(
	...
	pickup_contact_person: function(frm) {
		if (frm.doc.pickup_contact_person) {
			frappe.call({
				method:
					"erpnext.stock.doctype.shipment.shipment.get_company_contact",
	...
	pickup_from: function(frm) {
		...
		if (
			frm.doc.pickup_from &&
			frm.doc.pickup_date == frappe.datetime.get_today()
		) {
			...
			if (
				...
				(pickup_hour == current_hour && pickup_min < current_min)
			) {
				frm.set_value("pickup_from", pickup_time);
	...
	get_pickup_time: function() {
		...
		let current_min = new Date().toLocaleString("en-US", {
			minute: "numeric"
		});
		if (current_min < 30) {
			current_min = "30";
