...
class TestShipment(unittest.TestCase):
    def test_shipment_from_delivery_note(self):
        ...
        self.assertEqual(second_shipment.value_of_goods,
                         delivery_note.grand_total)
        self.assertEqual(len(second_shipment.shipment_delivery_note), 1)
        self.assertEqual(
            second_shipment.shipment_delivery_note[0].delivery_note, delivery_note.name)
...
def get_shipment_customer_contact(customer_name):
    ...
    contacts = frappe.get_all("Contact", fields=["name"], filters={
                              "name": customer_name})
    if len(contacts):
        return contacts[0]
    else:
...
def get_shipment_customer_address(customer_name):
    ...
    customer_address = frappe.get_all("Address", fields=["name"], filters={
...
def get_shipment_customer():
    ...
    customer = frappe.get_all("Customer", fields=["name"], filters={
                              "name": customer_name})
    if len(customer):
        return customer[0]
    else:
...
def get_shipment_company_address(company_name):
    ...
    addresses = frappe.get_all("Address", fields=["name"], filters={
                               "address_title": address_title})
    if len(addresses):
        return addresses[0]
    else:
...
def get_shipment_company():
    ...
    abbr = 'SC'
    companies = frappe.get_all("Company", fields=["name"], filters={
