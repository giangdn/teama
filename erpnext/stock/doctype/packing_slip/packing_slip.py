...
class PackingSlip(Document):
    ...
    def validate_delivery_note(self):
        ...
        if cint(frappe.db.get_value("Delivery Note", self.delivery_note, "docstatus")) != 0:
            frappe.throw(_("Delivery Note {0} must not be submitted").format(
                self.delivery_note))
    ...
    def validate_case_nos(self):
        ...
        if not cint(self.from_case_no):
            frappe.msgprint(
                _("Please specify a valid 'From Case No.'"), raise_exception=1)
        ...
        if res:
            frappe.throw(_("""Case No(s) already in use. Try from Case No {0}""").format(
    ...
    def recommend_new_qty(self, item, ps_item_qty, no_of_cases):
        ...
        item['recommended_qty'] = (
            flt(item['qty']) - flt(item['packed_qty'])) / no_of_cases
        item['specified_qty'] = flt(ps_item_qty[item['item_code']])
        if not item['packed_qty']:
            item['packed_qty'] = 0

        frappe.throw(_("Quantity for Item {0} must be less than {1}").format(
    ...
    def get_items(self):
        ...
        custom_fields = frappe.get_meta(
            "Delivery Note Item").get_custom_fields()
