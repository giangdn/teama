...
cur_frm.fields_dict["items"].grid.get_field("item_code").get_query = function(
	...
	if (!doc.delivery_note) {
		...
		return {
			query:
				"erpnext.stock.doctype.packing_slip.packing_slip.item_details",
...
cur_frm.cscript.validate_case_nos = function(doc) {
	...
	if (cint(doc.from_case_no) == 0) {
		...
		frappe.msgprint(
			__("'To Case No.' cannot be less than 'From Case No.'")
		);
		frappe.validated = false;
	}
};
...
cur_frm.cscript.validate_duplicate_items = function(doc, ps_detail) {
	for (var i = 0; i < ps_detail.length; i++) {
		for (var j = 0; j < ps_detail.length; j++) {
			if (
				i != j &&
				ps_detail[i].item_code &&
				ps_detail[i].item_code == ps_detail[j].item_code
			) {
				frappe.msgprint(
					__(
...
cur_frm.cscript.calc_net_total_pkg = function(doc, ps_detail) {
	...
	doc.net_weight_uom =
		ps_detail && ps_detail.length ? ps_detail[0].weight_uom : "";
...
var make_row = function(title, val, bold) {
	...
	var bend = "</b>";
	return (
		'<tr><td class="datalabelcell">' +
		(bold ? bstart : "") +
		title +
		(bold ? bend : "") +
		"</td>" +
		'<td class="datainputcell" style="text-align:left;">' +
		val +
		"</td>" +
		"</tr>"
	);
...
cur_frm.pformat.net_weight_pkg = function(doc) {
	return (
		...
		make_row("Net Weight", doc.net_weight_pkg) +
		"</table>"
	);
};
...
cur_frm.pformat.gross_weight_pkg = function(doc) {
	return (
		'<table style="width:100%">' +
		make_row("Gross Weight", doc.gross_weight_pkg) +
		"</table>"
	);
};
