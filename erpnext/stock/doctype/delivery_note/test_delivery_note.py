...
class TestDeliveryNote(unittest.TestCase):
    ...
    def test_delivery_note_gl_entry_packing_item(self):
        ...
        dn = create_delivery_note(item_code="_Test Product Bundle Item", company='_Test Company with perpetual inventory',
                                  warehouse='Stores - TCP1', cost_center='Main - TCP1', expense_account="Cost of Goods Sold - TCP1")

        stock_value_diff_rm1 = abs(frappe.db.get_value("Stock Ledger Entry",
                                                       {"voucher_type": "Delivery Note",
                                                           "voucher_no": dn.name, "item_code": "_Test Item"},
    ...
    def test_serialized_partial_sales_invoice(self):
        ...
        si = make_sales_invoice(dn.name)
        si.submit()
        self.assertEqual(si.items[0].qty, len(
            get_serial_nos(si.items[0].serial_no)))
    ...
    def test_sales_return_for_non_bundled_items_full(self):
        ...
        make_stock_entry(item_code="Box", target="Stores - TCP1",
    ...
    def test_return_entire_bundled_items(self):
        ...
        stock_in_hand_account = get_inventory_account(
            '_Test Company', dn1.items[0].warehouse)
    ...
    def test_closed_delivery_note(self):
        ...
        self.assertEqual(frappe.db.get_value(
            "Delivery Note", dn.name, "Status"), "Closed")
    ...
    def test_delivery_note_with_cost_center(self):
        ...
        stock_in_hand_account = get_inventory_account(
            '_Test Company with perpetual inventory')
        dn = create_delivery_note(company='_Test Company with perpetual inventory', warehouse='Stores - TCP1',
                                  expense_account="Cost of Goods Sold - TCP1", cost_center=cost_center)
    ...
    def test_make_sales_invoice_from_dn_for_returned_qty(self):
        ...
        dn1 = create_delivery_note(
            is_return=1, return_against=dn.name, qty=-1, do_not_submit=True)
    ...
    def test_make_sales_invoice_from_dn_with_returned_qty_duplicate_items(self):
        ...
        dn1 = create_delivery_note(
            is_return=1, return_against=dn.name, qty=-2, do_not_submit=True)
        dn1.items[0].dn_detail = dn.items[0].name
        dn1.submit()
