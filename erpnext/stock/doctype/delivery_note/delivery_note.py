...
class DeliveryNote(SellingController):
    ...
    def validate_packed_qty(self):
        ...
        for d in self.get("items"):
            if flt(d.get('qty')) != flt(d.get('packed_qty')):
                frappe.msgprint(_("Packed quantity must equal quantity for Item {0} in row {1}").format(
                    d.item_code, d.idx))
                has_error = True
        if has_error:
    ...
    def make_return_invoice(self):
        try:
            ...
            frappe.throw(
                _("Could not create Credit Note automatically, please uncheck 'Issue Credit Note' and submit again"))
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
    ...
    for dnd in dn_details:
        ...
        frappe.db.set_value("Delivery Note Item", dnd.name, "billed_amt",
                            billed_amt_agianst_dn, update_modified=update_modified)
