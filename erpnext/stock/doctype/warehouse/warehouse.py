...
class Warehouse(NestedSet):
    ...
    def on_trash(self):
        ...
        for d in bins:
            if d['actual_qty'] or d['reserved_qty'] or d['ordered_qty'] or \
                ...
                throw(_("Warehouse {0} can not be deleted as quantity exists for Item {1}").format(
                    self.name, d['item_code']))
            else:
                frappe.db.sql(
                    "delete from `tabBin` where name = %s", d['name'])

        if self.check_if_sle_exists():
            throw(
                _("Warehouse can not be deleted as stock ledger entry exists for this warehouse."))

        if self.check_if_child_exists():
            throw(
                _("Child warehouse exists for this warehouse. You can not delete this warehouse."))
    ...
    def get_new_warehouse_name_without_abbr(self, name):
        company_abbr = frappe.get_cached_value(
    ...
    def convert_to_ledger(self):
        if self.check_if_child_exists():
            frappe.throw(
                _("Warehouses with child nodes cannot be converted to ledger"))
        elif self.check_if_sle_exists():
            throw(
                _("Warehouses with existing transaction can not be converted to ledger."))
        else:
            self.is_group = 0
            self.save()
            return 1
    ...
    def convert_to_group(self):
        if self.check_if_sle_exists():
            throw(
                _("Warehouses with existing transaction can not be converted to group."))
        else:
            self.is_group = 1
            self.save()
            return 1
...
def get_children(doctype, parent=None, company=None, is_root=False):
    ...
    for wh in warehouses:
        ...
        if company:
            wh["company_currency"] = frappe.db.get_value(
...
def get_warehouses_based_on_account(account, company=None):
    ...
    if (not warehouses and company and
        ...
        warehouses = [d.name for d in frappe.get_all(
