...
class ItemPrice(Document):
    ...
    def validate_dates(self):
        if self.valid_from and self.valid_upto:
            if self.valid_from > self.valid_upto:
                frappe.throw(
                    _("Valid From Date must be lesser than Valid Upto Date."))
    ...
    def update_price_list_details(self):
        if self.price_list:
            ...
            if not price_list_details:
                ...
                frappe.throw(
                    "The price list {0} does not exist or is disabled".format(link))
    ...
    def check_duplicates(self):
        ...
        if price_list_rate:
            frappe.throw(
                _("Item Price appears multiple times based on Price List, Supplier/Customer, Currency, Item, Batch, UOM, Qty, and Dates."), ItemPriceDuplicateItem,)
