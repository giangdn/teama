...
def batch_uses_naming_series():
    ...
    :return: bool
    """
    use_naming_series = cint(frappe.db.get_single_value(
        'Stock Settings', 'use_naming_series'))
...
class Batch(Document):
    ...
    def before_save(self):
        ...
        if not self.expiry_date and has_expiry_date and shelf_life_in_days:
            self.expiry_date = add_days(
                self.manufacturing_date, shelf_life_in_days)
    ...
    def get_name_from_naming_series(self):
        ...
        naming_series_prefix = render_template(
            str(naming_series_prefix), self.__dict__)
...
def get_batches_by_oldest(item_code, warehouse):
    ...
    batches_dates = [[batch, frappe.get_value(
        'Batch', batch.batch_no, 'expiry_date')] for batch in batches]
...
def split_batch(batch_no, item_code, warehouse, qty, new_batch_id=None):
    ...
    batch = frappe.get_doc(
        dict(doctype='Batch', item=item_code, batch_id=new_batch_id)).insert()
        ...
        if has_batch_no and warehouse and qty > 0:
            if not d.batch_no:
                ...
                if flt(batch_qty, d.precision("qty")) < flt(qty, d.precision("qty")):
                    frappe.throw(_("Row #{0}: The batch {1} has only {2} qty. Please select another batch which has {3} qty available or split the row into multiple rows, to deliver/issue from multiple batches").format(
                        d.idx, d.batch_no, batch_qty, qty))
...
def get_batch_no(item_code, warehouse, qty=1, throw=False, serial_no=None):
    ...
    if not batch_no:
        frappe.msgprint(_('Please select a Batch for Item {0}. Unable to find a single batch that fulfills this requirement').format(
...
def get_batches(item_code, warehouse, qty=1, throw=False, serial_no=None):
    ...
    if serial_no and frappe.get_cached_value('Item', item_code, 'has_batch_no'):
        ...
        cond = " and `tabBatch`.name = %s" % (
            frappe.db.escape(batch[0].batch_no))
...
def validate_serial_no_with_batch(serial_nos, item_code):
    ...
    serial_no_link = ','.join(
        [get_link_to_form("Serial No", sn) for sn in serial_nos])
