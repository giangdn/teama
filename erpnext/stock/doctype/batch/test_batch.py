...
class TestBatch(unittest.TestCase):
    ...
    def test_stock_entry_incoming(self):
        ...
        self.assertEqual(get_batch_qty(
            stock_entry.items[0].batch_no, stock_entry.items[0].t_warehouse), 90)
    ...
    def test_batch_split(self):
        ...
        self.assertEqual(get_batch_qty(
            receipt.items[0].batch_no, receipt.items[0].warehouse), 78)
        self.assertEqual(get_batch_qty(
            new_batch, receipt.items[0].warehouse), 22)
    ...
    def test_get_batch_qty(self):
        ...
        self.make_new_batch_and_entry(
            'ITEM-BATCH-2', 'batch a', '_Test Warehouse - _TC')
        self.make_new_batch_and_entry(
            'ITEM-BATCH-2', 'batch b', '_Test Warehouse - _TC')
    ...
    def test_total_batch_qty(self):
        ...
        existing_batch_qty = flt(
            frappe.db.get_value("Batch", "B100", "batch_qty"))
        stock_entry = self.make_new_batch_and_entry(
            'ITEM-BATCH-3', 'B100', '_Test Warehouse - _TC')
...
def create_batch(item_code, rate, create_item_price_for_batch):
    ...
    batch = frappe.db.get_value(
        'Batch', {'item': item_code, 'reference_name': pi.name})
