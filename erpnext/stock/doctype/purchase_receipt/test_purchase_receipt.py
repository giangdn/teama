...
class TestPurchaseReceipt(unittest.TestCase):
    ...
    def test_batched_serial_no_purchase(self):
        item = frappe.db.exists(
            "Item", {'item_name': 'Batched Serialized Item'})
        if not item:
        ...
        self.assertTrue(frappe.db.get_value(
            'Batch', {'item': item.name, 'reference_name': pr.name}))
        ...
        self.assertFalse(frappe.db.get_value(
            'Batch', {'item': item.name, 'reference_name': pr.name}))
        self.assertFalse(frappe.db.get_all(
            'Serial No', {'batch_no': batch_no}))
    ...
    def test_purchase_receipt_gl_entry(self):
        ...
        stock_in_hand_account = get_inventory_account(
            pr.company, pr.items[0].warehouse)
        fixed_asset_account = get_inventory_account(
            pr.company, pr.items[1].warehouse)
    ...
    def test_subcontracting(self):
        ...
        make_stock_entry(item_code="_Test Item",
                         target="_Test Warehouse 1 - _TC", qty=100, basic_rate=100)
        make_stock_entry(item_code="_Test Item Home Desktop 100", target="_Test Warehouse 1 - _TC",
                         qty=100, basic_rate=100)
        pr = make_purchase_receipt(
            item_code="_Test FG Item", qty=10, rate=500, is_subcontracted="Yes")
    ...
    def test_serial_no_supplier(self):
        pr = make_purchase_receipt(
            item_code="_Test Serialized Item With Series", qty=1)
        self.assertEqual(frappe.db.get_value("Serial No", pr.get("items")[0].serial_no, "supplier"),
                         pr.supplier)

        pr.cancel()
        self.assertFalse(frappe.db.get_value(
            "Serial No", pr.get("items")[0].serial_no, "warehouse"))
    ...
    def test_rejected_serial_no(self):
        ...
        pr.get("items")[
            0].rejected_warehouse = "_Test Rejected Warehouse - _TC"
        pr.insert()
        pr.submit()
    ...
    def test_purchase_return_for_rejected_qty(self):
        ...
        pr = make_purchase_receipt(company="_Test Company with perpetual inventory", warehouse="Stores - TCP1",
                                   supplier_warehouse="Work in Progress - TCP1", received_qty=4, qty=2, rejected_warehouse=rejected_warehouse)

        return_pr = make_purchase_receipt(company="_Test Company with perpetual inventory", warehouse="Stores - TCP1", supplier_warehouse="Work in Progress - TCP1",
                                          is_return=1, return_against=pr.name, received_qty=-4, qty=-2, rejected_warehouse=rejected_warehouse)
    ...
    def test_purchase_return_for_multi_uom(self):
        ...
        pr = make_purchase_receipt(
            item_code=item_code, qty=1, uom="Box", conversion_factor=1.0)
    ...
    def test_closed_purchase_receipt(self):
        ...
        self.assertEqual(frappe.db.get_value(
            "Purchase Receipt", pr.name, "status"), "Closed")
    ...
    def test_serial_no_against_purchase_receipt(self):
        ...
        if not frappe.db.exists("Item", item_code):
            item = make_item(item_code, dict(
                has_serial_no=1, serial_no_series="KLJL.###"))
    ...
    def test_not_accept_duplicate_serial_no(self):
        ...
        item_code = frappe.db.get_value(
            'Item', {'has_serial_no': 1, 'is_fixed_asset': 0, "has_batch_no": 0})
        if not item_code:
            item = make_item("Test Serial Item 1", dict(
                has_serial_no=1, has_batch_no=0))
            item_code = item.name

        serial_no = random_string(5)
        pr1 = make_purchase_receipt(
            item_code=item_code, qty=1, serial_no=serial_no)
        dn = create_delivery_note(
            item_code=item_code, qty=1, serial_no=serial_no)

        pr2 = make_purchase_receipt(
            item_code=item_code, qty=1, serial_no=serial_no, do_not_submit=True)
    ...
    def test_auto_asset_creation(self):
        ...
        assets = frappe.db.get_all(
            'Asset', filters={'purchase_receipt': pr.name})
    ...
    def test_make_purchase_invoice_from_pr_for_returned_qty(self):
        ...
        pr1 = make_purchase_receipt(
            is_return=1, return_against=pr.name, qty=-1, do_not_submit=True)
    ...
    def test_make_purchase_invoice_from_pr_with_returned_qty_duplicate_items(self):
        ...
        pr2 = make_purchase_receipt(
            is_return=1, return_against=pr1.name, qty=-2, do_not_submit=True)
    ...
    def test_subcontracted_pr_for_multi_transfer_batches(self):
        ...
        for row in pr.supplied_items:
            self.assertEqual(transferred_batch.get(
                row.batch_no), row.consumed_qty)
...
def make_purchase_receipt(**args):
    ...
    uom = args.uom or frappe.db.get_value(
        "Item", item_code, "stock_uom") or "_Test UOM"
    pr.append("items", {
        "item_code": item_code,
        "warehouse": args.warehouse or "_Test Warehouse - _TC",
        "qty": qty,
