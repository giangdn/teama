...
from erpnext.setup.doctype.item_group.item_group import (
    get_parent_item_groups, invalidate_cache_for)
...
class Item(WebsiteGenerator):
    ...
    def validate(self):
        ...
        if not self.get("__islocal"):
            self.old_item_group = frappe.db.get_value(
                self.doctype, self.name, "item_group")
    ...
    def validate_customer_provided_part(self):
        if self.is_customer_provided_item:
            if self.is_purchase_item:
                frappe.throw(
                    _('"Customer Provided Item" cannot be Purchase Item also'))
            if self.valuation_rate:
                frappe.throw(
                    _('"Customer Provided Item" cannot have Valuation Rate'))
        ...
        for default in self.item_defaults or [frappe._dict({'company': frappe.defaults.get_defaults().company})]:
            ...
            if default_warehouse:
                warehouse_company = frappe.db.get_value(
                    "Warehouse", default_warehouse, "company")
    ...
    def validate_website_image(self):
        ...
        if not file_doc:
            ...
            if not auto_set_website_image:
                frappe.msgprint(
                    _("Website Image should be a public file or website URL"))
    ...
    def validate_fixed_asset(self):
        if self.is_fixed_asset:
            ...
            if not self.asset_category:
                frappe.throw(
                    _("Asset Category is mandatory for Fixed Asset item"))

            if self.stock_ledger_created():
                frappe.throw(
                    _("Cannot be a fixed asset item as Stock Ledger is created."))

        if not self.is_fixed_asset:
            asset = frappe.db.get_all(
                "Asset", filters={"item_code": self.name, "docstatus": 1}, limit=1)
            if asset:
                frappe.throw(
    ...
    def validate_retain_sample(self):
        if self.retain_sample and not frappe.db.get_single_value('Stock Settings', 'sample_retention_warehouse'):
            frappe.throw(
                _("Please select Sample Retention Warehouse in Stock Settings first"))
    ...
    def set_variant_context(self, context):
        if self.has_variants:
            ...
            context.variants = frappe.get_all("Item",
                                              filters={
                                                  "variant_of": self.name, "show_variant_in_website": 1},
                                              order_by="name asc")
    ...
    def set_attribute_context(self, context):
        if self.has_variants:
            ...
            for v in context.variants:
                ...
                for attr in v.attributes:
                    values = attribute_values_available.setdefault(
                        attr.attribute, [])
            ...
            for attr in self.attributes:
                values = context.attribute_values.setdefault(
                    attr.attribute, [])
    ...
    def set_disabled_attributes(self, context):
        ...
        for i, attr in enumerate(self.attributes):
            ...
            for prev_attr in self.attributes[:i]:
                combination_source.append(
                    [context.selected_attributes.get(prev_attr.attribute)])

            combination_source.append(context.attribute_values[attr.attribute])

            for combination in itertools.product(*combination_source):
                if not find_variant(combination):
                    context.disabled_attributes.setdefault(
                        attr.attribute, []).append(combination[-1])
    ...
    def set_shopping_cart_data(self, context):
        ...
        context.shopping_cart = get_product_info_for_website(
            self.name, skip_quotation_creation=True)
    ...
    def validate_item_type(self):
        if self.has_serial_no == 1 and self.is_stock_item == 0 and not self.is_fixed_asset:
            msgprint(
                _("'Has Serial No' can not be 'Yes' for non-stock item"), raise_exception=1)
        ...
        for d in self.get('taxes'):
            if d.item_tax_template:
                if d.item_tax_template in check_list:
                    frappe.throw(_("{0} entered twice in Item Tax").format(
                        d.item_tax_template))
                else:
    ...
    def validate_barcode(self):
        ...
        if len(self.barcodes) > 0:
            for item_barcode in self.barcodes:
                options = frappe.get_meta("Item Barcode").get_options(
                    "barcode_type").split('\n')
                if item_barcode.barcode:
                    ...
                    if item_barcode.barcode != item_barcode.name:
                        ...
                        frappe.delete_doc(
                            "Item Barcode", item_barcode_entry_name)
    ...
    def validate_warehouse_for_reorder(self):
        ...
        for d in self.get("reorder_levels"):
            ...
            if d.warehouse_reorder_level and not d.warehouse_reorder_qty:
                frappe.throw(
                    _("Row #{0}: Please set reorder quantity").format(d.idx))
    ...
    def on_trash(self):
        ...
        frappe.db.sql(
            "delete from `tabItem Price` where item_code=%s", self.name)
    ...
    def before_rename(self, old_name, new_name, merge=False):
        ...
        if merge:
            ...
            field_list = ["stock_uom", "is_stock_item",
                          "has_serial_no", "has_batch_no"]
            new_properties = [cstr(d) for d in frappe.db.get_value(
                "Item", new_name, field_list)]
    ...
    def after_rename(self, old_name, new_name, merge):
        if merge:
            self.validate_duplicate_item_in_stock_reconciliation(
                old_name, new_name)
    ...
    def validate_duplicate_item_in_stock_reconciliation(self, old_name, new_name):
        ...
        if not records:
            return
        document = _("Stock Reconciliation") if len(
            records) == 1 else _("Stock Reconciliations")

        msg = _("The items {0} and {1} are present in the following {2} : ").format(
            frappe.bold(old_name), frappe.bold(new_name), document)

        msg += '<br>'
        msg += ', '.join([get_link_to_form("Stock Reconciliation", d.parent)
                          for d in records]) + "<br><br>"
    ...
    def set_last_purchase_rate(self, new_name):
        last_purchase_rate = get_last_purchase_details(
            new_name).get("base_net_rate", 0)
        frappe.db.set_value(
            "Item", new_name, "last_purchase_rate", last_purchase_rate)
    ...
    def update_variants(self):
        if self.flags.dont_update_variants or \
            ...
            return
        if self.has_variants:
            variants = frappe.db.get_all("Item", fields=["item_code"], filters={
    ...
    def validate_attributes_in_variants(self):
        ...
        old_doc_attributes = set(
            [attr.attribute for attr in old_doc.attributes])
        ...
        items = [item["name"]
                 for item in frappe.get_all("Item", {"variant_of": self.name})]

        # get all deleted attributes
        deleted_attribute = list(
            old_doc_attributes.difference(set(own_attributes)))
        ...
        for docname, attr_list in not_included.items():
            link = "<a href='/app/Form/Item/{0}'>{0}</a>".format(
                frappe.bold(_(docname)))
        ...
        frappe.throw(message, title=_("Variant Attribute Error"),
    ...
    def validate_uom(self):
        ...
        if self.variant_of:
            template_uom = frappe.db.get_value(
                "Item", self.variant_of, "stock_uom")
    ...
    def validate_variant_attributes(self):
        if self.is_new() and self.variant_of and self.variant_based_on == 'Item Attribute':
            ...
            self.attributes = [d for d in self.attributes if cstr(
                d.attribute_value).strip()]
    ...
    def cant_change(self):
        if not self.get("__islocal"):
            fields = ("has_serial_no", "is_stock_item",
                      "valuation_method", "has_batch_no")

            values = frappe.db.get_value(
                "Item", self.name, fields, as_dict=True)
            if not values.get('valuation_method') and self.get('valuation_method'):
                values['valuation_method'] = frappe.db.get_single_value(
                    "Stock Settings", "valuation_method") or "FIFO"
                ...
                for field in fields:
                    if cstr(self.get(field)) != cstr(values.get(field)):
                        if not self.check_if_linked_document_exists(field):
                            ...
                            frappe.throw(_("As there are existing transactions against item {0}, you can not change the value of {1}").format(
                                self.name, frappe.bold(self.meta.get_label(field))))
    ...
    def check_if_linked_document_exists(self, field):
        ...
        if field == "is_stock_item":
            linked_doctypes += ["Sales Order Item", "Purchase Order Item",
                                "Material Request Item", "Product Bundle"]
    ...
    def validate_auto_reorder_enabled_in_stock_settings(self):
        if self.reorder_levels:
            enabled = frappe.db.get_single_value(
                'Stock Settings', 'auto_indent')
            if not enabled:
                frappe.msgprint(msg=_("You have to enable auto re-order in Stock Settings to maintain re-order levels."),
                                title=_("Enable Auto Re-Order"), indicator="orange")
...
def validate_end_of_life(item_code, end_of_life=None, disabled=None, verbose=1):
    if (not end_of_life) or (disabled is None):
        end_of_life, disabled = frappe.db.get_value(
...
def check_stock_uom_with_bin(item, stock_uom):
    ...
    if ref_uom:
        ...
        bin_list = frappe.db.sql(
            "select * from tabBin where item_code=%s", item, as_dict=1)
        for bin in bin_list:
        ...
        if matched and bin_list:
            frappe.db.sql(
                """update tabBin set stock_uom=%s where item_code=%s""", (stock_uom, item))
...
def get_uom_conv_factor(uom, stock_uom):
    ...
    if not value:
        uom_stock = frappe.db.get_value("UOM Conversion Factor", {"to_uom": stock_uom}, [
                                        "from_uom", "value"], as_dict=1)
        uom_row = frappe.db.get_value("UOM Conversion Factor", {"to_uom": uom}, [
                                      "from_uom", "value"], as_dict=1)
    ...
    for d in variants:
        ...
        if publish_progress:
            frappe.publish_progress(
                count*100/len(variants), title=_("Updating Variants..."))
