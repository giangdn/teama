...
frappe.ui.form.on("Item", {
	...
	onload: function(frm) {
		...
		if (frm.doc.variant_of) {
			frm.fields_dict["attributes"].grid.set_column_disp(
				"attribute_value",
				true
			);
		}
	...
	refresh: function(frm) {
		...
		if (frm.doc.has_variants) {
			...
			frm.add_custom_button(
				...
				function() {
					frappe.set_route("query-report", "Item Variant Details", {
						item: frm.doc.name
					});
				},
				__("View")
		...
		if (frm.doc.variant_of) {
			frm.set_intro(
				__("This Item is a Variant of {0} (Template).", [
					`<a href="/app/item/${frm.doc.variant_of}">${frm.doc.variant_of}</a>`
				]),
				true
			);
		}
		...
		if (
			frappe.defaults.get_default("item_naming_by") != "Naming Series" ||
			frm.doc.variant_of
		) {
		...
		const stock_exists =
			frm.doc.__onload && frm.doc.__onload.stock_exists ? 1 : 0;
	...
	gst_hsn_code: function(frm) {
		if ((!frm.doc.taxes || !frm.doc.taxes.length) && frm.doc.gst_hsn_code) {
			frappe.db
				.get_doc("GST HSN Code", frm.doc.gst_hsn_code)
				.then(hsn_doc => {
					$.each(hsn_doc.taxes || [], function(i, tax) {
						let a = frappe.model.add_child(
							cur_frm.doc,
							"Item Tax",
							"taxes"
						);
	...
	set_asset_naming_series: function(frm) {
		if (frm.doc.__onload && frm.doc.__onload.asset_naming_series) {
			frm.set_df_property(
				"asset_naming_series",
				"options",
				frm.doc.__onload.asset_naming_series
			);
		}
	},
...
frappe.ui.form.on("Item Reorder", {
	reorder_levels_add: function(frm, cdt, cdn) {
		...
		row.material_request_type =
			type == "Material Transfer" ? "Transfer" : type;
	}
});
...
$.extend(erpnext.item, {
	setup_queries: function(frm) {
		frm.fields_dict["item_defaults"].grid.get_field(
			"expense_account"
		...
		frm.fields_dict.customer_items.grid.get_field(
			"customer_name"
		).get_query = function(doc, cdt, cdn) {
			return { query: "erpnext.controllers.queries.customer_query" };
		};

		frm.fields_dict.supplier_items.grid.get_field(
			"supplier"
		).get_query = function(doc, cdt, cdn) {
		...
		frm.fields_dict.reorder_levels.grid.get_field(
			"warehouse_group"
		).get_query = function(doc, cdt, cdn) {
			return {
				filters: { is_group: 1 }
	...
	weight_to_validate: function(frm) {
		if (
			(frm.doc.nett_weight || frm.doc.gross_weight) &&
			!frm.doc.weight_uom
		) {
			frappe.msgprint(
				__('Weight is mentioned,\nPlease mention "Weight UOM" too')
			);
			frappe.validated = 0;
		}
	},
	...
	show_multiple_variants_dialog: function(frm) {
					...
					fields.push({
						...
						onchange: function() {
							...
							if (lengths.includes(0)) {
								...
								let no_of_combinations = lengths.reduce(
									(a, b) => a * b,
									1
								);
								me.multiple_variant_dialog
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(
				__("Create Variants"),
				() => {
					let selected_attributes = get_selected_attributes();
			...
			me.multiple_variant_dialog.$wrapper
				...
				.each((i, col) => {
					...
					checked_opts.each((i, opt) => {
						if ($(opt).is(":checked")) {
							selected_attributes[attribute_name].push(
								$(opt).attr("data-fieldname")
							);
						}
					});
				});
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on("input", function(e) {
					...
					frappe.call({
						method:
							"erpnext.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
						...
						callback: function(r) {
							if (r.message) {
								e.target.awesomplete.list = r.message.map(
									function(d) {
										return d.attribute_value;
									}
								);
