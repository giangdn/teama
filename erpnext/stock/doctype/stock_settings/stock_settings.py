...
class StockSettings(Document):
    ...
    def cant_change_valuation_method(self):
        db_valuation_method = frappe.db.get_single_value(
        ...
        if db_valuation_method and db_valuation_method != self.valuation_method:
            ...
            if sle:
                frappe.throw(
                    _("Can't change the valuation method, as there are transactions against some items which do not have its own valuation method"))
    ...
    def validate_clean_description_html(self):
        if int(self.clean_description_html or 0) \
            ...
            # changed to text
            frappe.enqueue(
                'erpnext.stock.doctype.stock_settings.stock_settings.clean_all_descriptions', now=frappe.flags.in_test)

    def on_update(self):
        self.toggle_warehouse_field_for_inter_warehouse_transfer()

    def toggle_warehouse_field_for_inter_warehouse_transfer(self):
        make_property_setter("Sales Invoice Item", "target_warehouse",
                             "hidden", 1 - cint(self.allow_from_dn), "Check")
        make_property_setter("Delivery Note Item", "target_warehouse",
                             "hidden", 1 - cint(self.allow_from_dn), "Check")
        make_property_setter("Purchase Invoice Item", "from_warehouse",
                             "hidden", 1 - cint(self.allow_from_pr), "Check")
        make_property_setter("Purchase Receipt Item", "from_warehouse",
                             "hidden", 1 - cint(self.allow_from_pr), "Check")
