...
class DeliveryTrip(Document):
    def __init__(self, *args, **kwargs):
        ...
        self.default_distance_uom = frappe.db.get_single_value(
            "Global Defaults", "default_distance_unit") or "Meter"
    ...
    def validate_stop_addresses(self):
        for stop in self.delivery_stops:
            if not stop.customer_address:
                stop.customer_address = get_address_display(
                    frappe.get_doc("Address", stop.address).as_dict())
    ...
    def update_delivery_notes(self, delete=False):
        ...
        delivery_notes = [get_link_to_form(
            "Delivery Note", note) for note in delivery_notes]
        frappe.msgprint(_("Delivery Notes {0} updated").format(
            ", ".join(delivery_notes)))
        ...
        for route in route_list:
            ...
            if directions:
                if optimize and len(directions.get("waypoint_order")) > 1:
                    self.rearrange_stops(directions.get(
                        "waypoint_order"), start=idx)

                # Avoid estimating last leg back to the home address
                legs = directions.get(
                    "legs")[:-1] if route == route_list[-1] else directions.get("legs")
                ...
                for leg in legs:
                    ...
                    departure_datetime = estimated_arrival + \
                        datetime.timedelta(minutes=cint(stop_delay))
                    idx += 1
    ...
    def form_route_list(self, optimize):
        ...
        if not self.driver_address:
            frappe.throw(
                _("Cannot Calculate Arrival Time as Driver Address is Missing."))

        home_address = get_address_display(frappe.get_doc(
            "Address", self.driver_address).as_dict())
        ...
        route_list = [[sanitize_address(address)
                       for address in route] for route in route_list]
...
def notify_customers(delivery_trip):
    ...
    if delivery_trip.driver:
        context.update(frappe.db.get_value(
            "Driver", delivery_trip.driver, "cell_number", as_dict=1))

    email_recipients = []

    for stop in delivery_trip.delivery_stops:
        contact_info = frappe.db.get_value("Contact", stop.contact, [
                                           "first_name", "last_name", "email_id"], as_dict=1)

        context.update({"items": []})
        if stop.delivery_note:
            items = frappe.get_all("Delivery Note Item", filters={
                                   "parent": stop.delivery_note, "docstatus": 1}, fields=["*"])
        ...
        if contact_info and contact_info.email_id:
            ...
            dispatch_template_name = frappe.db.get_single_value(
                "Delivery Settings", "dispatch_template")
            dispatch_template = frappe.get_doc(
                "Email Template", dispatch_template_name)
    ...
    if email_recipients:
        frappe.msgprint(_("Email sent to {0}").format(
...
def get_attachments(delivery_stop):
    ...
    dispatch_attachment = frappe.db.get_single_value(
        "Delivery Settings", "dispatch_attachment")
