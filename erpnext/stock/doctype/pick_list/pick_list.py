    ...
    def aggregate_item_qty(self):
        ...
        for item in locations:
            if not item.item_code:
                frappe.throw(
                    "Row #{0}: Item Code is Mandatory".format(item.idx))
...
def get_items_with_location_and_quantity(item_doc, item_location_map, docstatus):
    ...
    remaining_stock_qty = item_doc.qty if (
        docstatus == 1 and item_doc.stock_qty == 0) else item_doc.stock_qty
    ...
    while remaining_stock_qty > 0 and available_locations:
        ...
        if qty_diff > 0:
            ...
            if item_location.serial_no:
                ...
                item_location.serial_no = item_location.serial_no[-int(
                    qty_diff):]
...
def get_available_item_locations_for_serial_and_batched_item(item_code, from_warehouses, required_qty, company):
    ...
    for location in locations:
        ...
        location.qty = required_qty if location.qty > required_qty else location.qty
...
def get_available_item_locations_for_other_item(item_code, from_warehouses, required_qty, company):
    ...
    warehouses = [x.get('name') for x in frappe.get_list(
...
def create_stock_entry(pick_list):
    ...
    if pick_list.get('work_order'):
        ...
        stock_entry = update_stock_entry_items_with_no_reference(
            pick_list, stock_entry)
...
def get_item_details(item_code, uom=None):
    details = frappe.db.get_value(
        'Item', item_code, ['stock_uom', 'name'], as_dict=1)
...
def update_delivery_note_item(source, target, delivery_note):
    cost_center = frappe.db.get_value(
        'Project', delivery_note.project, 'cost_center')
    if not cost_center:
        cost_center = get_cost_center(
            source.item_code, 'Item', delivery_note.company)
