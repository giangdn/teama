...
class TestPickList(unittest.TestCase):
    ...
    def test_pick_list_picks_warehouse_for_each_item(self):
        ...
        self.assertEqual(
            pick_list.locations[0].item_code, '_Test Item Home Desktop 100')
        self.assertEqual(
    ...
    def test_pick_list_splits_row_according_to_warehouse_availability(self):
        ...
        self.assertEqual(
            pick_list.locations[0].item_code, '_Test Item Warehouse Group Wise Reorder')
        self.assertEqual(
            pick_list.locations[0].warehouse, '_Test Warehouse Group-C1 - _TC')
        self.assertEqual(pick_list.locations[0].qty, 5)

        self.assertEqual(
            pick_list.locations[1].item_code, '_Test Item Warehouse Group Wise Reorder')
        self.assertEqual(
            pick_list.locations[1].warehouse, '_Test Warehouse 2 - _TC')
    ...
    def test_pick_list_shows_serial_no_for_serialized_item(self):
        ...
        pick_list.set_item_locations()
        self.assertEqual(
            pick_list.locations[0].item_code, '_Test Serialized Item')
        self.assertEqual(
            pick_list.locations[0].warehouse, '_Test Warehouse - _TC')
        self.assertEqual(pick_list.locations[0].qty, 5)
        self.assertEqual(
            pick_list.locations[0].serial_no, '123450\n123451\n123452\n123453\n123454')
    ...
    def test_pick_list_shows_batch_no_for_batched_item(self):
        ...
        pr1 = make_purchase_receipt(
            item_code="Batched Item", qty=1, rate=100.0)

        pr1.load_from_db()
        oldest_batch_no = pr1.items[0].batch_no

        pr2 = make_purchase_receipt(
            item_code="Batched Item", qty=2, rate=100.0)
    ...
    def test_pick_list_for_batched_and_serialised_item(self):
        ...
        item = frappe.db.exists(
            "Item", {'item_name': 'Batched and Serialised Item'})
        if not item:
        ...
        pr1 = make_purchase_receipt(
            item_code="Batched and Serialised Item", qty=2, rate=100.0)
    ...
    def test_pick_list_for_items_from_multiple_sales_orders(self):
        ...
        self.assertEqual(pick_list.locations[0].qty, 5)
        self.assertEqual(
            pick_list.locations[0].sales_order_item, '_T-Sales Order-1_item')

        self.assertEqual(
            pick_list.locations[1].item_code, '_Test Item Home Desktop 100')
        self.assertEqual(
            pick_list.locations[1].warehouse, '_Test Warehouse - _TC')
        self.assertEqual(pick_list.locations[1].qty, 5)
        self.assertEqual(
            pick_list.locations[1].sales_order_item, sales_order.items[0].name)
