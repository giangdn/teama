...
class ItemAlternative(Document):
    ...
    def has_alternative_item(self):
        if (self.item_code and
            ...
            frappe.throw(
                _("Not allow to set alternative item for the item {0}").format(self.item_code))
    ...
    def validate_alternative_item(self):
        ...
        fields = ["is_stock_item", "include_item_in_manufacturing",
                  "has_serial_no", "has_batch_no"]
        item_data = frappe.db.get_values(
            "Item", self.item_code, fields, as_dict=1)
        alternative_item_data = frappe.db.get_values(
            "Item", self.alternative_item_code, fields, as_dict=1)

        for field in fields:
            if item_data[0].get(field) != alternative_item_data[0].get(field):
                raise_exception, alert = [
                    1, False] if field == "is_stock_item" else [0, True]
    ...
    def validate_duplicate(self):
        if frappe.db.get_value("Item Alternative", {'item_code': self.item_code,
            ...
            frappe.throw(
                _("Already record exists for the item {0}").format(self.item_code))
