...
class SerialNo(StockController):
    ...
    def validate_item(self):
        ...
        if item.has_serial_no != 1:
            frappe.throw(_("Item {0} is not setup for Serial Nos. Check Item master").format(
                self.item_code))
    ...
    if item_det.has_serial_no == 0:
        ...
        if serial_nos:
            if cint(sle.actual_qty) != flt(sle.actual_qty):
                frappe.throw(_("Serial No {0} quantity {1} cannot be a fraction").format(
                    sle.item_code, sle.actual_qty))
            ...
            if len(serial_nos) != len(set(serial_nos)):
                frappe.throw(_("Duplicate Serial No entered for Item {0}").format(
            ...
            for serial_no in serial_nos:
                if frappe.db.exists("Serial No", serial_no):
                    ...
                    if cint(sle.actual_qty) > 0 and has_serial_no_exists(sr, sle):
                        doc_name = frappe.bold(get_link_to_form(
                            sr.purchase_document_type, sr.purchase_document_no))
                    ...
                    if (sr.delivery_document_no and sle.voucher_type not in ['Stock Entry', 'Stock Reconciliation']
                        ...
                        if return_against and return_against != sr.delivery_document_no:
                            frappe.throw(
                                _("Serial no {0} has been already returned").format(sr.name))
                    ...
                    # transfer out
                    frappe.throw(_("Serial No {0} not in stock").format(
        ...
        for serial_no in serial_nos:
            sr = frappe.db.get_value("Serial No", serial_no, [
                                     "name", "warehouse"], as_dict=1)
...
def update_serial_nos(sle, item_det):
    ...
    if not sle.is_cancelled and not sle.serial_no and cint(sle.actual_qty) > 0 \
        ...
        serial_nos = get_auto_serial_nos(
            item_det.serial_no_series, sle.actual_qty)
...
def auto_make_serial_nos(args):
    ...
    form_links = list(
        map(lambda d: get_link_to_form('Serial No', d), created_numbers))
    ...
    if len(form_links) == 1:
        frappe.msgprint(_("Serial No {0} Created").format(
            form_links[0]), singular_title)
...
def update_maintenance_status():
    ...
    for serial_no in serial_nos:
        ...
        frappe.db.set_value('Serial No', doc.name,
...
def auto_fetch_serial_number(qty, item_code, warehouse, posting_date=None, batch_nos=None, for_doctype=None):
    ...
    if for_doctype == 'POS Invoice':
        ...
        serial_numbers = fetch_serial_numbers(
            filters, qty, do_not_include=reserved_sr_nos)
    else:
...
def fetch_serial_numbers(filters, qty, do_not_include=[]):
    ...
    if batch_nos:
        batch_no_condition = """and sr.batch_no in ({}) """.format(
            ', '.join(["'%s'" % d for d in batch_nos]))
    ...
    excluded_sr_nos = ", ".join(
        ["" + frappe.db.escape(sr) + "" for sr in do_not_include]) or "''"
