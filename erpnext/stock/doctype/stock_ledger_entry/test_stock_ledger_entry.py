...
class TestStockLedgerEntry(unittest.TestCase):
    def setUp(self):
        ...
        frappe.db.sql("delete from `tabStock Ledger Entry` where item_code in (%s)" % (
            ', '.join(['%s']*len(items))), items)
        frappe.db.sql("delete from `tabBin` where item_code in (%s)" %
                      (', '.join(['%s']*len(items))), items)
    ...
    def test_item_cost_reposting(self):
        ...
        repack = create_repack_entry(
            company=company, posting_date='2020-05-05', posting_time='14:00')
    ...
    def test_sales_return_valuation_reposting(self):
        ...
        lcv = create_landed_cost_voucher(
            "Purchase Receipt", pr.name, pr.company)
    ...
    def test_sub_contracted_item_costing(self):
        ...
        make_bom(item=subcontracted_item, raw_materials=[
                 rm_item_code], currency="INR")
