...
class Bin(Document):
    def before_save(self):
        if self.get("__islocal") or not self.stock_uom:
            self.stock_uom = frappe.get_cached_value(
                'Item', self.item_code, 'stock_uom')
    ...
    def update_qty(self, args):
        ...
        if args.get("voucher_type") == "Stock Reconciliation":
            ...
            self.actual_qty = flt(self.actual_qty) + \
                flt(args.get("actual_qty"))

        self.ordered_qty = flt(self.ordered_qty) + flt(args.get("ordered_qty"))
        self.reserved_qty = flt(self.reserved_qty) + \
            flt(args.get("reserved_qty"))
        self.indented_qty = flt(self.indented_qty) + \
            flt(args.get("indented_qty"))
    ...
    def update_reserved_qty_for_production(self):
        ...
        self.db_set('reserved_qty_for_production',
