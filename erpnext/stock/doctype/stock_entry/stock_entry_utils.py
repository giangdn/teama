...
def make_stock_entry(**args):
    ...
    def process_serial_numbers(serial_nos_list):
        serial_nos_list = [
            '\n'.join(serial_num['serial_no']
                      for serial_num in serial_nos_list if serial_num.serial_no)
        ]
    ...
    if not args.company:
        if args.source:
            args.company = frappe.db.get_value(
                'Warehouse', args.source, 'company')
        elif args.target:
            args.company = frappe.db.get_value(
                'Warehouse', args.target, 'company')
    ...
    if not args.cost_center:
        args.cost_center = frappe.get_value(
            'Company', s.company, 'cost_center')

    if not args.expense_account and s.is_opening == "No":
        args.expense_account = frappe.get_value(
            'Company', s.company, 'stock_adjustment_account')
