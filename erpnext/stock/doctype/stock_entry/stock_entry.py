...
class StockEntry(StockController):
    ...
    def update_cost_in_project(self):
        ...
        if self.project:
            ...
            frappe.db.set_value('Project', self.project,
                                'total_consumed_material_cost', amount)
    ...
    def validate_difference_account(self):
        ...
        for d in self.get("items"):
            if not d.expense_account:
                ...
                frappe.throw(
                    _("Difference Account must be a Asset/Liability type account, since this Stock Entry is an Opening Entry"), OpeningEntryAccountError)
    ...
    def check_if_operations_completed(self):
        ...
        for d in prod_order.get("operations"):
            ...
            if total_completed_qty > flt(completed_qty):
                ...
                work_order_link = frappe.utils.get_link_to_form(
                    'Work Order', self.work_order)
                job_card_link = frappe.utils.get_link_to_form(
                    'Job Card', job_card)
    ...
    def set_actual_qty(self):
        allow_negative_stock = cint(frappe.db.get_value(
        ...
        for d in self.get('items'):
            ...
            if d.docstatus == 1 and d.s_warehouse and not allow_negative_stock and flt(d.actual_qty, d.precision("actual_qty")) < flt(d.transfer_qty, d.precision("actual_qty")):
                frappe.throw(_("Row {0}: Quantity not available for {4} in warehouse {1} at posting time of the entry ({2} {3})").format(d.idx,
                                                                                                                                         frappe.bold(d.s_warehouse), formatdate(
                                                                                                                                             self.posting_date),
    ...
    def distribute_additional_costs(self):
        ...
        if self.purpose in ("Repack", "Manufacture"):
            incoming_items_cost = sum(
                [flt(t.basic_amount) for t in self.get("items") if t.is_finished_item])
        else:
            ...
            for row in self.items:
                if not row.subcontracted_item:
                    ...
                    po_detail = frappe.db.get_value(
                        "Purchase Order Item Supplied", filters, "name")
                    if po_detail:
    ...
    def update_work_order(self):
        def _validate_work_order(pro_doc):
            if flt(pro_doc.docstatus) != 1:
                frappe.throw(
                    _("Work Order {0} must be submitted").format(self.work_order))

            if pro_doc.status == 'Stopped':
                frappe.throw(_("Transaction not allowed against stopped Work Order {0}").format(
                    self.work_order))
    ...
    def get_item_details(self, args=None, for_update=False):
        ...
        if not item:
            frappe.throw(_("Item {0} is not active or end of life has been reached").format(
                args.get("item_code")))
        ...
        if (args.get('s_warehouse', None) and args.get('qty') and
            ...
            args.batch_no = get_batch_no(
                args['item_code'], args['s_warehouse'], args['qty'])
    ...
    def get_transfered_raw_materials(self):
        ...
        for d in materials_already_backflushed:
            backflushed_materials.setdefault(
                d.item_code, []).append({d.warehouse: d.qty})
                    ...
                    if consumed_qty != 0:
                        ...
                        if remaining_qty > exhaust_qty:
                            if (remaining_qty/(req_qty_each * flt(self.fg_completed_qty))) >= 1:
                                ...
                                qty = (
                                    req_qty_each * flt(self.fg_completed_qty)) - remaining_qty
                    else:
                        if self.flags.backflush_based_on == "Material Transferred for Manufacture":
                            qty = (item.qty/trans_qty) * \
    ...
    def get_pending_raw_materials(self, backflush_based_on=None):
        ...
        to_transfer_qty = flt(
            self.pro_doc.material_transferred_for_manufacturing) + flt(self.fg_completed_qty)
    ...
    def add_to_stock_entry_detail(self, item_dict, bom_no=None):
        for d in item_dict:
            stock_uom = item_dict[d].get(
                "stock_uom") or frappe.db.get_value("Item", d, "stock_uom")

            se_child = self.append('items')
            se_child.s_warehouse = item_dict[d].get("from_warehouse")
            se_child.t_warehouse = item_dict[d].get("to_warehouse")
            se_child.item_code = item_dict[d].get('item_code') or cstr(d)
            se_child.uom = item_dict[d]["uom"] if item_dict[d].get(
                "uom") else stock_uom
            se_child.stock_uom = stock_uom
            se_child.qty = flt(item_dict[d]["qty"], se_child.precision("qty"))
            se_child.allow_alternative_item = item_dict[d].get(
            ...
            se_child.conversion_factor = flt(
                item_dict[d].get("conversion_factor")) or 1
            se_child.transfer_qty = flt(
                item_dict[d]["qty"]*se_child.conversion_factor, se_child.precision("qty"))
    ...
    def validate_with_material_request(self):
        for item in self.get("items"):
            ...
            if self.purpose == 'Material Transfer' and self.outgoing_stock_entry:
                parent_se = frappe.get_value("Stock Entry Detail", item.ste_detail, [
                                             'material_request', 'material_request_item'], as_dict=True)
                if parent_se:
    ...
    def update_so_in_serial_number(self):
        so_name, item_code = frappe.db.get_value("Work Order", self.work_order, [
                    ...
                    for item in self.items:
                        if item.item_code == item_code:
                            ...
                            for serial_no in serial_nos:
                                if qty_to_reserve > 0:
                                    frappe.db.set_value(
                                        "Serial No", serial_no, "sales_order", so_name)
                                    qty_to_reserve -= 1
            ...
            if sample_quantity:
                ...
                if item.get('serial_no'):
                    ...
                    if serial_nos and len(serial_nos) > item.get('sample_quantity'):
                        serial_no_list = serial_nos[:-
                                                    (len(serial_nos)-item.get('sample_quantity'))]
                        sample_serial_nos = '\n'.join(serial_no_list)
...
def validate_sample_quantity(item_code, sample_quantity, qty, batch_no=None):
    if cint(qty) < cint(sample_quantity):
        frappe.throw(_("Sample quantity {0} cannot be more than received quantity {1}").format(
            sample_quantity, qty))
    retention_warehouse = frappe.db.get_single_value(
        'Stock Settings', 'sample_retention_warehouse')
    retainted_qty = 0
