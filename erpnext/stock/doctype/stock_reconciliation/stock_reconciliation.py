...
class StockReconciliation(StockController):
    def __init__(self, *args, **kwargs):
        ...
        self.head_row = ["Item Code", "Warehouse",
                         "Quantity", "Valuation Rate"]
    ...
    def update_valuation_rate_for_serial_nos(self, row, serial_nos):
        ...
        for d in serial_nos:
            frappe.db.set_value(
                "Serial No", d, 'purchase_rate', valuation_rate)
    ...
    def merge_similar_item_serial_nos(self, sl_entries):
        ...
        for d in sl_entries:
            ...
            if key not in merge_similar_entries:
                ...
                data.valuation_rate = (
                    data.valuation_rate + d.valuation_rate) / data.actual_qty
                data.serial_no += '\n' + d.serial_no

                if data.incoming_rate:
                    data.incoming_rate = (
                        data.incoming_rate + d.incoming_rate) / data.actual_qty
    ...
    def validate_expense_account(self):
        ...
        if not self.expense_account:
            ...
            if frappe.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
                frappe.throw(
                    _("Difference Account must be a Asset/Liability type account, since this Stock Reconciliation is an Opening Entry"), OpeningEntryAccountError)

    def set_total_qty_and_amount(self):
        for d in self.get("items"):
            d.amount = flt(d.qty, d.precision("qty")) * \
                flt(d.valuation_rate, d.precision("valuation_rate"))
...
def get_stock_balance_for(item_code, warehouse,
    ...
    if item_dict.get("has_batch_no"):
        qty = get_batch_qty(
            batch_no, warehouse, posting_date=posting_date, posting_time=posting_time) or 0
