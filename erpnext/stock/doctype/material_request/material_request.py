        ...
        for so_no in so_items.keys():
            for item in so_items[so_no].keys():
                ...
                if actual_so_qty and (flt(so_items[so_no][item]) + already_indented > actual_so_qty):
                    frappe.throw(_("Material Request of maximum {0} can be made for Item {1} against Sales Order {2}").format(
    ...
    def set_title(self):
        ...
        if not self.title:
            ...
            self.title = _('{0} Request for {1}').format(
                self.material_request_type, items)[:100]
        ...
        if frappe.flags.args and frappe.flags.args.default_supplier:
            ...
            for d in target_doc.items:
                default_supplier = get_item_defaults(
                    d.item_code, target_doc.company).get('default_supplier')
...
def get_material_requests_based_on_supplier(doctype, txt, searchfield, start, page_len, filters):
    ...
    if filters.get("transaction_date"):
        ...
        conditions += "and mr.transaction_date between '{0}' and '{1}' ".format(
            date[0], date[1])
...
def raise_work_orders(material_request):
    ...
    if work_orders:
        work_orders_list = [get_link_to_form(
            "Work Order", d) for d in work_orders]
