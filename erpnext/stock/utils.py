    ...
    for d in data:
        if d.actual_qty > 0:
            ...
            serial_nos = list(set(serial_nos) -
                              set(get_serial_nos_data(d.serial_no)))
...
def get_latest_stock_balance():
    ...
    for d in frappe.db.sql("""SELECT item_code, warehouse, stock_value as stock_value
        ...
        bin_map.setdefault(d.warehouse, {}).setdefault(
            d.item_code, flt(d.stock_value))
...
def update_bin(args, allow_negative_stock=False, via_landed_cost_voucher=False):
    ...
    if is_stock_item:
        ...
        frappe.msgprint(_("Item {0} ignored since it is not a stock item").format(
            args.get("item_code")))
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
    ...
    if (args.get("serial_no") or "").strip():
        ...
        if valuation_method == 'FIFO':
            if previous_sle:
                ...
                in_rate = get_fifo_rate(previous_stock_queue, args.get(
                    "qty") or 0) if previous_stock_queue else 0
    ...
    if not in_rate:
        ...
        in_rate = get_valuation_rate(args.get('item_code'), args.get('warehouse'),
                                     args.get('voucher_type'), voucher_no, args.get(
...
def get_valuation_method(item_code):
    ...
    if not val_method:
        val_method = frappe.db.get_value(
            "Stock Settings", None, "valuation_method") or "FIFO"
    return val_method
    ...
    for val in serial_nos:
        if val:
            ...
            if val in valid_serial_nos:
                frappe.throw(
                    _("Serial number {0} entered more than once").format(val))
            else:
                valid_serial_nos.append(val)

    if qty and len(valid_serial_nos) != abs(qty):
        frappe.throw(_("{0} valid serial nos for Item {1}").format(
            abs(qty), item_code))
...
def is_group_warehouse(warehouse):
    if frappe.db.get_value("Warehouse", warehouse, "is_group"):
        frappe.throw(
            _("Group node warehouse is not allowed to select for transactions"))


def validate_disabled_warehouse(warehouse):
    if frappe.db.get_value("Warehouse", warehouse, "disabled"):
        frappe.throw(_("Disabled Warehouse {0} cannot be used for this transaction.").format(
...
def add_additional_uom_columns(columns, result, include_uom, conversion_factors):
    ...
    for row_idx, row in enumerate(result):
        for convertible_col, data in convertible_column_map.items():
            ...
            if for_type == 'rate':
                row[data.converted_col] = flt(
                    value_before_conversion) * conversion_factor
            else:
                row[data.converted_col] = flt(
                    value_before_conversion) / conversion_factor
