...
class MaintenanceSchedule(TransactionBase):
    ...
    def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
        ...
        employee = frappe.db.get_value(
            "Sales Person", sales_person, "employee")
        if employee:
    ...
    def validate_maintenance_detail(self):
        ...
        for d in self.get('items'):
            if not d.item_code:
                ...
                throw(_("Please select Start Date and End Date for Item {0}").format(
                    d.item_code))
            elif not d.no_of_visits:
                throw(_("Please mention no of visits required"))
            elif not d.sales_person:
                throw(_("Please select a Sales Person for item: {0}").format(
                    d.item_name))

            if getdate(d.start_date) >= getdate(d.end_date):
                throw(_("Start date should be less than end date for Item {0}").format(
                    d.item_code))
    ...
    def validate_sales_order(self):
        for d in self.get('items'):
            if d.sales_order:
                ...
                if chk:
                    throw(_("Maintenance Schedule {0} exists against {1}").format(
                        chk[0][0], d.sales_order))
        ...
        for m in self.get('schedules'):
            if serial_present:
                if m.item_code in serial_present and not m.serial_no:
                    throw(_("Please click on 'Generate Schedule' to fetch Serial No added for Item {0}").format(
                        m.item_code))
