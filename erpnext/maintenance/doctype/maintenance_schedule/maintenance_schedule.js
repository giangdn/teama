			...
			this.frm.add_custom_button(
				...
				function() {
					erpnext.utils.map_current_doc({
						method:
							"erpnext.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
	...
	set_no_of_visits: function(doc, cdt, cdn) {
		...
		if (item.start_date && item.end_date && item.periodicity) {
			if (item.start_date > item.end_date) {
				frappe.msgprint(
					__("Row {0}:Start Date must be before End Date", [item.idx])
				);
				return;
			}

			var date_diff =
				frappe.datetime.get_diff(item.end_date, item.start_date) + 1;
			...
			var no_of_visits = cint(
				date_diff / days_in_period[item.periodicity]
			);
