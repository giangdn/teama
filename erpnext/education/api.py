...
def enroll_student(source_name):
    ...
    program_enrollment.program = frappe.db.get_value(
        "Student Applicant", source_name, "program")
    frappe.publish_realtime('enroll_student_progress', {
...
def get_fee_components(fee_structure):
    ...
    """
    if fee_structure:
        fs = frappe.get_list("Fee Component", fields=["fees_category", "description", "amount"], filters={
                             "parent": fee_structure}, order_by="idx")
        return fs
...
def get_grade(grading_scale, percentage):
    ...
    if not hasattr(frappe.local, 'grading_scale'):
        grading_scale = frappe.get_all("Grading Scale Interval", fields=[
                                       "grade_code", "threshold"], filters={"parent": grading_scale})
