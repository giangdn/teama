...
def execute(filters=None):
    ...
    for student in result_dict:
        ...
        for criteria in assessment_criteria_dict:
            ...
            if criteria in result_dict[student][args.course][args.assessment_group]:
                ...
                student_row[scrub_criteria +
                            "_score"] = result_dict[student][args.course][args.assessment_group][criteria]["score"]
    ...
    chart = get_chart_data(
        grades, assessment_criteria_list, grade_wise_analysis)
...
def get_formatted_result(args, get_assessment_criteria=False, get_course=False, get_all_assessment_groups=False):
    ...
    if args.students:
        cond4 = " and ar.student in (%s)" % (
            ', '.join(['%s']*len(args.students)))
    ...
    def add_score_and_recalculate_grade(result, assessment_group, assessment_criteria):
        ...
        formatted_assessment_result[result.student][result.course][
            assessment_group][assessment_criteria]["score"] += result.score
        ...
        formatted_assessment_result[result.student][result.course][assessment_group][assessment_criteria]["grade"] = tmp_grade
    ...
    def add_total_score(result, assessment_group):
        if "Final Grade" not in formatted_assessment_result[result.student][result.course][assessment_group]:
            ...
            add_score_and_recalculate_grade(
                result, assessment_group, "Final Grade")
    ...
    for result in assessment_result:
        ...
        if not formatted_assessment_result[result.student][result.course]:
            formatted_assessment_result[result.student][result.course] = defaultdict(
                dict)

        if not create_total_dict:
            formatted_assessment_result[result.student][result.course][
                result.assessment_group][result.assessment_criteria] = assessment_criteria_details
        ...
        # create the total of all the assessment groups criteria-wise
            ...
            if not formatted_assessment_result[result.student][result.course][args.assessment_group]:
                formatted_assessment_result[result.student][result.course][args.assessment_group] = defaultdict(
                    dict)
                formatted_assessment_result[result.student][result.course][
                    args.assessment_group][result.assessment_criteria] = assessment_criteria_details
        ...
        total_maximum_score = formatted_assessment_result[result.student][
            result.course][args.assessment_group]["Final Grade"]["maximum_score"]
        if get_assessment_criteria:
            assessment_criteria_dict[result.assessment_criteria] = formatted_assessment_result[result.student][
...
def get_child_assessment_groups(assessment_group):
    assessment_groups = []
    group_type = frappe.get_value(
        "Assessment Group", assessment_group, "is_group")
    if group_type:
