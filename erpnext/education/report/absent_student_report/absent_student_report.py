...
def execute(filters=None):
    ...
    if is_holiday(holiday_list, filters.get("date")):
        msgprint(_("No attendance has been marked for {0} as it is a Holiday").format(
    ...
    for student in absent_students:
        if not student.student in leave_applicants:
            ...
            stud_details = frappe.db.get_value("Student", student.student, [
                                               'student_email_id', 'student_mobile_number'], as_dict=True)
...
def get_transportation_details(date, student_list):
    academic_year = frappe.get_all("Academic Year", filters=[
    ...
    for d in transportation_details:
        transportation_map[d.student] = [
            d.mode_of_transportation, d.vehicle_no]
