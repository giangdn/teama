...
def preview_report_card(doc):
    ...
    letterhead = get_letter_head(frappe._dict(
        {"letter_head": doc.letterhead}), not doc.add_letterhead)
...
def get_attendance_count(student, academic_year, academic_term=None):
    if academic_year:
        from_date, to_date = frappe.db.get_value("Academic Year", academic_year, [
                                                 "year_start_date", "year_end_date"])
    elif academic_term:
        from_date, to_date = frappe.db.get_value("Academic Term", academic_term, [
    ...
    if from_date and to_date:
        ...
        frappe.throw(
            _("Provide the academic year and set the starting and ending date."))
