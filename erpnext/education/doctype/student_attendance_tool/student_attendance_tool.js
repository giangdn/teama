...
frappe.ui.form.on("Student Attendance Tool", {
	...
	refresh: function(frm) {
		if (frappe.route_options) {
			...
			frm.set_value(
				"course_schedule",
				frappe.route_options.course_schedule
			);
	...
	student_group: function(frm) {
		if (
			(frm.doc.student_group && frm.doc.date) ||
			frm.doc.course_schedule
		) {
			var method =
				"erpnext.education.doctype.student_attendance_tool.student_attendance_tool.get_student_attendance_records";
	...
	make: function(frm, students) {
		...
		student_toolbar
			...
			.on("click", function() {
				...
				var studs = [];
				$(me.wrapper.find('input[type="checkbox"]')).each(function(
					i,
					check
				) {
					var $check = $(check);
					...
					function() {
						...
						if (!frappe.request.ajax_count) {
							frappe.call({
								...
								callback: function(r) {
									$(me.wrapper.find(".btn-mark-att")).attr(
										"disabled",
										false
									);
									frm.trigger("student_group");
