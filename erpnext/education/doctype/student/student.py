...
class Student(Document):
    ...
    def check_unique(self):
        ...
        student = frappe.db.sql(
            "select name from `tabStudent` where student_applicant=%s and name!=%s", (self.student_applicant, self.name))
        if student:
            frappe.throw(_("Student {0} exist against student applicant {1}").format(
    ...
    def get_all_course_enrollments(self):
        ...
        course_enrollments = frappe.get_all("Course Enrollment", filters={
                                            "student": self.name}, fields=['course', 'name'])
        if not course_enrollments:
            return None
        else:
            enrollments = {item['course']: item['name']
                           for item in course_enrollments}
            return enrollments

    def get_program_enrollments(self):
        """Returns a list of course enrollments linked with the current student"""
        program_enrollments = frappe.get_all("Program Enrollment", filters={
                                             "student": self.name}, fields=['program'])
        ...
        if contents:
            for content in contents:
                if content.doctype in ('Article', 'Video'):
                    ...
                    progress.append({'content': content.name, 'content_type': content.doctype,
                                     'is_complete': status, 'score': score, 'result': result})
        return progress
    ...
    def enroll_in_program(self, program_name):
        try:
            ...
            enrollment_name = frappe.get_list("Program Enrollment", filters={
                                              "student": self.name, "Program": program_name})[0].name
    ...
    def enroll_in_course(self, course_name, program_enrollment, enrollment_date=frappe.utils.datetime.datetime.now()):
        try:
            ...
            enrollment_name = frappe.get_list("Course Enrollment", filters={
                                              "student": self.name, "course": course_name, "program_enrollment": program_enrollment})[0].name
