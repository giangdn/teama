...
class StudentGroup(Document):
    ...
    def validate_strength(self):
        ...
        if self.max_strength and len(self.students) > self.max_strength:
            frappe.throw(_("""Cannot enroll more than {0} students for this student group.""").format(
                self.max_strength))

    def validate_students(self):
        program_enrollment = get_program_enrollment(
            self.academic_year, self.academic_term, self.program, self.batch, self.student_category, self.course)
        students = [
        ...
        for d in self.students:
            ...
            if (self.group_based_on == "Batch") and cint(frappe.defaults.get_defaults().validate_batch)\
                ...
                frappe.throw(_("{0} - {1} is not enrolled in the Batch {2}").format(
            ...
            if (self.group_based_on == "Course") and cint(frappe.defaults.get_defaults().validate_course)\
                    and (d.student not in students):
                frappe.throw(_("{0} - {1} is not enrolled in the Course {2}").format(
