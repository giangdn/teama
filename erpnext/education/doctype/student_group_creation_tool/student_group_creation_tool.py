# -*- coding: utf-8 -*-
# Copyright (c) 2015, TeamA and contributors
...
class StudentGroupCreationTool(Document):
    def get_courses(self):
        ...
        for batch in batches:
            group_list.append(
                {"group_based_on": "Batch", "batch": batch.batch})
        ...
        for group in group_list:
            if group.get("group_based_on") == "Batch":
                student_group_name = self.program + "/" + group.get("batch") + "/" + (
                ...
                student_group_name = group.get("course") + "/" + self.program + ("/" + group.get("batch") if group.get(
                    "batch") else "") + "/" + (self.academic_term if self.academic_term else self.academic_year)
                group.update({"student_group_name": student_group_name})
    ...
    def create_student_groups(self):
        ...
        for d in self.courses:
            if not d.student_group_name:
                frappe.throw(
                    _("""Student Group Name is mandatory in row {0}""".format(d.idx)))

            if d.group_based_on == "Course" and not d.course:
                frappe.throw(
                    _("""Course is mandatory in row {0}""".format(d.idx)))

            if d.group_based_on == "Batch" and not d.batch:
                frappe.throw(
                    _("""Batch is mandatory in row {0}""".format(d.idx)))

            frappe.publish_realtime('student_group_creation_progress', {
                                    "progress": [d.idx, l]}, user=frappe.session.user)
