...
class Guardian(Document):
    ...
    def load_students(self):
        ...
        self.students = []
        students = frappe.get_all("Student Guardian", filters={
                                  "guardian": self.name}, fields=["parent"])
...
def invite_guardian(guardian):
    ...
    if not guardian_doc.email_address:
        ...
        guardian_as_user = frappe.get_value(
            'User', dict(email=guardian_doc.email_address))
        if guardian_as_user:
            frappe.msgprint(_("User {0} already exists").format(
            ...
            frappe.msgprint(_("User {0} created").format(
                getlink("User", user.name)))
            return user.name
