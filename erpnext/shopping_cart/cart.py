        ...
        for item in sales_order.get("items"):
            ...
            if is_stock_item:
                ...
                if item.qty > item_stock.stock_qty[0][0]:
                    throw(_("Only {0} in Stock for item {1}").format(
...
def update_cart_address(address_type, address_name):
    ...
    if address_type.lower() == "billing":
        ...
        address_doc = next(
            (doc for doc in get_billing_addresses() if doc["name"] == address_name), None)
...
def _get_cart_quotation(party=None):
    ...
    quotation = frappe.get_all("Quotation", fields=["name"], filters={"party_name": party.name, "order_type": "Shopping Cart", "docstatus": 0},
...
def get_debtors_account(cart_settings):
    ...
    payment_gateway_account_currency = \
        frappe.get_doc("Payment Gateway Account",
                       cart_settings.payment_gateway_account).currency
