...
def get_product_info_for_website(item_code, skip_quotation_creation=False):
    ...
    selling_price_list = cart_quotation.get(
        "selling_price_list") if cart_quotation else _set_price_list(cart_settings, None)
    ...
    if product_info["price"]:
        if frappe.session.user != "Guest":
            item = cart_quotation.get(
                {"item_code": item_code}) if cart_quotation else None
            if item:
...
def set_product_info_for_website(item):
    ...
    if product_info:
        ...
        if product_info.get("price"):
            ...
            item["price_sales_uom"] = product_info.get(
                "price").get("formatted_price_sales_uom")
        else:
