...
def execute():
    for company in frappe.db.sql("select name, expenses_included_in_valuation from tabCompany", as_dict=1):
        frozen_date = get_frozen_date(
            company.name, company.expenses_included_in_valuation)
                    ...
                    if len(expense_account) == 1:
                        ...
                        for item in purchase_invoice.items:
                            if not item.expense_account:
                                item.db_set(
                                    "expense_account", expense_account, update_modified=False)
...
def get_frozen_date(company, account):
    ...
    last_adjustment_date = cstr(
        last_adjustment_entry[0][0]) if last_adjustment_entry else None
    ...
    last_closing_date = cstr(
        last_closing_entry[0][0]) if last_closing_entry else None

    frozen_date = max(
        [accounts_frozen_upto, last_adjustment_date, last_closing_date])
