# Copyright (c) 2019, TeamA and Contributors
...
def execute():
    if "leave_policy" in frappe.db.get_table_columns("Employee"):
        employees_with_leave_policy = frappe.db.sql(
            "SELECT name, leave_policy FROM `tabEmployee` WHERE leave_policy IS NOT NULL and leave_policy != ''", as_dict=1)
        ...
        for employee in employees_with_leave_policy:
            alloc = frappe.db.exists("Leave Allocation", {
    ...
    if "default_leave_policy" in frappe.db.get_table_columns("Employee"):
        employee_grade_with_leave_policy = frappe.db.sql(
            "SELECT name, default_leave_policy FROM `tabEmployee Grade` WHERE default_leave_policy IS NOT NULL and default_leave_policy!=''", as_dict=1)
        ...
        for grade in employee_grade_with_leave_policy:
            ...
            for employee in employees:
                ...
                if employee not in employee_with_assignment:  # Will ensure no duplicate
                    ...
                    if not alloc:
                        create_assignment(
                            employee.name, grade.default_leave_policy)
    ...
    leave_allocations = frappe.db.sql(
        "SELECT leave_policy, leave_period, employee FROM `tabLeave Allocation` WHERE leave_policy IS NOT NULL and leave_policy != '' and docstatus = 1 ", as_dict=1)
...
def create_assignment(employee, leave_policy, leave_period=None, allocation_exists=False):
    ...
    if not frappe.db.exists("Leave Policy Assignment", filters):
        ...
        if allocation_exists:
            ...
            frappe.db.sql(
                "Update `tabLeave Allocation` set leave_policy_assignment = %s", lpa.name)
