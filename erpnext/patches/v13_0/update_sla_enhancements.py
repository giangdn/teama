...
def execute():
    ...
    if frappe.db.exists('DocType', 'Service Level Agreement'):
        sla_details = frappe.db.get_all('Service Level Agreement', fields=[
                                        'name', 'service_level'])
        ...
        for entry in sla_details:
            values = frappe.db.get_value('Service Level', entry.service_level, [
        ...
        if frappe.db.exists('DocType', 'Service Level'):
            service_levels = frappe.db.get_all('Service Level', filters={
                                               'service_level': ('not in', sl)}, fields=['*'])
