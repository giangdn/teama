...
def create_receivable_payable_account():
    ...
    def _create_account(args):
        if args["parent_account"] and frappe.db.exists("Account", args["parent_account"]):
            ...
            receivable_payable_accounts.setdefault(
                args["company"], {}).setdefault(args["account_type"], account_id)
...
def set_party_in_jv_and_gl_entry(receivable_payable_accounts):
    ...
    for dt in ["Journal Entry Account", "GL Entry"]:
        ...
        for i, d in enumerate(records):
            ...
            account_type = "Receivable" if account_details.get(
                "master_type") == "Customer" else "Payable"
            new_account = receivable_payable_accounts[account_details.get(
