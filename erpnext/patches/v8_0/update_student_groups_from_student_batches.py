        ...
        for student_batch in student_batches:
            if frappe.db.exists("Student Group", student_batch.get("name")):
                ...
                if frappe.db.table_exists("Student Batch Student"):
                    ...
                    student_list = list(
                        set(batch_student_list)-set(current_student_list))
                    if student_list:
                        student_group.extend(
                            "students", [{"student": d} for d in student_list])
                ...
                if frappe.db.table_exists("Student Batch Instructor"):
                    ...
                    instructor_list = list(
                        set(batch_instructor_list)-set(current_instructor_list))
                    if instructor_list:
                        student_group.extend(
                            "instructors", [{"instructor": d} for d in instructor_list])
