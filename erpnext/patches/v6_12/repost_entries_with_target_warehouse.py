...
def repost_dn(dn_failed_list):
    ...
    for dn in dn_list:
        ...
        try:
            ...
            frappe.db.sql(
                "update `tabDelivery Note Item` set target_warehouse='' where parent=%s", dn.name)
            dn_doc = frappe.get_doc("Delivery Note", dn.name)
            dn_doc.docstatus = 1
            dn_doc.on_submit()
            frappe.db.commit()
    ...
    frappe.db.sql(
        "update `tabDelivery Note Item` set target_warehouse='' where docstatus=0")
...
def repost_si(si_failed_list):
    ...
    for si in si_list:
        ...
        try:
            ...
            frappe.db.sql(
                "update `tabSales Invoice Item` set target_warehouse='' where parent=%s", si.name)
            si_doc = frappe.get_doc("Sales Invoice", si.name)
            si_doc.docstatus = 1
            si_doc.update_stock_ledger()
    ...
    frappe.db.sql(
        "update `tabSales Invoice Item` set target_warehouse='' where docstatus=0")
