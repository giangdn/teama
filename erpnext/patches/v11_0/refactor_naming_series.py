...
def set_series(doctype, options, default):
    def _make_property_setter(property_name, value):
        ...
        if property_setter:
            ...
            make_property_setter(doctype, "naming_series",
                                 "options", value, "Text")
...
def get_series():
    ...
    for doctype in doctype_series_map:
        ...
        if not frappe.get_meta(doctype).has_field('naming_series'):
            continue
        series_to_preserve = list(
            filter(None, get_series_to_preserve(doctype)))
        default_series = get_default_series(doctype)

        if not series_to_preserve:
            continue
        existing_series = (frappe.get_meta(doctype).get_field(
            "naming_series").options or "").split("\n")
        existing_series = list(
            filter(None, [d.strip() for d in existing_series]))
        ...
        if series_to_preserve:
            series_to_set[doctype] = {"options": "\n".join(
                series_to_preserve), "default": default_series}
...
def get_series_to_preserve(doctype):
    series_to_preserve = frappe.db.sql_list(
        """select distinct naming_series from `tab{doctype}` where ifnull(naming_series, '') != ''""".format(doctype=doctype))
