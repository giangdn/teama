    ...
    if len(companies) == 1:
        ...
        for item_group in item_groups:
            ...
            def insert_into_item_defaults(doc_field_name, doc_field_value, company):
                ...
                if(len(doc_field_name) > 1):
                    item_group_defaults[len(
                        item_group_defaults)-1][doc_field_name[1]] = doc_field_value
            ...
            for d in [
                ...
                if item_group.get(d[0]):
                    company = frappe.get_value(
                        d[1], item_group.get(d[0]), "company", cache=True)
                    doc_field_name = item_group_dict.get(d[0])

                    insert_into_item_defaults(
                        doc_field_name, item_group.get(d[0]), company)
