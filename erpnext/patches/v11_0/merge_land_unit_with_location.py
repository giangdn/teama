...
def execute():
    ...
    if frappe.db.table_exists('Linked Land Unit') and not frappe.db.table_exists('Linked Location'):
        frappe.rename_doc('DocType', 'Linked Land Unit',
                          'Linked Location', force=True)
    ...
    if not frappe.db.exists("Location", "All Land Units"):
        frappe.get_doc({"doctype": "Location", "is_group": True,
                        "location_name": "All Land Units"}).insert(ignore_permissions=True)
