    ...
    for attribute, value in args.items():
        ...
        if attribute.lower() in numeric_values:
            ...
            validate_is_incremental(
                numeric_attribute, attribute, value, item.name)
...
def validate_is_incremental(numeric_attribute, attribute, value, item):
    ...
    if increment == 0:
        ...
        frappe.throw(
            _("Increment for Attribute {0} cannot be 0").format(attribute))

    is_in_range = from_range <= flt(value) <= to_range
    precision = max(
        len(cstr(v).split(".")[-1].rstrip("0")) for v in (value, increment))
...
def validate_item_attribute_value(attributes_list, attribute, attribute_value, item, from_variant=True):
    ...
    if allow_rename_attribute_value:
        ...
        if from_variant:
            ...
            msg += "<br>" + _("To still proceed with editing this Attribute Value, enable {0} in Item Variant Settings.").format(
                frappe.bold("Allow Rename Attribute Value"))

            frappe.throw(msg, InvalidItemAttributeValueError,
    ...
    if not frappe.flags.attribute_values:
        ...
        for t in frappe.get_all("Item Attribute Value", fields=["parent", "attribute_value"]):
            attribute_values.setdefault(
                t.parent.lower(), []).append(t.attribute_value)

        for t in frappe.get_all('Item Variant Attribute',
                                fields=["attribute", "from_range",
                                        "to_range", "increment"],
...
def find_variant(template, args, variant_item_code=None):
    ...
    possible_variants = [i for i in get_item_codes_by_attributes(
        args, template) if i != variant_item_code]
...
def generate_keyed_value_combinations(args):
    ...
    # ]
    key_value_lists = [[(key, val) for val in args[key]]
...
def copy_attributes_to_variant(item, variant):
    ...
    allow_fields = [d.field_name for d in frappe.get_all(
        "Variant Field", fields=['field_name'])]
        ...
        if item.variant_based_on == 'Item Attribute':
            if variant.attributes:
                ...
                for d in variant.attributes:
                    attributes_description += "<div>" + d.attribute + \
                        ": " + cstr(d.attribute_value) + "</div>"
...
def make_variant_item_code(template_item_code, template_item_name, variant):
    ...
    for attr in variant.attributes:
        ...
        abbr_or_value = cstr(
            attr.attribute_value) if item_attribute[0].numeric_values else item_attribute[0].abbr
...
def create_variant_doc_for_quick_entry(template, args):
    variant_based_on = frappe.db.get_value(
        "Item", template, "variant_based_on")
    args = json.loads(args)
