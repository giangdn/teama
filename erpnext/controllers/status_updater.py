...
class StatusUpdater(Document):
        ...
        for args in self.status_updater:
            ...
            for d in self.get_all_children():
                if hasattr(d, 'qty') and d.qty < 0 and not self.get('is_return'):
                    frappe.throw(
                        _("For an item {0}, quantity must be positive number").format(d.item_code))

                if hasattr(d, 'qty') and d.qty > 0 and self.get('is_return'):
                    frappe.throw(
                        _("For an item {0}, quantity must be negative number").format(d.item_code))
                ...
                if d.doctype == args['source_dt'] and d.get(args["join_field"]):
                    ...
                    if item:
                        item = item[0]
                        item['idx'] = d.idx
                        item['target_ref_field'] = args['target_ref_field'].replace(
                            '_', ' ')
    ...
    def check_overflow_with_allowance(self, item, args):
        ...
        if overflow_percent - allowance > 0.01:
            item['max_allowed'] = flt(
                item[args['target_ref_field']] * (100+allowance)/100)
    ...
    def update_billing_status_for_zero_amount_refdoc(self, ref_dt):
        ...
        ref_docs = [item.get(ref_fieldname) for item in (
            self.get('items') or []) if item.get(ref_fieldname)]
        if not ref_docs:
            return
        ...
        if zero_amount_refdocs:
            self.update_billing_status(
                zero_amount_refdocs, ref_dt, ref_fieldname)
...
def get_allowance_for(item_code, item_allowance={}, global_qty_allowance=None, global_amount_allowance=None, qty_or_amount="qty"):
    ...
    if qty_or_amount == "qty" and not qty_allowance:
        if global_qty_allowance == None:
            global_qty_allowance = flt(frappe.db.get_single_value(
                'Stock Settings', 'over_delivery_receipt_allowance'))
        qty_allowance = global_qty_allowance
    elif qty_or_amount == "amount" and not over_billing_allowance:
        if global_amount_allowance == None:
            global_amount_allowance = flt(frappe.db.get_single_value(
                'Accounts Settings', 'over_billing_allowance'))
        over_billing_allowance = global_amount_allowance

    if qty_or_amount == "qty":
        allowance = qty_allowance
        item_allowance.setdefault(
            item_code, frappe._dict()).setdefault("qty", qty_allowance)
    else:
        allowance = over_billing_allowance
        item_allowance.setdefault(item_code, frappe._dict()).setdefault(
