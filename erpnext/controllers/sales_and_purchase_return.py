    ...
    if not frappe.db.exists(doc.doctype, doc.return_against):
        ...
        if ref_doc.company == doc.company and ref_doc.get(party_type) == doc.get(party_type) and ref_doc.docstatus == 1:
            ...
            if get_datetime(return_posting_datetime) < get_datetime(ref_posting_datetime):
                frappe.throw(_("Posting timestamp must be after {0}").format(
        ...
        if d.item_code and (flt(d.qty) < 0 or flt(d.get('received_qty')) < 0):
            if d.item_code not in valid_items:
                ...
                if ref.rate and doc.doctype in ("Delivery Note", "Sales Invoice") and flt(d.rate) > ref.rate:
                    ...
                    if not d.serial_no:
                        frappe.throw(
                            _("Row # {0}: Serial No is mandatory").format(d.idx))
                    else:
...
def validate_quantity(doc, args, ref, valid_items, already_returned_items):
    ...
    for column in fields:
        returned_qty = flt(already_returned_data.get(column, 0)) if len(
            already_returned_data) > 0 else 0
        ...
        if column == 'stock_qty':
            ...
            current_stock_qty = args.get(
                column) * args.get("conversion_factor", 1.0)

        max_returnable_qty = flt(
            reference_qty, stock_qty_precision) - returned_qty
...
def get_returned_qty_map_for_row(row_name, doctype):
    ...
    reference_field = "dn_detail" if doctype == "Delivery Note" else frappe.scrub(
        child_doctype)
    ...
    if doctype in ("Purchase Receipt", "Purchase Invoice"):
        fields += [
            "sum(abs(`tab{0}`.rejected_qty)) as rejected_qty".format(
                child_doctype),
            "sum(abs(`tab{0}`.received_qty)) as received_qty".format(
                child_doctype)
        ]

        if doctype == "Purchase Receipt":
            fields += ["sum(abs(`tab{0}`.received_stock_qty)) as received_stock_qty".format(
                child_doctype)]
...
def make_return_doc(doctype, source_name, target_doc=None):
    ...
    def update_item(source_doc, target_doc, source_parent):
        ...
        if source_doc.serial_no:
            ...
            serial_nos = list(
                set(get_serial_nos(source_doc.serial_no)) - set(returned_serial_nos))
            if serial_nos:
...
def get_rate_for_return(voucher_type, voucher_no, item_code, return_against=None, item_row=None, voucher_detail_no=None):
    if not return_against:
        return_against = frappe.get_cached_value(
            voucher_type, voucher_no, "return_against")
