...
class StockController(AccountsController):
    ...
    def update_stock_ledger_entries(self, sle):
        ...
        sle.stock_value = flt(sle.qty_after_transaction) * \
            flt(sle.valuation_rate)
        sle.stock_value_difference = flt(
            sle.actual_qty) * flt(sle.valuation_rate)
    ...
    def delete_auto_created_batches(self):
        for d in self.items:
            ...
            if serial_nos:
                frappe.db.set_value(
                    "Serial No", {'name': ['in', serial_nos]}, "batch_no", None)
...
def check_if_future_sle_exists(args):
    ...
    distinct_item_warehouses = list(
        set([(d.item_code, d.warehouse) for d in sl_entries]))
