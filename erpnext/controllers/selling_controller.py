...
class SellingController(StockController):
    ...
    def validate_selling_price(self):
        ...
        for it in self.get("items"):
            ...
            if flt(it.base_net_rate) < flt(last_purchase_rate_in_sales_uom):
                throw_message(it.idx, frappe.bold(
                    it.item_name), last_purchase_rate_in_sales_uom, "last purchase rate")
            ...
            if last_valuation_rate:
                ...
                if is_stock_item and flt(it.base_net_rate) < flt(last_valuation_rate_in_sales_uom) \
                    ...
                    throw_message(it.idx, frappe.bold(
                        it.item_name), last_valuation_rate_in_sales_uom, "valuation rate")
    ...
    def get_po_nos(self, ref_doctype, ref_fieldname, po_nos):
        ...
        if doc_list:
            po_nos += [d.po_no for d in frappe.get_all(ref_doctype, 'po_no', filters={
                                                       'name': ('in', doc_list)}) if d.get('po_no')]
        ...
        for d in self.get('items'):
            if self.doctype == "Sales Invoice":
                stock_items = [d.item_code, d.description, d.warehouse,
                               d.sales_order or d.delivery_note, d.batch_no or '']
                non_stock_items = [d.item_code, d.description,
                                   d.sales_order or d.delivery_note]
            elif self.doctype == "Delivery Note":
                stock_items = [d.item_code, d.description, d.warehouse,
                               d.against_sales_order or d.against_sales_invoice, d.batch_no or '']
                non_stock_items = [d.item_code, d.description,
