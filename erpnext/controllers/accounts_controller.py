...
force_item_fields = ("item_group", "brand", "stock_uom",
                     "is_fixed_asset", "item_tax_rate", "pricing_rules")
...
class AccountsController(TransactionBase):
    ...
    def on_trash(self):
        ...
        if frappe.db.get_single_value('Accounts Settings', 'delete_linked_ledger_entries'):
            ...
            frappe.db.sql(
                "delete from `tabStock Ledger Entry` where voucher_type=%s and voucher_no=%s", (self.doctype, self.name))
        ...
        if hasattr(self, "items"):
            ...
            for item in self.get("items"):
                if item.get("item_code"):
                    ...
                    args["ignore_pricing_rule"] = self.ignore_pricing_rule if hasattr(
                    ...
                    if self.doctype in ["Purchase Invoice", "Sales Invoice"] and item.meta.get_field('is_fixed_asset'):
                        item.set('is_fixed_asset', ret.get(
                            'is_fixed_asset', 0))
    ...
    def append_taxes_from_master(self, tax_master_doctype=None):
        if self.get("taxes_and_charges"):
            ...
            self.extend("taxes", get_taxes_and_charges(
    ...
    def validate_tax_account_company(self):
        for d in self.get("taxes"):
            if d.account_head:
                tax_account_company = frappe.db.get_value(
    ...
    def clear_unallocated_advances(self, childtype, parentfield):
        self.set(parentfield, self.get(parentfield, {
    ...
    def is_inclusive_tax(self):
        is_inclusive = cint(frappe.db.get_single_value(
            "Accounts Settings", "show_inclusive_tax_in_print"))
    ...
    def raise_missing_debit_credit_account_error(self, party_type, party):
        ...
        db_or_cr = frappe.bold(
            "Debit To") if self.doctype == "Sales Invoice" else frappe.bold("Credit To")
        ...
        message += "<br><ul><li>" + \
            _("'Account' in the Accounting section of Customer {0}").format(
                link_to_party) + "</li>"
        message += "<li>" + _("'Default {0} Account' in Company {1}").format(
            rec_or_pay, link_to_company) + "</li></ul>"
        ...
        if not self.get("payment_schedule"):
            if self.get("payment_terms_template"):
                data = get_payment_terms(
                    self.payment_terms_template, posting_date, grand_total)
                for item in data:
                    self.append("payment_schedule", item)
            else:
                data = dict(due_date=due_date, invoice_portion=100,
    ...
    def validate_payment_schedule_amount(self):
        ...
        if self.get("payment_schedule"):
            ...
            if party_account_currency == self.company_currency:
                ...
                grand_total = flt(self.get(
                    "base_rounded_total") or self.base_grand_total, self.precision('base_grand_total'))
            else:
                total = flt(total, self.precision("grand_total"))
                grand_total = flt(
                    self.get("rounded_total") or self.grand_total, self.precision('grand_total'))
...
def validate_taxes_and_charges(tax):
    if tax.charge_type in ['Actual', 'On Net Total'] and tax.row_id:
        ...
        if cint(tax.idx) == 1:
            ...
            frappe.throw(_("Please specify a valid Row ID for row {0} in table {1}").format(
...
def validate_inclusive_tax(tax, doc):
    ...
    if cint(getattr(tax, "included_in_print_rate", None)):
        if tax.charge_type == "Actual":
            ...
            throw(_("Charge of type 'Actual' in row {0} cannot be included in Item Rate").format(
                tax.idx))
...
def get_advance_journal_entries(party_type, party, party_account, amount_field,
    ...
    reference_condition = " and (" + " or ".join(conditions) + \
        ")" if conditions else ""
...
def get_payment_terms(terms_template, posting_date=None, grand_total=None, bill_date=None):
    ...
    for d in terms_doc.get("terms"):
        term_details = get_payment_term_details(
            d, posting_date, grand_total, bill_date)
...
def set_order_defaults(parent_doctype, parent_doctype_name, child_doctype, child_docname, trans_item):
    ...
    conversion_factor = flt(get_conversion_factor(
        item.item_code, child_item.uom).get("conversion_factor"))
    child_item.conversion_factor = flt(trans_item.get(
        'conversion_factor')) or conversion_factor
    ...
    for d in deleted_children:
        if parent.doctype == "Sales Order":
            if flt(d.delivered_qty):
                frappe.throw(_("Row #{0}: Cannot delete item {1} which has already been delivered").format(
                    d.idx, d.item_code))
            if flt(d.work_order_qty):
                frappe.throw(_("Row #{0}: Cannot delete item {1} which has work order assigned to it.").format(
                    d.idx, d.item_code))
            if flt(d.ordered_qty):
                frappe.throw(_("Row #{0}: Cannot delete item {1} which is assigned to customer's purchase order.").format(
                    d.idx, d.item_code))

        if parent.doctype == "Purchase Order" and flt(d.received_qty):
            frappe.throw(_("Row #{0}: Cannot delete item {1} which has already been received").format(
                d.idx, d.item_code))

        if flt(d.billed_amt):
            frappe.throw(_("Row #{0}: Cannot delete item {1} which has already been billed.").format(
                d.idx, d.item_code))
    ...
    for d in data:
        ...
        if not d.get("docname"):
            ...
            prev_rate, new_rate = flt(
                child_item.get("rate")), flt(d.get("rate"))
            prev_qty, new_qty = flt(child_item.get("qty")), flt(d.get("qty"))
            prev_con_fac, new_con_fac = flt(child_item.get(
                "conversion_factor")), flt(d.get("conversion_factor"))
            ...
            date_unchanged = prev_date == new_date if prev_date and new_date else False
        ...
        if d.get("uom"):
            ...
            conversion_factor = flt(get_conversion_factor(
                child_item.item_code, child_item.uom).get("conversion_factor"))
            child_item.conversion_factor = flt(
                d.get('conversion_factor'), conv_fac_precision) or conversion_factor
