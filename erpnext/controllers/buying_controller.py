...
class BuyingController(StockController):
    ...
    def validate_from_warehouse(self):
        for item in self.get('items'):
            ...
            if item.get('from_warehouse') and self.get('is_subcontracted') == 'Yes':
                frappe.throw(
                    _("Row #{0}: Cannot select Supplier Warehouse while suppling raw materials to subcontractor").format(item.idx))
            ...
            if item.item_code and item.qty and item.item_code in stock_and_asset_items:
                ...
                if flt(item.conversion_factor) == 0.0:
                    item.conversion_factor = get_conversion_factor(
                        item.item_code, item.uom).get("conversion_factor") or 1.0
        ...
        for d in items:
            if not cint(self.get("is_return")):
                ...
                if not d.get(frappe.scrub(ref_doctype)):
                    ...
                    rate = frappe.db.get_value(ref_doctype, d.get(
                        frappe.scrub(ref_doctype)), 'rate')
    ...
    def get_supplied_items_cost(self, item_row_id, reset_outgoing_rate=True):
        ...
        for d in self.get("supplied_items"):
            if d.reference_name == item_row_id:
                ...
                d.amount = flt(flt(d.consumed_qty) *
                               flt(d.rate), d.precision("amount"))
            ...
            if self.doctype == "Purchase Order":
                for supplied_item in self.get("supplied_items"):
                    if not supplied_item.reserve_warehouse:
                        frappe.throw(_("Reserved Warehouse is mandatory for Item {0} in Raw Materials supplied").format(
                            frappe.bold(supplied_item.rm_item_code)))
            ...
            if (self.doctype == 'Purchase Receipt' and
                ...
                for item in self.get("items"):
                    ...
                    if item.bom and item.item_code in self.sub_contracted_items:
                        self.update_raw_materials_supplied_based_on_bom(
                            item, raw_material_table)

                        if [item.item_code, item.name] not in parent_items:
                            parent_items.append([item.item_code, item.name])

                self.cleanup_raw_materials_supplied(
                    parent_items, raw_material_table)
    ...
    def update_raw_materials_supplied_based_on_stock_entries(self):
        ...
        backflushed_raw_materials_map = get_backflushed_subcontracted_raw_materials(
            purchase_orders)
        ...
        for item in self.get('items'):
            ...
            transferred_raw_materials = get_subcontracted_raw_materials_from_se(
            ...
            transferred_batch_qty_map = get_transferred_batch_qty_map(
                item.purchase_order, item.item_code)
            # backflushed_batch_qty_map = get_backflushed_batch_qty_map(item.purchase_order, item.item_code)

            for raw_material in transferred_raw_materials + non_stock_items:
                rm_item_key = (raw_material.rm_item_code,
                               item.item_code, item.purchase_order)
                raw_material_data = backflushed_raw_materials_map.get(
                    rm_item_key, {})
                ...
                if fg_yet_to_be_received == item.qty:
                    ...
                    qty = (rm_qty_to_be_consumed /
                           fg_yet_to_be_received) * item.qty
                ...
                if raw_material.batch_nos:
                    backflushed_batch_qty_map = raw_material_data.get(
                        'consumed_batch', {})
                    ...
                    for batch_data in batches_qty:
                        ...
                        self.append_raw_material_to_be_backflushed(
                            item, raw_material, qty)
                else:
                    self.append_raw_material_to_be_backflushed(
                        item, raw_material, qty)
            ...
            if d.meta.get_field("stock_qty"):
                ...
                if self.doctype == "Purchase Receipt" and d.meta.get_field("received_stock_qty"):
                    ...
                    d.received_stock_qty = flt(
                        d.received_qty) * flt(d.conversion_factor, d.precision("conversion_factor"))
        ...
        for d in self.get("items"):
            if flt(d.rejected_qty) and not d.rejected_warehouse:
                ...
                if not d.rejected_warehouse:
                    frappe.throw(_("Row #{0}: Rejected Warehouse is mandatory against rejected Item {1}").format(
                        d.idx, d.item_code))
    ...
    def validate_accepted_rejected_qty(self):
        for d in self.get("items"):
            ...
            if (flt(val, d.precision("received_qty")) != flt(d.received_qty, d.precision("received_qty"))):
                frappe.throw(
                    _("Accepted + Rejected Qty must be equal to Received quantity for Item {0}").format(d.item_code))
                ...
                if pr_qty:
                    ...
                    if self.is_return:
                        ...
                        val_rate_db_precision = 6 if cint(
                            self.precision("valuation_rate", d)) <= 6 else 9
                        incoming_rate = flt(
                            d.valuation_rate, val_rate_db_precision)
                        sle.update({
                ...
                if item_data.get('auto_create_assets'):
                    ...
                    if item_data.get('asset_naming_series'):
                        ...
                        if len(created_assets) > 5:
                            ...
                            messages.append(_('{} Assets created for {}').format(
    ...
    def delete_linked_asset(self):
        ...
        frappe.db.sql(
            "delete from `tabAsset Movement` where reference_name=%s", self.name)
    ...
    def validate_schedule_date(self):
        ...
        earliest_schedule_date = min(
            [d.schedule_date for d in self.get("items")])
...
def get_items_from_bom(item_code, bom, exploded_item=1):
    ...
    if not bom_items:
        msgprint(_("Specified BOM {0} does not exist for Item {1}").format(
            bom, item_code), raise_exception=1)
...
def get_backflushed_subcontracted_raw_materials(purchase_orders):
    ...
    for args, references in iteritems(distinct_purchase_receipts):
        purchase_receipt_supplied_items = get_supplied_items(
            args[1], args[2], references)
        ...
        for data in purchase_receipt_supplied_items:
            ...
            if data.get("batch_no"):
                if data.get("batch_no") in row.consumed_batch:
                    row.consumed_batch[data.get(
                        "batch_no")] += data.consumed_qty
                else:
                    row.consumed_batch[data.get(
                        "batch_no")] = data.consumed_qty
...
def get_supplied_items(item_code, purchase_receipt, references):
    return frappe.get_all("Purchase Receipt Item Supplied",
                          fields=["rm_item_code", "main_item_code",
                                  "consumed_qty", "serial_no", "batch_no"],
...
def validate_item_type(doc, fieldname, message):
    ...
    if invalid_items:
        ...
        if len(invalid_items) > 1:
            error_message = _(
                "Following items {0} are not marked as {1} item. You can enable them as {1} item from its Item master").format(items, message)
        else:
            error_message = _(
                "Following item {0} is not marked as {1} item. You can enable them as {1} item from its Item master").format(items, message)
...
def set_serial_nos(raw_material, consumed_serial_nos, qty):
    ...
    if serial_nos and qty <= len(serial_nos):
        raw_material.serial_no = '\n'.join(
            list(serial_nos)[0:frappe.utils.cint(qty)])
...
def get_backflushed_batch_qty_map(purchase_order, fg_item):
    ...
    for batch_data in backflushed_batches:
        backflushed_batch_qty_map.setdefault(
            (batch_data.item_code, fg_item), {})
        backflushed_batch_qty_map[(
            batch_data.item_code, fg_item)][batch_data.batch_no] = batch_data.qty
...
def get_batches_with_qty(item_code, fg_item, required_qty, transferred_batch_qty_map, backflushed_batches, po):
    ...
    transferred_batches = transferred_batch_qty_map.get(
        (item_code, fg_item), {})
