...
class calculate_taxes_and_totals(object):
    ...
    def _set_in_company_currency(self, doc, fields):
        ...
        for f in fields:
            val = flt(flt(doc.get(f), doc.precision(f)) *
                      self.doc.conversion_rate, doc.precision("base_" + f))
            doc.set("base_" + f, val)
    ...
    def determine_exclusive_rate(self):
        ...
        for item in self.doc.get("items"):
            ...
            for i, tax in enumerate(self.doc.get("taxes")):
                ...
                total_inclusive_tax_amount_per_qty += inclusive_tax_amount_per_qty * \
                    flt(item.qty)
    ...
    def get_current_tax_fraction(self, tax, item_tax_map):
        ...
        if cint(tax.included_in_print_rate):
            ...
            if tax.charge_type == "On Net Total":
                ...
                current_tax_fraction = (tax_rate / 100.0) * \
                    self.doc.get("taxes")[
                    cint(tax.row_id) - 1].tax_fraction_for_current_item

            elif tax.charge_type == "On Previous Row Total":
                current_tax_fraction = (tax_rate / 100.0) * \
                    self.doc.get("taxes")[
                    cint(tax.row_id) - 1].grand_total_fraction_for_current_item
    ...
    def calculate_net_total(self):
        ...
        self.doc.round_floats_in(
            self.doc, ["total", "base_total", "net_total", "base_net_total"])
    ...
    def calculate_taxes(self):
        ...
        for n, item in enumerate(self.doc.get("items")):
            ...
            for i, tax in enumerate(self.doc.get("taxes")):
                ...
                current_tax_amount = self.get_current_tax_amount(
                ...
                if i == 0:
                    ...
                    tax.grand_total_for_current_item = \
                        flt(self.doc.get("taxes")[
                            i-1].grand_total_for_current_item + current_tax_amount)
    ...
    def set_cumulative_total(self, row_idx, tax):
        ...
        if row_idx == 0:
            ...
            tax.total = flt(self.doc.get("taxes")[
                            row_idx-1].total + tax_amount, tax.precision("total"))
    ...
    def get_current_tax_amount(self, item, tax, item_tax_map):
        ...
        if tax.charge_type == "Actual":
            ...
            current_tax_amount = item.net_amount*actual / \
                self.doc.net_total if self.doc.net_total else 0.0

        elif tax.charge_type == "On Net Total":
            current_tax_amount = (tax_rate / 100.0) * item.net_amount
        elif tax.charge_type == "On Previous Row Amount":
            current_tax_amount = (tax_rate / 100.0) * \
                self.doc.get("taxes")[cint(tax.row_id) -
                                      1].tax_amount_for_current_item
        elif tax.charge_type == "On Previous Row Total":
            current_tax_amount = (tax_rate / 100.0) * \
                self.doc.get("taxes")[cint(tax.row_id) -
                                      1].grand_total_for_current_item
        elif tax.charge_type == "On Item Quantity":
    ...
    def set_rounded_total(self):
        if self.doc.meta.get_field("rounded_total"):
            ...
            self._set_in_company_currency(
                self.doc, ["rounding_adjustment", "rounded_total"])
        ...
        if self.doc.discount_amount:
            ...
            if total_for_discount_amount:
                ...
                for i, item in enumerate(self.doc.get("items")):
                    ...
                    item.net_rate = flt(
                        item.net_amount / item.qty, item.precision("net_rate")) if item.qty else 0
        ...
        if self.doc.apply_discount_on == "Net Total":
            ...
            for tax in self.doc.get("taxes"):
                if tax.charge_type in ["Actual", "On Item Quantity"]:
                    ...
                    actual_tax_amount = flt(actual_taxes_dict.get(
                        tax.row_id, 0)) * flt(tax.rate) / 100
    ...
    def calculate_outstanding_amount(self):
        ...
        if self.doc.is_return and self.doc.return_against and not self.doc.get('is_pos') or \
                self.is_internal_invoice():
            return

        self.doc.round_floats_in(
            self.doc, ["grand_total", "total_advance", "write_off_amount"])
    ...
    def calculate_paid_amount(self):
        ...
        self.doc.paid_amount = flt(
            paid_amount, self.doc.precision("paid_amount"))
        self.doc.base_paid_amount = flt(
            base_paid_amount, self.doc.precision("base_paid_amount"))
        ...
        if item.price_list_rate:
            ...
            if item.margin_type and item.margin_rate_or_amount:
                margin_value = item.margin_rate_or_amount if item.margin_type == 'Amount' else flt(
                    item.price_list_rate) * flt(item.margin_rate_or_amount) / 100
                rate_with_margin = flt(
                    item.price_list_rate) + flt(margin_value)
                base_rate_with_margin = flt(
                    rate_with_margin) * flt(self.doc.conversion_rate)

        return rate_with_margin, base_rate_with_margin

    def set_item_wise_tax_breakup(self):
        self.doc.other_charges_calculation = get_itemised_tax_breakup_html(
            self.doc)
...
def get_itemised_tax_breakup_html(doc):
    ...
    headers = get_itemised_tax_breakup_header(
        doc.doctype + " Item", tax_accounts)
...
class init_landed_taxes_and_totals(object):
        ...
        for d in self.doc.get(self.tax_field):
            if not d.account_currency:
                account_currency = frappe.db.get_value(
                    'Account', d.expense_account, 'account_currency')
    ...
    def set_amounts_in_company_currency(self):
        for d in self.doc.get(self.tax_field):
            ...
            d.base_amount = flt(
                d.amount * flt(d.exchange_rate), d.precision("base_amount"))
