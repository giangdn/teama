    ...
    for entry in gl_map:
        ...
        if flt(entry.debit_in_account_currency) < 0:
            entry.credit_in_account_currency = \
                ...
                flt(entry.debit_in_account_currency)
...
def merge_similar_entries(gl_map):
    ...
    for entry in gl_map:
        ...
        if same_head:
            ...
            same_head.debit_in_account_currency = \
                flt(same_head.debit_in_account_currency) + \
                flt(entry.debit_in_account_currency)
            same_head.credit = flt(same_head.credit) + flt(entry.credit)
            same_head.credit_in_account_currency = \
                flt(same_head.credit_in_account_currency) + \
                flt(entry.credit_in_account_currency)
        else:
    ...
    precision = get_field_precision(frappe.get_meta(
        "GL Entry").get_field("debit"), company_currency)

    # filter zero debit and credit entries
    merged_gl_map = filter(lambda x: flt(x.debit, precision) != 0 or flt(
        x.credit, precision) != 0, merged_gl_map)
...
def validate_cwip_accounts(gl_map):
    cwip_enabled = any([cint(ac.enable_cwip_accounting) for ac in frappe.db.get_all(
        "Asset Category", "enable_cwip_accounting")])
...
def make_reverse_gl_entries(gl_entries=None, voucher_type=None, voucher_no=None,
    ...
    if gl_entries:
        ...
        set_as_cancel(gl_entries[0]['voucher_type'],
        ...
        for entry in gl_entries:
            ...
            debit_in_account_currency = entry.get(
                'debit_in_account_currency', 0)
            credit_in_account_currency = entry.get(
