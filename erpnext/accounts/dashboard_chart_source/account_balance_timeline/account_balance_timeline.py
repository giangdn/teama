...
def get(chart_name=None, chart=None, no_cache=None, filters=None, from_date=None,
    ...
    filters = frappe.parse_json(
        filters) or frappe.parse_json(chart.filters_json)
...
def get_gl_entries(account, to_date):
    child_accounts = get_descendants_of(
        'Account', account, ignore_permissions=True)
...
def get_dates_from_timegrain(from_date, to_date, timegrain):
    ...
    while getdate(dates[-1]) < getdate(to_date):
        date = get_period_ending(add_to_date(
            dates[-1], years=years, months=months, days=days), timegrain)
        dates.append(date)
    return dates
