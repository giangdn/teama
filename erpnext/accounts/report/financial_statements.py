...
from frappe.utils import (flt, getdate, get_first_day,
                          add_months, add_days, formatdate, cstr, cint)
    ...
    for i in range(cint(math.ceil(months / months_to_add))):
        ...
        if not ignore_fiscal_year:
            period.to_date_fiscal_year = get_fiscal_year(
                period.to_date, company=company)[0]
    ...
    for opts in period_list:
        ...
        if periodicity == "Monthly" and not accumulated_values:
            ...
            if not accumulated_values:
                label = get_label(
                    periodicity, opts["from_date"], opts["to_date"])
            else:
                if reset_period_on_fy_change:
                    ...
                    label = get_label(
                        periodicity, period_list[0].from_date, opts["to_date"])
    ...
    if accounts:
        ...
        if filters and filters.get('presentation_currency'):
            convert_to_presentation_currency(
                gl_entries, get_currency(filters), filters.get('company'))
...
def get_cost_centers_with_children(cost_centers):
    if not isinstance(cost_centers, list):
        cost_centers = [d.strip()
                        for d in cost_centers.strip().split(',') if d]

    all_cost_centers = []
    for d in cost_centers:
        if frappe.db.exists("Cost Center", d):
            lft, rgt = frappe.db.get_value("Cost Center", d, ["lft", "rgt"])
            children = frappe.get_all("Cost Center", filters={
                                      "lft": [">=", lft], "rgt": ["<=", rgt]})
