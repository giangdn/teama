...
from erpnext.accounts.report.financial_statements import (
    get_period_list, get_columns, get_data)
...
def setup_mappers(mappers):
    ...
    for mapping in mappers:
        ...
        for label in account_types_labels:
            names = [d['name']
                     for d in account_types if d['label'] == label[0]]
        ...
        for label in fc_adjustment_labels:
            names = [d['name']
                     for d in finance_costs_adjustments if d['label'] == label[0]]
            m = dict(label=label[0], names=names)
            mapping['finance_costs_adjustments'].append(m)

        for label in unique_liability_labels:
            names = [d['name']
                     for d in tax_liabilities if d['label'] == label[0]]
            m = dict(label=label[0], names=names,
                     tax_liability=label[1], tax_expense=label[2])
        ...
        for label in unique_finance_costs_labels:
            names = [d['name']
                     for d in finance_costs if d['label'] == label[0]]
...
def add_data_for_operating_activities(
    ...
    if not mapper['finance_costs_adjustments']:
        mapper['finance_costs_adjustments'] = [
            dict(label='Interest Paid', names=[''])]
...
def calculate_adjustment(filters, non_expense_mapper, expense_mapper, use_accumulated_values, period_list):
    ...
    data = _calculate_adjustment(
        non_expense_closing, non_expense_opening, expense_data)
    return data
...
def _calculate_adjustment(non_expense_closing, non_expense_opening, expense_data):
    ...
    for month in non_expense_opening.keys():
        if non_expense_opening[month] and non_expense_closing[month]:
            account_data[month] = non_expense_opening[month] - \
                expense_data[month] + non_expense_closing[month]
...
def execute(filters=None):
    ...
    net_profit_loss = get_net_profit_loss(
        income, expense, period_list, filters.company)

    company_currency = frappe.get_cached_value(
        'Company',  filters.company,  "default_currency")

    data = compute_data(filters, company_currency, net_profit_loss,
                        period_list, mappers, cash_flow_accounts)

    _add_total_row_account(data, data, _(
        "Net Change in Cash"), period_list, company_currency)
    ...
    for period in period_list:
        ...
        if opening_balances:
            ...
            if accumulated_values:
                start, end = add_to_date(
                    start_date, years=-1), add_to_date(period['to_date'], years=-1)
            else:
