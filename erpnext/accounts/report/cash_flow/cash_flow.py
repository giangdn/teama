...
def get_cash_flow_accounts():
    operation_accounts = {
        ...
        "account_types": [
            ...
            {"account_type": "Receivable", "label": _(
                "Net Change in Accounts Receivable")},
            {"account_type": "Payable", "label": _(
                "Net Change in Accounts Payable")},
            {"account_type": "Stock", "label": _(
    ...
    investing_accounts = {
        ...
        "account_types": [
            {"account_type": "Fixed Asset", "label": _(
...
def get_account_type_based_gl_data(company, start_date, end_date, account_type, filters={}):
    ...
    if filters.include_default_book_entries:
        company_fb = frappe.db.get_value(
...
def get_chart_data(columns, data):
    ...
    datasets = [{'name': account.get('account').replace("'", ""), 'values': [account.get(
        'total')]} for account in data if account.get('parent_account') == None and account.get('currency')]
    datasets = datasets[:-1]
