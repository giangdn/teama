...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
    ...
    for inv in invoice_list:
        # invoice details
        purchase_order = list(set(invoice_po_pr_map.get(
            inv.name, {}).get("purchase_order", [])))
        purchase_receipt = list(set(invoice_po_pr_map.get(
            inv.name, {}).get("purchase_receipt", [])))
        project = list(set(invoice_po_pr_map.get(
            inv.name, {}).get("project", [])))
        ...
        row += [
            ...
            ", ".join(purchase_order), ", ".join(
                purchase_receipt), company_currency
        ]
        ...
        for expense_acc in expense_accounts:
            if inv.is_internal_supplier and inv.company == inv.represents_company:
                ...
                expense_amount = flt(invoice_expense_map.get(
                    inv.name, {}).get(expense_acc))
        ...
        row += [total_tax, inv.base_grand_total,
                flt(inv.base_grand_total, 0), inv.outstanding_amount]
        data.append(row)
...
def get_columns(invoice_list, additional_table_columns):
    ...
    columns += [
        _("Supplier Group") + ":Link/Supplier Group:120", _("Tax Id") +
        "::80", _("Payable Account") + ":Link/Account:120",
        _("Mode of Payment") +
        ":Link/Mode of Payment:80", _("Project") + ":Link/Project:80",
        _("Bill No") + "::120", _("Bill Date") +
        ":Date:80", _("Remarks") + "::150",
    ...
    expense_columns = [(account + ":Currency/currency:120")
            ...
            if filters.get(dimension.fieldname):
                if frappe.get_cached_value('DocType', dimension.document_type, 'is_tree'):
                    ...
                    conditions += common_condition + \
                        "and ifnull(`tabPurchase Invoice Item`.{0}, '') in %({0})s)".format(
                            dimension.fieldname)
                else:
                    conditions += common_condition + \
                        "and ifnull(`tabPurchase Invoice Item`.{0}, '') in (%({0})s))".format(
...
def get_invoice_expense_map(invoice_list):
    ...
    for d in expense_details:
        invoice_expense_map.setdefault(
            d.parent, frappe._dict()).setdefault(d.expense_account, [])
    ...
    for d in tax_details:
        if d.account_head in expense_accounts:
            ...
            invoice_tax_map.setdefault(
                d.parent, frappe._dict()).setdefault(d.account_head, [])
