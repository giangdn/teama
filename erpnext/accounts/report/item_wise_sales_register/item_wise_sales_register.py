...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
    ...
    for d in item_list:
        ...
        if filters.get('group_by'):
            ...
            add_sub_total_row(row, total_row_map, d.get(
                group_by_field, ''), tax_columns)

        data.append(row)

    if filters.get('group_by') and item_list:
        total_row = total_row_map.get(
            prev_group_by_value or d.get('item_name'))
...
def get_tax_accounts(item_list, columns, company_currency,
    ...
    for d in item_list:
        ...
        item_row_map.setdefault(d.parent, {}).setdefault(
            d.item_code or d.item_name, []).append(d)
...
def add_total_row(data, filters, prev_group_by_value, item, total_row_map,
    ...
    if prev_group_by_value != item.get(group_by_field, ''):
        if prev_group_by_value:
            ...
            data.append(total_row)
            data.append({})
            add_sub_total_row(total_row, total_row_map,
                              'total_row', tax_columns)
...
def add_sub_total_row(item, total_row_map, group_by_value, tax_columns):
    ...
    for tax in tax_columns:
        ...
        total_row[frappe.scrub(tax + ' Amount')
                  ] += flt(item[frappe.scrub(tax + ' Amount')])
