...
def set_filters(filters):
    ...
    filters.naming_series = frappe.db.get_single_value(
...
def get_result(filters):
    ...
    for d in gle_map:
        ...
        account_list = [
            i.account for i in tds_doc.accounts if i.company == filters.company]
...
def get_supplier_map(filters):
    ...
    supplier_detail = frappe.db.get_all('Supplier',
                                        {"name": [
                                            "in", [d.supplier for d in filters["invoices"]]]},
    ...
    for d in supplier_detail:
        if d.get("tax_withholding_category") not in tds_docs:
            tds_docs[d.get("tax_withholding_category")] = \
                frappe.get_doc("Tax Withholding Category",
                               d.get("tax_withholding_category"))
...
def get_gle_map(filters):
    ...
    gle = frappe.db.get_all('GL Entry',
                            {"voucher_no": [
                                "in", [d.get("name") for d in filters["invoices"]]], 'is_cancelled': 0},
