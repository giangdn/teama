    ...
    if asset and (liability or equity):
        ...
        for period in period_list:
            ...
            provisional_profit_loss[key] = flt(
                asset[-2].get(key)) - effective_liability
...
def check_opening_balance(asset, liability, equity):
    ...
    if asset:
        opening_balance = flt(asset[0].get(
            "opening_balance", 0), float_precision)
    if liability:
        opening_balance -= flt(liability[0].get(
            "opening_balance", 0), float_precision)
    if equity:
        opening_balance -= flt(equity[0].get("opening_balance",
                                             0), float_precision)
