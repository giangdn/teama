...
def execute(filters=None):
    ...
    data = get_pos_sales_payment_data(filters) if filters.get(
        'is_pos') else get_sales_payment_data(filters, columns)
    return columns, data
...
def get_mode_of_payments(filters):
    ...
    invoice_list_names = ",".join(
        ['"' + invoice['name'] + '"' for invoice in invoice_list])
    if invoice_list:
        ...
        for d in inv_mop:
            mode_of_payments.setdefault(
                d["owner"]+cstr(d["posting_date"]), []).append(d.mode_of_payment)
    ...
    if invoice_list:
        ...
        for d in inv_mop_detail:
            mode_of_payment_details.setdefault(
                d["owner"]+cstr(d["posting_date"]), []).append((d.mode_of_payment, d.paid_amount))
