...
def execute(filters=None):
    ...
    fiscal_year = get_fiscal_year_data(filters.get(
        'from_fiscal_year'), filters.get('to_fiscal_year'))
    ...
    if filters.get('report') == "Balance Sheet":
        ...
        data, report_summary = get_cash_flow_data(
            fiscal_year, companies, filters)
...
def get_balance_sheet_data(fiscal_year, companies, columns, filters):
    ...
    liability = get_data(companies, "Liability", "Credit",
                         fiscal_year, filters=filters)

    equity = get_data(companies, "Equity", "Credit",
                      fiscal_year, filters=filters)
    ...
    if opening_balance and round(opening_balance, 2) != 0:
        ...
        for company in companies:
            ...
            if provisional_profit_loss:
                provisional_profit_loss[company] = provisional_profit_loss[company] - \
                    opening_balance
...
def get_profit_loss_data(fiscal_year, companies, columns, filters):
    ...
    chart = get_pl_chart_data(filters, columns, income,
                              expense, net_profit_loss)

    report_summary = get_pl_summary(
        companies, '', income, expense, net_profit_loss, company_currency, True)
...
def get_income_expense_data(companies, fiscal_year, filters):
    company_currency = get_company_currency(filters)
    income = get_data(companies, "Income", "Credit",
                      fiscal_year, filters, True)

    expense = get_data(companies, "Expense", "Debit",
                       fiscal_year, filters, True)
...
def get_data(companies, root_type, balance_must_be, fiscal_year, filters=None, ignore_closing_entries=False):
    ...
    calculate_values(accounts_by_name, gl_entries_by_account,
                     companies, start_date, filters)
    accumulate_values_into_parents(accounts, accounts_by_name, companies)
    out = prepare_data(accounts, start_date, end_date,
                       balance_must_be, companies, company_currency)

    if out:
        add_total_row(out, root_type, balance_must_be,
                      companies, company_currency)
        ...
        if d.parent_account:
            ...
            for company in companies:
                accounts_by_name[account][company] = \
                    accounts_by_name[account].get(
                        company, 0.0) + d.get(company, 0.0)
    ...
    for d in companies:
        ...
        if filters and filters.get('presentation_currency') != d.default_currency:
            ...
            convert_to_presentation_currency(
                gl_entries, currency_info, filters.get('company'))
...
def validate_entries(key, entry, accounts_by_name):
    if key not in accounts_by_name:
        ...
        frappe.throw(
            _("{0} {1} is not present in the parent company").format(field, key))
...
def get_additional_conditions(from_date, ignore_closing_entries, filters):
    ...
    if ignore_closing_entries:
        additional_conditions.append(
            "ifnull(gl.voucher_type, '')!='Period Closing Voucher'")
