...
def execute(filters=None):
    ...
    for dimension in dimensions:
        ...
        if dimension_items:
            data = get_final_data(dimension, dimension_items,
                                  filters, period_month_ranges, data, 0)
        else:
...
def get_actual_details(name, filters):
    ...
    if filters.get("budget_against") == "Cost Center":
        cc_lft, cc_rgt = frappe.db.get_value(
            "Cost Center", name, ["lft", "rgt"])
        cond = """
				and lft >= "{lft}"
				and rgt <= "{rgt}"
