...
class PartyLedgerSummaryReport(object):
    ...
    def get_data(self):
        ...
        for party, row in iteritems(self.party_data):
            if row.opening_balance or row.invoiced_amount or row.paid_amount or row.return_amount or row.closing_amount:
                total_party_adjustment = sum([amount for amount in itervalues(
                    self.party_adjustment_details.get(party, {}))])
    ...
    def prepare_conditions(self):
        ...
        if self.filters.finance_book:
            conditions.append(
                "ifnull(finance_book,'') in (%(finance_book)s, '')")
    ...
    def get_party_adjustment_amounts(self):
        ...
        for gle in gl_entries:
            ...
            adjustment_voucher_entries[(
                gle.voucher_type, gle.voucher_no)].append(gle)
        ...
        for voucher_gl_entries in itervalues(adjustment_voucher_entries):
            ...
            for gle in voucher_gl_entries:
                if gle.account == round_off_account:
                    ...
                    parties[gle.party] += gle.get(reverse_dr_or_cr) - \
                        gle.get(invoice_dr_or_cr)
                elif frappe.get_cached_value("Account", gle.account, "account_type") == income_or_expense:
                    accounts.setdefault(gle.account, 0)
                    accounts[gle.account] += gle.get(
                        invoice_dr_or_cr) - gle.get(reverse_dr_or_cr)
                else:
            ...
            if parties and accounts:
                if len(parties) == 1:
                    ...
                    for account, amount in iteritems(accounts):
                        ...
                        self.party_adjustment_details[party].setdefault(
                            account, 0)
