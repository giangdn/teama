...
def _get_party_details(party=None, account=None, party_type="Customer", company=None, posting_date=None,
    ...
    if not ignore_permissions and not (frappe.has_permission(party_type, "read", party) or frappe.has_permission(party_type, "select", party)):
        frappe.throw(_("Not permitted for {0}").format(
            party), frappe.PermissionError)
    ...
    party_address, shipping_address = set_address_details(
        party_details, party, party_type, doctype, company, party_address, company_address, shipping_address)
...
def set_address_details(party_details, party, party_type, doctype=None, company=None, party_address=None, company_address=None, shipping_address=None):
    ...
    if doctype:
        party_details.update(get_fetch_values(
            doctype, billing_address_field, party_details[billing_address_field]))
    # address display
    ...
    if party_type in ["Customer", "Lead"]:
        ...
        if doctype:
            party_details.update(get_fetch_values(
                doctype, 'shipping_address_name', party_details.shipping_address_name))
    ...
    if doctype and doctype in ['Delivery Note', 'Sales Invoice', 'Sales Order']:
        ...
        if party_details.company_address:
            ...
            party_details.shipping_address_display = get_address_display(
                party_details["shipping_address"])
            party_details.update(get_fetch_values(
                doctype, 'shipping_address', party_details.shipping_address))
        get_regional_address_details(party_details, doctype, company)
    ...
    if existing_gle_currency:
        if account:
            account_currency = frappe.db.get_value(
                "Account", account, "account_currency", cache=True)
...
def get_due_date_from_template(template_name, posting_date, bill_date):
    ...
    for term in template.terms:
        if term.due_date_based_on == 'Day(s) after invoice date':
            ...
            due_date = max(due_date, add_months(
                get_last_day(due_date), term.credit_months))
    return due_date
...
def get_dashboard_info(party_type, party, loyalty_program=None):
    ...
    company_wise_grand_total = frappe.get_all(doctype,
                                              ...
                                              group_by="company",
                                              fields=["company", "sum(grand_total) as grand_total",
    ...
    if party_type == "Customer":
        loyalty_point_details = frappe._dict(frappe.get_all("Loyalty Point Entry",
                                                            ...
                                                            group_by="company",
                                                            fields=[
                                                                "company", "sum(loyalty_points) as loyalty_points"],
                                                            as_list=1
                                                            ))
...
def get_partywise_advanced_payment_amount(party_type, posting_date=None, future_payment=0, company=None):
    ...
    if posting_date:
        if future_payment:
            cond = "posting_date <= '{0}' OR DATE(creation) <= '{0}' """.format(
                posting_date)
        else:
