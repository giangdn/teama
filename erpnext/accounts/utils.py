...
def get_fiscal_years(transaction_date=None, fiscal_year=None, label="Date", verbose=1, company=None, as_dict=False):
    ...
    if company:
        error_msg = _("""{0} for {1}""").format(
            error_msg, frappe.bold(company))
...
def validate_fiscal_year(date, fiscal_year, company, label="Date", doc=None):
    years = [f[0] for f in get_fiscal_years(
        date, label=_(label), company=company)]
    if fiscal_year not in years:
        if doc:
            doc.fiscal_year = years[0]
        else:
            throw(_("{0} '{1}' not in Fiscal Year {2}").format(
            ...
            if fieldname not in ('invoiced_amount', 'payables'):
                ...
                if ((not gle.against_voucher) or (gle.against_voucher_type in ["Sales Order", "Purchase Order"]) or
                    ...
                    outstanding_amount = flt(
                        gle.get(dr_or_cr)) - flt(gle.get(cr_or_dr)) - payment_amount
...
def update_reference_in_journal_entry(d, jv_obj):
    ...
    if d['allocated_amount'] < d['unadjusted_amount']:
        ...
        amount_in_account_currency = flt(
            d['unadjusted_amount']) - flt(d['allocated_amount'])
        amount_in_company_currency = amount_in_account_currency * \
            flt(jvd[0]['exchange_rate'])
        ...
        ch.set('debit' if d['dr_or_cr'] ==
               'debit_in_account_currency' else 'credit', amount_in_company_currency)

        ch.set('credit_in_account_currency' if d['dr_or_cr'] == 'debit_in_account_currency'
               else 'debit_in_account_currency', 0)
        ch.set('credit' if d['dr_or_cr'] ==
               'debit_in_account_currency' else 'debit', 0)
...
def update_reference_in_payment_entry(d, payment_entry, do_not_save=False):
    ...
    if d.voucher_detail_no:
        existing_row = payment_entry.get(
            "references", {"name": d["voucher_detail_no"]})[0]
...
def get_stock_rbnb_difference(posting_date, company):
    ...
    stock_rbnb_account = "Stock Received But Not Billed - " + \
        frappe.get_cached_value('Company',  company,  "abbr")
    sys_bal = get_balance_on(
        stock_rbnb_account, posting_date, in_account_currency=False)
...
def get_outstanding_invoices(party_type, party, account, condition=None, filters=None):
    ...
    for d in payment_entries:
        pe_map.setdefault(
            (d.against_voucher_type, d.against_voucher), d.payment_amount)
...
def get_children(doctype, parent, company, is_root=False):
    ...
    filters.append(['ifnull(`{0}`,"")'.format(
        parent_fieldname), '=', '' if is_root else parent])

    if is_root:
        fields += ['root_type', 'report_type',
                   'account_currency'] if doctype == 'Account' else []
    ...
    if doctype == 'Account':
        ...
        for each in acc:
            ...
            each["balance"] = flt(get_balance_on(
                each.get("value"), in_account_currency=False, company=company))

            if each.account_currency != company_currency:
                each["balance_in_account_currency"] = flt(
                    get_balance_on(each.get("value"), company=company))
...
def update_cost_center(docname, cost_center_name, cost_center_number, company, merge):
    ...
    '''
    validate_field_number("Cost Center", docname,
                          cost_center_number, company, "cost_center_number")
    ...
    frappe.db.set_value("Cost Center", docname,
                        "cost_center_name", cost_center_name.strip())

    new_name = get_autoname_with_number(
        cost_center_number, cost_center_name, docname, company)
    if docname != new_name:
        frappe.rename_doc("Cost Center", docname,
                          new_name, force=1, merge=merge)
        return new_name
...
def get_autoname_with_number(number_value, doc_title, name, company):
    ...
    if name:
        ...
        abbr = frappe.get_cached_value(
            'Company',  company,  ["abbr"], as_dict=True)
...
def get_coa(doctype, parent, is_root, chart=None):
    ...
    accounts = build_tree_from_json(chart)
