...
class Subscription(Document):
    ...
    def set_status_grace_period(self):
        ...
        if self.status == 'Past Due Date' and self.is_past_grace_period():
            self.status = 'Cancelled' if cint(
                subscription_settings.cancel_after_grace) else 'Unpaid'
    ...
    def get_current_invoice(self):
        ...
        if len(self.invoices):
            ...
            if frappe.db.exists(doctype, current.get('invoice')):
                ...
                frappe.throw(_('Invoice {0} no longer exists').format(
                    current.get('invoice')))
    ...
    def validate_trial_period(self):
        ...
        if self.trial_period_start and self.trial_period_end:
            if getdate(self.trial_period_end) < getdate(self.trial_period_start):
                frappe.throw(
                    _('Trial Period End Date Cannot be before Trial Period Start Date'))

        if self.trial_period_start and not self.trial_period_end:
            frappe.throw(
                _('Both Trial Period Start Date and Trial Period End Date must be set'))

        if self.trial_period_start and getdate(self.trial_period_start) > getdate(self.start_date):
            frappe.throw(
                _('Trial Period Start date cannot be after Subscription Start Date'))
    ...
    def validate_to_follow_calendar_months(self):
        if self.follow_calendar_months:
            ...
            if not self.end_date:
                frappe.throw(
                    _('Subscription End Date is mandatory to follow calendar months'))
    ...
    def is_postpaid_to_invoice(self):
        return getdate() > getdate(self.current_invoice_end) or \
            (getdate() >= getdate(self.current_invoice_end) and getdate(
                self.current_invoice_end) == getdate(self.current_invoice_start))
    ...
    def process_for_active(self):
        ...
        if not self.is_current_invoice_generated() and (self.is_postpaid_to_invoice() or self.is_prepaid_to_invoice()):
            prorate = frappe.db.get_single_value(
                'Subscription Settings', 'prorate')
    ...
    def process_for_past_due_date(self):
        ...
        current_invoice = self.get_current_invoice()
        if not current_invoice:
            frappe.throw(_('Current invoice {0} is missing').format(
    ...
    def cancel_subscription(self):
        ...
        if self.status != 'Cancelled':
            ...
            to_prorate = frappe.db.get_single_value(
                'Subscription Settings', 'prorate')
            self.status = 'Cancelled'
            self.cancelation_date = nowdate()
    ...
    def restart_subscription(self):
        ...
        if self.status == 'Cancelled':
            ...
            frappe.throw(
                _('You cannot restart a Subscription that is not cancelled.'))
