...
class Account(NestedSet):
    ...
    def autoname(self):
        ...
        self.name = get_autoname_with_number(
            self.account_number, self.account_name, None, self.company)
    ...
    def validate(self):
        ...
        validate_field_number(
            "Account", self.name, self.account_number, self.company, "account_number")
    ...
    def validate_parent(self):
        ...
        if self.parent_account:
            ...
            if not par:
                throw(_("Account {0}: Parent account {1} does not exist").format(
                    self.name, self.parent_account))
            elif par.name == self.name:
                throw(_("Account {0}: You can not assign itself as parent account").format(
                    self.name))
            elif not par.is_group:
                throw(_("Account {0}: Parent account {1} can not be a ledger").format(
                    self.name, self.parent_account))
    ...
    def validate_root_details(self):
        ...
        if not self.parent_account and not self.is_group:
            frappe.throw(_("The root account {0} must be a group").format(
    ...
    def validate_root_company_and_sync_account_to_children(self):
        ...
        if ancestors:
            ...
            if not frappe.db.get_value("Account",
                ...
                frappe.throw(
                    _("Please add the account to root level Company - {}").format(ancestors[0]))
            ...
            self.create_account_for_child_company(
                parent_acc_name_map, descendants, parent_acc_name)

    def validate_group_or_ledger(self):
        if self.get("__islocal"):
            return

        existing_is_group = frappe.db.get_value(
            "Account", self.name, "is_group")
        if cint(self.is_group) != cint(existing_is_group):
            if self.check_gle_exists():
                throw(
                    _("Account with existing transaction cannot be converted to ledger"))
            elif self.is_group:
    ...
    def create_account_for_child_company(self, parent_acc_name_map, descendants, parent_acc_name):
        for company in descendants:
            ...
            if (frappe.db.get_value("Account", self.parent_account, "is_group")
                ...
                msg = _("While creating account for Child Company {0}, parent account {1} found as a ledger account.").format(
                    company_bold, parent_acc_name_bold)
                msg += "<br><br>"
...
def get_account_autoname(account_number, account_name, company):
    ...
    company = frappe.get_cached_value(
        'Company',  company,  ["abbr", "name"], as_dict=True)
    if not company:
...
def update_account_number(name, account_name, account_number=None, from_descendant=False):
    ...
    if account_number:
        frappe.db.set_value(
            "Account", name, "account_number", account_number.strip())
    else:
    ...
    new_name = get_account_autoname(
        account_number, account_name, account.company)
    if name != new_name:
...
def sync_update_account_number_in_child(descendants, old_acc_name, account_name, account_number=None, old_acc_number=None):
    ...
    for d in frappe.db.get_values('Account', filters=filters, fieldname=["company", "name"], as_dict=True):
        update_account_number(d["name"], account_name,
                              account_number, from_descendant=True)
