...
def go():
    ...
    for country_dir in country_dirs:
        ...
        manifest = ast.literal_eval(
            open(os.path.join(country_path, "__openerp__.py")).read())
    ...
    for root in root_list:
        for node in root[0].findall("record"):
            if node.get("model") == "account.account.type":
                ...
                node_id = prefix + "." + \
                    node.get("id") if prefix else node.get("id")
                types[node_id] = data
    ...
    if csv_content and csv_content[0][0] == "id":
        for row in csv_content[1:]:
            ...
            if data and data.get("id"):
                node_id = prefix + "." + \
                    data.get("id") if prefix else data.get("id")
                types[node_id] = data
    return types
            ...
            with open(fpath, "r") as chartfile:
                ...
                if not old_content or (json.loads(old_content).get("is_active", "No") == "No"
                    ...
                    with open(fpath, "w") as chartfile:
                        chartfile.write(json.dumps(
                            chart, indent=4, sort_keys=True))
