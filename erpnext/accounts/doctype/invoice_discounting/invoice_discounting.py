...
class InvoiceDiscounting(AccountsController):
    ...
    def set_end_date(self):
        if self.loan_start_date and self.loan_period:
            self.loan_end_date = add_days(
                self.loan_start_date, self.loan_period)

    def validate_mandatory(self):
        if self.docstatus == 1 and not (self.loan_start_date and self.loan_period):
            frappe.throw(
                _("Loan Start Date and Loan Period are mandatory to save the Invoice Discounting"))
    ...
    def calculate_total_amount(self):
        self.total_amount = sum([flt(d.outstanding_amount)
    ...
    def update_sales_invoice(self):
        for d in self.invoices:
            ...
            frappe.db.set_value("Sales Invoice", d.sales_invoice,
    ...
    def make_gl_entries(self):
        ...
        gl_entries = []
        invoice_fields = ["debit_to", "party_account_currency",
                          "conversion_rate", "cost_center"]
        ...
        for d in self.invoices:
            inv = frappe.db.get_value(
                "Sales Invoice", d.sales_invoice, invoice_fields, as_dict=1)
        ...
        make_gl_entries(gl_entries, cancel=(
            self.docstatus == 2), update_outstanding='No')
