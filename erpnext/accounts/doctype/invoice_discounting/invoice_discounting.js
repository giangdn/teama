...
frappe.ui.form.on("Invoice Discounting", {
	setup: frm => {
		...
		frm.events.filter_accounts("bank_account", frm, [
			["account_type", "=", "Bank"]
		]);
		frm.events.filter_accounts("bank_charges_account", frm, [
			["root_type", "=", "Expense"]
		]);
		frm.events.filter_accounts("short_term_loan", frm, [
			["root_type", "=", "Liability"]
		]);
	...
	get_invoices: frm => {
		var d = new frappe.ui.Dialog({
			...
			primary_action: function() {
				...
				frappe.call({
					method:
						"erpnext.accounts.doctype.invoice_discounting.invoice_discounting.get_invoices",
					args: {
						filters: data
					},
					callback: function(r) {
						if (!r.exc) {
							...
							$.each(r.message, function(i, v) {
								frm.doc.invoices = frm.doc.invoices.filter(
									row => row.sales_invoice
								);
