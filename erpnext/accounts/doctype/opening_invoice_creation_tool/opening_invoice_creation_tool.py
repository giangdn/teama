...
class OpeningInvoiceCreationTool(Document):
    ...
    def get_invoice_dict(self, row=None):
        def get_item_dict():
            ...
            if not cost_center:
                frappe.throw(_("Please set the Default Cost Center in {0} company.").format(
...
def start_import(invoices):
    ...
    for idx, d in enumerate(invoices):
        try:
            ...
            frappe.db.rollback()
            message = "\n".join(["Data:", dumps(
                d, default=str, indent=4), "--" * 50, "\nException:", traceback.format_exc()])
            frappe.log_error(
                title="Error while creating Opening Invoice", message=message)
            frappe.db.commit()
    if errors:
