...
class TestOpeningInvoiceCreationTool(unittest.TestCase):
    ...
    def check_expected_values(self, invoices, expected_value, invoice_type="Sales"):
        ...
        for invoice_idx, invoice in enumerate(invoices or []):
            ...
            for field_idx, field in enumerate(expected_value["keys"]):
                self.assertEqual(si.get(field, ""),
                                 expected_value[invoice_idx][field_idx])

    def test_opening_purchase_invoice_creation(self):
        invoices = self.make_invoices(
            invoice_type="Purchase", company="_Test Opening Invoice Company")
    ...
    def test_opening_sales_invoice_creation_with_missing_debit_account(self):
        ...
        party_1, party_2 = make_customer(
            "Customer A"), make_customer("Customer B")
        ...
        error_log = frappe.db.exists("Error Log", {"error": [
                                     "like", "%erpnext.controllers.accounts_controller.AccountMissingError%"]})
...
def get_opening_invoice_creation_dict(**args):
    party = "Customer" if args.get(
        "invoice_type", "Sales") == "Sales" else "Supplier"
