...
def get_party_tax_withholding_details(ref_doc, tax_withholding_category=None):
    ...
    if not tax_withholding_category:
        tax_withholding_category, pan_no = frappe.db.get_value(
            'Supplier', ref_doc.supplier, ['tax_withholding_category', 'pan'])
    ...
    if pan_no:
        suppliers = [d.name for d in frappe.get_all(
            'Supplier', fields=['name'], filters={'pan': pan_no})]

    if not suppliers:
        suppliers.append(ref_doc.supplier)

    fy = get_fiscal_year(ref_doc.posting_date, company=ref_doc.company)
    tax_details = get_tax_withholding_details(
        tax_withholding_category, fy[0], ref_doc.company)
    if not tax_details:
...
def get_tax_withholding_details(tax_withholding_category, fiscal_year, company):
    tax_withholding = frappe.get_doc(
...
def get_tax_withholding_rates(tax_withholding, fiscal_year):
    ...
    frappe.throw(
        _("No Tax Withholding data found for the current Fiscal Year."))
...
def get_tds_amount(suppliers, net_total, company, tax_details, fiscal_year_details, posting_date, pan_no=None):
    ...
    advance_vouchers = get_advance_vouchers(
        suppliers, fiscal_year=fiscal_year, company=company)
    ...
    if tds_deducted:
        ...
        debit_note_amount = get_debit_note_amount(
            suppliers, year_start_date, year_end_date)
...
def get_advance_vouchers(suppliers, fiscal_year=None, company=None, from_date=None, to_date=None):
    ...
    if from_date and to_date:
        condition += "and posting_date between %s and %s" % (
            from_date, to_date)
