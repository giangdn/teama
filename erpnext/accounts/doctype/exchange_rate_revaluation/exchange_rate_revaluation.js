...
frappe.ui.form.on("Exchange Rate Revaluation", {
			...
			frappe.db.get_value(
				...
				r => {
					...
					frm.doc.accounts.forEach(d => {
						total_amt =
							total_amt + d["new_balance_in_base_currency"];
					});
					if (total_amt !== r.sum) {
	...
	get_total_gain_loss: function(frm) {
		...
		frm.doc.accounts.forEach(d => {
			d.gain_loss =
				flt(
					d.new_balance_in_base_currency,
					precision("new_balance_in_base_currency", d)
				) -
				flt(
					d.balance_in_base_currency,
					precision("balance_in_base_currency", d)
				);
