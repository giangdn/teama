...
def generate_data_from_excel(file_doc, extension, as_dict=False):
    ...
    for row in rows:
        if as_dict:
            data.append({frappe.scrub(header): row[index]
...
def get_coa(doctype, parent, is_root=False, file_name=None):
    ...
    accounts = build_tree_from_json("", chart_data=forest)
...
def build_forest(data):
    ...
    def set_nested(d, path, value):
        reduce(lambda d, k: d.setdefault(k, {}),
               path[:-1], d)[path[-1]] = value
        return d
    ...
    for i in data:
        ...
        if not account_name:
            error_messages.append(
                "Row {0}: Please enter Account Name".format(line_no))
    ...
    for path in paths:
        for n, account_name in enumerate(path):
            ...
            set_nested(out, path[:n+1], charts_map[account_name])
...
def get_template(template_type):
    ...
    fields = ["Account Name", "Parent Account", "Account Number",
              "Is Group", "Account Type", "Root Type"]
    ...
    if template_type == 'Blank Template':
        ...
        for account_type in get_mandatory_account_types():
            writer.writerow(['', '', '', 0, account_type.get(
                'account_type'), account_type.get('root_type')])
    else:
...
def get_sample_template(writer):
    template = [
        ...
        ["Income", "", "", 1, "", "Income"],
        ["Bank Accounts",
         "Application Of Funds(Assets)", "", 1, "Bank", "Asset"],
        ["Cash In Hand",
         "Application Of Funds(Assets)", "", 1, "Cash", "Asset"],
        ["Stock Assets",
         "Application Of Funds(Assets)", "", 1, "Stock", "Asset"],
        ["Cost Of Goods Sold", "Expenses", "",
         0, "Cost of Goods Sold", "Expense"],
        ["Asset Depreciation", "Expenses", "", 0, "Depreciation", "Expense"],
        ["Fixed Assets",
         "Application Of Funds(Assets)", "", 0, "Fixed Asset", "Asset"],
        ["Accounts Payable",
         "Sources Of Funds(Liabilities)", "", 0, "Payable", "Liability"],
        ["Accounts Receivable",
         "Application Of Funds(Assets)", "", 1, "Receivable", "Asset"],
...
def validate_root(accounts):
    roots = [accounts[d]
             for d in accounts if not accounts[d].get('parent_account')]
    if len(roots) < 4:
    ...
    for account in roots:
        if not account.get("root_type") and account.get("account_name"):
            error_messages.append(
                "Please enter Root Type for account- {0}".format(account.get("account_name")))
        elif account.get("root_type") not in get_root_types() and account.get("account_name"):
            error_messages.append("Root Type for {0} must be one of the Asset, Liability, Income, Expense and Equity".format(
...
def validate_account_types(accounts):
    account_types_for_ledger = ["Cost of Goods Sold", "Depreciation",
                                "Fixed Asset", "Payable", "Receivable", "Stock Adjustment"]
    account_types = [accounts[d]["account_type"]
                     for d in accounts if not accounts[d]['is_group'] == 1]
    ...
    # fix logic bug
    account_groups = [accounts[d]["account_type"]
                      for d in accounts if accounts[d]['is_group'] == 1]
