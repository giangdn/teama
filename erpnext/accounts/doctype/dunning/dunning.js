...
frappe.ui.form.on("Dunning", {
	...
	calculate_interest_and_amount: function(frm) {
		const interest_per_year =
			(frm.doc.outstanding_amount * frm.doc.rate_of_interest) / 100;
		const interest_amount = flt(
			(interest_per_year * cint(frm.doc.overdue_days)) / 365 || 0,
			precision("interest_amount")
		);
		const dunning_amount = flt(
			interest_amount + frm.doc.dunning_fee,
			precision("dunning_amount")
		);
		const grand_total = flt(
			frm.doc.outstanding_amount + dunning_amount,
