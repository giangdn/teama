...
class Dunning(AccountsController):
    ...
    def validate_overdue_days(self):
        self.overdue_days = (getdate(self.posting_date) -
                             getdate(self.due_date)).days or 0

    def validate_amount(self):
        amounts = calculate_interest_and_amount(
            self.posting_date, self.outstanding_amount, self.rate_of_interest, self.dunning_fee, self.overdue_days)
        if self.interest_amount != amounts.get('interest_amount'):
            self.interest_amount = flt(amounts.get(
                'interest_amount'), self.precision('interest_amount'))
        if self.dunning_amount != amounts.get('dunning_amount'):
            self.dunning_amount = flt(amounts.get(
                'dunning_amount'), self.precision('dunning_amount'))
        if self.grand_total != amounts.get('grand_total'):
            self.grand_total = flt(amounts.get(
                'grand_total'), self.precision('grand_total'))
    ...
    for reference in doc.references:
        if reference.reference_doctype == 'Sales Invoice' and reference.outstanding_amount <= 0:
            ...
            for dunning in dunnings:
                frappe.db.set_value("Dunning", dunning.name,
                                    "status", 'Resolved')
...
def calculate_interest_and_amount(posting_date, outstanding_amount, rate_of_interest, dunning_fee, overdue_days):
    ...
    grand_total = 0
    if rate_of_interest:
        interest_per_year = flt(outstanding_amount) * \
            flt(rate_of_interest) / 100
        interest_amount = (interest_per_year * cint(overdue_days)) / 365
        grand_total = flt(outstanding_amount) + \
            flt(interest_amount) + flt(dunning_fee)
