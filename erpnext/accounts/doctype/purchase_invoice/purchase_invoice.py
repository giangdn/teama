...
class PurchaseInvoice(BuyingController):
    ...
    def set_expense_account(self, for_validate=False):
        ...
        asset_items = [
            d.is_fixed_asset for d in self.items if d.is_fixed_asset]
            ...
            if auto_accounting_for_stock and item.item_code in stock_items \
                ...
                if self.update_stock and (not item.from_warehouse):
                    if for_validate and item.expense_account and item.expense_account != warehouse_account[item.warehouse]["account"]:
                        msg = _("Row {}: Expense Head changed to {} ").format(
                            item.idx, frappe.bold(warehouse_account[item.warehouse]["account"]))
                        msg += _("because account {} is not linked to warehouse {} ").format(
                    ...
                    if item.purchase_receipt:
                        ...
                        if negative_expense_booked_in_pr:
                            if for_validate and item.expense_account and item.expense_account != stock_not_billed_account:
                                msg = _("Row {}: Expense Head changed to {} ").format(
                                    item.idx, frappe.bold(stock_not_billed_account))
                                msg += _("because expense is booked against this account in Purchase Receipt {}").format(
                        ...
                        if for_validate and item.expense_account and item.expense_account != stock_not_billed_account:
                            msg = _("Row {}: Expense Head changed to {} ").format(
                                item.idx, frappe.bold(stock_not_billed_account))
                            msg += _("as no Purchase Receipt is created against Item {}. ").format(
        ...
        if frappe.db.get_value("Buying Settings", None, "po_required") == 'Yes':
            ...
            for d in self.get('items'):
                if not d.purchase_order:
                    ...
                    msg += "<br><br>"
                    msg += _("To submit the invoice without purchase order please set {} ").format(
                        frappe.bold(_('Purchase Order Required')))
                    msg += _("as {} in {}").format(frappe.bold('No'), get_link_to_form(
            ...
            for d in self.get('items'):
                if not d.purchase_receipt and d.item_code in stock_items:
                    msg = _("Purchase Receipt Required for item {}").format(
                        frappe.bold(d.item_code))
                    msg += "<br><br>"
                    msg += _("To submit the invoice without purchase receipt please set {} ").format(
            ...
            if flt(item.base_net_amount):
                ...
                if self.update_stock and self.auto_accounting_for_stock and item.item_code in stock_items:
                    ...
                    if not item.is_fixed_asset:
                        ...
                        amount = flt(
                            item.base_net_amount + item.item_tax_amount, item.precision("base_net_amount"))

                    auto_accounting_for_non_stock_items = cint(frappe.db.get_value(
                        'Company', self.company, 'enable_perpetual_inventory_for_non_stock_items'))
                    ...
                    if self.update_stock and item.landed_cost_voucher_amount:
                        ...
                        assets = frappe.db.get_all('Asset',
                                                   filters={
                                                       'purchase_invoice': self.name, 'item_code': item.item_code}
                                                   )
                        for asset in assets:
                            frappe.db.set_value(
                                "Asset", asset.name, "gross_purchase_amount", flt(item.valuation_rate))
                            frappe.db.set_value(
    ...
    def get_asset_gl_entry(self, gl_entries):
        arbnb_account = self.get_company_default(
            "asset_received_but_not_billed")
        eiiav_account = self.get_company_default(
            "expenses_included_in_asset_valuation")

        for item in self.get("items"):
            if item.is_fixed_asset:
                asset_amount = flt(item.net_amount) + \
                    flt(item.item_tax_amount/self.conversion_rate)
                base_asset_amount = flt(
                ...
                if not self.update_stock:
                    ...
                    if item.item_tax_amount:
                        asset_eiiav_currency = get_account_currency(
                            eiiav_account)
                        gl_entries.append(self.get_gl_dict({
        ...
        for tax in self.get("taxes"):
            ...
            if self.is_opening == "No" and tax.category in ("Valuation", "Valuation and Total") and flt(tax.base_tax_amount_after_discount_amount) \
                ...
                if self.auto_accounting_for_stock and not tax.cost_center:
                    frappe.throw(_("Cost Center is required in row {0} in Taxes table for type {1}").format(
        ...
        if self.bill_no:
            if cint(frappe.db.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
                ...
                if pi:
                    pi = pi[0][0]
                    frappe.throw(
                        _("Supplier Invoice No exists in Purchase Invoice {0}").format(pi))
    ...
    def update_billing_status_in_pr(self, update_modified=True):
        ...
        for d in self.get("items"):
            if d.pr_detail:
                ...
                frappe.db.set_value("Purchase Receipt Item", d.pr_detail,
...
def on_doctype_update():
    frappe.db.add_index("Purchase Invoice", [
                        "supplier", "is_return", "return_against"])
