...
class POSInvoice(SalesInvoice):
    ...
    def validate(self):
        if not cint(self.is_pos):
            frappe.throw(_("POS Invoice should have {} field checked.").format(
    ...
    def on_submit(self):
        ...
        if self.loyalty_program:
            ...
            against_psi_doc = frappe.get_doc(
                "POS Invoice", self.return_against)
        ...
        for d in self.get('items'):
            ...
            if d.serial_no:
                ...
                if flt(available_stock) <= 0:
                    msg = (_('Row #{}: Item Code: {} is not available under warehouse {}.').format(
    ...
    def validate_non_stock_items(self):
        for d in self.get("items"):
            is_stock_item = frappe.get_cached_value(
                "Item", d.get("item_code"), "is_stock_item")
    ...
    def validate_change_account(self):
        if self.change_amount and self.account_for_change_amount and \
            ...
            frappe.throw(_("The selected change account {} doesn't belongs to Company {}.").format(
                self.account_for_change_amount, self.company))

    def validate_change_amount(self):
        grand_total = flt(self.rounded_total) or flt(self.grand_total)
        base_grand_total = flt(self.base_rounded_total) or flt(
        ...
        if not flt(self.change_amount) and grand_total < flt(self.paid_amount):
            ...
            self.base_change_amount = flt(
                self.base_paid_amount - base_grand_total + flt(self.base_write_off_amount))
