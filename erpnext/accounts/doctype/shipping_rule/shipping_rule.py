...
class ShippingRule(Document):
    ...
    def validate_from_to_values(self):
        ...
        for d in self.get("conditions"):
            ...
            if not d.to_value:
                ...
                throw(_("From value must be less than to value in row {0}").format(
                    d.idx), FromGreaterThanToError)
    ...
    def validate_countries(self, doc):
        ...
        if self.countries:
            ...
            if shipping_country not in [d.country for d in self.countries]:
                frappe.throw(_('Shipping rule not applicable for country {0} in Shipping Address').format(
                    shipping_country))
    ...
    def sort_shipping_rule_conditions(self):
        ...
        self.shipping_rules_conditions = sorted(
            self.conditions, key=lambda d: flt(d.from_value))
