...
class Budget(Document):
        ...
        for d in self.get('accounts'):
            if d.account:
                ...
                if account_details.is_group:
                    frappe.throw(
                        _("Budget cannot be assigned against Group Account {0}").format(d.account))
                ...
                if d.account in account_list:
                    frappe.throw(
                        _("Account {0} has been entered multiple times").format(d.account))
                else:
    ...
    def validate_applicable_for(self):
        if (self.applicable_on_material_request
            ...
            frappe.throw(
                _("Please enable Applicable on Booking Actual Expenses"))
...
def validate_expense_against_budget(args):
    ...
    if args.get('company') and not args.fiscal_year:
        args.fiscal_year = get_fiscal_year(
            args.get('posting_date'), company=args.get('company'))[0]
...
def get_expense_cost_center(doctype, args):
    if doctype == 'Item Group':
        return frappe.db.get_value('Item Default',
                                   {'parent': args.get(frappe.scrub(
                                       doctype)), 'company': args.get('company')},
                                   ['buying_cost_center', 'expense_account'])
    else:
