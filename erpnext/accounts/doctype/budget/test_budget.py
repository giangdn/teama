...
class TestBudget(unittest.TestCase):
    ...
    def test_exception_approver_role(self):
        ...
        self.assertEqual(frappe.db.get_value(
            'Journal Entry', jv.name, 'docstatus'), 1)
        jv.cancel()
    ...
    def test_monthly_budget_crossed_for_po(self):
        ...
        po = create_purchase_order(
            transaction_date=nowdate(), do_not_submit=True)
...
def make_budget(**args):
    ...
    if budget_against == "Project":
        ...
        budget_list = frappe.get_all("Budget", fields=["name"], filters={
                                     "name": ("like", project_name)})
    else:
        cost_center_name = "{0}%".format(
            cost_center or "_Test Cost Center - _TC/" + fiscal_year)
        budget_list = frappe.get_all("Budget", fields=["name"], filters={
                                     "name": ("like", cost_center_name)})
    for d in budget_list:
        frappe.db.sql("delete from `tabBudget` where name = %(name)s", d)
        frappe.db.sql(
            "delete from `tabBudget Account` where parent = %(name)s", d)

    budget = frappe.new_doc("Budget")

    if budget_against == "Project":
        budget.project = frappe.get_value(
            "Project", {"project_name": "_Test Project"})
    else:
        budget.cost_center = cost_center or "_Test Cost Center - _TC"

    monthly_distribution = frappe.get_doc(
        "Monthly Distribution", "_Test Distribution")
