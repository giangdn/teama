...
class PaymentRequest(Document):
    ...
    def validate_reference_document(self):
        if not self.reference_doctype or not self.reference_name:
            frappe.throw(
                _("To create a Payment Request reference document is required"))
    ...
    def validate_currency(self):
        ...
        if self.payment_account and ref_doc.currency != frappe.db.get_value("Account", self.payment_account, "account_currency"):
            frappe.throw(
                _("Transaction currency must be same as Payment Gateway currency"))
    ...
    def validate_subscription_details(self):
        if self.is_a_subscription:
            ...
            for subscription_plan in self.subscription_plans:
                ...
                rate = get_plan_rate(subscription_plan.plan,
                                     quantity=subscription_plan.qty)

                amount += rate

            if amount != self.grand_total:
                frappe.msgprint(_("The amount of {0} set in this payment request is different from the calculated amount of all payment plans: {1}. Make sure this is correct before submitting the document.").format(
    ...
    def get_request_amount(self):
        ...
        request_amounts = sum([json.loads(d).get('request_amount')
                               for d in data_of_completed_requests])
    ...
    def make_invoice(self):
        ...
        if (hasattr(ref_doc, "order_type") and getattr(ref_doc, "order_type") == "Shopping Cart"):
            ...
            si = make_sales_invoice(
                self.reference_name, ignore_permissions=True)
        ...
        if status in ["Authorized", "Completed"]:
            ...
            if (shopping_cart_settings.enabled and hasattr(frappe.local, "session")
                ...
                if success_url:
                    ...
                    redirect_to = get_url(
                        "/orders/{0}".format(self.reference_name))
...
def make_payment_request(**args):
    ...
    if args.loyalty_points and args.dt == "Sales Order":
        ...
        frappe.db.set_value("Sales Order", args.dn, "loyalty_points", int(
            args.loyalty_points), update_modified=False)
        frappe.db.set_value("Sales Order", args.dn, "loyalty_amount",
                            loyalty_amount, update_modified=False)
    ...
    if existing_payment_request:
        frappe.db.set_value("Payment Request", existing_payment_request,
                            "grand_total", grand_total, update_modified=False)
...
def get_gateway_details(args):
    ...
    if args.order_type == "Shopping Cart":
        payment_gateway_account = frappe.get_doc(
            "Shopping Cart Settings").payment_gateway_account
...
def get_subscription_details(reference_doctype, reference_name):
    if reference_doctype == "Sales Invoice":
        subscriptions = frappe.db.sql(
            """SELECT parent as sub_name FROM `tabSubscription Invoice` WHERE invoice=%s""", reference_name, as_dict=1)
        subscription_plans = []
