...
class ProcessStatementOfAccounts(Document):
    def validate(self):
        ...
        if self.enable_auto_email:
            ...
            self.from_date = add_months(
                self.to_date, -1 * self.filter_duration)
...
def fetch_customers(customer_collection, collection_name, primary_mandatory):
    ...
    for customer in customers:
        ...
        billing_email = get_customer_emails(
            customer.name, 1, billing_and_primary=False)
...
def get_customer_emails(customer_name, primary_mandatory, billing_and_primary=True):
    ...
    if len(billing_email) == 0 or (billing_email[0][0] is None):
        if billing_and_primary:
            frappe.throw(
                'No billing email found for customer: ' + customer_name)
        else:
            return ''
    ...
    if billing_and_primary:
        ...
        if primary_email is None and int(primary_mandatory):
            frappe.throw(
                'No primary email found for customer: ' + customer_name)
...
def send_emails(document_name, from_scheduler=False):
    ...
    if report:
        ...
        if doc.enable_auto_email and from_scheduler:
            ...
            doc.add_comment('Comment', 'Emails sent on: ' +
...
@frappe.whitelist()
def send_auto_email():
    selected = frappe.get_list('Process Statement Of Accounts', filters={
                               'to_date': format_date(today()), 'enable_auto_email': 1})
