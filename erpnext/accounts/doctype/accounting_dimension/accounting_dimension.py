...
class AccountingDimension(Document):
    ...
    def validate(self):
        if self.document_type in core_doctypes_list + ('Accounting Dimension', 'Project',
            ...
            msg = _("Not allowed to create accounting dimension for {0}").format(
                self.document_type)
            frappe.throw(msg)

        exists = frappe.db.get_value("Accounting Dimension", {
    ...
    def validate_document_type_change(self):
        doctype_before_save = frappe.db.get_value(
            "Accounting Dimension", self.name, "document_type")
...
def delete_accounting_dimension(doc):
    ...
    budget_against_property = frappe.get_doc(
        "Property Setter", "Budget-budget_against-options")
...
def get_accounting_dimensions(as_list=True):
    accounting_dimensions = frappe.get_all("Accounting Dimension", fields=[
                                           "label", "fieldname", "disabled", "document_type"])
