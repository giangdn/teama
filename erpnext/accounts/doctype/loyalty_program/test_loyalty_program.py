...
class TestLoyaltyProgram(unittest.TestCase):
    ...
    def test_loyalty_points_earned_single_tier(self):
        ...
        lpe = frappe.get_doc('Loyalty Point Entry', {
                             'invoice_type': 'Sales Invoice', 'invoice': si_original.name, 'customer': si_original.customer})

        self.assertEqual(si_original.get('loyalty_program'),
                         customer.loyalty_program)
        self.assertEqual(lpe.get('loyalty_program_tier'),
        ...
        lpe_redeem = frappe.get_doc('Loyalty Point Entry', {
                                    'invoice_type': 'Sales Invoice', 'invoice': si_redeem.name, 'redeem_against': lpe.name})
        lpe_earn = frappe.get_doc('Loyalty Point Entry', {
    ...
    def test_loyalty_points_earned_multiple_tier(self):
        frappe.db.set_value("Customer", "Test Loyalty Customer",
                            "loyalty_program", "Test Multiple Loyalty")
        # assign multiple tier program to the customer
        customer = frappe.get_doc(
            'Customer', {"customer_name": "Test Loyalty Customer"})
        customer.loyalty_program = frappe.get_doc(
            'Loyalty Program', {'loyalty_program_name': 'Test Multiple Loyalty'}).name
        customer.save()
        ...
        self.assertEqual(lpe_earn.loyalty_points, earned_after_redemption)
        self.assertEqual(lpe_redeem.loyalty_points, (-1*earned_points))
        self.assertEqual(lpe_earn.loyalty_program_tier,
                         customer.loyalty_program_tier)
    ...
    def test_sales_invoice_return(self):
        ...
        earned_points = get_points_earned(si_original)
        lpe_original = frappe.get_doc('Loyalty Point Entry', {
                                      'invoice_type': 'Sales Invoice', 'invoice': si_original.name, 'customer': si_original.customer})
        ...
        earned_points = get_points_earned(si_original)
        lpe_after_return = frappe.get_doc('Loyalty Point Entry', {
                                          'invoice_type': 'Sales Invoice', 'invoice': si_original.name, 'customer': si_original.customer})
        self.assertEqual(lpe_after_return.loyalty_points, earned_points)
        self.assertEqual(True, (lpe_original.loyalty_points >
                                lpe_after_return.loyalty_points))
    ...
    def test_loyalty_points_for_dashboard(self):
        ...
        company_wise_info = get_dashboard_info(
            "Customer", doc.name, doc.loyalty_program)
...
def get_points_earned(self):
    ...
    if lp_details and getdate(lp_details.from_date) <= getdate(self.posting_date) and \
        ...
        eligible_amount = flt(self.grand_total) - \
            cint(self.loyalty_amount) - returned_amount
