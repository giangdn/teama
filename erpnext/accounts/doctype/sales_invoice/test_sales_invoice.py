...
class TestSalesInvoice(unittest.TestCase):
    ...
    def test_add_terms_after_save(self):
        ...
        self.assertEqual(
            getdate(si.payment_schedule[0].due_date), getdate(si.due_date))
    ...
    def test_discount_amount_gl_entry(self):
        frappe.db.set_value("Company", "_Test Company",
                            "round_off_account", "Round Off - _TC")
        si = frappe.copy_doc(test_records[3])
        si.discount_amount = 104.94
        si.append("taxes", {
    ...
    def test_payment(self):
        ...
        self.assertEqual(frappe.db.get_value(
            "Sales Invoice", w.name, "outstanding_amount"), 162.0)
        ...
        self.assertTrue(link_doctypes.index('GL Entry') >
                        link_doctypes.index('Journal Entry Account'))

        jv.cancel()
        self.assertEqual(frappe.db.get_value(
            "Sales Invoice", w.name, "outstanding_amount"), 562.0)
    ...
    def test_pos_gl_entry_with_perpetual_inventory(self):
        ...
        pos.append("payments", {'mode_of_payment': 'Bank Draft',
                                'account': '_Test Bank - TCP1', 'amount': 50})
        pos.append("payments", {'mode_of_payment': 'Cash',
                                'account': 'Cash - TCP1', 'amount': 50})
    ...
    def test_pos_returns_with_repayment(self):
        ...
        pos.append("payments", {'mode_of_payment': 'Bank Draft',
                                'account': '_Test Bank - _TC', 'amount': 500})
        pos.append("payments", {'mode_of_payment': 'Cash',
                                'account': 'Cash - _TC', 'amount': 500})
        pos.insert()
        pos.submit()
    ...
    def pos_gl_entry(self, si, pos, cash_amount):
        ...
        stock_in_hand = get_inventory_account(
        ...
        expected_gl_entries = sorted([
            ...
            [pos.items[0].expense_account, abs(
    ...
    def test_serialized(self):
        ...
        self.assertFalse(frappe.db.get_value(
            "Serial No", serial_nos[0], "warehouse"))
    ...
    def test_serialized_cancel(self):
        ...
        self.assertEqual(frappe.db.get_value(
            "Serial No", serial_nos[0], "warehouse"), "_Test Warehouse - _TC")
        self.assertFalse(frappe.db.get_value("Serial No", serial_nos[0],
                                             "delivery_document_no"))
        self.assertFalse(frappe.db.get_value(
            "Serial No", serial_nos[0], "sales_invoice"))
    ...
    def test_serial_numbers_against_delivery_note(self):
        ...
        dn = create_delivery_note(item=se.get(
            "items")[0].item_code, serial_no=serial_nos[0])
        dn.submit()
        ...
        self.assertEqual(
            si.get("items")[0].serial_no, dn.get("items")[0].serial_no)

    def test_return_sales_invoice(self):
        make_stock_entry(item_code="_Test Item",
                         target="Stores - TCP1", qty=50, basic_rate=100)

        actual_qty_0 = get_qty_after_transaction(
            item_code="_Test Item", warehouse="Stores - TCP1")

        si = create_sales_invoice(qty=5, rate=500, update_stock=1, company="_Test Company with perpetual inventory", debit_to="Debtors - TCP1", item_code="_Test Item",
                                  warehouse="Stores - TCP1", income_account="Sales - TCP1", expense_account="Cost of Goods Sold - TCP1", cost_center="Main - TCP1")
        ...
        # return entry
        si1 = create_sales_invoice(is_return=1, return_against=si.name, qty=-2, rate=500, update_stock=1, company="_Test Company with perpetual inventory", debit_to="Debtors - TCP1",
                                   item_code="_Test Item", warehouse="Stores - TCP1", income_account="Sales - TCP1", expense_account="Cost of Goods Sold - TCP1", cost_center="Main - TCP1")
        ...
        self.assertEqual(frappe.db.get_value(
            "Sales Invoice", si.name, "outstanding_amount"), 1500)
        ...
        for d in si.get("taxes"):
            for i, k in enumerate(expected_values["keys"]):
                if expected_values.get(d.account_head):
                    self.assertEqual(
                        d.get(k), expected_values[d.account_head][i])
    ...
    def test_multi_currency_gle(self):
        ...
        for field in ("account_currency", "debit", "debit_in_account_currency", "credit", "credit_in_account_currency"):
            for i, gle in enumerate(gl_entries):
                self.assertEqual(
                    expected_values[gle.account][field], gle[field])
    ...
    def test_create_so_with_margin(self):
        si = create_sales_invoice(
            item_code="_Test Item", qty=1, do_not_submit=True)
        price_list_rate = 100
        si.items[0].price_list_rate = price_list_rate
        si.items[0].margin_type = 'Percentage'
        si.items[0].margin_rate_or_amount = 25
        si.save()
        self.assertEqual(si.get("items")[0].rate, flt(
    ...
    def test_item_wise_tax_breakup_india(self):
        ...
        itemised_tax, itemised_taxable_amount = get_itemised_tax_breakup_data(
            si)
        ...
        self.assertEqual(itemised_taxable_amount,
                         expected_itemised_taxable_amount)
    ...
    def test_company_monthly_sales(self):
        ...
        current_month_sales = frappe.get_cached_value(
            'Company',  "_Test Company",  "total_monthly_sales")
        self.assertEqual(current_month_sales,
                         existing_current_month_sales + si.base_grand_total)
    ...
    def test_duplicate_due_date_in_terms(self):
        si = create_sales_invoice(do_not_save=1)
        si.append('payment_schedule', dict(due_date='2017-01-01',
                                           invoice_portion=50.00, payment_amount=50))
        si.append('payment_schedule', dict(due_date='2017-01-01',
                                           invoice_portion=50.00, payment_amount=50))
    ...
    def test_internal_transfer_gl_entry(self):
        ...
        check_gl_entries(self, target_doc.name, pi_gl_entries,
                         add_days(nowdate(), -1))
    ...
    def test_eway_bill_json(self):
        ...
        self.assertEqual(data['billLists'][0]['itemList']
                         [0]['taxableAmount'], 60000)

    def test_einvoice_submission_without_irn(self):
        # init
        frappe.db.set_value('E Invoice Settings',
                            'E Invoice Settings', 'enable', 1)
    ...
    def test_einvoice_json(self):
        ...
        for item in einvoice['ItemList']:
            ...
            self.assertTrue(item['TotItemVal'], item['AssAmt'] +
        ...
        self.assertTrue(value_details['TotInvVal'] -
                        calculated_invoice_value < 0.1)
...
def check_gl_entries(doc, voucher_no, expected_gle, posting_date):
    ...
    def test_item_tax_validity(self):
        ...
        sales_invoice = create_sales_invoice(
            item="_Test Item 2", do_not_save=1)
