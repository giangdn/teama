...
class SalesInvoice(SellingController):
    ...
    def validate(self):
        ...
        self.validate_multiple_billing(
            "Delivery Note", "dn_detail", "amount", "items")
            ...
            if d.is_fixed_asset and d.meta.get_field("asset") and d.asset:
                ...
                if self.doctype == "Sales Invoice" and self.docstatus == 1:
                    if self.update_stock:
                        ...
                        frappe.throw(_("Row #{0}: Asset {1} cannot be submitted, it is already {2}").format(
    ...
    def validate_item_cost_centers(self):
        for item in self.items:
            ...
            if cost_center_company != self.company:
                frappe.throw(_("Row #{0}: Cost Center {1} does not belong to company {2}").format(
    ...
    def on_cancel(self):
        ...
        self.ignore_linked_doctypes = (
            'GL Entry', 'Stock Ledger Entry', 'Repost Item Valuation')
    ...
    def set_pos_fields(self, for_validate=False):
        ...
        if pos:
            ...
            if self.customer:
                ...
                selling_price_list = customer_price_list or customer_group_price_list or pos.get(
                    'selling_price_list')
            else:
            ...
            for item in self.get("items"):
                if item.get('item_code'):
                    profile_details = get_pos_profile_item_details(
                        pos, frappe._dict(item.as_dict()), pos)
    ...
    def validate_debit_to_acc(self):
        ...
        if account.report_type != "Balance Sheet":
            msg = _("Please ensure {} account is a Balance Sheet account. ").format(
    ...
    def so_dn_required(self):
        ...
        prev_doc_field_map = {'Sales Order': [
            'so_required', 'is_pos'], 'Delivery Note': ['dn_required', 'update_stock']}
            ...
            if frappe.db.get_single_value('Selling Settings', value[0]) == 'Yes':
                ...
                for d in self.get('items'):
                    if (d.item_code and not d.get(key.lower().replace(' ', '_')) and not self.get(value[1])):
                        msgprint(_("{0} is mandatory for Item {1}").format(
                            key, d.item_code), raise_exception=1)
    ...
    def validate_item_code(self):
        for d in self.get('items'):
            if not d.item_code and self.is_opening == "No":
                msgprint(_("Item Code required at Row No {0}").format(
    ...
    def validate_delivery_note(self):
        for d in self.get("items"):
            if d.delivery_note:
                msgprint(_("Stock cannot be updated against Delivery Note {0}").format(
                    d.delivery_note), raise_exception=1)
    ...
    def update_current_stock(self):
        for d in self.get('items'):
            if d.item_code and d.warehouse:
                bin = frappe.db.sql(
                    "select actual_qty from `tabBin` where item_code = %s and warehouse = %s", (d.item_code, d.warehouse), as_dict=1)
                d.actual_qty = bin and flt(bin[0]['actual_qty']) or 0

        for d in self.get('packed_items'):
            bin = frappe.db.sql(
                "select actual_qty, projected_qty from `tabBin` where item_code =	%s and warehouse = %s", (d.item_code, d.warehouse), as_dict=1)
            d.actual_qty = bin and flt(bin[0]['actual_qty']) or 0
        ...
        if gl_entries:
            ...
            if self.docstatus == 1:
                make_gl_entries(gl_entries, update_outstanding=update_outstanding,
                                merge_entries=False, from_repost=from_repost)
    ...
    def make_customer_gl_entry(self, gl_entries):
        ...
        grand_total = self.rounded_total if (
    ...
    def update_billing_status_in_dn(self, update_modified=True):
        ...
        for d in self.get("items"):
            if d.dn_detail:
                ...
                frappe.db.set_value("Delivery Note Item", d.dn_detail,
                                    "billed_amt", billed_amt, update_modified=update_modified)
        ...
        for item in self.items:
            ...
            for serial_no in item.serial_no.split("\n"):
                if serial_no and frappe.db.get_value('Serial No', serial_no, 'item_code') == item.item_code:
                    frappe.db.set_value(
                        'Serial No', serial_no, 'sales_invoice', invoice)
    ...
    def validate_serial_against_delivery_note(self):
        ...
        for item in self.items:
            ...
            serial_nos = frappe.db.get_value(
                "Delivery Note Item", item.dn_detail, "serial_no") or ""
            ...
            if si_serial_nos - dn_serial_nos:
                frappe.throw(
                    _("Serial Numbers in row {0} does not match with Delivery Note").format(item.idx))
        ...
        for item in self.items:
            ...
            for serial_no in item.serial_no.split("\n"):
                ...
                if serial_no_details.sales_invoice and serial_no_details.item_code == item.item_code \
                    ...
                    sales_invoice_company = frappe.db.get_value(
                        "Sales Invoice", serial_no_details.sales_invoice, "company")
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
    ...
    if inter_company_reference:
        ...
        if not frappe.db.get_value(partytype, {"represents_company": doc.company}, "name") == party:
            frappe.throw(
                _("Invalid {0} for Inter Company Transaction.").format(partytype))
        if not frappe.get_cached_value(ref_partytype, ref_party, "represents_company") == company:
            frappe.throw(_("Invalid Company for Inter Company Transaction."))

    elif frappe.db.get_value(partytype, {"name": party, internal: 1}, "name") == party:
        companies = frappe.get_all("Allowed To Transact With", fields=[
                                   "company"], filters={"parenttype": partytype, "parent": party})
        companies = [d.company for d in companies]
        if not company in companies:
            frappe.throw(_("{0} not allowed to transact with {1}. Please change the Company.").format(
                partytype, company))
...
def set_account_for_mode_of_payment(self):
    for data in self.payments:
        if not data.account:
            data.account = get_bank_cash_account(
                data.mode_of_payment, self.company).get("account")


def get_inter_company_details(doc, doctype):
    if doctype in ["Sales Invoice", "Sales Order", "Delivery Note"]:
        parties = frappe.db.get_all("Supplier", fields=["name"], filters={
        ...
        if not parties:
            frappe.throw(_('No Supplier found for Inter Company Transactions which represents company {0}').format(
                frappe.bold(doc.company)))

        party = get_internal_party(parties, "Supplier", doc)
    else:
        parties = frappe.db.get_all("Customer", fields=["name"], filters={
                                    "disabled": 0, "is_internal_customer": 1, "represents_company": doc.company})
        company = frappe.get_cached_value(
            "Supplier", doc.supplier, "represents_company")

        if not parties:
            frappe.throw(_('No Customer found for Inter Company Transactions which represents company {0}').format(
                frappe.bold(doc.company)))
...
def validate_inter_company_transaction(doc, doctype):
    ...
    price_list = doc.selling_price_list if doctype in [
        "Sales Invoice", "Sales Order", "Delivery Note"] else doc.buying_price_list
    ...
    def update_details(source_doc, target_doc, source_parent):
        ...
        if target_doc.doctype in ["Purchase Invoice", "Purchase Order"]:
            ...
            update_address(target_doc, 'shipping_address',
                           'shipping_address_display', source_doc.customer_address)
            ...
            update_address(target_doc, 'company_address',
                           'company_address_display', source_doc.supplier_address)
        ...
        if doc.doctype == 'Purchase Receipt':
            ...
            if so_item_map:
                pd_item_map, parent_child_map, warehouse_map = \
                    get_pd_details('Purchase Order Item',
                                   so_item_map, 'sales_order_item')
            ...
            if list(dn_item_map.values()):
                pd_item_map, parent_child_map, warehouse_map = \
                    get_pd_details('Purchase Receipt Item',
                                   dn_item_map, 'delivery_note_item')
...
def update_pr_items(doc, sales_item_map, purchase_item_map, parent_child_map, warehouse_map):
    for item in doc.get('items'):
        item.purchase_order_item = purchase_item_map.get(
            sales_item_map.get(item.delivery_note_item))
        item.warehouse = warehouse_map.get(
            sales_item_map.get(item.delivery_note_item))
        item.purchase_order = parent_child_map.get(
            sales_item_map.get(item.delivery_note_item))
...
def update_multi_mode_option(doc, pos_profile):
    ...
    for pos_payment_method in pos_profile.get('payments'):
        ...
        payment_mode = get_mode_of_payment_info(
            pos_payment_method.mode_of_payment, doc.company)
        if not payment_mode:
            invalid_modes.append(get_link_to_form(
                "Mode of Payment", pos_payment_method.mode_of_payment))
            continue
...
def create_dunning(source_name, target_doc=None):
    ...
    def set_missing_values(source, target):
        ...
        overdue_days = (getdate(target.posting_date) -
                        getdate(source.due_date)).days
        ...
        if frappe.db.exists('Dunning Type', {'start_day': [
            ...
            letter_text = get_dunning_letter_text(
                dunning_type=dunning_type.name, doc=target.as_dict())
            if letter_text:
