...
def get_customers_list(pos_profile={}):
    ...
    if pos_profile.get('customer_groups'):
        ...
        for d in pos_profile.get('customer_groups'):
            customer_groups.extend([d.get('name') for d in get_child_nodes(
                'Customer Group', d.get('customer_group'))])
...
def get_items_list(pos_profile, company):
    ...
    if pos_profile.get('item_groups'):
        ...
        for d in pos_profile.get('item_groups'):
            args_list.extend(
                [d.name for d in get_child_nodes('Item Group', d.item_group)])
        if args_list:
            cond = "and i.item_group in (%s)" % (
                ', '.join(['%s'] * len(args_list)))
