...
class POSProfile(Document):
    ...
    def validate_duplicate_groups(self):
        ...
        if len(customer_groups) != len(set(customer_groups)):
            frappe.throw(_("Duplicate customer group found in the cutomer group table"),
        ...
        for d in self.payments:
            ...
            if not account:
                invalid_modes.append(get_link_to_form(
                    "Mode of Payment", d.mode_of_payment))
...
def get_item_groups(pos_profile):
    ...
    if pos_profile.get('item_groups'):
        ...
        for data in pos_profile.get('item_groups'):
            item_groups.extend(["%s" % frappe.db.escape(d.name)
