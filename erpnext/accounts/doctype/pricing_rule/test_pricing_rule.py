...
class TestPricingRule(unittest.TestCase):
    ...
    def test_pricing_rule_for_stock_qty(self):
        ...
        so = make_sales_order(item_code="_Test Item", qty=2,
                              uom="Box", do_not_submit=True)
        so.items[0].price_list_rate = 100
        so.submit()
        so = frappe.get_doc('Sales Order', so.name)
    ...
    def test_pricing_rule_with_margin_and_discount(self):
        ...
        make_pricing_rule(selling=1, margin_type="Percentage",
                          margin_rate_or_amount=10, discount_percentage=10)
    ...
    def test_cumulative_pricing_rule(self):
        frappe.delete_doc_if_exists(
            'Pricing Rule', '_Test Cumulative Pricing Rule')
        test_record = {
            "doctype": "Pricing Rule",
    ...
    def test_pricing_rule_for_condition(self):
        ...
        si = create_sales_invoice(
            do_not_submit=True, customer="_Test Customer 2", is_return=0)
        si.items[0].price_list_rate = 1000
        si.submit()
        item = si.items[0]
        ...
        si = create_sales_invoice(
            do_not_submit=True, customer="_Test Customer 1", is_return=1, qty=-1)
        si.items[0].price_list_rate = 1000
        si.submit()
        item = si.items[0]
        self.assertEquals(item.rate, 100)
        ...
        si = create_sales_invoice(
            do_not_submit=True, customer="_Test Customer 1", is_return=0)
        si.items[0].price_list_rate = 1000
        si.submit()
        item = si.items[0]
    ...
    def test_multiple_pricing_rules(self):
        ...
        si = create_sales_invoice(
            do_not_submit=True, customer="_Test Customer 1", qty=1)
