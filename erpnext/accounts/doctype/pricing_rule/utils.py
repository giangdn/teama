        ...
        if pricing_rule:
            ...
            if pricing_rule.get('apply_multiple_pricing_rules'):
                pricing_rule_dict.setdefault(
                    cint(pricing_rule.get("priority")), []).append(pricing_rule)
...
def _get_pricing_rules(apply_on, args, values):
    ...
    if apply_on_field in ['item_code', 'brand']:
        ...
        item_conditions = _get_tree_conditions(
            args, "Item Group", child_doc, False)

    conditions += get_other_conditions(conditions, values, args)
    warehouse_conditions = _get_tree_conditions(
        args, "Warehouse", '`tabPricing Rule`')
    ...
    if args.get(field):
        ...
        try:
            lft, rgt = frappe.db.get_value(
                parenttype, args.get(field), ["lft", "rgt"])
        except TypeError:
        ...
        if parent_groups:
            ...
            condition = "ifnull({table}.{field}, '') in ({parent_groups})".format(
                table=table,
                field=field,
                parent_groups=", ".join([frappe.db.escape(d)
                                         for d in parent_groups])
            )
...
def get_other_conditions(conditions, values, args):
    for field in ["company", "customer", "supplier", "campaign", "sales_partner"]:
        if args.get(field):
            conditions += " and ifnull(`tabPricing Rule`.{0}, '') in (%({1})s, '')".format(
                field, field)
            values[field] = args.get(field)
        else:
            conditions += " and ifnull(`tabPricing Rule`.{0}, '') = ''".format(
                field)

    for parenttype in ["Customer Group", "Territory", "Supplier Group"]:
        group_condition = _get_tree_conditions(
...
def filter_pricing_rules(args, pricing_rules, doc=None):
    ...
    if pricing_rules:
        ...
        if max_priority:
            pricing_rules = list(filter(lambda x: cint(
                x.priority) == max_priority, pricing_rules))
...
def apply_internal_priority(pricing_rules, field_set, args):
    ...
    for field in field_set:
        if args.get(field):
            ...
            filtered_rules = list(filter(lambda x: x.get(
                field) == args.get(field), pricing_rules))
            if filtered_rules:
                break
        ...
        for row in doc.get('items'):
            ...
            if pr_doc.mixed_conditions:
                ...
                if args.get("item_code") != row.get("item_code"):
                    amt = flt(row.get('qty')) * \
                        flt(row.get("price_list_rate") or args.get("rate"))

                sum_qty += flt(row.get("stock_qty")
                               ) or flt(args.get("stock_qty")) or flt(args.get("qty"))
                sum_amt += amt
...
def get_qty_amount_data_for_cumulative(pr_doc, doc, items=[]):
    ...
    date_field = 'transaction_date' if frappe.get_meta(
        doctype).has_field('transaction_date') else 'posting_date'
        ...
        for d in pricing_rules:
            if d.price_or_product_discount == 'Price':
                ...
                apply_pricing_rule_for_free_items(
                    doc, item_details.free_item_data)
...
def validate_coupon_code(coupon_name):
    ...
    if coupon.valid_from:
        if coupon.valid_from > getdate(today()):
            frappe.throw(
                _("Sorry, this coupon code's validity has not started"))
    ...
    if coupon:
        if transaction_type == 'used':
            if coupon.used < coupon.maximum_use:
                ...
                frappe.throw(_("{0} Coupon used are {1}. Allowed quantity is exhausted").format(
