...
frappe.ui.form.on("Journal Entry", {
	...
	voucher_type: function(frm) {
		...
		if (
			...
			((frm.doc.accounts || []).length === 1 &&
				!frm.doc.accounts[0].account)
		) {
	...
	from_template: function(frm) {
		if (frm.doc.from_template) {
			frappe.db
				.get_doc("Journal Entry Template", frm.doc.from_template)
				.then(doc => {
					frappe.model.clear_table(frm.doc, "accounts");
					frm.set_value({
						company: doc.company,
...
erpnext.accounts.JournalEntry = frappe.ui.form.Controller.extend({
	...
	setup_queries: function() {
		...
		me.frm.set_query("party_type", "accounts", function(doc, cdt, cdn) {
			...
			return {
				query:
					"erpnext.setup.doctype.party_type.party_type.get_party_type",
				filters: {
					account: row.account
				}
			};
		...
		me.frm.set_query("reference_name", "accounts", function(doc, cdt, cdn) {
			...
			if (jvd.reference_type === "Payroll Entry") {
				return {
					query:
						"erpnext.payroll.doctype.payroll_entry.payroll_entry.get_payroll_entries_for_jv"
				};
			}
			...
			if (
				...
				var party_account_field =
					jvd.reference_type === "Sales Invoice"
						? "debit_to"
						: "credit_to";
				out.filters.push([
					jvd.reference_type,
			...
			if (
				in_list(["Sales Order", "Purchase Order"], jvd.reference_type)
			) {
	...
	reference_name: function(doc, cdt, cdn) {
		...
		if (d.reference_name) {
			if (d.reference_type === "Purchase Invoice" && !flt(d.debit)) {
				...
				d.reference_type === "Journal Entry" &&
				!flt(d.credit) &&
				!flt(d.debit)
			) {
				this.get_outstanding(
...
cur_frm.cscript.select_print_heading = function(doc, cdt, cdn) {
	...
	} else cur_frm.pformat.print_heading = __("Journal Entry");
};
...
frappe.ui.form.on("Journal Entry Account", {
	party: function(frm, cdt, cdn) {
		...
		if (!d.account && d.party_type && d.party) {
			...
			return frm.call({
				method:
					"erpnext.accounts.doctype.journal_entry.journal_entry.get_party_account_and_balance",
				child: d,
				args: {
					company: frm.doc.company,
	...
	exchange_rate: function(frm, cdt, cdn) {
		var company_currency = frappe.get_doc(":Company", frm.doc.company)
			.default_currency;
...
$.extend(erpnext.journal_entry, {
	toggle_fields_based_on_currency: function(frm) {
		...
		$.each(field_label_map, function(fieldname, label) {
			...
			df.label = frm.doc.multi_currency
				? label + " in Account Currency"
				: label;
		});
	},
	...
	account_query: function(frm) {
		...
		if (!frm.doc.multi_currency) {
			$.extend(filters, {
				account_currency: frappe.get_doc(":Company", frm.doc.company)
					.default_currency
			});
		}
