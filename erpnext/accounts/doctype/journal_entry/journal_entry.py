...
class JournalEntry(AccountsController):
        ...
        for d in self.get("accounts"):
            if d.is_advance:
                if d.reference_type in ("Sales Order", "Purchase Order", "Employee Advance"):
                    advance_paid.setdefault(
                        d.reference_type, []).append(d.reference_name)

        for voucher_type, order_list in iteritems(advance_paid):
            for voucher_no in list(set(order_list)):
                frappe.get_doc(
                    voucher_type, voucher_no).set_total_advance_paid()
    ...
    def validate_stock_accounts(self):
        stock_accounts = get_stock_accounts(
            self.company, self.doctype, self.name)
    ...
    def update_invoice_discounting(self):
        def _validate_invoice_discounting_status(inv_disc, id_status, expected_status, row_id):
            ...
            if id_status != expected_status:
                frappe.throw(_("Row #{0}: Status must be {1} for Invoice Discounting {2}").format(
                    d.idx, expected_status, id_link))

        invoice_discounting_list = list(set(
        ...
        for d in self.get("accounts"):
            ...
            if account_type in ["Receivable", "Payable"]:
                if not (d.party_type and d.party):
                    frappe.throw(
                        _("Row {0}: Party Type and Party is required for Receivable / Payable account {1}").format(d.idx, d.account))
        ...
        for d in self.get('accounts'):
            if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
                if (d.party_type == 'Customer' and flt(d.credit) > 0) or \
                    ...
                    if d.is_advance == "No":
                        msgprint(_("Row {0}: Please check 'Is Advance' against Account {1} if this is an advance entry.").format(
                            d.idx, d.account), alert=True)
                    elif d.reference_type in ("Sales Order", "Purchase Order") and d.is_advance != "Yes":
                        frappe.throw(
                            _("Row {0}: Payment against Sales/Purchase Order should always be marked as advance").format(d.idx))
        ...
        for d in self.get("accounts"):
            ...
            if d.reference_type and d.reference_name and (d.reference_type in list(field_dict)):
                ...
                if d.reference_type == "Sales Order" and flt(d.debit) > 0:
                    frappe.throw(_("Row {0}: Debit entry can not be linked with a {1}").format(
                        d.idx, d.reference_type))

                if d.reference_type == "Purchase Order" and flt(d.credit) > 0:
                    frappe.throw(_("Row {0}: Credit entry can not be linked with a {1}").format(
                        d.idx, d.reference_type))
                ...
                if self.voucher_type not in ('Deferred Revenue', 'Deferred Expense'):
                    self.reference_totals[d.reference_name] += flt(
                        d.get(dr_or_cr))
                ...
                if not against_voucher:
                    frappe.throw(_("Row {0}: Invalid reference {1}").format(
                    ...
                    if self.voucher_type in ('Deferred Revenue', 'Deferred Expense') and d.reference_detail_no:
                        ...
                        if d.reference_type == "Sales Invoice":
                            party_account = get_party_account_based_on_invoice_discounting(
                                d.reference_name) or against_voucher[1]
                        else:
        ...
        for reference_name, total in iteritems(self.reference_totals):
            ...
            if reference_type in ("Sales Order", "Purchase Order"):
                ...
                if order.docstatus != 1:
                    frappe.throw(_("{0} {1} is not submitted").format(
                        reference_type, reference_name))

                if flt(order.per_billed) >= 100:
                    frappe.throw(_("{0} {1} is fully billed").format(
                        reference_type, reference_name))

                if cstr(order.status) == "Closed":
                    frappe.throw(_("{0} {1} is closed").format(
                        reference_type, reference_name))
    ...
    def set_amounts_in_company_currency(self):
        for d in self.get("accounts"):
            ...
            d.debit = flt(d.debit_in_account_currency *
                          flt(d.exchange_rate), d.precision("debit"))
            d.credit = flt(d.credit_in_account_currency *
                           flt(d.exchange_rate), d.precision("credit"))
    ...
    def set_exchange_rate(self):
        for d in self.get("accounts"):
            ...
            if not d.exchange_rate:
                frappe.throw(
                    _("Row {0}: Exchange Rate is mandatory").format(d.idx))
    ...
    def create_remarks(self):
        ...
        if self.cheque_no:
            if self.cheque_date:
                r.append(_('Reference #{0} dated {1}').format(
                    self.cheque_no, formatdate(self.cheque_date)))
            else:
                msgprint(_("Please enter Reference date"),
    ...
    def get_outstanding_invoices(self):
        ...
        for d in self.get_values():
            total += flt(d.outstanding_amount,
                         self.precision("credit", "accounts"))
    ...
    def validate_expense_claim(self):
        for d in self.accounts:
            if d.reference_type == "Expense Claim":
                ...
                pending_amount = flt(sanctioned_amount) - \
                    flt(reimbursed_amount)
                if d.debit > pending_amount:
                    frappe.throw(_("Row No {0}: Amount cannot be greater than Pending Amount against Expense Claim {1}. Pending Amount is {2}").format(
                        d.idx, d.reference_name, pending_amount))
    ...
    def validate_credit_debit_note(self):
        if self.stock_entry:
            ...
            if frappe.db.exists({"doctype": "Journal Entry", "stock_entry": self.stock_entry, "docstatus": 1}):
                frappe.msgprint(_("Warning: Another {0} # {1} exists against stock entry {2}").format(
...
def get_default_bank_cash_account(company, account_type=None, mode_of_payment=None, account=None):
    ...
    if mode_of_payment:
        account = get_bank_cash_account(
            mode_of_payment, company).get("account")
...
def get_payment_entry(ref_doc, args):
    ...
    if args.get("party_account"):
        ...
        exchange_rate = get_exchange_rate(ref_doc.get("posting_date") or ref_doc.get("transaction_date"),
                                          args.get("party_account"), args.get(
