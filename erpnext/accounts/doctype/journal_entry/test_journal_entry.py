...
class TestJournalEntry(unittest.TestCase):
    ...
    def jv_against_voucher_testcase(self, base_jv, test_voucher):
        dr_or_cr = "credit" if test_voucher.doctype in [
            "Sales Order", "Journal Entry"] else "debit"
        ...
        base_jv.get("accounts")[0].is_advance = "Yes" if (
            test_voucher.doctype in ["Sales Order", "Purchase Order"]) else "No"
        ...
        submitted_voucher = frappe.get_doc(
    ...
    def test_inter_company_jv(self):
        frappe.db.set_value("Account", "Sales Expenses - _TC",
                            "inter_company_account", 1)
        frappe.db.set_value("Account", "Buildings - _TC",
                            "inter_company_account", 1)
        frappe.db.set_value("Account", "Sales Expenses - _TC1",
                            "inter_company_account", 1)
        frappe.db.set_value("Account", "Buildings - _TC1",
                            "inter_company_account", 1)
        jv = make_journal_entry("Sales Expenses - _TC", "Buildings - _TC",
                                100, posting_date=nowdate(), cost_center="Main - _TC", save=False)
        jv.voucher_type = "Inter Company Journal Entry"
        jv.multi_currency = 0
        jv.insert()
        jv.submit()

        jv1 = make_journal_entry("Sales Expenses - _TC1", "Buildings - _TC1",
                                 100, posting_date=nowdate(), cost_center="Main - _TC1", save=False)
    ...
    def test_jv_with_project(self):
        ...
        if not frappe.db.exists("Project", {"project_name": "Journal Entry Project"}):
            ...
            project_name = frappe.get_value(
                "Project", {"project_name": "_Test Project"})

        jv = make_journal_entry(
            "_Test Cash - _TC", "_Test Bank - _TC", 100, save=False)
    ...
    def test_jv_account_and_party_balance_with_cost_centre(self):
        ...
        account_balance = get_balance_on(
            account="_Test Bank - _TC", cost_center=cost_center)
        jv.voucher_type = "Bank Entry"
        jv.multi_currency = 0
        jv.cheque_no = "112233"
        jv.cheque_date = nowdate()
