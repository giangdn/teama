...
class CostCenter(NestedSet):
        ...
        if cint(self.enable_distributed_cost_center):
            ...
            if not self.get('__islocal'):
                if not cint(frappe.get_cached_value("Cost Center", {"name": self.name}, "enable_distributed_cost_center")) \
                    ...
                    frappe.throw(
                        _("Cannot enable Distributed Cost Center for a Cost Center already allocated in another Distributed Cost Center"))
                if next((True for x in self.distributed_cost_center if x.cost_center == x.parent), False):
                    frappe.throw(
                        _("Parent Cost Center cannot be added in Distributed Cost Center"))
            if check_if_distributed_cost_center_enabled(list(x.cost_center for x in self.distributed_cost_center)):
                frappe.throw(
                    _("A Distributed Cost Center cannot be added in the Distributed Cost Center allocation table."))
        else:
    ...
    def convert_group_to_ledger(self):
        if self.check_if_child_exists():
            frappe.throw(
                _("Cannot convert Cost Center to ledger as it has child nodes"))
        elif self.check_gle_exists():
            frappe.throw(
                _("Cost Center with existing transactions can not be converted to ledger"))
        else:
            self.is_group = 0
            self.save()
            return 1
    ...
    def convert_ledger_to_group(self):
        ...
        if self.check_gle_exists():
            frappe.throw(
                _("Cost Center with existing transactions can not be converted to group"))
        self.is_group = 1
        self.save()
        return 1
    ...
    def before_rename(self, olddn, newdn, merge=False):
        ...
        super(CostCenter, self).before_rename(
            olddn, new_cost_center, merge, "is_group")
        if not merge:
            new_cost_center = get_name_with_number(
                new_cost_center, self.cost_center_number)
...
def check_if_distributed_cost_center_enabled(cost_center_list):
    value_list = frappe.get_list("Cost Center", {
                                 "name": ["in", cost_center_list]}, "enable_distributed_cost_center", as_list=1)
