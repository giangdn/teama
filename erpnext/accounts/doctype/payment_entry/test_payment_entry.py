...
class TestPaymentEntry(unittest.TestCase):
    def test_payment_entry_against_order(self):
        ...
        so_advance_paid = frappe.db.get_value(
            "Sales Order", so.name, "advance_paid")
    ...
    def test_payment_entry_for_blocked_supplier_payments_past_date(self):
        ...
        with self.assertRaises(Exception):
            try:
                ...
                get_payment_entry('Purchase Invoice', pi.name,
                                  bank_account="_Test Bank - _TC")
    ...
    def test_payment_entry_against_si_usd_to_usd(self):
        ...
        outstanding_amount = flt(frappe.db.get_value(
            "Sales Invoice", si.name, "outstanding_amount"))
    ...
    def test_payment_entry_against_pi(self):
        ...
        outstanding_amount = flt(frappe.db.get_value(
            "Sales Invoice", pi.name, "outstanding_amount"))
        self.assertEqual(outstanding_amount, 0)
    ...
    def test_payment_entry_against_payment_terms(self):
        ...
        self.assertEqual(
            pe.references[0].payment_term, 'Basic Amount Receivable')
    ...
    def test_payment_entry_against_ec(self):
        ...
        payable = frappe.get_cached_value(
            'Company',  "_Test Company",  'default_payable_account')
        ec = make_expense_claim(
            payable, 300, 300, "_Test Company", "Travel Expenses - _TC")
        ...
        outstanding_amount = flt(frappe.db.get_value("Expense Claim", ec.name, "total_sanctioned_amount")) - \
            flt(frappe.db.get_value("Expense Claim",
                                    ec.name, "total_amount_reimbursed"))
        self.assertEqual(outstanding_amount, 0)
    ...
    def test_payment_against_negative_sales_invoice(self):
        ...
        pe2 = get_payment_entry("Sales Invoice", si1.name,
                                bank_account="_Test Cash - _TC")
        pe2.insert()
        pe2.submit()
        ...
        si1_outstanding = frappe.db.get_value(
            "Sales Invoice", si1.name, "outstanding_amount")
        self.assertEqual(si1_outstanding, -100)

        # pay more than outstanding against si1
        pe3 = get_payment_entry("Sales Invoice", si1.name,
                                bank_account="_Test Cash - _TC")
    ...
    def test_payment_entry_account_and_party_balance_with_cost_center(self):
        ...
        account_balance = get_balance_on(
            account="_Test Bank - _TC", cost_center=si.cost_center)
        party_balance = get_balance_on(
            party_type="Customer", party=si.customer, cost_center=si.cost_center)
        party_account_balance = get_balance_on(
            si.debit_to, cost_center=si.cost_center)
        ...
        account_balance = get_balance_on(
            account=pe.paid_to, cost_center=pe.cost_center)
        party_balance = get_balance_on(
            party_type="Customer", party=pe.party, cost_center=pe.cost_center)
        party_account_balance = get_balance_on(
            account=pe.paid_from, cost_center=pe.cost_center)
