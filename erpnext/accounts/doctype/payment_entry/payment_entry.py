...
class PaymentEntry(AccountsController):
    ...
    def set_missing_values(self):
        if self.payment_type == "Internal Transfer":
            ...
            _party_name = "title" if self.party_type in (
                "Student", "Shareholder") else self.party_type.lower() + "_name"
                ...
                if not frappe.db.exists(d.reference_doctype, d.reference_name):
                    ...
                    if d.reference_doctype in ("Sales Invoice", "Purchase Invoice", "Expense Claim", "Fees"):
                        if self.party_type == "Customer":
                            ref_party_account = get_party_account_based_on_invoice_discounting(
                                d.reference_name) or ref_doc.debit_to
            ...
            if reference.payment_term and reference.reference_name:
                ...
                if not invoice_paid_amount_map.get(key):
                    ...
                    for term in payment_schedule:
                        ...
                        invoice_paid_amount_map[invoice_key]['outstanding'] = term.payment_amount - \
                            term.paid_amount
        ...
        for key, amount in iteritems(invoice_payment_amount_map):
            if cancel:
                ...
                if amount > outstanding:
                    frappe.throw(_('Cannot allocate more than {0} against payment term {1}').format(
                        outstanding, key[0]))
    ...
    def set_unallocated_amount(self):
        ...
        if self.party:
            total_deductions = sum([flt(d.amount)
    ...
    def set_remarks(self):
        ...
        if self.payment_type == "Internal Transfer":
            ...
            remarks = [_("Amount {0} {1} {2} {3}").format(
                ...
                _("received from") if self.payment_type == "Receive" else _(
                    "to"), self.party
            )]
    ...
    def add_party_gl_entries(self, gl_entries):
        if self.party_account:
            ...
            dr_or_cr = "credit" if erpnext.get_party_account_type(
                self.party_type) == 'Receivable' else "debit"
            ...
            if self.unallocated_amount:
                base_unallocated_amount = base_unallocated_amount = self.unallocated_amount * \
                    (self.source_exchange_rate if self.payment_type ==
                     "Receive" else self.target_exchange_rate)
        ...
        if self.payment_type in ("Receive", "Pay") and self.party:
            for d in self.get("references"):
                if d.allocated_amount \
                    ...
                    frappe.get_doc(d.reference_doctype,
                                   d.reference_name).set_total_advance_paid()
...
def get_reference_details(reference_doctype, reference_name, party_account_currency):
    ...
    if reference_doctype == "Fees":
        ...
        if ref_doc.doctype == "Expense Claim":
            total_amount = flt(ref_doc.total_sanctioned_amount) + \
                flt(ref_doc.total_taxes_and_charges)
        ...
        if not exchange_rate:
            ...
            exchange_rate = ref_doc.get("conversion_rate") or \
                get_exchange_rate(party_account_currency,
        ...
        if reference_doctype in ("Sales Invoice", "Purchase Invoice"):
            ...
            outstanding_amount = (
                flt(ref_doc.advance_amount) - flt(ref_doc.paid_amount))
            if party_account_currency != ref_doc.currency:
                outstanding_amount = flt(
                    outstanding_amount) * flt(exchange_rate)
                if party_account_currency == company_currency:
                    exchange_rate = 1
        else:
...
def set_grand_total_and_outstanding_amount(party_amount, dt, party_account_currency, doc):
    ...
    if party_amount:
        ...
        if party_account_currency != doc.currency:
            ...
            outstanding_amount = (
                flt(doc.advance_amount) - flt(doc.paid_amount)) * flt(doc.exchange_rate)
    elif dt == "Fees":
