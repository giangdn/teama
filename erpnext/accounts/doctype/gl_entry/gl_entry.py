...
class GLEntry(Document):
    ...
    def validate_dimensions_for_pl_and_bs(self):
        account_type = frappe.db.get_value(
            "Account", self.account, "report_type")
...
def validate_balance_type(account, adv_adj=False):
    if not adv_adj and account:
        balance_must_be = frappe.db.get_value(
        ...
        if balance_must_be:
            ...
            if (balance_must_be == "Debit" and flt(balance) < 0) or \
                ...
                frappe.throw(_("Balance for Account {0} must always be {1}").format(
                    account, _(balance_must_be)))
...
def update_outstanding_amt(account, party_type, party, against_voucher_type, against_voucher, on_cancel=False):
    ...
    if against_voucher_type == "Sales Invoice":
        ...
        account_condition = " and account = {0}".format(
            frappe.db.escape(account))
    ...
    if against_voucher_type == 'Purchase Invoice':
        ...
        if bal < 0 and not on_cancel:
            frappe.throw(_("Outstanding for {0} cannot be less than zero ({1})").format(
    ...
    if against_voucher_type in ["Sales Invoice", "Purchase Invoice", "Fees"]:
        ...
        frappe.db.set_value(against_voucher_type,
                            against_voucher, 'outstanding_amount', bal)
...
def validate_frozen_account(account, adv_adj=None):
    ...
    if frozen_account == 'Yes' and not adv_adj:
        ...
        if not frozen_accounts_modifier:
            ...
            frappe.throw(
                _("Not authorized to edit frozen Account {0}").format(account))
...
def rename_temporarily_named_docs(doctype):
    ...
    for doc in docs_to_rename:
        ...
        newname = doc.name
        frappe.db.sql("""UPDATE `tab{}` SET name = %s, to_rename = 0 where name = %s""".format(
            doctype), (newname, oldname))
