...
class TaxRule(Document):
    ...
    def validate_filters(self):
        ...
        for d in filters:
            if conds:
                conds += " and "
            conds += """ifnull({0}, '') = {1}""".format(d,
                                                        frappe.db.escape(cstr(filters[d])))
        ...
        if self.from_date and self.to_date:
            ...
            conds += """ and to_date > '{from_date}'""".format(
                from_date=self.from_date)

        elif self.to_date and not self.from_date:
            conds += """ and from_date < '{to_date}'""".format(
        ...
        if tax_rule:
            if tax_rule[0].priority == self.priority:
                frappe.throw(_("Tax Rule Conflicts with {0}").format(
                    tax_rule[0].name), ConflictingTaxRule)
    ...
    def validate_use_for_shopping_cart(self):
        ...
        if (not self.use_for_shopping_cart
            ...
            frappe.msgprint(
                _("Enabling 'Use for Shopping Cart', as Shopping Cart is enabled and there should be at least one Tax Rule for Shopping Cart"))
    ...
    if args:
        if args.get('billing_address'):
            billing_address = frappe.get_doc(
                'Address', args.get('billing_address'))
        ...
        shipping_address_name = get_default_address(
            party_type, party, 'is_shipping_address')
...
def get_customer_group_condition(customer_group):
    condition = ""
    customer_groups = ["%s" % (frappe.db.escape(d.name))
                       for d in get_parent_customer_groups(customer_group)]
    if customer_groups:
        condition = ",".join(['%s'] * len(customer_groups)
