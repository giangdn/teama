...
class POSInvoiceMergeLog(Document):
    ...
    def validate_customer(self):
        for d in self.pos_invoices:
            if d.customer != self.customer:
                frappe.throw(_("Row #{}: POS Invoice {} is not against customer {}").format(
    ...
    def validate_pos_invoice_status(self):
        for d in self.pos_invoices:
            ...
            if docstatus != 1:
                frappe.throw(_("Row #{}: POS Invoice {} is not submitted yet").format(
                    d.idx, bold_pos_invoice))
            if status == "Consolidated":
                frappe.throw(_("Row #{}: POS Invoice {} has been {}").format(
    ...
    def on_submit(self):
        pos_invoice_docs = [frappe.get_doc(
            "POS Invoice", d.pos_invoice) for d in self.pos_invoices]
            ...
            for payment in doc.get('payments'):
                ...
                for pay in payments:
                    if pay.account == payment.account and pay.mode_of_payment == payment.mode_of_payment:
                        ...
                        pay.base_amount = flt(
                            pay.base_amount) + flt(payment.base_amount)
                        found = True
                if not found:
    ...
    def update_pos_invoices(self, invoice_docs, sales_invoice='', credit_note=''):
        for doc in invoice_docs:
            doc.load_from_db()
            doc.update({'consolidated_invoice': None if self.docstatus == 2 else (
                credit_note if doc.is_return else sales_invoice)})
...
def consolidate_pos_invoices(pos_invoices=[], closing_entry={}):
    invoices = pos_invoices or closing_entry.get(
        'pos_transactions') or get_all_unconsolidated_invoices()
