...
class POSClosingEntry(StatusUpdater):
    def validate(self):
        if frappe.db.get_value("POS Opening Entry", self.pos_opening_entry, "status") != "Open":
            frappe.throw(_("Selected POS Opening Entry should be open."), title=_(
    ...
    def update_opening_entry(self, for_cancel=False):
        opening_entry = frappe.get_doc(
            "POS Opening Entry", self.pos_opening_entry)
...
def get_pos_invoices(start, end, pos_profile, user):
    ...
    data = list(filter(lambda d: get_datetime(start) <=
                       get_datetime(d.timestamp) <= get_datetime(end), data))
