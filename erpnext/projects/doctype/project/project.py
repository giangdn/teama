...
class Project(Document):
    ...
    def copy_from_template(self):
        ...
        if self.project_template and not frappe.db.get_all('Task', dict(project=self.name), limit=1):
            ...
            template = frappe.get_doc(
                'Project Template', self.project_template)
    ...
    def calculate_start_date(self, task_details):
        self.start_date = add_days(
            self.expected_start_date, task_details.start)
    ...
    def dependency_mapping(self, template_tasks, project_tasks):
        for template_task in template_tasks:
            project_task = list(
                filter(lambda x: x.subject == template_task.subject, project_tasks))[0]
            project_task = frappe.get_doc("Task", project_task.name)
            self.check_depends_on_value(
                template_task, project_task, project_tasks)
            self.check_for_parent_tasks(
                template_task, project_task, project_tasks)

    def check_depends_on_value(self, template_task, project_task, project_tasks):
        if template_task.get("depends_on") and not project_task.get("depends_on"):
            for child_task in template_task.get("depends_on"):
                child_task_subject = frappe.db.get_value(
                    "Task", child_task.task, "subject")
    ...
    def check_for_parent_tasks(self, template_task, project_task, project_tasks):
        if template_task.get("parent_task") and not project_task.get("parent_task"):
            parent_task_subject = frappe.db.get_value(
                "Task", template_task.get("parent_task"), "subject")
            corresponding_project_task = list(
        ...
        if not total:
            ...
            if (self.percent_complete_method == "Task Weight" and total > 0):
                ...
                for row in weighted_progress:
                    pct_complete += row["progress"] * \
                        frappe.utils.safe_div(row["task_weight"], weight_sum)
    ...
    def calculate_gross_margin(self):
        ...
        if self.total_billed_amount:
            self.per_gross_margin = (
                self.gross_margin / flt(self.total_billed_amount)) * 100
