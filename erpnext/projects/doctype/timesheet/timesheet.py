...
class Timesheet(Document):
    ...
    def calculate_total_amounts(self):
        ...
        for d in self.get("time_logs"):
            ...
            if d.billable:
                ...
                self.total_billed_amount += flt(
                    d.billing_amount) if d.sales_invoice else 0.0
                self.total_billed_hours += flt(
                    d.billing_hours) if d.sales_invoice else 0.0

    def calculate_percentage_billed(self):
        self.per_billed = 0
        if self.total_billed_amount > 0 and self.total_billable_amount > 0:
            self.per_billed = (self.total_billed_amount *
                               100) / self.total_billable_amount
    ...
    def validate_mandatory_fields(self):
        for data in self.time_logs:
            ...
            if not data.activity_type and self.employee:
                frappe.throw(
                    _("Row {0}: Activity Type is mandatory.").format(data.idx))

            if flt(data.hours) == 0.0:
                frappe.throw(
                    _("Row {0}: Hours value must be greater than zero.").format(data.idx))
    ...
    def validate_overlap(self, data):
        ...
        self.validate_overlap_for(
            "user", data, self.user, settings.ignore_user_time_overlap)
        self.validate_overlap_for(
            "employee", data, self.employee, settings.ignore_employee_time_overlap)

    def validate_task_project(self):
        for log in self.time_logs:
            log.project = log.project or frappe.db.get_value(
                "Task", log.task, "project")
...
def make_sales_invoice(source_name, item_code=None, customer=None):
    ...
    hours = flt(timesheet.total_billable_hours) - \
        flt(timesheet.total_billed_hours)
    billing_amount = flt(timesheet.total_billable_amount) - \
        flt(timesheet.total_billed_amount)
...
def get_timesheets_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
    ...
    if customer:
        sales_invoices = [d.name for d in frappe.get_all(
            'Sales Invoice', filters={'customer': customer})] or [None]
        projects = [d.name for d in frappe.get_all(
            'Project', filters={'customer': customer})]
