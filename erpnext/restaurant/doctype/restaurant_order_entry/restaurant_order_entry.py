...
def get_invoice(table):
    ...
    invoice_name = frappe.get_value(
        'Sales Invoice', dict(restaurant_table=table, docstatus=0))
    ...
    if invoice_name:
        ...
        invoice.naming_series = frappe.db.get_value(
            'Restaurant', restaurant, 'invoice_series_prefix')
        invoice.is_pos = 1
        default_customer = frappe.db.get_value(
            'Restaurant', restaurant, 'default_customer')
    ...
    invoice.taxes_and_charges = frappe.db.get_value(
        'Restaurant', restaurant, 'default_tax_template')
...
def make_invoice(table, customer, mode_of_payment):
    ...
    invoice.append('payments', dict(
        mode_of_payment=mode_of_payment, amount=invoice.grand_total))
    invoice.save()
    invoice.submit()
...
def item_query_restaurant(doctype='Item', txt='', searchfield='name', start=0, page_len=20, filters=None, as_dict=False):
    ...
    items = frappe.db.get_all('Restaurant Menu Item', [
                              'item'], dict(parent=menu))
    del filters['table']
