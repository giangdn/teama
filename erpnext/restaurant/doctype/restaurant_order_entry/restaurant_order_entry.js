...
frappe.ui.form.on("Restaurant Order Entry", {
	setup: function(frm) {
		let get_item_query = () => {
			return {
				query:
					"erpnext.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant",
				filters: {
	...
	restaurant_table: function(frm) {
		...
		return frappe.call({
			method:
				"erpnext.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.get_invoice",
			args: {
				table: frm.doc.restaurant_table
			},
			callback: r => {
	...
	sync: function(frm) {
		return frappe.call({
			method:
				"erpnext.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.sync",
			args: {
				table: frm.doc.restaurant_table,
				items: frm.doc.items
			},
		...
		frm.trigger("sync").then(() => {
			frappe.prompt(
				...
				data => {
					...
					return frappe.call({
						method:
							"erpnext.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.make_invoice",
						args: {
							table: frm.doc.restaurant_table,
