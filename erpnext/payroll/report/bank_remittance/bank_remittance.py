...
def get_bank_accounts():
    accounts = [d.name for d in get_all(
        "Account", filters={"account_type": "Bank"})]
    return accounts
...
def get_payroll_entries(accounts, filters):
    ...
    entries = get_all("Payroll Entry", payroll_filter,
...
def get_salary_slips(payroll_entries):
    ...
    salary_slips = get_all("Salary Slip", filters=[("payroll_entry", "IN", payroll)],
                           fields=["modified", "net_pay", "bank_name", "bank_account_no",
                                   "payroll_entry", "employee", "employee_name", "status"]
                           )
...
def get_emp_bank_ifsc_code(salary_slips):
    ...
    ifsc_codes = get_all("Employee", [("name", "IN", emp_names)], [
                         "ifsc_code", "name"])
...
def set_company_account(payment_accounts, payroll_entries):
    company_accounts = get_all("Bank Account", [("account", "in", payment_accounts)], [
