...
class TestSalaryStructure(unittest.TestCase):
    ...
    def test_salary_structure_deduction_based_on_gross_pay(self):
        ...
        sal_struct = make_salary_structure(
            "Salary Structure 2", "Monthly", dont_submit=True)
    ...
    def test_amount_totals(self):
        ...
        sal_slip = frappe.get_value(
            "Salary Slip", {"employee_name": "test_employee_2@salary.com"})
        if not sal_slip:
            sal_slip = make_employee_salary_slip(
                "test_employee_2@salary.com", "Monthly", "Salary Structure Sample")
            self.assertEqual(sal_slip.get("salary_structure"),
            ...
            self.assertEqual(sal_slip.get("net_pay"), 78000 -
                             sal_slip.get("total_deduction"))
    ...
    def test_salary_structures_assignment(self):
        ...
        salary_structure_assignment = frappe.get_doc("Salary Structure Assignment", {
...
def make_salary_structure(salary_structure, payroll_frequency, employee=None, dont_submit=False, other_details=None,
    ...
    if test_tax:
        frappe.db.sql(
            """delete from `tabSalary Structure` where name=%s""", (salary_structure))
