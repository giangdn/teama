...
class SalaryStructure(Document):
    ...
    def set_missing_values(self):
        overwritten_fields = ["depends_on_payment_days",
                              "variable_based_on_taxable_salary", "is_tax_applicable", "is_flexible_benefit"]
        overwritten_fields_if_missing = [
            "amount_based_on_formula", "formula", "amount"]
    ...
    def validate_max_benefits_with_flexi(self):
        ...
        if self.earnings:
            ...
            if have_a_flexi and flt(self.max_benefits) == 0:
                frappe.throw(
                    _("Max benefits should be greater than zero to dispense benefits"))
...
def assign_salary_structure_for_employees(employees, salary_structure, payroll_payable_account=None, from_date=None, base=None, variable=None, income_tax_slab=None):
    ...
    for employee in employees:
        ...
        frappe.publish_progress(count*100/len(set(employees) - set(
            existing_assignments_for)), title=_("Assigning Structures..."))
...
def create_salary_structures_assignment(employee, salary_structure, payroll_payable_account, from_date, base, variable, income_tax_slab=None):
    if not payroll_payable_account:
        ...
        if not payroll_payable_account:
            frappe.throw(
                _('Please set "Default Payroll Payable Account" in Company Defaults'))
    ...
    if payroll_payable_account_currency != salary_structure.currency and payroll_payable_account_currency != company_curency:
        frappe.throw(_("Invalid Payroll Payable Account. The account currency must be {0} or {1}").format(
