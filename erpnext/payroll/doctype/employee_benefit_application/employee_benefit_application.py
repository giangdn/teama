...
class EmployeeBenefitApplication(Document):
    def validate(self):
        ...
        if not self.max_benefits:
            self.max_benefits = get_max_benefits_remaining(
                self.employee, self.date, self.payroll_period)
    ...
    def validate_remaining_benefit_amount(self):
        ...
        if salary_struct_name:
                ...
                for earnings in salary_structure.earnings:
                    if earnings.is_flexible_benefit == 1 and earnings.salary_component not in benefit_components:
                        pay_against_benefit_claim, max_benefit_amount = frappe.db.get_value(
                            "Salary Component", earnings.salary_component, ["pay_against_benefit_claim", "max_benefit_amount"])
            ...
            if pro_rata_amount == 0 and non_pro_rata_amount == 0:
                frappe.throw(_("Please add the remaining benefits {0} to any of the existing component").format(
    ...
    def validate_max_benefit_for_component(self):
        if self.employee_benefits:
            ...
            if max_benefit_amount > self.max_benefits:
                frappe.throw(_("Maximum benefit amount of employee {0} exceeds {1}").format(
    ...
    def validate_max_benefit(self, earning_component_name):
        ...
        if rounded(benefit_amount, 2) > max_benefit_amount:
            frappe.throw(_("Maximum benefit amount of component {0} exceeds {1}").format(
    ...
    def validate_duplicate_on_payroll_period(self):
        ...
        if application:
            frappe.throw(_("Employee {0} already submited an apllication {1} for the payroll period {2}").format(
...
def get_max_benefits(employee, on_date):
    ...
    if sal_struct:
        max_benefits = frappe.db.get_value(
            "Salary Structure", sal_struct, "max_benefits")
        if max_benefits > 0:
            return max_benefits
    return False
...
def get_max_benefits_remaining(employee, on_date, payroll_period):
    ...
    if max_benefits and max_benefits > 0:
        ...
        if prev_sal_slip_flexi_total > 0:
            ...
            if sal_struct_name:
                sal_struct = frappe.get_doc(
                    "Salary Structure", sal_struct_name)
            ...
            if have_depends_on_payment_days and per_day_amount_total > 0:
                ...
                working_days = date_diff(
                    on_date, payroll_period_obj.start_date) + 1
                leave_days = calculate_lwp(
                    employee, payroll_period_obj.start_date, holidays, working_days)
...
def get_benefit_amount_based_on_pro_rata(sal_struct, component_max_benefit):
    ...
    for d in sal_struct.get("earnings"):
        if d.is_flexible_benefit == 1:
            component = frappe.db.get_value("Salary Component", d.salary_component, [
                                            "max_benefit_amount", "pay_against_benefit_claim"], as_dict=1)
            if not component.pay_against_benefit_claim:
                max_benefits_total += component.max_benefit_amount

    if max_benefits_total > 0:
        benefit_amount = sal_struct.max_benefits * \
            component.max_benefit_amount / max_benefits_total
