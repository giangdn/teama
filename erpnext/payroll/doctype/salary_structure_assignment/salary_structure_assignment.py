...
class SalaryStructureAssignment(Document):
    ...
    def validate_income_tax_slab(self):
        ...
        if self.currency != income_tax_slab_currency:
            frappe.throw(_("Currency of selected Income Tax Slab should be {0} instead of {1}").format(
...
def get_employee_currency(employee):
    employee_currency = frappe.db.get_value('Salary Structure Assignment', {
                                            'employee': employee}, 'currency')
    if not employee_currency:
        frappe.throw(
            _("There is no Salary Structure assigned to {0}. First assign a Salary Stucture.").format(employee))
