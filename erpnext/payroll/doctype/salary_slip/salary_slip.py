...
class SalarySlip(TransactionBase):
    ...
    def get_emp_and_working_day_details(self):
        ...
        if self.employee:
            ...
            if struct:
                ...
                ps = frappe.db.get_value("Payroll Settings", None, [
                                         "payroll_based_on", "consider_unmarked_attendance_as"], as_dict=1)
    ...
    def get_working_days_details(self, joining_date=None, relieving_date=None, lwp=None, for_preview=0):
        ...
        if flt(payment_days) > flt(lwp):
            ...
            consider_unmarked_attendance_as = frappe.db.get_value(
    ...
    def calculate_lwp_ppl_and_absent_days_based_on_attendance(self, holidays):
        ...
        for d in attendances:
            ...
            if d.leave_type:
                fraction_of_daily_salary_per_leave = leave_type_map[
                    d.leave_type]["fraction_of_daily_salary_per_leave"]
    ...
    def set_net_pay(self):
        ...
        self.base_total_deduction = flt(flt(
            self.total_deduction) * flt(self.exchange_rate), self.precision('base_total_deduction'))
        self.net_pay = flt(
    ...
    def calculate_variable_tax(self, payroll_period, tax_component):
        ...
        total_taxable_earnings = previous_taxable_earnings + current_structured_taxable_earnings + future_structured_taxable_earnings \
            + current_additional_earnings + other_incomes + \
            unclaimed_taxable_benefits - total_exemption_amount

        # Total taxable earnings without additional earnings with full tax
        total_taxable_earnings_without_full_tax_addl_components = total_taxable_earnings - \
        ...
        current_structured_tax_amount = (
            total_structured_tax_amount - previous_total_paid_taxes) / remaining_sub_periods
    ...
    def get_income_tax_slabs(self, payroll_period):
        ...
        if not income_tax_slab:
            frappe.throw(_("Income Tax Slab not set in Salary Structure Assignment: {0}").format(
                ss_assignment_name))
                ...
                if ded.exempted_from_income_tax:
                    ...
                    if based_on_payment_days:
                        amount = self.get_amount_based_on_payment_days(
                            ded, joining_date, relieving_date)[0]
    ...
    def get_amount_based_on_payment_days(self, row, joining_date, relieving_date):
        ...
        if frappe.get_cached_value("Salary Component", row.salary_component, "round_to_the_nearest_integer"):
            amount, additional_amount = rounded(
                amount), rounded(additional_amount)
        ...
        for slab in tax_slab.slabs:
            ...
            if not slab.to_amount and annual_taxable_earning >= slab.from_amount:
                tax_amount += (annual_taxable_earning -
                               slab.from_amount + 1) * slab.percent_deduction * .01
                continue
    ...
    def set_component_amounts_based_on_payment_days(self):
        ...
        for component_type in ("earnings", "deductions"):
            for d in self.get(component_type):
                d.amount = flt(self.get_amount_based_on_payment_days(
                    d, joining_date, relieving_date)[0], d.precision("amount"))
    ...
    def pull_emp_details(self):
        emp = frappe.db.get_value("Employee", self.employee, [
                                  "bank_name", "bank_ac_no", "salary_mode"], as_dict=1)
        if emp:
