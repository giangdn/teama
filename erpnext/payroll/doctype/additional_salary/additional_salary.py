...
class AdditionalSalary(Document):
    ...
    def on_submit(self):
        if self.ref_doctype == "Employee Advance" and self.ref_docname:
            frappe.db.set_value("Employee Advance",
                                self.ref_docname, "return_amount", self.amount)
    ...
    def validate_recurring_additional_salary_overlap(self):
        if self.is_recurring:
            ...
            additional_salaries = [
                salary.name for salary in additional_salaries]
    ...
    def get_amount(self, sal_start_date, sal_end_date):
        ...
        total_days = date_diff(getdate(self.to_date),
                               getdate(self.from_date)) + 1
...
def get_additional_salary_component(employee, start_date, end_date, component_type):
    ...
    overwrites_components = [
        ele.salary_component for ele in additional_salaries if ele.overwrite_salary_structure_amount == 1]

    component_fields = ["depends_on_payment_days", "salary_component_abbr",
                        "is_tax_applicable", "variable_based_on_taxable_salary", 'type']
    for d in additional_salaries:

        if d.salary_component not in existing_salary_components:
            component = frappe.get_all("Salary Component", filters={
                                       'name': d.salary_component}, fields=component_fields)
        ...
        if overwrites_components.count(d.salary_component) > 1:
            frappe.throw(_("Multiple Additional Salaries with overwrite property exist for Salary Component: {0} between {1} and {2}.".format(
                d.salary_component, start_date, end_date)), title=_("Error"))
        else:
