...
def execute(filters=None):
    ...
    chart = prepare_chart_data(data, filters) if filters.get(
        "group_by") not in ("Asset Category", "Location") else {}
...
def get_data(filters):
    ...
    if group_by == "asset_category":
        fields = ["asset_category", "gross_purchase_amount",
                  "opening_accumulated_depreciation"]
        assets_record = frappe.db.get_all(
            "Asset", filters=conditions, fields=fields, group_by=group_by)

    elif group_by == "location":
        fields = ["location", "gross_purchase_amount",
                  "opening_accumulated_depreciation"]
        assets_record = frappe.db.get_all(
            "Asset", filters=conditions, fields=fields, group_by=group_by)
        ...
        assets_record = frappe.db.get_all(
            "Asset", filters=conditions, fields=fields)
...
def prepare_chart_data(data, filters):
    ...
    for d in period_list:
        labels_values_map.setdefault(d.get('label'), frappe._dict(
            {'asset_value': 0, 'depreciated_amount': 0}))
    ...
    return {
        "data": {
            ...
            "datasets": [
                {'name': _('Asset Value'), 'values': [
                    d.get("asset_value") for d in labels_values_map.values()]},
                {'name': _('Depreciatied Amount'), 'values': [
                    d.get("depreciated_amount") for d in labels_values_map.values()]}
            ]
        },
        "type": "bar",
        "barOptions": {
