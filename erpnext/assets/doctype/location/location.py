...
class Location(NestedSet):
    ...
    def update_ancestor_location_features(self):
        ...
        for ancestor in self.get_ancestors():
            ...
            ancestor_doc.db_set('area', ancestor_doc.area +
                                self.area_difference, commit=True)
    ...
    def remove_ancestor_location_features(self):
        for ancestor in self.get_ancestors():
            ...
            ancestor_doc.set_location_features(features=ancestor_features)
            ancestor_doc.db_set(
                'area', ancestor_doc.area - self.area, commit=True)

    def add_child_property(self):
        features = self.get_location_features()
        filter_features = [feature for feature in features if not feature.get(
            'properties').get('child_feature')]
...
def _ring_area(coords):
    ...
    if coords_length > 2:
        for i in range(coords_length):
            ...
            area += (math.radians(p3[0]) - math.radians(p1[0])
