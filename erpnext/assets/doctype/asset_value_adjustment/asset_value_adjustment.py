...
class AssetValueAdjustment(Document):
    ...
    def validate_date(self):
        asset_purchase_date = frappe.db.get_value(
            'Asset', self.asset, 'purchase_date')
    ...
    def set_difference_amount(self):
        self.difference_amount = flt(
            self.current_asset_value - self.new_asset_value)
    ...
    def make_depreciation_entry(self):
        ...
        for dimension in accounting_dimensions:
            if dimension.get('mandatory_for_bs'):
                credit_entry.update({
                    dimension['fieldname']: self.get(
                        dimension['fieldname']) or dimension.get('default_dimension')
                })
    ...
    def reschedule_depreciations(self, asset_value):
        ...
        for d in asset.finance_books:
            ...
            if d.depreciation_method in ("Straight Line", "Manual"):
                end_date = max(
                    [s.schedule_date for s in asset.schedules if cint(s.finance_book_id) == d.idx])
                total_days = date_diff(end_date, self.date)
                rate_per_day = flt(
                    d.value_after_depreciation) / flt(total_days)
                from_date = self.date
            else:
