        ...
        for d in self.finance_books:
            for field in ("Total Number of Depreciations", "Frequency of Depreciation"):
                if cint(d.get(frappe.scrub(field))) < 1:
                    frappe.throw(_("Row {0}: {1} must be greater than 0").format(
                        d.idx, field), frappe.MandatoryError)
            ...
            for type_of_account in account_types:
                if d.get(type_of_account):
                    ...
                    if account_currency != company_currency:
                        invalid_accounts.append(frappe._dict(
        ...
        for d in self.accounts:
            for fieldname in account_type_map.keys():
                if d.get(fieldname):
                    ...
                    key_to_match = next(iter(account_type_map.get(fieldname)))
                    selected_key_type = frappe.db.get_value(
                        'Account', selected_account, key_to_match)
    ...
    if item and frappe.db.get_value("Item", item, "is_fixed_asset"):
        ...
        if not account:
            asset_details = frappe.db.get_value(
                "Asset", asset, ["asset_category", "company"])
