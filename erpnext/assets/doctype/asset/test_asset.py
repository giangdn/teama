...
class TestAsset(unittest.TestCase):
    ...
    def test_asset_sale(self):
        ...
        si.cancel()
        self.assertEqual(frappe.db.get_value(
            "Asset", asset.name, "status"), "Partially Depreciated")
    ...
    def test_cwip_accounting(self):
        ...
        month_end_date = get_last_day(nowdate())
        asset_doc.available_for_use_date = nowdate() if nowdate(
        ) != month_end_date else add_days(nowdate(), -15)
    ...
    def test_asset_cwip_toggling_cases(self):
        cwip = frappe.db.get_value(
            "Asset Category", "Computers", "enable_cwip_accounting")
        name = frappe.db.get_value("Asset Category Account", filters={
                                   "parent": "Computers"}, fieldname=["name"])
        ...
        asset_doc.submit()
        gle = frappe.db.sql(
            """select name from `tabGL Entry` where voucher_type='Asset' and voucher_no = %s""", asset_doc.name)
...
def create_fixed_asset_item():
    ...
    naming_series = meta.get_field("naming_series").options.splitlines()[
        0] or 'ACC-ASS-.YYYY.-'
    try:
        frappe.get_doc({
            "doctype": "Item",
