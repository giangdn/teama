...
frappe.ui.form.on("Asset", {
	...
	setup_chart: function(frm) {
		...
		if (frm.doc.opening_accumulated_depreciation) {
			last_depreciation_date = frappe.datetime.add_months(
				frm.doc.next_depreciation_date,
				-1 * frm.doc.frequency_of_depreciation
			);

			x_intervals.push(last_depreciation_date);
			asset_values.push(
				flt(frm.doc.gross_purchase_amount) -
		...
		$.each(frm.doc.schedules || [], function(i, v) {
			...
			var asset_value =
				flt(frm.doc.gross_purchase_amount) -
				flt(v.accumulated_depreciation_amount);
			if (v.journal_entry) {
	...
	make_schedules_editable: function(frm) {
		if (frm.doc.finance_books) {
			var is_editable =
				frm.doc.finance_books.filter(
					d => d.depreciation_method == "Manual"
				).length > 0
					? true
					: false;

			frm.toggle_enable("schedules", is_editable);
			frm.fields_dict["schedules"].grid.toggle_enable(
				"schedule_date",
				is_editable
			);
			frm.fields_dict["schedules"].grid.toggle_enable(
	...
	create_asset_maintenance: function(frm) {
		frappe.call({
			...
			method:
				"erpnext.assets.doctype.asset.asset.create_asset_maintenance",
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_adjustment: function(frm) {
		frappe.call({
			...
			method:
				"erpnext.assets.doctype.asset.asset.create_asset_adjustment",
			freeze: 1,
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	purchase_receipt: frm => {
		...
		if (frm.doc.purchase_receipt) {
			if (frm.doc.item_code) {
				frappe.db
					.get_doc("Purchase Receipt", frm.doc.purchase_receipt)
					.then(pr_doc => {
						frm.events.set_values_from_purchase_doc(
							frm,
							"Purchase Receipt",
							pr_doc
						);
	...
	purchase_invoice: frm => {
		...
		if (frm.doc.purchase_invoice) {
			if (frm.doc.item_code) {
				frappe.db
					.get_doc("Purchase Invoice", frm.doc.purchase_invoice)
					.then(pi_doc => {
						frm.events.set_values_from_purchase_doc(
							frm,
							"Purchase Invoice",
	...
	set_values_from_purchase_doc: function(frm, doctype, purchase_doc) {
		...
		const item = purchase_doc.items.find(
		...
		frm.set_value(
			...
			item.base_net_rate + item.item_tax_amount
		);
		frm.set_value(
		...
		frm.set_value(
			"cost_center",
			item.cost_center || purchase_doc.cost_center
		);
	},
				...
				callback: function(r) {
					if (r.message) {
						...
						frappe.model.set_value(
							...
							flt(
								r.message,
								precision("rate_of_depreciation", row)
							)
						);
					}
				}
...
erpnext.asset.set_accumulated_depreciation = function(frm) {
	...
	var accumulated_depreciation = flt(
		frm.doc.opening_accumulated_depreciation
	);
...
erpnext.asset.restore_asset = function(frm) {
	frappe.confirm(
		__("Do you really want to restore this scrapped asset?"),
		function() {
			frappe.call({
				args: {
					asset_name: frm.doc.name
