...
class AssetMaintenance(Document):
    def validate(self):
        for task in self.get('asset_maintenance_tasks'):
            if task.end_date and (getdate(task.start_date) >= getdate(task.end_date)):
                throw(_("Start date should be less than end date for task {0}").format(
    ...
    def on_update(self):
        for task in self.get('asset_maintenance_tasks'):
            assign_tasks(self.name, task.assign_to,
                         task.maintenance_task, task.next_due_date)
...
def update_maintenance_log(asset_maintenance, item_code, item_name, task):
    ...
    if not asset_maintenance_log:
        ...
        maintenance_log = frappe.get_doc(
            'Asset Maintenance Log', asset_maintenance_log)
