...
def create_stripe_subscription(gateway_controller, data):
    ...
    try:
        ...
        stripe_settings.payment_plans = frappe.get_doc(
            "Payment Request", stripe_settings.data.reference_docname).subscription_plans
...
def create_subscription_on_stripe(stripe_settings):
    ...
    try:
        customer = stripe.Customer.create(description=stripe_settings.data.payer_name,
                                          email=stripe_settings.data.payer_email, source=stripe_settings.data.stripe_token_id)
        ...
        if subscription.status == "active":
            ...
            frappe.log_error('Subscription N°: ' +
                             subscription.id, 'Stripe Payment not completed')
