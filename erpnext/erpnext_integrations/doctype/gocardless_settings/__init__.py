...
def set_mandate_status(event):
    ...
    for mandate in mandates:
        frappe.db.set_value("GoCardless Mandate",
                            mandate, "disabled", disabled)
...
def authenticate_signature(r):
    ...
    for key in get_webhook_keys():
        computed_signature = hmac.new(key.encode(
            "utf-8"), r.get_data(), hashlib.sha256).hexdigest()
...
def get_webhook_keys():
    def _get_webhook_keys():
        webhook_keys = [d.webhooks_secret for d in frappe.get_all(
            "GoCardless Settings", fields=["webhooks_secret"],) if d.webhooks_secret]
