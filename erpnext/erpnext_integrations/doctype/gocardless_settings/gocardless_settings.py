...
class GoCardlessSettings(Document):
    ...
    def on_update(self):
        create_payment_gateway('GoCardless-' + self.gateway_name,
                               settings='GoCardLess Settings', controller=self.gateway_name)
    ...
    def check_mandate_validity(self, data):
        ...
        if frappe.db.exists("GoCardless Mandate", dict(customer=data.get('payer_name'), disabled=0)):
            registered_mandate = frappe.db.get_value("GoCardless Mandate", dict(
                customer=data.get('payer_name'), disabled=0), 'mandate')
            self.initialize_client()
    ...
    def validate_transaction_currency(self, currency):
        if currency not in self.supported_currencies:
            frappe.throw(
                _("Please select another payment method. Stripe does not support transactions in currency '{0}'").format(currency))
    ...
    def create_charge_on_gocardless(self):
        ...
        reference_doc = frappe.get_doc(self.data.get(
            'reference_doctype'), self.data.get('reference_docname'))
        self.initialize_client()
        ...
        try:
            ...
            if payment.status == "pending_submission" or payment.status == "pending_customer_approval" or payment.status == "submitted":
                ...
                frappe.log_error(
                    _("Payment Cancelled. Please check your GoCardless Account for more details"), "GoCardless Payment Error")
                self.integration_request.db_set(
                ...
                frappe.log_error(
                    _("Payment Failed. Please check your GoCardless Account for more details"), "GoCardless Payment Error")
                self.integration_request.db_set(
                    'error', payment.status, update_modified=False)
        ...
        if self.flags.status_changed_to == "Completed":
            ...
            if redirect_message:
                redirect_url += '&' + \
                    urlencode({'redirect_message': redirect_message})
