...
class ShopifySettings(unittest.TestCase):
    ...
    def test_order(self):
        ...
        create_customer(shopify_customer.get(
            "customer"), self.shopify_settings)
        ...
        create_order(shopify_order.get("order"),
                     self.shopify_settings, False, company="_Test Company")

        sales_order = frappe.get_doc(
            "Sales Order", {"shopify_order_id": cstr(shopify_order.get("order").get("id"))})

        self.assertEqual(cstr(shopify_order.get("order").get(
            "id")), sales_order.shopify_order_id)
        ...
        self.assertEqual(delivery_note_count, len(
            shopify_order.get("order").get("fulfillments")))
