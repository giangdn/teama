    ...
    def create_delete_custom_fields(self):
        if self.enable_sync:
            ...
            for doctype in ["Customer", "Sales Order", "Item", "Address"]:
                df = dict(fieldname='woocommerce_id', label='Woocommerce ID',
                          fieldtype='Data', read_only=1, print_hide=1)
                create_custom_field(doctype, df)

            for doctype in ["Customer", "Address"]:
                df = dict(fieldname='woocommerce_email', label='Woocommerce Email',
                          fieldtype='Data', read_only=1, print_hide=1)
                create_custom_field(doctype, df)
