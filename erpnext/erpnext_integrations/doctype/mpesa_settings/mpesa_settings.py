# -*- coding: utf-8 -*-
# Copyright (c) 2020, TeamA Technologies and contributors
...
class MpesaSettings(Document):
    ...
    def validate_transaction_currency(self, currency):
        if currency not in self.supported_currencies:
            frappe.throw(
                _("Please select another payment method. Mpesa does not support transactions in currency '{0}'").format(currency))

    def on_update(self):
        create_custom_pos_fields()
        create_payment_gateway('Mpesa-' + self.payment_gateway_name,
                               settings='Mpesa Settings', controller=self.payment_gateway_name)
        call_hook_method('payment_gateway_enabled', gateway='Mpesa-' +
                         self.payment_gateway_name, payment_channel="Phone")
    ...
    def request_for_payment(self, **kwargs):
        ...
        request_amounts = self.split_request_amount_according_to_transaction_limit(
            args)
        ...
        for i, amount in enumerate(request_amounts):
            ...
            if frappe.flags.in_test:
                ...
                response = frappe._dict(
                    get_payment_request_response_payload(amount))
            else:
    ...
    def split_request_amount_according_to_transaction_limit(self, args):
        ...
        if request_amount > self.transaction_limit:
            ...
            requests_to_be_made = frappe.utils.ceil(
                request_amount / self.transaction_limit)  # 480/150 = ceil(3.2) = 4
    ...
    def handle_api_response(self, global_id, request_dict, response):
        ...
        if error:
            frappe.throw(_(getattr(response, "errorMessage")),
...
def generate_stk_push(**kwargs):
    ...
    try:
        ...
        mpesa_settings = frappe.get_doc(
            "Mpesa Settings", args.payment_gateway[6:])
        ...
        frappe.throw(_("Issue detected with Mpesa configuration, check the error logs for more details"), title=_(
            "Mpesa Express Error"))
    ...
    if transaction_response['ResultCode'] == 0:
        if integration_request.reference_doctype and integration_request.reference_docname:
            try:
                ...
                frappe.db.set_value(
                    "POS Invoice", pr.reference_name, "mpesa_receipt_number", mpesa_receipts)
...
def get_account_balance(request_payload):
    ...
    try:
        ...
        callback_url = get_request_site_address(
            True) + "/api/method/erpnext.erpnext_integrations.doctype.mpesa_settings.mpesa_settings.process_balance_info"

        response = connector.get_balance(mpesa_settings.initiator_name, mpesa_settings.security_credential,
                                         mpesa_settings.till_number, 4, mpesa_settings.name, callback_url, callback_url)
        return response
    except Exception:
...
def process_balance_info(**kwargs):
    ...
    if account_balance_response["ResultCode"] == 0:
        try:
            ...
            balance_info = fetch_param_value(
                result_params, "AccountBalance", "Key")
            ...
            frappe.log_error(title=_(
                "Mpesa Account Balance Processing Error"), message=account_balance_response)
    else:
        request.handle_failure(account_balance_response)
