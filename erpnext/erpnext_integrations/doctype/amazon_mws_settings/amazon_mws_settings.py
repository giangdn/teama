...
class AmazonMWSSettings(Document):
    ...
    def get_products_details(self):
        if self.enable_amazon == 1:
            frappe.enqueue(
                'erpnext.erpnext_integrations.doctype.amazon_mws_settings.amazon_methods.get_products_details')

    def get_order_details(self):
        if self.enable_amazon == 1:
            after_date = dateutil.parser.parse(
                self.after_date).strftime("%Y-%m-%d")
            frappe.enqueue(
                'erpnext.erpnext_integrations.doctype.amazon_mws_settings.amazon_methods.get_orders', after_date=after_date)
...
def schedule_get_order_details():
    ...
    if mws_settings.enable_sync and mws_settings.enable_amazon:
        after_date = dateutil.parser.parse(
            mws_settings.after_date).strftime("%Y-%m-%d")
