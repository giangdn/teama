# -*- coding: utf-8 -*-
# Copyright (c) 2018, TeamA Technologies and contributors
...
def get_products_details():
    ...
    for marketplace in market_place_list:
        ...
        if report_id:
            ...
            # Get ASIN Codes
            string_io = StringIO(frappe.safe_decode(
                listings_response.original))
            ...
            for asin_list in asin_chunked_list:
                ...
                for product in matching_products_list:
                    skus = [row["sku"]
                            for row in sku_asin if row["asin"] == product.ASIN]
                    for sku in skus:
...
def request_and_fetch_report_id(report_type, start_date=None, end_date=None, marketplaceids=None):
    ...
    for x in range(1, 10):
        report_request_list_response = reports.get_report_request_list(requestids=[
                                                                       report_request_id])
        report_status = report_request_list_response.parsed[
            "ReportRequestInfo"]["ReportProcessingStatus"]["value"]
        ...
        if report_status == "_SUBMITTED_" or report_status == "_IN_PROGRESS_":
            ...
            generated_report_id = report_request_list_response.parsed[
                "ReportRequestInfo"]["GeneratedReportId"]["value"]
            break
...
def call_mws_method(mws_method, *args, **kwargs):
    ...
    frappe.throw(
        _("Sync has been temporarily disabled because maximum retries have been exceeded"))
...
def create_item_code(amazon_item_json, sku):
    ...
    item_group = frappe.db.get_value(
        "Item Group", filters={"item_group_name": temp_item_group})
    ...
    try:
        ...
        while True:
            ...
            if "Order" in orders_response.parsed.Orders:
                orders_list = return_as_list(
                    orders_response.parsed.Orders.Order)
            ...
            orders_response = call_mws_method(
                orders.list_orders_by_next_token, next_token)
...
def create_sales_order(order_json, after_date):
    ...
    if not so:
        ...
        delivery_date = dateutil.parser.parse(
            order_json.LatestShipDate).strftime("%Y-%m-%d")
        transaction_date = dateutil.parser.parse(
            order_json.PurchaseDate).strftime("%Y-%m-%d")
...
def get_order_items(market_place_order_id):
    ...
    order_items_list = return_as_list(
        order_items_response.parsed.OrderItems.OrderItem)

    warehouse = frappe.db.get_value(
        "Amazon MWS Settings", "Amazon MWS Settings", "warehouse")
    ...
    while True:
        ...
        order_items_response = call_mws_method(
            mws_orders.list_order_items_by_next_token, next_token)
        order_items_list = return_as_list(
            order_items_response.parsed.OrderItems.OrderItem)
...
def get_charges_and_fees(market_place_order_id):
    ...
    response = call_mws_method(
        finances.list_financial_events, amazon_order_id=market_place_order_id)

    shipment_event_list = return_as_list(
        response.parsed.FinancialEvents.ShipmentEventList)

    for shipment_event in shipment_event_list:
        if shipment_event:
            shipment_item_list = return_as_list(
                shipment_event.ShipmentEvent.ShipmentItemList.ShipmentItem)

            for shipment_item in shipment_item_list:
                charges, fees = [], []

                if 'ItemChargeList' in shipment_item.keys():
                    charges = return_as_list(
                        shipment_item.ItemChargeList.ChargeComponent)

                if 'ItemFeeList' in shipment_item.keys():
                    fees = return_as_list(
                        shipment_item.ItemFeeList.FeeComponent)
