...
class TestPlaidSettings(unittest.TestCase):
    ...
    def test_add_account_type(self):
        ...
        self.assertEqual(frappe.get_doc(
            "Bank Account Type", "brokerage").name, "brokerage")

    def test_add_account_subtype(self):
        add_account_subtype("loan")
        self.assertEqual(frappe.get_doc(
            "Bank Account Subtype", "loan").name, "loan")
    ...
    def test_default_bank_account(self):
        ...
        bank = json.dumps(frappe.get_doc(
            "Bank", "Citi").as_dict(), default=json_handler)
        company = frappe.db.get_single_value(
            'Global Defaults', 'default_company')
        frappe.db.set_value("Company", company, "default_bank_account", None)

        self.assertRaises(frappe.ValidationError, add_bank_accounts,
                          response=bank_accounts, bank=bank, company=company)
    ...
    def test_new_transaction(self):
        ...
        if frappe.db.get_value("Company", company, "default_bank_account") is None:
            frappe.db.set_value("Company", company, "default_bank_account",
