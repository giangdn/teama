...
class QuickBooksMigrator(Document):
    ...
    def _migrate(self):
        try:
            ...
            entities_for_gl_transform = ["Advance Payment", "Tax Payment",
                                         "Sales Tax Payment", "Purchase Tax Payment", "Inventory Qty Adjust"]
    ...
    def _make_custom_fields(self):
        doctypes_for_quickbooks_id_field = ["Account", "Customer", "Address",
                                            "Item", "Supplier", "Sales Invoice", "Journal Entry", "Purchase Invoice"]
    ...
    def _save_entries(self, entity, entries):
        ...
        for index, entry in enumerate(entries, start=1):
            self._publish({"event": "progress", "message": _(
                "Saving {0}").format(entity), "count": index, "total": total})
    ...
    def _get_account_type(self, account):
        ...
        if account_type is None:
            account_type_mapping = {"Accounts Payable": "Payable",
                                    "Accounts Receivable": "Receivable", "Bank": "Bank", "Credit Card": "Bank"}
                ...
                if linked_transaction["TxnType"] == "Invoice":
                    ...
                    if frappe.db.exists({"doctype": "Journal Entry", "quickbooks_id": si_quickbooks_id, "company": self.company}):
                        ...
                        customer_account_line = list(
                            filter(lambda acc: acc.party_type == "Customer", journal_entry.accounts))[0]
            ...
            for line in purchase["Line"]:
                if line["DetailType"] == "AccountBasedExpenseLineDetail":
                    ...
                    account = frappe.get_doc("Item",
                                             {"quickbooks_id": line["ItemBasedExpenseLineDetail"]
                                                 ["ItemRef"]["value"], "company": self.company}
