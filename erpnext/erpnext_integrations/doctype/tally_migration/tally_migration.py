# -*- coding: utf-8 -*-
# Copyright (c) 2019, TeamA Technologies Pvt. Ltd. and contributors
...
class TallyMigration(Document):
    def validate(self):
        ...
        sorted_failed_import_log = sorted(
            failed_import_log, key=lambda row: row["doc"]["creation"])
    ...
    def _process_master_data(self):
        ...
        def get_coa_customers_suppliers(collection):
            ...
            accounts = list(get_groups(collection.find_all("GROUP"))) + \
                list(get_ledgers(collection.find_all("LEDGER")))
            children, parents = get_children_and_parent_dict(accounts)
            group_set = [acc[1] for acc in accounts if acc[2]]
            children, customers, suppliers = remove_parties(
                parents, children, group_set)

            try:
                coa = traverse({}, children, roots, roots, group_set)
            except RecursionError:
                self.log(
                    _("Error occured while parsing Chart of Accounts: Please make sure that no two accounts have the same name"))
                ...
                if account in group_set or account in roots:
                    if account in children:
                        tree[account] = traverse(
                            {}, children, children[account], roots, group_set)
                    else:
        ...
        def get_parties_addresses(collection, customers, suppliers):
            ...
            for account in collection.find_all("LEDGER"):
                ...
                if account.NAME.string.strip() in customers:
                    ...
                    links.append({"link_doctype": party_type,
                                  "link_name": account["NAME"]})
                ...
                if party_type:
                    address = "\n".join([a.string.strip()
                                         for a in account.find_all("ADDRESS")])
                    addresses.append({
                        "doctype": "Address",
        ...
        def get_stock_items_uoms(collection):
            ...
            for uom in collection.find_all("UNIT"):
                uoms.append(
                    {"doctype": "UOM", "uom_name": uom.NAME.string.strip()})
        ...
        try:
            self.publish("Process Master Data", _(
                "Reading Uploaded File"), 1, 5)
            ...
            self.publish("Process Master Data", _(
                "Processing Chart of Accounts and Parties"), 2, 5)
            chart_of_accounts, customers, suppliers = get_coa_customers_suppliers(
                collection)

            self.publish("Process Master Data", _(
                "Processing Party Addresses"), 3, 5)
            parties, addresses = get_parties_addresses(
                collection, customers, suppliers)

            self.publish("Process Master Data", _(
                "Processing Items and UOMs"), 4, 5)
            items, uoms = get_stock_items_uoms(collection)
            data = {"chart_of_accounts": chart_of_accounts, "parties": parties,
                    "addresses": addresses, "items": items, "uoms": uoms}
    ...
    def _import_master_data(self):
        ...
        def create_parties_and_addresses(parties_file_url, addresses_file_url):
            ...
            addresses_file = frappe.get_doc(
                "File", {"file_url": addresses_file_url})
        ...
        try:
            self.publish("Import Master Data", _(
                "Creating Company and Importing Chart of Accounts"), 1, 4)
            create_company_and_coa(self.chart_of_accounts)

            self.publish("Import Master Data", _(
                "Importing Parties and Addresses"), 2, 4)
            create_parties_and_addresses(self.parties, self.addresses)

            self.publish("Import Master Data", _(
                "Importing Items and UOMs"), 3, 4)
    ...
    def _process_day_book_data(self):
        def get_vouchers(collection):
            ...
            for voucher in collection.find_all("VOUCHER"):
                if voucher.ISCANCELLED.string.strip() == "Yes":
                    continue
                inventory_entries = voucher.find_all("INVENTORYENTRIES.LIST") + voucher.find_all(
                    "ALLINVENTORYENTRIES.LIST") + voucher.find_all("INVENTORYENTRIESIN.LIST") + voucher.find_all("INVENTORYENTRIESOUT.LIST")
        ...
        def voucher_to_journal_entry(voucher):
            ...
            ledger_entries = voucher.find_all(
                "ALLLEDGERENTRIES.LIST") + voucher.find_all("LEDGERENTRIES.LIST")
            for entry in ledger_entries:
                account = {"account": encode_company_abbr(entry.LEDGERNAME.string.strip(
                ), self.erpnext_company), "cost_center": self.default_cost_center}
        ...
        def voucher_to_invoice(voucher):
            if voucher.VOUCHERTYPENAME.string.strip() in ["Sales", "Credit Note"]:
                ...
                account_name = encode_company_abbr(
                    self.tally_debtors_account, self.erpnext_company)
                ...
                account_name = encode_company_abbr(
                    self.tally_creditors_account, self.erpnext_company)
            ...
            for entry in ledger_entries:
                if entry.ISPARTYLEDGER.string.strip() == "No":
                    tax_account = encode_company_abbr(
                        entry.LEDGERNAME.string.strip(), self.erpnext_company)
                    taxes.append({
                        "charge_type": "Actual",
        ...
        try:
            self.publish("Process Day Book Data", _(
                "Reading Uploaded File"), 1, 3)
            collection = self.get_collection(self.day_book_data)

            self.publish("Process Day Book Data",
                         _("Processing Vouchers"), 2, 3)
    ...
    def _import_day_book_data(self):
        def create_fiscal_years(vouchers):
            ...
            oldest_year = frappe.get_all("Fiscal Year", fields=[
                                         "year_start_date", "year_end_date"], order_by="year_start_date")[0]
            while earliest_date < oldest_year.year_start_date:
                ...
                new_year.year_end_date = add_years(
                    oldest_year.year_end_date, -1)
                if new_year.year_start_date.year == new_year.year_end_date.year:
                    ...
                    new_year.year = "{}-{}".format(
                        new_year.year_start_date.year, new_year.year_end_date.year)
                new_year.save()
        ...
        try:
            frappe.db.set_value("Account", encode_company_abbr(
                self.tally_creditors_account, self.erpnext_company), "account_type", "Payable")
            frappe.db.set_value("Account", encode_company_abbr(
                self.tally_debtors_account, self.erpnext_company), "account_type", "Receivable")
            frappe.db.set_value("Company", self.erpnext_company,
                                "round_off_account", self.default_round_off_account)
            ...
            for index in range(0, total, VOUCHER_CHUNK_SIZE):
                if index + VOUCHER_CHUNK_SIZE >= total:
                    is_last = True
                frappe.enqueue_doc(self.doctype, self.name, "_import_vouchers", queue="long",
                                   timeout=3600, start=index+1, total=total, is_last=is_last)
    ...
    def _import_vouchers(self, start, total, is_last=False):
        ...
        for index, voucher in enumerate(chunk, start=start):
            try:
                ...
                voucher_doc.insert()
                voucher_doc.submit()
                self.publish("Importing Vouchers", _(
                    "{} of {}").format(index, total), index, total)
                frappe.db.commit()
            except:
                frappe.db.rollback()
    ...
    def process_master_data(self):
        ...
        frappe.enqueue_doc(self.doctype, self.name,
                           "_process_master_data", queue="long", timeout=3600)

    def import_master_data(self):
        self.set_status("Importing Master Data")
        frappe.enqueue_doc(self.doctype, self.name,
                           "_import_master_data", queue="long", timeout=3600)

    def process_day_book_data(self):
        self.set_status("Processing Day Book Data")
        frappe.enqueue_doc(self.doctype, self.name,
                           "_process_day_book_data", queue="long", timeout=3600)

    def import_day_book_data(self):
        self.set_status("Importing Day Book Data")
        frappe.enqueue_doc(self.doctype, self.name,
                           "_import_day_book_data", queue="long", timeout=3600)
    ...
    def log(self, data=None):
        if isinstance(data, frappe.model.document.Document):
            if sys.exc_info()[1].__class__ != frappe.DuplicateEntryError:
                ...
                self.failed_import_log = json.dumps(
                    failed_import_log, separators=(',', ':'))
                self.save()
                frappe.db.commit()
