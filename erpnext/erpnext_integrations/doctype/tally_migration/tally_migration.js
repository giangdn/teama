...
frappe.ui.form.on("Tally Migration", {
	...
	refresh: function(frm) {
		...
		erpnext.tally_migration.failed_import_log = JSON.parse(
			frm.doc.failed_import_log
		);
		erpnext.tally_migration.fixed_errors_log = JSON.parse(
			frm.doc.fixed_errors_log
		);
		...
		if (frm.doc.day_book_data && !frm.doc.is_day_book_data_imported) {
			if (frm.doc.is_day_book_data_processed) {
				if (frm.doc.status != "Importing Day Book Data") {
					frm.events.add_button(
						frm,
						__("Import Day Book Data"),
						"import_day_book_data"
					);
				}
			} else {
				if (frm.doc.status != "Processing Day Book Data") {
					frm.events.add_button(
						frm,
						__("Process Day Book Data"),
						"process_day_book_data"
					);
				}
			}
		}
	...
	show_error_summary(frm) {
		let summary = erpnext.tally_migration.failed_import_log.reduce(
			(summary, row) => {
				if (row.doc) {
	...
	show_logs_preview(frm) {
		...
		let render_section = !(
			import_log === completed_log && import_log === empty
		);
...
erpnext.tally_migration.getError = traceback => {
	...
	if (is_multiline) {
		...
		let split_str_idx =
			error_line.indexOf(":") > 0 ? error_line.indexOf(":") + 1 : 0;
...
erpnext.tally_migration.unresolve = document => {
	...
	let modified_fixed_log = fixed_log.filter(row => {
		if (
			!frappe.utils.deep_equal(
				erpnext.tally_migration.cleanDoc(row.doc),
				document
			)
		) {
			return row;
...
erpnext.tally_migration.create_new_doc = document => {
	...
	return frappe.call({
		type: "POST",
		method:
			"erpnext.erpnext_integrations.doctype.tally_migration.tally_migration.new_doc",
		args: {
			document
		},
		freeze: true,
		...
		.map(({ doc, exc }) => {
			...
			let show_doc = `
				...
				<div class="collapse margin-top" id="${id}-doc">
					<div class="well">
						<pre style="font-size: smaller;">${JSON.stringify(
							erpnext.tally_migration.cleanDoc(doc),
							null,
							1
						)}</pre>
					</div>
