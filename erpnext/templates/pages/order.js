...
frappe.ready(function() {
	var loyalty_points_input = document.getElementById(
		"loyalty-point-to-redeem"
	);
	var loyalty_points_status = document.getElementById(
			...
			frappe.call({
				...
				callback: function(r) {
					if (r) {
						...
						if (
							doc_info.grand_total &&
							doc_info.grand_total < loyalty_amount
						) {
							let redeemable_amount = parseInt(
								doc_info.grand_total / r.message
							);
							message =
								"You can only redeem max " +
								redeemable_amount +
							...
							message =
								loyalty_points +
								" Loyalty Points of amount " +
								loyalty_amount +
								" is applied.";
							frappe.msgprint(__(message));
							var remaining_amount =
								flt(doc_info.grand_total) - flt(loyalty_amount);
							var payment_button = document.getElementById(
								"pay-for-order"
							);
