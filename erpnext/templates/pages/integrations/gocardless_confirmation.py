...
def confirm_payment(redirect_flow_id, reference_doctype, reference_docname):
    ...
    try:
        ...
        if gocardless_success_page:
            confirmation_url = frappe.get_attr(
                gocardless_success_page[-1])(reference_doctype, reference_docname)
...
def create_mandate(data):
    ...
    if frappe.db.exists("GoCardless Mandate", mandate):
        ...
        reference_doc = frappe.db.get_value(data.get('reference_doctype'), data.get(
            'reference_docname'), ["reference_doctype", "reference_name"], as_dict=1)
