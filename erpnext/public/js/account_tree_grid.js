...
erpnext.AccountTreeGrid = frappe.views.TreeGridReport.extend({
	...
	setup_filters: function() {
		...
		this.filter_inputs.fiscal_year.change(function() {
			...
			$.each(frappe.report_dump.data["Fiscal Year"], function(i, v) {
				if (v.name == fy) {
					me.filter_inputs.from_date.val(
						frappe.datetime.str_to_user(v.year_start_date)
					);
					me.filter_inputs.to_date.val(
						frappe.datetime.str_to_user(v.year_end_date)
					);
				}
			});
			me.refresh();
	...
	prepare_balances: function() {
		...
		this.opening_date = frappe.datetime.user_to_obj(
			this.filter_inputs.from_date.val()
		);
		this.closing_date = frappe.datetime.user_to_obj(
			this.filter_inputs.to_date.val()
		);
	...
	update_balances: function(account, posting_date, v) {
		...
		if (posting_date < this.opening_date || v.is_opening === "Yes") {
			...
			this.opening_date <= posting_date &&
			posting_date <= this.closing_date
		) {
			// in between
					...
					$.each(me.columns, function(c, col) {
						if (col.formatter == me.currency_formatter) {
							if (col.field == "opening_dr") {
								...
								in_list(["debit", "credit"], col.field)
							) {
	...
	set_fiscal_year: function() {
		...
		$.each(frappe.report_dump.data["Fiscal Year"], function(i, v) {
			if (
				me.opening_date >=
					frappe.datetime.str_to_obj(v.year_start_date) &&
