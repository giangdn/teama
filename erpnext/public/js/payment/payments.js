...
erpnext.payments = erpnext.stock.StockController.extend({
	...
	show_payment_details: function() {
		...
		if (this.frm.doc.payments.length) {
			$.each(this.frm.doc.payments, function(index, data) {
				...
				if (
					data.type == "Cash" &&
					data.amount == me.frm.doc.paid_amount
				) {
					me.idx = data.idx;
		...
		if (in_list(["change_amount", "write_off_amount"], this.idx)) {
			...
			if (me.idx == "change_amount") {
				...
				if (
					...
					value = flt(
						me.frm.doc.outstanding_amount /
							me.frm.doc.conversion_rate,
						precision(me.idx)
					);
				}
	...
	show_amounts: function() {
		...
		$(this.$body)
			.find(".paid_amount")
			.text(
				format_currency(this.frm.doc.paid_amount, this.frm.doc.currency)
			);
		$(this.$body)
			.find(".change_amount")
			.val(
		...
		$(this.$body)
			...
			.text(
				format_currency(
					...
					frappe.get_doc(":Company", this.frm.doc.company)
						.default_currency
				)
			);
		this.update_invoice();
