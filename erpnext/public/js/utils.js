...
$.extend(erpnext.utils, {
	...
	copy_parent_value_in_all_row: function(
		doc,
		dt,
		dn,
		table_fieldname,
	...
	const dialog = new frappe.ui.Dialog({
					...
					},
					{
						...
						onchange: function() {
							...
							if (item_code && warehouse) {
								frappe.call({
									method:
										"erpnext.stock.utils.get_latest_stock_qty",
									args: {
										item_code: item_code,
										warehouse: warehouse
									},
						...
						get_query: e => {
							return {
								query:
									"erpnext.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
		...
		primary_action: function() {
			...
			alternative_items.forEach(d => {
				...
				frm.script_manager
					.trigger(item_field, row.doctype, row.name)
					.then(() => {
...
erpnext.utils.update_child_items = function(opts) {
	...
	const child_docname =
		typeof opts.cannot_add_row === "undefined"
			? "items"
			: opts.child_docname;
			...
			if (already_set) {
				opts.source_name.forEach(function(src) {
					frappe.model.with_doc(opts.source_doctype, src, function(
						...
						var source_doc = frappe.model.get_doc(
							opts.source_doctype,
							src
						);
	...
	if (opts.source_doctype) {
		var d = new frappe.ui.form.MultiSelectDialog({
			...
			action: function(selections, args) {
				...
				if (values.length === 0) {
					frappe.msgprint(
						__("Please select {0}", [opts.source_doctype])
					);
					return;
				}
