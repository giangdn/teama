	...
	setup: function(frm) {
		...
		$(frm.wrapper).on("grid-row-render", function(e, grid_row) {
			if (
				...
				erpnext.taxes.set_conditional_mandatory_rate_or_amount(
					grid_row
				);
			}
		});
...
frappe.ui.form.on("Purchase Invoice", {
	mode_of_payment: function(frm) {
		get_payment_mode_account(frm, frm.doc.mode_of_payment, function(
			account
		) {
...
cur_frm.cscript.validate_taxes_and_charges = function(cdt, cdn) {
	...
	if (!d.charge_type && (d.row_id || d.rate || d.tax_amount)) {
		...
		(d.charge_type == "Actual" || d.charge_type == "On Net Total") &&
		d.row_id
	) {
		msg = __(
	...
	if (cint(tax.included_in_print_rate)) {
		if (tax.charge_type == "Actual") {
			...
			if (taxes_not_included.length > 0) {
				...
				on_previous_row_error(
					tax.row_id == 1 ? "1" : "1 - " + tax.row_id
				);
			}
		} else if (tax.category == "Valuation") {
			frappe.throw(
				__("Valuation type charges can not marked as Inclusive")
			);
		}
	}
};
...
if (!erpnext.taxes.flags[cur_frm.cscript.tax_table]) {
	...
	frappe.ui.form.on(cur_frm.cscript.tax_table, "row_id", function(
		frm,
		cdt,
		cdn
	) {
	...
	frappe.ui.form.on(cur_frm.cscript.tax_table, "rate", function(
		frm,
		cdt,
		cdn
	) {
	...
	frappe.ui.form.on(cur_frm.cscript.tax_table, "tax_amount", function(
		frm,
		cdt,
		cdn
	) {
	...
	frappe.ui.form.on(cur_frm.cscript.tax_table, "charge_type", function(
		frm,
		cdt,
		cdn
	) {
...
cur_frm.pformat.total = function(doc) {
	return "";
};
...
cur_frm.pformat.taxes = function(doc) {
	...
	function print_hide(fieldname) {
		var doc_field = frappe.meta.get_docfield(
			doc.doctype,
			fieldname,
			doc.name
		);
	...
	if (!doc.print_without_amount) {
		...
		// outer table
		var out =
			'<div><table class="noborder" style="width:100%"><tr><td style="width: 60%"></td><td>';
