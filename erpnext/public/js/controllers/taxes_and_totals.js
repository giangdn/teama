...
erpnext.taxes_and_totals = erpnext.payments.extend({
	...
	apply_pricing_rule_on_item: function(item) {
		...
		if (item.discount_percentage) {
			item.discount_amount =
				(flt(item.rate_with_margin) * flt(item.discount_percentage)) /
				100;
		}
	...
	initialize_taxes: function() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			if (!this.discount_amount_applied && cur_frm) {
				cur_frm.cscript.validate_taxes_and_charges(
					tax.doctype,
					tax.name
				);
	...
	determine_exclusive_rate: function() {
		...
		$.each(me.frm.doc["items"] || [], function(n, item) {
			...
			if (
				...
				var amount =
					flt(item.amount) - total_inclusive_tax_amount_per_qty;
				item.net_amount = flt(amount / (1 + cumulated_tax_fraction));
				item.net_rate = item.qty
					? flt(
							item.net_amount / item.qty,
							precision("net_rate", item)
					  )
					: 0;
		...
		$.each(this.frm.doc["items"] || [], function(n, item) {
			...
			$.each(me.frm.doc["taxes"] || [], function(i, tax) {
				...
				if (tax.category) {
					...
					current_tax_amount =
						tax.category == "Valuation" ? 0.0 : current_tax_amount;

					current_tax_amount *=
						tax.add_deduct_tax == "Deduct" ? -1.0 : 1.0;
				}
				...
				if (i == 0) {
					...
					tax.grand_total_for_current_item = flt(
						me.frm.doc["taxes"][i - 1]
							.grand_total_for_current_item + current_tax_amount
					);
				}
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				if (
					diff &&
					Math.abs(diff) <=
						5.0 / Math.pow(10, precision("tax_amount", last_tax))
				) {
	...
	calculate_totals: function() {
		...
		var me = this;
		var tax_count = this.frm.doc["taxes"]
	...
	_cleanup: function() {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			$.each(this.frm.doc["taxes"] || [], function(i, tax) {
				...
				tax.item_wise_tax_detail = JSON.stringify(
					tax.item_wise_tax_detail
				);
			});
		}
	...
	set_discount_amount: function() {
		if (this.frm.doc.additional_discount_percentage) {
			this.frm.doc.discount_amount = flt(
				(flt(
					this.frm.doc[frappe.scrub(this.frm.doc.apply_discount_on)]
				) *
		...
		if (this.frm.doc.apply_discount_on == "Net Total") {
			...
			$.each(this.frm.doc["taxes"] || [], function(i, tax) {
				if (in_list(["Actual", "On Item Quantity"], tax.charge_type)) {
					var tax_amount =
						tax.category == "Valuation" ? 0.0 : tax.tax_amount;
					tax_amount *= tax.add_deduct_tax == "Deduct" ? -1.0 : 1.0;
					actual_taxes_dict[tax.idx] = tax_amount;
				} else if (actual_taxes_dict[tax.row_id] !== null) {
					var actual_tax_amount =
						(flt(actual_taxes_dict[tax.row_id]) * flt(tax.rate)) /
						100;
		...
		var total_allocated_amount = frappe.utils.sum(
			$.map(this.frm.doc["advances"] || [], function(adv) {
				return flt(
					...
					precision("allocated_amount", adv)
				);
			})
		);
	...
	calculate_outstanding_amount: function(update_paid_amount) {
		...
		if (
			...
			if (this.frm.doc.party_account_currency == this.frm.doc.currency) {
				...
				var total_amount_to_pay = flt(
					flt(
						grand_total * this.frm.doc.conversion_rate,
						precision("grand_total")
					) -
						this.frm.doc.total_advance -
			...
			if (
				...
				let total_amount_for_payment =
					this.frm.doc.redeem_loyalty_points &&
					this.frm.doc.loyalty_amount
						? flt(
								total_amount_to_pay -
									this.frm.doc.loyalty_amount,
								precision("base_grand_total")
