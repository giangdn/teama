// Copyright (c) 2015, TeamA Technologies Pvt. Ltd. and Contributors
...
erpnext.TransactionController = erpnext.taxes_and_totals.extend({
	setup: function() {
		...
		frappe.ui.form.on(this.frm.cscript.tax_table, "tax_amount", function(
			frm,
			cdt,
			cdn
		) {
		...
		frappe.ui.form.on(this.frm.cscript.tax_table, "row_id", function(
			frm,
			cdt,
			cdn
		) {
		...
		frappe.ui.form.on(
			...
			function(frm) {
				if (!frm.doc.apply_discount_on) {
					frappe.msgprint(
						__("Please set 'Apply Additional Discount On'")
					);
					return;
				}
				...
				frm.set_value("discount_amount", discount_amount).then(
		...
		if (this.frm.fields_dict["items"].grid.get_field("blanket_order")) {
			this.frm.set_query("blanket_order", "items", function(
				doc,
				cdt,
				cdn
			) {
	...
	setup_quality_inspection: function() {
		...
		var quality_inspection_field = this.frm.get_docfield(
			"items",
			"quality_inspection"
		);
		...
		this.frm.set_query("quality_inspection", "items", function(
			doc,
			cdt,
			cdn
		) {
	...
	make_payment_request: function() {
		var me = this;
		const payment_request_type = in_list(
		...
		frappe.call({
			...
			args: {
				...
				party_type:
					payment_request_type == "Outward" ? "Supplier" : "Customer",
				party:
	...
	onload_post_render: function() {
		...
		if (
			...
			this.frm
				.get_field("items")
				.grid.set_multiple_add("item_code", "qty");
		}
	},
	...
	refresh: function() {
		...
		if (scan_barcode_field && scan_barcode_field.get_value()) {
			...
			if (frappe.is_mobile()) {
				if (
					scan_barcode_field.$input_wrapper.find(".input-group")
						.length
				)
					return;

				let $input_group = $('<div class="input-group">');
				scan_barcode_field.$input_wrapper
					.find(".control-input")
					.append($input_group);
		...
		if (this.frm.doc.scan_barcode) {
			frappe
				...
				.then(r => {
					...
					const blank_item_row = this.frm.doc.items.find(
						d => !d.item_code
					);
					...
					show_description(
						row_to_modify.idx,
						row_to_modify.item_code
					);

					this.frm.from_barcode = this.frm.from_barcode
						? this.frm.from_barcode + 1
						: 1;
					...
					["serial_no", "batch_no", "barcode"].forEach(field => {
						if (
							...
							let value =
								row_to_modify[field] && field === "serial_no"
									? row_to_modify[field] + "\n" + data[field]
									: data[field];
						...
						frappe.run_serially([
							() => {
								...
								if (r.message.taxes_and_charges) {
									me.frm.doc.taxes_and_charges =
										r.message.taxes_and_charges;
								}
	...
	item_code: function(doc, cdt, cdn) {
		...
		this.frm.from_barcode =
			this.frm.from_barcode - 1 >= 0 ? this.frm.from_barcode - 1 : 0;

		if (item.item_code || item.barcode || item.serial_no) {
			if (!this.validate_company_and_party()) {
				this.frm.fields_dict["items"].grid.grid_rows[
					item.idx - 1
				].remove();
			} else {
				return this.frm.call({
					...
					args: {
						...
						args: {
							...
							customer:
								me.frm.doc.customer || me.frm.doc.party_name,
		...
		if (item && item.serial_no) {
			if (!item.item_code) {
				...
				for (var x = 0; x <= serial_nos.length - 1; x++) {
					if (
						serial_nos[x].trim() != "" &&
						valid_serial_nos.indexOf(serial_nos[x].trim()) == -1
					) {
	...
	company: function() {
		...
		var set_pricing = function() {
			if (me.frm.doc.company && me.frm.fields_dict.currency) {
				...
				var company_doc = frappe.get_doc(
					":Company",
					me.frm.doc.company
				);
	...
	service_start_date: function(frm, cdt, cdn) {
		...
		if (child.service_start_date) {
			frappe.call({
				method:
					"erpnext.stock.get_item_details.calculate_service_end_date",
				args: { args: child },
				callback: function(r) {
					frappe.model.set_value(
						cdt,
	...
	change_grid_labels: function(company_currency) {
		...
		if (this.frm.fields_dict["operations"]) {
			...
			$.each(["base_operating_cost", "base_hour_rate"], function(
				i,
				fname
			) {
		...
		if (this.frm.fields_dict["taxes"]) {
			...
			this.frm.set_currency_labels(
				[
					"base_tax_amount",
					"base_total",
					"base_tax_amount_after_discount"
				],
				company_currency,
				"taxes"
			);
		...
		$.each(["base_rate", "base_price_list_rate", "base_amount"], function(
			i,
			fname
		) {
			if (frappe.meta.get_docfield(item_grid.doctype, fname))
	...
	ignore_pricing_rule: function() {
		if (this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method:
					"erpnext.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	_set_values_for_item_list: function(children) {
		...
		for (var i = 0, l = children.length; i < l; i++) {
			var d = children[i];
			var existing_pricing_rule = frappe.model.get_value(
				d.doctype,
				d.name,
				"pricing_rules"
			);
			...
			if (
				!me.frm.doc.ignore_pricing_rule &&
	...
	apply_product_discount: function(free_item_data) {
		const items =
			this.frm.doc.items.filter(
				d => d.item_code == free_item_data.item_code && d.is_free_item
			) || [];

		if (!items.length) {
			let row_to_modify = frappe.model.add_child(
				this.frm.doc,
				this.frm.doc.doctype + " Item",
				"items"
			);
	...
	validate_company_and_party: function() {
		...
		$.each(["company", "customer"], function(i, fieldname) {
			if (
				...
				me.frm.doc.doctype != "Purchase Order"
			) {
	...
	get_terms: function() {
		...
		erpnext.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function(
			r
		) {
			if (!r.exc) {
				...
				callback: function(r) {
					if (!r.exc) {
						$.each(me.frm.doc.items || [], function(i, item) {
							if (
								item.item_code &&
								r.message.hasOwnProperty(item.item_code)
							) {
								if (!item.item_tax_template) {
									...
									item.item_tax_rate =
										r.message[item.item_code].item_tax_rate;
								}
								me.add_taxes_from_item_tax_template(
									item.item_tax_rate
								);
	...
	blanket_order: function(doc, cdt, cdn) {
		...
		if (
			...
			frappe.call({
				method:
					"erpnext.stock.get_item_details.get_blanket_order_details",
				args: {
					args: {
						item_code: item.item_code,
...
erpnext.show_serial_batch_selector = function(
	...
	if (frm.doc.is_return) {
		if (
			["Purchase Receipt", "Purchase Invoice"].includes(frm.doc.doctype)
		) {
			existing_stock = true;
			warehouse = d.warehouse;
		} else if (
			["Delivery Note", "Sales Invoice"].includes(frm.doc.doctype)
		) {
...
erpnext.apply_putaway_rule = (frm, purpose = null) => {
	...
	frappe.call({
		method:
			"erpnext.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
