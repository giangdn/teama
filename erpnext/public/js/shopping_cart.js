...
frappe.ready(function() {
	...
	if (
		referral_coupon_code &&
		$(".tot_quotation_discount").val() == undefined
	) {
...
$.extend(shopping_cart, {
		...
		if (frappe.session.user === "Guest") {
			...
			return frappe.call({
				...
				args: {
					...
					qty: opts.qty,
					additional_notes:
						opts.additional_notes !== undefined
							? opts.additional_notes
							: undefined,
				...
				callback: function(r) {
					...
					if (r.message.shopping_cart_menu) {
						$(".shopping-cart-menu").html(
							r.message.shopping_cart_menu
						);
					}
	...
	show_cart_navbar: function() {
		frappe.call({
			...
			callback: function(r) {
				$(".shopping-cart").toggleClass(
					"hidden",
					r.message ? false : true
				);
			}
		});
	}
