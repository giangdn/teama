...
class LoanInterestAccrual(AccountsController):
    ...
    def update_is_accrued(self):
        frappe.db.set_value('Repayment Schedule',
                            self.repayment_schedule_name, 'is_accrued', 0)
...
def calculate_accrual_amount_for_demand_loans(loan, posting_date, process_loan_interest, accrual_type):
    ...
    pending_amounts = calculate_amounts(
        loan.name, posting_date, payment_type='Loan Closure')
...
def make_loan_interest_accrual_entry(args):
    ...
    loan_interest_accrual.pending_principal_amount = flt(
        args.pending_principal_amount, precision)
    loan_interest_accrual.interest_amount = flt(
        args.interest_amount, precision)
...
def get_no_of_days_for_interest_accural(loan, posting_date):
    ...
    no_of_days = date_diff(posting_date or nowdate(),
                           last_interest_accrual_date) + 1
