...
class LoanApplication(Document):
    ...
    def validate_loan_type(self):
        ...
        if company != self.company:
            frappe.throw(_("Please select Loan Type for company {0}").format(
    ...
    def validate_loan_amount(self):
        ...
        maximum_loan_limit = frappe.db.get_value(
            'Loan Type', self.loan_type, 'maximum_loan_amount')
        if maximum_loan_limit and self.loan_amount > maximum_loan_limit:
            frappe.throw(_("Loan Amount cannot exceed Maximum Loan Amount of {0}").format(
                maximum_loan_limit))

        if self.maximum_loan_amount and self.loan_amount > self.maximum_loan_amount:
            frappe.throw(_("Loan Amount exceeds maximum loan amount of {0} as per proposed securities").format(
    ...
    def set_pledge_amount(self):
        for proposed_pledge in self.proposed_pledges:
            ...
            proposed_pledge.loan_security_price = get_loan_security_price(
                proposed_pledge.loan_security)

            if not proposed_pledge.qty:
                proposed_pledge.qty = cint(
                    proposed_pledge.amount/proposed_pledge.loan_security_price)

            proposed_pledge.amount = proposed_pledge.qty * \
                proposed_pledge.loan_security_price
    ...
    def calculate_payable_amount(self):
        ...
        while(balance_amount > 0):
            ...
            balance_amount = rounded(
                balance_amount + interest_amount - self.repayment_amount)
...
def create_loan(source_name, target_doc=None, submit=0):
    def update_accounts(source_doc, target_doc, source_parent):
        account_details = frappe.get_all("Loan Type",
                                         fields=["mode_of_payment", "payment_account", "loan_account",
...
def get_proposed_pledge(securities):
    ...
    for security in securities:
        ...
        security.post_haircut_amount = security.amount - \
            (security.amount * security.haircut/100)
