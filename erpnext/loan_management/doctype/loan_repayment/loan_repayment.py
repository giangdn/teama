...
class LoanRepayment(AccountsController):
    ...
    def validate_amount(self):
        ...
        if self.amount_paid < self.penalty_amount:
            msg = _("Paid amount cannot be less than {0}").format(
                self.penalty_amount)
            frappe.throw(msg)
    ...
    def mark_as_unpaid(self):
        ...
        for payment in self.repayment_details:
            ...
            if payment.accrual_type == 'Repayment' and payment.idx == no_of_repayments:
                lia_doc = frappe.get_doc(
                    'Loan Interest Accrual', payment.loan_interest_accrual)
                lia_doc.cancel()
    ...
    def make_gl_entries(self, cancel=0, adv_adj=0):
        ...
        if gle_map:
            make_gl_entries(gle_map, cancel=cancel,
...
def get_amounts(amounts, against_loan, posting_date):
    ...
    for entry in accrued_interest_entries:
        ...
        if no_of_late_days > 0 and (not against_loan_doc.repay_from_salary) and entry.accrual_type == 'Regular':
            penalty_amount += (entry.interest_amount *
                               (loan_type_details.penalty_interest_rate / 100) * no_of_late_days)
    ...
    amounts["pending_principal_amount"] = flt(
        pending_principal_amount, precision)
    amounts["payable_principal_amount"] = flt(
        payable_principal_amount, precision)
...
def calculate_amounts(against_loan, posting_date, payment_type=''):
    ...
    if payment_type == 'Loan Closure':
        ...
        amounts['payable_amount'] = amounts['payable_principal_amount'] + \
