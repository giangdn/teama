...
class LoanSecurityPledge(Document):
    ...
    def validate_loan_security_type(self):
        ...
        if self.loan:
            existing_pledge = frappe.db.get_value(
                'Loan Security Pledge', {'loan': self.loan}, ['name'])
        ...
        for security in self.securities:
            if ltv_ratio_map.get(security.loan_security_type) != ltv_ratio:
                frappe.throw(
                    _("Loan Securities with different LTV ratio cannot be pledged against one loan"))
    ...
    def set_pledge_amount(self):
        ...
        for pledge in self.securities:
            ...
            pledge.post_haircut_amount = cint(
                pledge.amount - (pledge.amount * pledge.haircut/100))
...
def update_loan(loan, maximum_value_against_pledge):
    maximum_loan_value = frappe.db.get_value(
        'Loan', {'name': loan}, ['maximum_loan_amount'])
