...
class LoanDisbursement(AccountsController):
    ...
    def set_missing_values(self):
        ...
        if not self.bank_account and self.applicant_type == "Customer":
            self.bank_account = frappe.db.get_value(
                "Customer", self.applicant, "default_bank_account")

    def validate_disbursal_amount(self):
        possible_disbursal_amount = get_disbursal_amount(self.against_loan)

        if self.disbursed_amount > possible_disbursal_amount:
            frappe.throw(_("Disbursed Amount cannot be greater than {0}").format(
                possible_disbursal_amount))
    ...
    def set_status_and_amounts(self, cancel=0):
        ...
        if cancel:
            disbursed_amount, status, total_payment = self.get_values_on_cancel(
                loan_details)
        else:
            disbursed_amount, status, total_payment = self.get_values_on_submit(
                loan_details)
...
def get_total_pledged_security_value(loan):
    ...
    loan_security_price_map = frappe._dict(frappe.get_all("Loan Security Price",
                                                          fields=[
                                                              "loan_security", "loan_security_price"],
                                                          filters={
    ...
    for security, qty in pledged_securities.items():
        ...
        security_value += (loan_security_price_map.get(security)
                           * qty * after_haircut_percentage)/100
