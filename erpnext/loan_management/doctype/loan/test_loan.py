...
class TestLoan(unittest.TestCase):
    def setUp(self):
        ...
        create_loan_security_price("Test Security 1", 500, "Nos", get_datetime(
        ), get_datetime(add_to_date(nowdate(), hours=24)))
        create_loan_security_price("Test Security 2", 250, "Nos", get_datetime(
        ), get_datetime(add_to_date(nowdate(), hours=24)))

        self.applicant1 = make_employee("robert_loan@loan.com")
        make_salary_structure("Test Salary Structure Loan", "Monthly",
                              employee=self.applicant1, currency='INR', company="_Test Company")
        if not frappe.db.exists("Customer", "_Test Loan Customer"):
            frappe.get_doc(get_customer_dict('_Test Loan Customer')).insert(
                ignore_permissions=True)

        self.applicant2 = frappe.db.get_value(
            "Customer", {'name': '_Test Loan Customer'}, 'name')
    ...
    def test_loan(self):
        ...
        for idx, principal_amount, interest_amount, balance_loan_amount in [[3, 13369, 1683, 227079], [19, 14941, 105, 0], [17, 14740, 312, 29785]]:
            ...
            self.assertEqual(
                schedule[idx].balance_loan_amount, balance_loan_amount)
    ...
    def test_loan_disbursement(self):
        ...
        loan_disbursement_entry1 = make_loan_disbursement_entry(
            loan.name, 500000)
        loan_disbursement_entry2 = make_loan_disbursement_entry(
            loan.name, 500000)

        loan = frappe.get_doc("Loan", loan.name)
        gl_entries1 = frappe.db.get_all("GL Entry",
                                        fields=["name"],
                                        filters={'voucher_type': 'Loan Disbursement',
                                                 'voucher_no': loan_disbursement_entry1.name}
                                        )

        gl_entries2 = frappe.db.get_all("GL Entry",
                                        fields=["name"],
                                        filters={'voucher_type': 'Loan Disbursement',
                                                 'voucher_no': loan_disbursement_entry2.name}
                                        )
    ...
    def test_regular_loan_repayment(self):
        ...
        self.assertEquals(flt(repayment_entry.penalty_amount,
                              0), flt(penalty_amount, 0))

        amounts = frappe.db.get_all('Loan Interest Accrual', {
                                    'loan': loan.name}, ['paid_interest_amount'])
    ...
    def test_loan_closure(self):
        ...
        amount = frappe.db.get_value('Loan Interest Accrual', {'loan': loan.name}, [
    ...
    def test_security_shortfall(self):
        ...
        loan_security_shortfall = frappe.get_doc(
            "Loan Security Shortfall", {"loan": loan.name})
    ...
    def test_loan_security_unpledge(self):
        ...
        repayment_entry = create_repayment_entry(loan.name, self.applicant2, add_days(
            last_date, 5), flt(loan.loan_amount + accrued_interest_amount))
        repayment_entry.submit()
        ...
        amounts = amounts = calculate_amounts(
            loan.name, add_days(last_date, 5))
    ...
    def test_partial_loan_security_unpledge(self):
        ...
        unpledge_request = unpledge_security(
            loan=loan.name, security_map=unpledge_map, save=1)
        unpledge_request.submit()
        unpledge_request.status = 'Approved'
        unpledge_request.save()
        unpledge_request.submit()
    ...
    def test_partial_unaccrued_interest_payment(self):
        ...
        self.assertEqual(
            flt(repayment_entry.total_interest_paid, 0), flt(interest_amount, 0))
    ...
    def test_penalty(self):
        ...
        penalty_amount = flt(
            (((penalty_applicable_amount * 25) / 100) * penalty_days), 2)
    ...
    def test_loan_amount_write_off(self):
        ...
        we = make_loan_write_off(
            loan.name, amount=amounts['pending_principal_amount'])
        we.submit()
