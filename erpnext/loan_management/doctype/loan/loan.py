...
class Loan(AccountsController):
    def validate(self):
        if self.applicant_type == 'Employee' and self.repay_from_salary:
            validate_employee_currency_with_company_currency(
                self.applicant, self.company)
    ...
    def validate_accounts(self):
        for fieldname in ['payment_account', 'loan_account', 'interest_income_account', 'penalty_income_account']:
            company = frappe.get_value(
                "Account", self.get(fieldname), 'company')
    ...
    def set_missing_fields(self):
        ...
        if self.repayment_method == "Repay Over Number of Periods":
            self.monthly_repayment_amount = get_monthly_repayment_amount(
                self.repayment_method, self.loan_amount, self.rate_of_interest, self.repayment_periods)

    def check_sanctioned_amount_limit(self):
        total_loan_amount = get_total_loan_amount(
        ...
        if sanctioned_amount_limit and flt(self.loan_amount) + flt(total_loan_amount) > flt(sanctioned_amount_limit):
            frappe.throw(_("Sanctioned Amount limit crossed for {0} {1}").format(
    ...
    def validate_repay_from_salary(self):
        if not self.is_term_loan and self.repay_from_salary:
            frappe.throw(
                _("Repay From Salary can be selected only for term loans"))
    ...
    def make_repayment_schedule(self):
        ...
        while(balance_amount > 0):
            interest_amount = rounded(
                balance_amount * flt(self.rate_of_interest) / (12*100))
            principal_amount = self.monthly_repayment_amount - interest_amount
            balance_amount = rounded(
                balance_amount + interest_amount - self.monthly_repayment_amount)
    ...
    def set_loan_amount(self):
        if self.loan_application and not self.loan_amount:
            self.loan_amount = frappe.db.get_value(
                'Loan Application', self.loan_application, 'loan_amount')

    def validate_loan_amount(self):
        if self.maximum_loan_amount and self.loan_amount > self.maximum_loan_amount:
            msg = _("Loan amount cannot be greater than {0}").format(
    ...
    def unlink_loan_security_pledge(self):
        pledges = frappe.get_all('Loan Security Pledge', fields=[
                                 'name'], filters={'loan': self.name})
...
def update_total_amount_paid(doc):
    ...
    frappe.db.set_value(
        "Loan", doc.name, "total_amount_paid", total_amount_paid)


def get_total_loan_amount(applicant_type, applicant, company):
    return frappe.db.get_value('Loan',
                               {'applicant_type': applicant_type, 'company': company,
...
def validate_repayment_method(repayment_method, loan_amount, monthly_repayment_amount, repayment_periods, is_term_loan):
    ...
    if repayment_method == "Repay Fixed Amount per Period":
        ...
        if monthly_repayment_amount > loan_amount:
            frappe.throw(
                _("Monthly Repayment Amount cannot be greater than Loan Amount"))
...
def get_monthly_repayment_amount(repayment_method, loan_amount, rate_of_interest, repayment_periods):
    if rate_of_interest:
        ...
        monthly_repayment_amount = math.ceil(
            flt(loan_amount) / repayment_periods)
...
def request_loan_closure(loan, posting_date=None):
    ...
    write_off_limit = frappe.get_value(
        'Loan Type', loan_type, 'write_off_amount')
    ...
    if pending_amount < write_off_limit:
        ...
        frappe.throw(
            _("Cannot close loan as there is an outstanding of {0}").format(pending_amount))
...
def unpledge_security(loan=None, loan_security_pledge=None, security_map=None, as_dict=0, save=0, submit=0, approve=0):
    ...
    # will unpledge qty based on loan security pledge
        ...
        security_map = {}
        pledge_doc = frappe.get_doc(
            'Loan Security Pledge', loan_security_pledge)
