...
def get_columns(filters):
    columns = [
        ...
        {"label": _("Current Value"), "fieldname": "current_value",
         "fieldtype": "Currency", "options": "currency", "width": 100},
        {"label": _("% Of Total Portfolio"), "fieldname": "portfolio_percent",
         "fieldtype": "Percentage", "width": 100},
        {"label": _("Pledged Applicant Count"), "fieldname": "pledged_applicant_count",
         "fieldtype": "Percentage", "width": 100},
...
def get_data(filters):
    ...
    for security, value in iteritems(current_pledges):
        if value.get('qty'):
            row = {}
            current_value = flt(value.get(
                'qty', 0) * loan_security_details.get(security, {}).get('latest_price', 0))
            valid_upto = loan_security_details.get(
                security, {}).get('valid_upto')
...
def get_company_wise_loan_security_details(filters, loan_security_details):
    ...
    for key, qty in iteritems(pledge_values):
        ...
        total_portfolio_value += flt(qty *
                                     loan_security_details.get(key[1])['latest_price'])
