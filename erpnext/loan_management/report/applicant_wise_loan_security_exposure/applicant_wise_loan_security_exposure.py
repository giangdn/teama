...
def get_columns(filters):
    columns = [
        ...
        {"label": _("Loan Security"), "fieldname": "loan_security",
         "fieldtype": "Link", "options": "Loan Security", "width": 160},
        {"label": _("Loan Security Code"), "fieldname": "loan_security_code",
         "fieldtype": "Data", "width": 100},
        {"label": _("Loan Security Name"), "fieldname": "loan_security_name",
         "fieldtype": "Data", "width": 150},
        {"label": _("Haircut"), "fieldname": "haircut",
         "fieldtype": "Percent", "width": 100},
        {"label": _("Loan Security Type"), "fieldname": "loan_security_type",
         "fieldtype": "Link", "options": "Loan Security Type", "width": 120},
        {"label": _("Disabled"), "fieldname": "disabled",
         "fieldtype": "Check", "width": 80},
        {"label": _("Total Qty"), "fieldname": "total_qty",
         "fieldtype": "Float", "width": 100},
        {"label": _("Latest Price"), "fieldname": "latest_price",
         "fieldtype": "Currency", "options": "currency", "width": 100},
        {"label": _("Price Valid Upto"), "fieldname": "price_valid_upto",
         "fieldtype": "Datetime", "width": 100},
        {"label": _("Current Value"), "fieldname": "current_value",
         "fieldtype": "Currency", "options": "currency", "width": 100},
        {"label": _("% Of Applicant Portfolio"), "fieldname": "portfolio_percent",
         "fieldtype": "Percentage", "width": 100},
...
def get_data(filters):
    ...
    for key, qty in iteritems(pledge_values):
        if qty:
            ...
            valid_upto = loan_security_details.get(
                key[1], {}).get('valid_upto')
...
def get_loan_security_details(filters):
    ...
    for security in loan_security_prices:
        loan_security_price_map.setdefault(
            security.loan_security, security.loan_security_price)
        lsp_validity_map.setdefault(
            security.loan_security, security.valid_upto)
...
def get_applicant_wise_total_loan_security_qty(filters, loan_security_details):
    ...
    for unpledge in unpledges:
        applicant_wise_unpledges.setdefault(
            (unpledge.applicant, unpledge.loan_security), unpledge.qty)
    ...
    for security in pledges:
        current_pledges.setdefault(
            (security.applicant, security.loan_security), security.qty)
        total_value_map.setdefault(security.applicant, 0.0)
        applicant_type_map.setdefault(
            security.applicant, security.applicant_type)

        current_pledges[(security.applicant, security.loan_security)] -= \
            applicant_wise_unpledges.get(
