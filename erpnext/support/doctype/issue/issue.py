        ...
        if self.service_level_agreement:
            ...
            if hold_statuses:
                ...
                if self.status not in hold_statuses and status in hold_statuses:
                    ...
                    resolution_by = add_to_date(
                        resolution_by, seconds=round(last_hold_time))
    ...
    def set_response_and_resolution_time(self, priority=None, service_level_agreement=None):
        ...
        if not service_level_agreement:
            if frappe.db.get_value("Issue", self.name, "service_level_agreement"):
                frappe.throw(_("Couldn't Set Service Level Agreement {0}.").format(
        ...
        if (service_level_agreement.customer and self.customer) and not (service_level_agreement.customer == self.customer):
            frappe.throw(_("This Service Level Agreement is specific to Customer {0}").format(
        ...
        self.response_by_variance = round(
            time_diff_in_seconds(self.response_by, now_datetime()))
        self.resolution_by_variance = round(
            time_diff_in_seconds(self.resolution_by, now_datetime()))
    ...
    def change_service_level_agreement_and_priority(self):
        if self.service_level_agreement and frappe.db.exists("Issue", self.name) and \
            ...
            if not self.service_level_agreement == frappe.db.get_value("Issue", self.name, "service_level_agreement"):
                self.set_response_and_resolution_time(
                    priority=self.priority, service_level_agreement=self.service_level_agreement)
                frappe.msgprint(_("Service Level Agreement has been changed to {0}.").format(
                    self.service_level_agreement))
        ...
        if not is_holiday(current_date_time, holidays) and current_weekday in support_days:
            ...
            if time_left_today <= 0:
                ...
                if time_left_today >= allotted_seconds:
                    ...
                    expected_time = add_to_date(
                        expected_time, seconds=allotted_seconds)
...
def set_service_level_agreement_variance(issue=None):
    ...
    for issue in frappe.get_list("Issue", filters=filters):
        ...
        if not doc.first_responded_on:  # first_responded_on set when first reply is sent to customer
            variance = round(time_diff_in_seconds(
                doc.response_by, current_time), 2)
            frappe.db.set_value(
        ...
        if not doc.resolution_date:  # resolution_date set when issue has been closed
            variance = round(time_diff_in_seconds(
                doc.resolution_by, current_time), 2)
            frappe.db.set_value(
...
def set_resolution_time(issue):
    ...
    resolution_time = time_diff_in_seconds(
        issue.resolution_date, issue.creation)
...
def set_user_resolution_time(issue):
    ...
    resolution_time_in_secs = time_diff_in_seconds(
        issue.resolution_date, issue.creation)
...
def auto_close_tickets():
    ...
    auto_close_after_days = frappe.db.get_value(
        "Support Settings", "Support Settings", "close_issue_after_days") or 7
...
def has_website_permission(doc, ptype, user, verbose=False):
    ...
    permission_based_on_customer = has_website_permission(
        doc, ptype, user, verbose)
...
def update_issue(contact, method):
    ...
    frappe.db.sql(
        """UPDATE `tabIssue` set contact='' where contact=%s""", contact.name)
...
def make_issue_from_communication(communication, ignore_communication_links=False):
    ...
    link_communication_to_document(
        doc, "Issue", issue.name, ignore_communication_links)
