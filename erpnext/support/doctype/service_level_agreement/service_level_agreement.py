    ...
    def check_priorities(self):
        ...
        for priority in self.priorities:
            ...
            if not priority.response_time or not priority.resolution_time:
                frappe.throw(_("Set Response Time and Resolution Time for Priority {0} in row {1}.").format(
            ...
            if response > resolution:
                frappe.throw(_("Response Time for {0} priority in row {1} can't be greater than Resolution Time.").format(
                    priority.priority, priority.idx))
        ...
        if not len(set(priorities)) == len(priorities):
            ...
            frappe.throw(
                _("Priority {0} has been repeated.").format(repeated_priority))
    ...
    def validate_doc(self):
        ...
        if self.entity_type and self.entity:
            if frappe.db.exists("Service Level Agreement", {"entity_type": self.entity_type, "entity": self.entity, "name": ["!=", self.name]}):
                frappe.throw(_("Service Level Agreement with Entity Type {0} and Entity {1} already exists.").format(
                    self.entity_type, self.entity))

    def get_service_level_agreement_priority(self, priority):
        priority = frappe.get_doc("Service Level Priority", {
                                  "priority": priority, "parent": self.name})
...
def check_agreement_status():
    ...
    for service_level_agreement in service_level_agreements:
        doc = frappe.get_doc("Service Level Agreement",
                             service_level_agreement.name)
        if doc.end_date and getdate(doc.end_date) < getdate(frappe.utils.getdate()):
            frappe.db.set_value("Service Level Agreement",
                                service_level_agreement.name, "active", 0)
...
def get_active_service_level_agreement_for(priority, customer=None, service_level_agreement=None):
    ...
    or_filters = [
        ["Service Level Agreement", "entity", "in", [
    ...
    or_filters.append(["Service Level Agreement",
