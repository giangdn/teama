...
class IssueAnalytics(object):
    ...
    def get_issues(self):
        ...
        self.entries = frappe.db.get_all('Issue',
                                         fields=[self.field_map.get(
                                             self.filters.based_on), 'name', 'opening_date'],
                                         filters=filters
                                         )
            ...
            if self.filters.based_on == 'Assigned To':
                if d._assign:
                    for entry in json.loads(d._assign):
                        self.issue_periodic_data.setdefault(
                            entry, frappe._dict()).setdefault(period, 0.0)
                ...
                self.issue_periodic_data.setdefault(
                    value, frappe._dict()).setdefault(period, 0.0)
