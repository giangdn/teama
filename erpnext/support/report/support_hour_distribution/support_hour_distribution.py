...
def get_data(filters):
    ...
    while(start_date <= getdate(filters.to_date)):
        ...
        for key, value in iteritems(time_slots):
            ...
            start_time = get_datetime("{0} {1}".format(
                start_date.strftime("%Y-%m-%d"), start_time))
            end_time = get_datetime("{0} {1}".format(
                start_date.strftime("%Y-%m-%d"), end_time))
            hours_count[key] = get_hours_count(start_time, end_time)
            time_slot_wise_total_count[key] = time_slot_wise_total_count.get(
                key, 0) + hours_count[key]
