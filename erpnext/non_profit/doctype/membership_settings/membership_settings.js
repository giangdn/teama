...
frappe.ui.form.on("Membership Settings", {
	refresh: function(frm) {
		...
		let docs_url =
			"https://docs.erpnext.com/docs/user/manual/en/non_profit/membership";
		...
		if (frm.doc.webhook_secret) {
			frm.add_custom_button(__("Copy Webhook URL"), () => {
				frappe.utils.copy_to_clipboard(
					`https://${frappe.boot.sitename}/api/method/erpnext.non_profit.doctype.membership.membership.trigger_razorpay_subscription`
				);
			});
		}
	}
