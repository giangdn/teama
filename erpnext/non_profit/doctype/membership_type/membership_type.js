...
frappe.ui.form.on("Membership Type", {
	refresh: function(frm) {
		frappe.db
			.get_single_value("Membership Settings", "enable_razorpay")
			.then(val => {
				if (val)
					frm.set_df_property("razorpay_plan_id", "hidden", false);
			});

		frappe.db
			.get_single_value("Membership Settings", "enable_invoicing")
			.then(val => {
