...
class Membership(Document):
    ...
    def create_member_from_website_user(self):
        member_name = frappe.get_value(
            "Member", dict(email_id=frappe.session.user))
    ...
    def on_payment_authorized(self, status_changed_to=None):
        ...
        if settings.enable_invoicing and settings.create_for_web_forms:
            self.generate_invoice(
                with_payment_entry=settings.make_payment_entry, save=True)
    ...
    def generate_invoice(self, save=True, with_payment_entry=False):
        ...
        if not member.customer:
            frappe.throw(_("No customer linked to member {0}").format(
    ...
    def send_acknowlement(self):
        ...
        if not member.email_id:
            frappe.throw(_("Email address of member {0} is missing").format(
        ...
        if self.invoice and settings.send_invoice:
            attachments.append(frappe.attach_print(
                "Sales Invoice", self.invoice, print_format=settings.inv_print_format))

        email_template = frappe.get_doc(
            "Email Template", settings.email_template)
        ...
        if not frappe.flags.in_test:
            frappe.enqueue(method=frappe.sendmail, queue="short",
...
def trigger_razorpay_subscription(*args, **kwargs):
    ...
    try:
        ...
        member.subscription_start = datetime.fromtimestamp(
        ...
        message = "{0}\n\n{1}\n\n{2}: {3}".format(
            e, frappe.get_traceback(), __("Payment ID"), payment.id)
        log = frappe.log_error(message, _(
            "Error creating membership entry for {0}").format(member.name))
        notify_failure(log)
        return {"status": "Failed", "reason": e}
...
def get_plan_from_razorpay_id(plan_id):
    plan = frappe.get_all("Membership Type", filters={
                          "razorpay_plan_id": plan_id}, order_by="creation desc")
