...
def setup_item():
    items = json.loads(open(frappe.get_app_path(
        'erpnext', 'demo', 'data', 'item_education.json')).read())
    for i in items:
    ...
    with open(file_path, "r") as open_file:
        ...
        for idx, d in enumerate(random_student_data):
            ...
            student_applicant.student_email_id = d.get('first_name') + "_" + \
                student_applicant.middle_name + "_" + \
                d.get('last_name') + "@example.com"
            if count < 5:
...
def make_fees_structure():
    for d in frappe.db.get_list("Program"):
        ...
        for academic_term in ["2017-18 (Semester 1)", "2017-18 (Semester 2)", "2017-18 (Semester 3)"]:
            ...
            fee_structure.academic_term = random.choice(
                frappe.db.get_list("Academic Term")).name
            for j in range(1, 4):
                temp = {"fees_category": random.choice(frappe.db.get_list(
                    "Fee Category")).name, "amount": random.randint(500, 1000)}
                fee_structure.append("components", temp)
            fee_structure.insert()
            program.append("fees", {"academic_term": academic_term,
                                    "fee_structure": fee_structure.name, "amount": fee_structure.total_amount})
        program.save()
    frappe.db.commit()
