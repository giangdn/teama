...
def make_patient():
    ...
    with open(file_path, "r") as open_file:
        ...
        for d in enumerate(patient_data):
            ...
            patient.email_id = d[1]['patient_name'] + "_" + \
                patient.date_of_birth.strftime('%m/%d/%Y') + "@example.com"
            if count < 5:
                patient.insert()
                frappe.db.commit()
            count += 1
...
def make_consulation():
    for i in range(3):
        ...
        encounter = set_encounter(
            patient, patient_sex, practitioner, department, getdate(), i)
...
def consulation_on_appointment():
    for i in range(3):
        ...
        encounter = set_encounter(appointment.patient, appointment.patient_sex,
                                  appointment.practitioner, appointment.department, appointment.appointment_date, i)
...
def lab_test_on_encounter():
    ...
    while i <= 2:
        ...
        set_lab_test(encounter.patient, encounter.patient_sex,
                     encounter.practitioner, test_rx.test_code, test_rx.name)
        i += 1
...
def import_drug():
    ...
    data = json.loads(open(frappe.get_app_path(
        'erpnext', 'demo', 'data', 'drug_list.json')).read())
    for d in data:
