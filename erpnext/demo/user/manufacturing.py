...
def work():
    ...
    if random.random() < 0.4:
        for pro in query_report.run("Open Work Orders")["result"][:how_many("Stock Entry for WIP")]:
            make_stock_entry_from_pro(
                pro[0], "Material Transfer for Manufacture")
...
def make_stock_entry_from_pro(pro_id, purpose):
    ...
    try:
        ...
        for d in st.get("items"):
            d.cost_center = "Main - " + \
                frappe.get_cached_value('Company',  st.company,  'abbr')
        st.insert()
        frappe.db.commit()
        st.submit()
        frappe.db.commit()
...
def submit_job_cards():
    work_orders = frappe.get_all("Work Order", ["name", "creation"], {
