...
class LinkedInSettings(Document):
    def get_authorization_url(self):
        ...
        url = "https://www.linkedin.com/oauth/v2/authorization?{}".format(
            params)
    ...
    def post(self, text, media=None):
        if not media:
            ...
            if media_id:
                ...
                frappe.log_error("Failed to upload media.",
    ...
    def upload_image(self, media):
        ...
        if response.status_code == 200:
            ...
            response = self.http_post(
                upload_url, headers=headers, data=open(media, "rb"))
            if response.status_code < 200 and response.status_code > 299:
                frappe.throw(_("Error While Uploading Image"), title="{0} {1}".format(
    ...
    def http_post(self, url, headers=None, body=None, data=None):
        try:
            ...
            if response.status_code == 401:
                ...
                frappe.db.commit()
                frappe.throw(content["message"],
                             title="LinkedIn Error - Unauthorized")
            elif response.status_code == 403:
                frappe.msgprint(
                    _("You Didn't have permission to access this API"))
                frappe.throw(content["message"],
                             title="LinkedIn Error - Access Denied")
            else:
