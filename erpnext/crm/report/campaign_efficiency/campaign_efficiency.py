...
def get_lead_data(filters, based_on):
    ...
    for based_on_value, leads in lead_map.items():
        ...
        row["opp_lead"] = flt(row["opp_count"]) / \
            flt(row["lead_count"] or 1.0) * 100.0
        row["quot_lead"] = flt(row["quot_count"]) / \
            flt(row["lead_count"] or 1.0) * 100.0

        row["order_quot"] = flt(row["order_count"]) / \
            flt(row["quot_count"] or 1.0) * 100.0
