...
def execute(filters=None):
    ...
    for row in data:
        ...
        total_tickets += row[3]
    data.append(['Average', total_interactions/len(data),
                 total_duration/len(data), total_tickets/len(data)])
    return columns, data
...
def get_communication_details(filters):
    ...
    for d in opportunities:
        ...
        support_tickets = len(frappe.db.get_all(
            'Issue', {'raised_by': d.contact_email}))
        communication_list.append({'customer': d.customer_name, 'interactions': communication_count,
                                   'duration': duration, 'support_tickets': support_tickets})
