...
class ProductionPlanReport(object):
    ...
    def get_open_orders(self):
        ...
        order_by = " ORDER BY %s" % (
            order_mapper[self.filters.based_on][self.filters.order_by])
    ...
    def prepare_other_conditions(self, filters, doctype):
        if self.filters.docnames:
            field = "name" if doctype == "Work Order" else "`tab{} Item`.parent".format(
                doctype)
            filters += " and %s in (%s)" % (field,
                                            ','.join(['%s'] * len(self.filters.docnames)))

        if doctype != "Work Order":
            filters += " and `tab{doc}`.name = `tab{doc} Item`.parent".format(
                doc=doctype)

        if self.filters.company:
            filters += " and `tab%s`.company = %s" % (
                doctype, frappe.db.escape(self.filters.company))
    ...
    def get_raw_materials(self):
        ...
        if self.filters.based_on == "Work Order":
            ...
            for d in self.orders:
                bom_no = d.bom_no or frappe.get_cached_value(
                    "Item", d.production_item, "default_bom")
    ...
    def get_bin_details(self):
        ...
        if self.filters.raw_material_warehouse:
            self.mrp_warehouses.extend(get_child_warehouses(
    ...
    def get_purchase_details(self):
        ...
        for d in frappe.get_all("Purchase Order Item",
                                fields=[
                                    "item_code", "min(schedule_date) as arrival_date", "qty as arrival_qty", "warehouse"],
                                filters={"item_code": ("in", self.item_codes), "warehouse": (
                                    "in", self.warehouses)},
    ...
    def prepare_data(self):
        ...
        for d in self.orders:
            ...
            bin_data = self.bin_details.get(
                (d.production_item, d.warehouse)) or {}
            d.update({
