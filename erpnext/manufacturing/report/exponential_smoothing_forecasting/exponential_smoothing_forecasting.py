        ...
        for key, value in self.period_wise_data.items():
            ...
            for period in self.period_list:
                ...
                if value.get(period.key) and not forecast_data:
                    value[forecast_key] = flt(
                        value.get("avg", 0)) or flt(value.get(period.key))

                elif forecast_data:
                    previous_period_data = forecast_data[-1]
                    value[forecast_key] = (previous_period_data[1] +
                                           flt(self.filters.smoothing_constant) * (
                        flt(previous_period_data[0]) -
                        flt(previous_period_data[1])
                    )
                    )
                ...
                if value.get(forecast_key):
                    ...
                    forecast_data.append(
                        [value.get(period.key), value.get(forecast_key)])
...
class ForecastingReport(ExponentialSmoothingForecast):
    ...
    def prepare_periodical_data(self):
        ...
        from_date = add_years(self.filters.from_date,
                              cint(self.filters.no_of_years) * -1)
        ...
        for entry in order_data:
            ...
            for period in self.period_list:
                ...
                if (entry.posting_date >= period.from_date and entry.posting_date <= period.to_date):
                    period_data[period.key] = period_data.get(
                        period.key, 0.0) + flt(entry.get(self.based_on_field))

        for key, value in self.period_wise_data.items():
            list_of_period_value = [
                value.get(p.key, 0) for p in self.period_list]

            if list_of_period_value:
                total_qty = [1 for d in list_of_period_value if d]
                if total_qty:
                    value["avg"] = flt(
                        sum(list_of_period_value)) / flt(sum(total_qty))

    def get_data_for_forecast(self):
        cond = ""
        if self.filters.item_code:
            cond = " AND soi.item_code = %s" % (
                frappe.db.escape(self.filters.item_code))

        warehouses = []
        if self.filters.warehouse:
            warehouses = get_child_warehouses(self.filters.warehouse)
            cond += " AND soi.warehouse in ({})".format(
    ...
    def get_columns(self):
        ...
        width = 180 if self.filters.periodicity in [
            'Yearly', "Half-Yearly", "Quarterly"] else 100
        for period in self.period_list:
    ...
    def get_chart_data(self):
        ...
        for period in self.period_list:
            ...
            if period.from_date < getdate(self.filters.from_date):
                ...
                self.total_history_forecast.append(
                    self.data[-1].get(forecast_key, 0))
            else:
                self.total_future_forecast.append(
                    self.data[-1].get(forecast_key, 0))
