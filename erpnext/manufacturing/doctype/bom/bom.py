...
class BOM(WebsiteGenerator):
    ...
    def autoname(self):
        names = frappe.db.sql_list(
            """select name from `tabBOM` where item=%s""", self.item)
        ...
        if names:
            ...
            names = [d[-1][1:]
                     for d in filter(lambda x: len(x) > 1 and x[-1], names)]
        ...
        if arg.get('scrap_items'):
            ...
            if not frappe.db.get_value('Item', arg["item_code"], 'is_customer_provided_item') and not arg.get('sourced_by_supplier'):
                if arg.get('bom_no') and self.set_rate_of_sub_assembly_item_based_on_bom:
                    rate = flt(self.get_bom_unitcost(
                        arg['bom_no'])) * (arg.get("conversion_factor") or 1)
                else:
    ...
    def validate_main_item(self):
        ...
        if not item:
            frappe.throw(
                _("Item {0} does not exist in the system or has expired").format(self.item))
        else:
            ret = frappe.db.get_value(
                "Item", self.item, ["description", "stock_uom", "item_name"])
    ...
    def update_stock_qty(self):
        for m in self.get('items'):
            if not m.conversion_factor:
                m.conversion_factor = flt(get_conversion_factor(
                    m.item_code, m.uom)['conversion_factor'])
            if m.uom and m.qty:
    ...
    def validate_materials(self):
        ...
        for m in self.get('items'):
            ...
            if flt(m.qty) <= 0:
                frappe.throw(_("Quantity required for Item {0} in row {1}").format(
                    m.item_code, m.idx))
            check_list.append(m)
    ...
    def check_recursion(self, bom_list=[]):
        ...
        if raise_exception:
            frappe.throw(_("BOM recursion: {0} cannot be parent or child of {1}").format(
                self.name, self.name))
    ...
    def calculate_cost(self):
        ...
        self.total_cost = self.operating_cost + \
            self.raw_material_cost - self.scrap_material_cost
        self.base_total_cost = self.base_operating_cost + \
            self.base_raw_material_cost - self.base_scrap_material_cost
    ...
    def calculate_op_cost(self):
        ...
        for d in self.get('operations'):
            if d.workstation:
                if not d.hour_rate:
                    hour_rate = flt(frappe.db.get_value(
                        "Workstation", d.workstation, "hour_rate"))
                    d.hour_rate = hour_rate / \
                        flt(self.conversion_rate) if self.conversion_rate else hour_rate

            if d.hour_rate and d.time_in_mins:
                d.base_hour_rate = flt(d.hour_rate) * flt(self.conversion_rate)
                d.operating_cost = flt(d.hour_rate) * \
                    flt(d.time_in_mins) / 60.0
                d.base_operating_cost = flt(
                    d.operating_cost) * flt(self.conversion_rate)
    ...
    def calculate_rm_cost(self):
        ...
        for d in self.get('items'):
            d.base_rate = flt(d.rate) * flt(self.conversion_rate)
            d.amount = flt(d.rate, d.precision("rate")) * \
                flt(d.qty, d.precision("qty"))
    ...
    def calculate_sm_cost(self):
        ...
        for d in self.get('scrap_items'):
            d.base_rate = flt(d.rate, d.precision(
                "rate")) * flt(self.conversion_rate, self.precision("conversion_rate"))
            d.amount = flt(d.rate, d.precision("rate")) * \
                flt(d.stock_qty, d.precision("stock_qty"))
            d.base_amount = flt(d.amount, d.precision(
                "amount")) * flt(self.conversion_rate, self.precision("conversion_rate"))
    ...
    def add_exploded_items(self, save=True):
        ...
        if save:
            frappe.db.sql(
                """delete from `tabBOM Explosion Item` where parent=%s""", self.name)
    ...
    def validate_bom_links(self):
        if not self.is_active:
            ...
            if act_pbom and act_pbom[0][0]:
                frappe.throw(
                    _("Cannot deactivate or cancel BOM as it is linked with other BOMs"))
        ...
        if self.with_operations:
            for d in self.operations:
                if not d.description:
                    d.description = frappe.db.get_value(
                        'Operation', d.operation, 'description')
...
def get_bom_item_rate(args, bom_doc):
    if bom_doc.rm_cost_as_per == 'Valuation Rate':
        ...
        bom_args = frappe._dict({
            ...
            # Passed conversion rate as 1 purposefully, as conversion rate is applied at the end of the function
            "conversion_rate": 1,
...
def get_valuation_rate(args):
    ...
    if not valuation_rate:
        valuation_rate = frappe.db.get_value(
            "Item", args['item_code'], "valuation_rate")
...
def get_bom_items(bom, company, qty=1, fetch_exploded=1):
    items = get_bom_items_as_dict(
        bom, company, qty, fetch_exploded, include_non_stock_items=True).values()
    items = list(items)
...
def validate_bom_no(item, bom_no):
    ...
    if item:
        ...
        if not rm_item_exists:
            frappe.throw(
                _("BOM {0} does not belong to Item {1}").format(bom_no, item))
...
def add_additional_cost(stock_entry, work_order):
    ...
    add_non_stock_items_cost(stock_entry, work_order,
                             expenses_included_in_valuation)
    add_operations_cost(stock_entry, work_order,
                        expenses_included_in_valuation)


def add_non_stock_items_cost(stock_entry, work_order, expense_account):
    bom = frappe.get_doc('BOM', work_order.bom_no)
    table = 'exploded_items' if work_order.get(
        'use_multi_level_bom') else 'items'
    ...
    for name in non_stock_items:
        non_stock_items_cost += flt(items.get(name[0])) * flt(
            stock_entry.fg_completed_qty) / flt(bom.quantity)
...
def add_operations_cost(stock_entry, work_order=None, expense_account=None):
    ...
    operating_cost_per_unit = get_operating_cost_per_unit(
        work_order, stock_entry.bom_no)
        ...
        if df.fieldtype in table_fields:
            ...
            for i, d in enumerate(new_value):
                if d.get(identifier) in old_row_by_identifier:
                    diff = get_diff(
                        old_row_by_identifier[d.get(identifier)], d, for_child=True)
...
def item_query(doctype, txt, searchfield, start, page_len, filters):
    ...
    if filters and filters.get("item_code"):
        has_variants = frappe.get_cached_value(
            "Item", filters.get("item_code"), "has_variants")
        if not has_variants:
