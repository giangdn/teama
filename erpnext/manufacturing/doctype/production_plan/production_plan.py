...
class ProductionPlan(Document):
    ...
    def validate_data(self):
        for d in self.get('po_items'):
            if not d.bom_no:
                frappe.throw(
                    _("Please select BOM for Item in Row {0}").format(d.idx))
            else:
                validate_bom_no(d.item_code, d.bom_no)

            if not flt(d.planned_qty):
                frappe.throw(_("Please enter Planned Qty for Item {0} at row {1}").format(
                    d.item_code, d.idx))
    ...
    def get_so_items(self):
        ...
        if not self.get("sales_orders") or not self.get_so_mr_list("sales_order", "sales_orders"):
            frappe.throw(_("Please fill the Sales Orders table"),
    ...
    def get_mr_items(self):
        ...
        if not self.get("material_requests") or not self.get_so_mr_list("material_request", "material_requests"):
            frappe.throw(_("Please fill the Material Requests table"),
        ...
        item_condition = ""
        if self.item_code:
            item_condition = " and mr_item.item_code ={0}".format(
                frappe.db.escape(self.item_code))
    ...
    def get_production_items(self):
        ...
        for d in self.po_items:
            ...
            if self.get_items_from == "Material Request":
                ...
                item_dict[(d.item_code, d.material_request_item,
                           d.warehouse)] = item_details
            else:
                item_details.update({
...
def get_bin_details(row, company, for_warehouse=None, all_warehouse=False):
    ...
    if not all_warehouse:
        warehouse = for_warehouse or row.get(
            'source_warehouse') or row.get('default_warehouse')
...
def get_items_for_material_requests(doc, warehouses=None):
    ...
    if not ignore_existing_ordered_qty and warehouses:
        ...
        for item in mr_items:
            get_materials_from_other_locations(
                item, warehouses, new_mr_items, company)
    ...
    if not mr_items:
        ...
        message = _("As there are sufficient raw materials, Material Request is not required for Warehouse {0}.").format(
            warehouse) + "<br><br>"
...
def get_materials_from_other_locations(item, warehouses, new_mr_items, company):
    ...
    for d in locations:
        ...
        quantity = required_qty if d.get(
            "qty") > required_qty else d.get("qty")
    ...
    for d in data:
        if d.expandable:
            ...
            bom_item["stock_qty"] += (d.stock_qty /
                                      d.parent_bom_qty) * flt(to_produce_qty)

            get_sub_assembly_items(bom_item.get(
                "bom_no"), bom_data, bom_item["stock_qty"])
