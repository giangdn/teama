...
frappe.ui.form.on("Production Plan", {
	setup: function(frm) {
		...
		frm.fields_dict["po_items"].grid.get_field(
			"warehouse"
		...
		frm.fields_dict["mr_items"].grid.get_field(
			"warehouse"
		).get_query = function(doc) {
			return {
				filters: {
...
frappe.ui.form.on("Production Plan Item", {
	item_code: function(frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.call({
				method:
					"erpnext.manufacturing.doctype.production_plan.production_plan.get_item_data",
				args: {
...
frappe.ui.form.on("Material Request Plan Item", {
	warehouse: function(frm, cdt, cdn) {
		...
		if (row.warehouse && row.item_code && frm.doc.company) {
			frappe.call({
				method:
					"erpnext.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row,
					company: frm.doc.company,
...
cur_frm.fields_dict["sales_orders"].grid.get_field(
	"sales_order"
).get_query = function() {
	return {
