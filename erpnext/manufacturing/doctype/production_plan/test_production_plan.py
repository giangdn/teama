...
class TestProductionPlan(unittest.TestCase):
    ...
    def test_production_plan_for_existing_ordered_qty(self):
        ...
        pln = create_production_plan(
            item_code='Test Production Item 1', ignore_existing_ordered_qty=0)
    ...
    def test_production_plan_with_non_stock_item(self):
        pln = create_production_plan(
            item_code='Test Production Item 1', include_non_stock_items=0)
        self.assertTrue(len(pln.mr_items), 3)
        pln.cancel()

    def test_production_plan_without_multi_level(self):
        pln = create_production_plan(
            item_code='Test Production Item 1', use_multi_level_bom=0)
        self.assertTrue(len(pln.mr_items), 2)
        pln.cancel()
    ...
    def test_production_plan_sales_orders(self):
        ...
        so_wo_qty = frappe.db.get_value(
            'Sales Order Item', sales_order_item, 'work_order_qty')
        ...
        sales_orders = [d.get('name')
                        for d in sales_orders if d.get('name') == sales_order]
    ...
    def test_pp_to_mr_customer_provided(self):
        ...
        material_request = frappe.db.get_value('Material Request Item', {
                                               'production_plan': production_plan.name, 'item_code': 'CUST-0987'}, 'parent')
    ...
    def test_production_plan_with_multi_level_bom(self):
        ...
        if not frappe.db.get_value('BOM', {'item': "Test BOM 3"}):
            make_bom(item="Test BOM 3", raw_materials=[
                     "Test RM BOM 1"], rm_qty=3)
