...
class WorkOrder(Document):
    ...
    def set_operation_start_end_time(self, idx, row):
        ...
        row.planned_end_time = get_datetime(
            row.planned_start_time) + relativedelta(minutes=row.time_in_mins)

        if row.planned_start_time == row.planned_end_time:
            frappe.throw(
                _("Capacity Planning Error, planned start time can not be same as end time"))

    def validate_cancel(self):
        if self.status == "Stopped":
            frappe.throw(
                _("Stopped Work Order cannot be cancelled, Unstop it first to cancel"))
    ...
    def set_lead_time(self):
        if self.actual_start_date and self.actual_end_date:
            self.lead_time = flt(time_diff_in_hours(
                self.actual_end_date, self.actual_start_date) * 60)
    ...
    def validate_production_item(self):
        if frappe.db.get_value("Item", self.production_item, "has_variants"):
            frappe.throw(
                _("Work Order cannot be raised against a Item Template"), ItemHasVariantError)
    ...
    def update_consumed_qty_for_required_items(self):
        ...
        for item in self.required_items:
            ...
            item.db_set('consumed_qty', flt(
                consumed_qty), update_modified=False)
...
def make_stock_entry(work_order_id, purpose, qty=None):
    ...
    stock_entry.fg_completed_qty = qty or (
        flt(work_order.qty) - flt(work_order.produced_qty))
...
def create_job_card(work_order, row, qty=0, enable_capacity_planning=False, auto_create=False):
    ...
    if auto_create:
        ...
        doc.insert()
        frappe.msgprint(_("Job card {0} created").format(
            get_link_to_form("Job Card", doc.name)))
...
def create_pick_list(source_name, target_doc=None, for_qty=None):
    ...
    max_finished_goods_qty = frappe.db.get_value(
        'Work Order', source_name, 'qty')

    def update_item_quantity(source, target, source_parent):
        pending_to_issue = flt(source.required_qty) - \
            flt(source.transferred_qty)
        desire_to_transfer = flt(source.required_qty) / \
            max_finished_goods_qty * flt(for_qty)
        ...
        if qty:
            ...
            target.uom = frappe.get_value(
                'Item', source.item_code, 'stock_uom')
