...
class TestWorkOrder(unittest.TestCase):
    ...
    def check_planned_qty(self):
        ...
        # from stores to wip
        s = frappe.get_doc(make_stock_entry(
            wo_order.name, "Material Transfer for Manufacture", 4))
        ...
        self.assertEqual(frappe.db.get_value(
            "Work Order", wo_order.name, "produced_qty"), 4)
    ...
    def test_production_item(self):
        wo_order = make_wo_order_test_record(
            item="_Test FG Item", qty=1, do_not_save=True)
        ...
        wo_order = make_wo_order_test_record(
            item="_Test Variant Item", qty=1, do_not_save=True)
    ...
    def test_reserved_qty_for_production_on_stock_entry(self):
        ...
        s = frappe.get_doc(make_stock_entry(
            self.wo_order.name, "Manufacture", 2))
    ...
    def test_backflush_qty_for_overpduction_manufacture(self):
        ...
        s = frappe.get_doc(make_stock_entry(
            wo_order.name, "Material Transfer for Manufacture", 60))
        s.submit()
    ...
    def test_scrap_material_qty(self):
        ...
        s = frappe.get_doc(make_stock_entry(
            wo_order.name, "Material Transfer for Manufacture", 2))
        for d in s.get("items"):
            d.s_warehouse = "Stores - _TC"
        s.insert()
        s.submit()
        ...
        for item in s.items:
            if item.bom_no and item.item_code in scrap_item_details:
                self.assertEqual(
                    wo_order_details.scrap_warehouse, item.t_warehouse)
                self.assertEqual(flt(wo_order_details.qty) *
    ...
    def test_allow_overproduction(self):
        ...
        s = frappe.get_doc(make_stock_entry(
            wo_order.name, "Material Transfer for Manufacture", 3))
        s.insert()
        self.assertRaises(StockOverProductionError, s.submit)
    ...
    def test_work_order_with_non_stock_item(self):
        items = {'Finished Good Test Item For non stock': 1,
                 '_Test FG Item': 1, '_Test FG Non Stock Item': 0}
        ...
        if not frappe.db.get_value('BOM', {'item': fg_item}):
            make_bom(item=fg_item, rate=1000, raw_materials=[
                     '_Test FG Item', '_Test FG Non Stock Item'])

        wo = make_wo_order_test_record(production_item=fg_item)

        se = frappe.get_doc(make_stock_entry(
            wo.name, "Material Transfer for Manufacture", 1))
        se.insert()
        se.submit()
    ...
    def test_job_card(self):
        ...
        ste = frappe.get_doc(make_stock_entry(
            work_order.name, "Material Transfer for Manufacture", 1))
        ste.submit()
        stock_entries.append(ste)

        job_cards = frappe.get_all(
            'Job Card', filters={'work_order': work_order.name})
        ...
        ste1 = frappe.get_doc(make_stock_entry(
    ...
    def test_work_order_with_non_transfer_item(self):
        items = {'Finished Good Transfer Item': 1,
                 '_Test FG Item': 1, '_Test FG Item 1': 0}
        ...
        if not frappe.db.get_value('BOM', {'item': fg_item}):
            make_bom(item=fg_item, raw_materials=[
                     '_Test FG Item', '_Test FG Item 1'])
    ...
    def test_cost_center_for_manufacture(self):
        ...
        ste = make_stock_entry(
            wo_order.name, "Material Transfer for Manufacture", wo_order.qty)
        self.assertEquals(ste.get("items")[0].get(
    ...
    def test_operation_time_with_batch_size(self):
        ...
        if not bom_name:
            bom = make_bom(item=fg_item, rate=1000,
                           raw_materials=[rm1], do_not_save=True)
    ...
    def test_partial_material_consumption(self):
        ...
        ste1 = frappe.get_doc(make_stock_entry(
            wo_order.name, "Manufacture", 2))
        ste1.submit()
        ste_cancel_list.append(ste1)

        ste3 = frappe.get_doc(make_stock_entry(
            wo_order.name, "Material Consumption for Manufacture", 2))
    ...
    def test_extra_material_transfer(self):
        ...
        ste2 = frappe.get_doc(make_stock_entry(
            wo_order.name, "Manufacture", 2))
