...
frappe.ui.form.on("Work Order", {
	setup: function(frm) {
		...
		frm.set_query("operation", "required_items", function() {
			return {
				query:
					"erpnext.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
	...
	refresh: function(frm) {
		...
		if (frm.doc.required_items && frm.doc.allow_alternative_item) {
			const has_alternative = frm.doc.required_items.find(
	...
	make_job_card: function(frm) {
		...
		dialog.fields_dict["operations"].grid.wrapper
			.find(".grid-add-row")
			.hide();
		...
		if (frm.doc.operations && frm.doc.operations.length) {
			...
			frm.doc.operations.forEach(d => {
				if (!status_wise_oprtation_data[d.status]) {
					status_wise_oprtation_data[d.status] = [
						d.completed_qty,
						d.operation
					];
				} else {
					status_wise_oprtation_data[d.status][0] += d.completed_qty;
					status_wise_oprtation_data[d.status][1] +=
						", " + d.operation;
				}
			});

			for (let key in status_wise_oprtation_data) {
				title = __("{0} Operations: {1}", [
					key,
					status_wise_oprtation_data[key][1].bold()
				]);
				bars.push({
					title: title,
					width:
						(status_wise_oprtation_data[key][0] /
							total_completed_qty) *
							100 +
						"%",
	...
	before_submit: function(frm) {
		frm.fields_dict.required_items.grid.toggle_reqd(
			"source_warehouse",
			true
		);
		frm.toggle_reqd(
		...
		frm.fields_dict.operations.grid.toggle_reqd(
			"workstation",
			frm.doc.operations
		);
	},
	...
	set_sales_order: function(frm) {
		if (frm.doc.production_item) {
			frappe.call({
				method:
					"erpnext.manufacturing.doctype.work_order.work_order.query_sales_order",
				...
				callback: function(r) {
					if (r.message) {
						frappe.model.set_value(cdt, cdn, {
							...
							allow_alternative_item:
								r.message.allow_alternative_item,
...
erpnext.work_order = {
		...
		if (doc.docstatus === 1) {
			...
			if (show_start_btn) {
				if (
					...
					var start_btn = frm.add_custom_button(
						__("Start"),
						function() {
			...
			if (!frm.doc.skip_transfer) {
				...
				if (
					...
					if (
						frm.doc.__onload &&
						frm.doc.__onload.material_consumption == 1
					) {
						...
						for (var i = 0, len = tbl_lenght; i < len; i++) {
							...
							if (
								flt(wo_item_qty) >
								flt(frm.doc.required_items[i].consumed_qty)
							) {
								counter += 1;
							}
						}
					...
					var finish_btn = frm.add_custom_button(
						__("Finish"),
						function() {
	...
	calculate_cost: function(doc) {
		if (doc.operations) {
			...
			for (var i = 0; i < op.length; i++) {
				var planned_operating_cost = flt(
	...
	calculate_total_cost: function(frm) {
		let variable_cost =
			flt(frm.doc.actual_operating_cost) ||
			flt(frm.doc.planned_operating_cost);
		frm.set_value(
			"total_operating_cost",
			flt(frm.doc.additional_operating_cost) + variable_cost
		);
	},
