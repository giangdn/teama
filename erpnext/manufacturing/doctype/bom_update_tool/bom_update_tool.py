...
class BOMUpdateTool(Document):
    def replace_bom(self):
        ...
        for bom in bom_list:
            try:
                ...
                bom_obj.update_new_bom(
                    self.current_bom, self.new_bom, unit_cost)
    ...
    def get_parent_boms(self, bom, bom_list=[]):
        ...
        for d in data:
            if self.new_bom == d[0]:
                frappe.throw(
                    _("BOM recursion: {0} cannot be child of {1}").format(bom, self.new_bom))
...
def enqueue_replace_bom(args):
    ...
    frappe.enqueue(
        "erpnext.manufacturing.doctype.bom_update_tool.bom_update_tool.replace_bom", args=args, timeout=40000)
    frappe.msgprint(
        _("Queued for replacing the BOM. It may take a few minutes."))
...
def update_cost():
    ...
    for bom in bom_list:
        frappe.get_doc("BOM", bom).update_cost(
            update_parent=False, from_child_bom=True)
