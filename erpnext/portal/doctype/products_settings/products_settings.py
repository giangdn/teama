...
class ProductsSettings(Document):
    def validate(self):
        if self.home_page_is_products:
            frappe.db.set_value("Website Settings", None,
                                "home_page", "products")
    ...
    def validate_field_filters(self):
        ...
        valid_fields = [df.fieldname for df in item_meta.fields if df.fieldtype in [
            'Link', 'Table MultiSelect']]

        for f in self.filter_fields:
            if f.fieldname not in valid_fields:
                frappe.throw(
                    _('Filter Fields Row #{0}: Fieldname <b>{1}</b> must be of type "Link" or "Table MultiSelect"').format(f.idx, f.fieldname))
...
def home_page_is_products(doc, method):
    ...
    home_page_is_products = cint(frappe.db.get_single_value(
        'Products Settings', 'home_page_is_products'))
