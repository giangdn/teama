...
class Analytics(object):
    ...
    def get_period(self, appointment_date):
        if self.filters.range == 'Weekly':
            ...
            year = get_fiscal_year(
                appointment_date, company=self.filters.company)
            period = str(year[0])
    ...
    def get_appointments_based_on_medical_department(self):
        ...
        self.entries = frappe.db.get_all('Patient Appointment',
                                         fields=[
                                             'appointment_date', 'name', 'patient', 'practitioner', 'department'],
                                         filters=filters
                                         )
    ...
    def get_periodic_data(self):
        ...
        for d in self.entries:
            ...
            if self.filters.tree_type == 'Healthcare Practitioner':
                self.appointment_periodic_data.setdefault(
                    d.practitioner, frappe._dict()).setdefault(period, 0.0)
                self.appointment_periodic_data[d.practitioner][period] += 1

            elif self.filters.tree_type == 'Medical Department':
                self.appointment_periodic_data.setdefault(
                    d.department, frappe._dict()).setdefault(period, 0.0)
