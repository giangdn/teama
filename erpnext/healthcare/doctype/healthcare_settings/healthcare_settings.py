...
def send_registration_sms(doc):
    if frappe.db.get_single_value('Healthcare Settings', 'send_registration_msg'):
        if doc.mobile:
            ...
            messages = frappe.db.get_single_value(
                'Healthcare Settings', 'registration_msg')
            messages = frappe.render_template(messages, context)
            number = [doc.mobile]
            send_sms(number, messages)
        else:
            frappe.msgprint(
                doc.name + ' has no mobile number to send registration SMS', alert=True)


def get_receivable_account(company):
    receivable_account = get_account(
        None, 'receivable_account', 'Healthcare Settings', company)
...
def get_income_account(practitioner, company):
    ...
    income_account = get_account(
        None, 'income_account', 'Healthcare Settings', company)
    if income_account:
        return income_account
