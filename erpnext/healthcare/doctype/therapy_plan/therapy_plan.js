...
frappe.ui.form.on("Therapy Plan", {
	...
	refresh: function(frm) {
		if (!frm.doc.__islocal) {
			...
			if (frm.doc.status != "Completed") {
				let therapy_types = (frm.doc.therapy_plan_details || []).map(
					function(d) {
					...
					function() {
						frappe.prompt(
							...
							data => {
								frappe.call({
									method:
										"erpnext.healthcare.doctype.therapy_plan.therapy_plan.make_therapy_session",
									args: {
	...
	make_sales_invoice: function(frm) {
		frappe.call({
			...
			method:
				"erpnext.healthcare.doctype.therapy_plan.therapy_plan.make_sales_invoice",
			callback: function(r) {
	...
	show_progress_for_therapies: function(frm) {
		...
		// completed sessions
		let title = __("{0} sessions completed", [
			frm.doc.total_sessions_completed
		]);
		if (frm.doc.total_sessions_completed === 1) {
			title = __("{0} session completed", [
				frm.doc.total_sessions_completed
			]);
		}
		...
		bars.push({
			title: title,
			width:
				(frm.doc.total_sessions_completed / frm.doc.total_sessions) *
					100 +
				"%",
