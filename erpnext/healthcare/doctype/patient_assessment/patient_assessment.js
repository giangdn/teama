...
frappe.ui.form.on("Patient Assessment", {
			...
			frappe.call({
				...
				callback: function(data) {
					...
					frm.set_value(
						...
						data.message.assessment_description
					);
					frm.set_value(
						"total_score",
						data.message.scale_max * data.message.parameters.length
					);
					frm.trigger("set_score_range");
	...
	show_patient_progress: function(frm) {
		...
		let title = __("{0} out of {1}", [
			frm.doc.total_score_obtained,
			frm.doc.total_score
		]);
		...
		bars.push({
			title: title,
			width:
				(frm.doc.total_score_obtained / frm.doc.total_score) * 100 +
				"%",
