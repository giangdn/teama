...
erpnext.ExerciseEditor = Class.extend({
	...
	make: function(frm, wrapper) {
		...
		this.exercise_toolbar = $(
			'<p>\
		<button class="btn btn-default btn-add btn-xs" style="margin-left: 10px;"></button>'
		).appendTo(this.wrapper);

		this.exercise_cards = $('<div class="exercise-cards"></div>').appendTo(
			this.wrapper
		);
	...
	show_add_card_dialog: function(frm) {
		...
		let d = new frappe.ui.Dialog({
			...
			primary_action: function() {
				...
				let step = frappe.model.add_child(
					frm.doc,
					"Exercise Type Step",
					"steps_table"
				);
