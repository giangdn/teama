...
class InpatientMedicationEntry(Document):
    ...
    def get_medication_orders(self):
        ...
        if orders:
            ...
            frappe.msgprint(
                _('No pending medication orders found for selected criteria'))
    ...
    def update_medication_orders(self, on_cancel=False):
        ...
        for order, count in order_entry_map.items():
            medication_order = frappe.get_doc(
                'Inpatient Medication Order', order)
            completed_orders = flt(count)
            current_value = frappe.db.get_value(
                'Inpatient Medication Order', order, 'completed_orders')
    ...
    def check_stock_qty(self):
        drug_shortage = get_drug_shortage_map(
            self.medication_orders, self.warehouse)
        ...
        if drug_shortage:
            ...
            frappe.throw(message, title=_('Insufficient Stock'),
    ...
    def make_stock_entry(self):
        ...
        for entry in self.medication_orders:
            ...
            se_child.uom = frappe.db.get_value(
                'Item', entry.drug_code, 'stock_uom')
    ...
    def cancel_stock_entries(self):
        stock_entries = frappe.get_all(
            'Stock Entry', {'inpatient_medication_entry': self.name})
...
def get_pending_medication_orders(entry):
    ...
    for doc in data:
        ...
        if inpatient_record:
            doc['service_unit'] = get_current_healthcare_service_unit(
                inpatient_record)
