...
frappe.ui.form.on("Practitioner Schedule", {
	refresh: function(frm) {
		cur_frm.fields_dict["time_slots"].grid.wrapper
			.find(".grid-add-row")
			.hide();
		cur_frm.fields_dict["time_slots"].grid.add_custom_button(
			__("Add Time Slots"),
			() => {
				let d = new frappe.ui.Dialog({
					fields: [
						...
						{
							fieldname: "duration",
							label: __("Appointment Duration (mins)"),
							fieldtype: "Int",
							default: 15,
							reqd: 1
						}
					...
					primary_action: () => {
						...
						if (values) {
							...
							function check_overlap_or_add_slot(
								week_day,
								cur_time,
								end_time,
								add_slots_to_child
								...
								while (cur_time < end_time) {
									...
									let to_time = cur_time
										.clone()
										.add(values.duration, "minutes");
									if (to_time <= end_time) {
										if (frm.doc.time_slots) {
											frm.doc.time_slots.forEach(function(
												slot
											) {
												if (slot.day == week_day) {
													let slot_from_moment = moment(
														slot.from_time,
														"HH:mm:ss"
													);
													let slot_to_moment = moment(
														slot.to_time,
														"HH:mm:ss"
													);
													if (
										...
										if (
											...
											frm.add_child("time_slots", {
												from_time: cur_time.format(
													"HH:mm:ss"
												),
												to_time: to_time.format(
													"HH:mm:ss"
												),
												day: week_day
											});
							...
							function add_slots(week_day) {
								...
								if (
									...
									frappe.confirm(
										__(
											"Schedules for {0} overlaps, do you want to proceed after skiping overlaped slots ?",
											[week_day]
										),
										function() {
											// if Yes
