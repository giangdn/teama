...
frappe.ui.form.on("Therapy Session", {
			...
			frm.add_custom_button(
				...
				function() {
					frappe.model.open_mapped_doc({
						method:
							"erpnext.healthcare.doctype.patient_assessment.patient_assessment.create_patient_assessment",
						frm: frm
					});
				},
				"Create"
					...
					if (r && !r.therapy_plan_template) {
						frm.add_custom_button(
							...
							function() {
								frappe.model.open_mapped_doc({
									method:
										"erpnext.healthcare.doctype.therapy_session.therapy_session.invoice_therapy_session",
									frm: frm
								});
							},
							"Create"
	...
	filter_therapy_types: function(frm) {
		frappe.call({
			...
			callback: function(data) {
				let therapy_types = (
					data.message.therapy_plan_details || []
				).map(function(d) {
	...
	patient: function(frm) {
		if (frm.doc.patient) {
			frappe.call({
				method:
					"erpnext.healthcare.doctype.patient.patient.get_patient_detail",
				args: {
		...
		if (frm.doc.therapy_type) {
			frappe.call({
				...
				callback: function(data) {
					...
					frm.set_value(
						"service_unit",
						data.message.healthcare_service_unit
					);
					frm.set_value(
						"department",
