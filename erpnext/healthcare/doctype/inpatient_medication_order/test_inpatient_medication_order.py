...
class TestInpatientMedicationOrder(unittest.TestCase):
    ...
    def test_order_creation(self):
        ...
        self.assertEqual(
            ipmo.medication_orders[0].date, add_days(getdate(), -1))
    ...
    def test_status(self):
        ...
        filters = frappe._dict(from_date=add_days(
            getdate(), -1), to_date=add_days(getdate(), -1), from_time='', to_time='')
        ...
        filters = frappe._dict(from_date=getdate(),
                               to_date=getdate(), from_time='', to_time='')
        ipme = create_ipme(filters)
        ipme.submit()
        ipmo.reload()
