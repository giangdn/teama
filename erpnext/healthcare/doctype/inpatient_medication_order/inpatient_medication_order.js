...
frappe.ui.form.on("Inpatient Medication Order", {
	...
	show_medication_order_button: function(frm) {
		frm.fields_dict["medication_orders"].grid.wrapper
			.find(".grid-add-row")
			.hide();
	...
	show_progress: function(frm) {
		...
		// completed sessions
		let title = __("{0} medication orders completed", [
			frm.doc.completed_orders
		]);
		if (frm.doc.completed_orders === 1) {
			title = __("{0} medication order completed", [
				frm.doc.completed_orders
			]);
		}
		...
		bars.push({
			title: title,
			width:
				(frm.doc.completed_orders / frm.doc.total_orders) * 100 + "%",
