...
class ImportSupplierInvoice(Document):
    ...
    def import_xml_data(self):
        ...
        self.default_uom = frappe.db.get_value(
            "Stock Settings", fieldname="stock_uom")
    ...
    def prepare_data_for_import(self, file_content, file_name, encoded_content):
        for line in file_content.find_all("DatiGeneraliDocumento"):
            ...
            pi_name = create_purchase_invoice(
                supplier_name, file_name, invoices_args, self.name)
    ...
    def process_file_data(self):
        ...
        frappe.enqueue_doc(self.doctype, self.name,
                           "import_xml_data", queue="long", timeout=3600)

    def publish(self, title, message, count, total):
        frappe.publish_realtime("import_invoice_update", {
...
def get_file_content(file_name, zip_file_object):
    ...
    try:
        ...
        try:
            ...
            frappe.log_error(
                message=e, title="UTF-16 encoding error for File Name: " + file_name)
...
def get_supplier_details(file_content):
    ...
    for line in file_content.find_all("CedentePrestatore"):
        supplier_info['tax_id'] = line.DatiAnagrafici.IdPaese.text + \
            line.DatiAnagrafici.IdCodice.text
...
def get_payment_terms_from_file(file_content):
    ...
    for line in file_content.find_all("DettaglioPagamento"):
        mop_code = line.ModalitaPagamento.text + '-' + \
            mop_dict.get(line.ModalitaPagamento.text)
        if line.find("DataScadenzaPagamento"):
            due_date = dateutil.parser.parse(
                line.DataScadenzaPagamento.text).strftime("%Y-%m-%d")
        else:
            due_date = today()
        terms.append({
...
def get_country(code):
    ...
    if existing_country_name:
        ...
        frappe.throw(
            _("Country Code in File does not match with country code set up in the system"))
