...
def send_reminder():
    ...
    last_sent = frappe.db.get_single_value(
        'GST Settings', 'gstin_email_sent_on')
...
def send_gstin_reminder_to_all_parties():
    ...
    for party in parties:
        ...
        try:
            email_id = _send_gstin_reminder(
                party[0], party[1], party[2], sent_to)
...
def send_gstin_reminder(party_type, party):
    ...
    if email:
        frappe.msgprint(_('Reminder to update GSTIN Sent'),
                        title='Reminder sent', indicator='green')
