...
def append_vat_on_sales(data, filters):
    ...
    append_data(data, '2',
                ...
                frappe.format(
                    (-1) * get_tourist_tax_return_total(filters), 'Currency'),
...
def standard_rated_expenses_emiratewise(data, filters):
    ...
    amounts_by_emirate = append_emiratewise_expenses(
        data, emirates, amounts_by_emirate)
    return emirates, amounts_by_emirate
...
def append_vat_on_expenses(data, filters):
    ...
    append_data(data, '9', _('Standard Rated Expenses'),
                frappe.format(get_standard_rated_expenses_total(
                    filters), 'Currency'),
    ...
    append_data(data, '10', _('Supplies subject to the reverse charge provision'),
                frappe.format(get_reverse_charge_recoverable_total(
                    filters), 'Currency'),
...
def append_data(data, no, legend, amount, vat_amount):
    ...
    data.append({"no": no, "legend": legend,
                 "amount": amount, "vat_amount": vat_amount})
...
def get_standard_rated_expenses_tax(filters):
    ...
    try:
        return frappe.db.get_all('Purchase Invoice',
                                 filters=query_filters,
                                 fields=[
                                     'sum(recoverable_standard_rated_expenses)'],
                                 as_list=True,
                                 limit=1
                                 )[0][0] or 0
    except (IndexError, TypeError):
