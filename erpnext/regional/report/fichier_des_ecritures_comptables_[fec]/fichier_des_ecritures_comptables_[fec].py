...
def get_result_as_list(data, filters):
    ...
    accounts = frappe.get_all("Account", filters={
                              "Company": filters.company}, fields=["name", "account_number"])
    ...
    for d in data:
        ...
        if d.get("voucher_no").startswith("{0}-".format(JournalCode)) or d.get("voucher_no").startswith("{0}/".format(JournalCode)):
            ...
            EcritureNum = re.search(
                "{0}(\d+)".format(JournalCode), d.get("voucher_no"), re.IGNORECASE).group(1)
        ...
        if account_number[0] is not None:
            ...
            frappe.throw(_("Account number for account {0} is not available.<br> Please setup your Chart of Accounts correctly.").format(
                d.get("account")))
        ...
        if Idevise != company_currency:
            Montantdevise = '{:.2f}'.format(d.get("debitCurr")).replace(".", ",") if d.get(
                "debitCurr") != 0 else '{:.2f}'.format(d.get("creditCurr")).replace(".", ",")
        else:
            Montantdevise = '{:.2f}'.format(d.get("debit")).replace(".", ",") if d.get(
                "debit") != 0 else '{:.2f}'.format(d.get("credit")).replace(".", ",")
