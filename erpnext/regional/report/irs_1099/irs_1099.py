...
def execute(filters=None):
    filters = filters if isinstance(
        filters, frappe._dict) else frappe._dict(filters)
    if not filters:
    ...
    if filters.supplier_group:
        conditions += "AND s.supplier_group = %s" % frappe.db.escape(
...
def irs_1099_print(filters):
    ...
    fiscal_year_doc = get_fiscal_year(
        fiscal_year=filters.fiscal_year, as_dict=True)
    ...
    for row in data:
        ...
        row["payments"] = fmt_money(
            row["payments"], precision=0, currency="USD")
...
def get_payer_address_html(company):
    ...
    if address_list:
        ...
        address_display = frappe.get_doc(
            "Address", company_address).get_display()
