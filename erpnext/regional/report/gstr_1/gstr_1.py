...
class Gstr1Report(object):
    ...
    def run(self):
        ...
        if self.invoices:
            ...
            self.invoice_fields = [d["fieldname"]
                                   for d in self.invoice_columns]
            self.get_data()
            ...
            for inv, items_based_on_rate in self.items_based_on_tax_rate.items():
                ...
                for rate, items in items_based_on_rate.items():
                    ...
                    if self.filters.get("type_of_business") == "CDNR":
                        row.append("Y" if invoice_details.posting_date <= date(
                            2017, 7, 1) else "N")
                        row.append(
    ...
    def get_b2c_data(self):
        ...
        for inv, items_based_on_rate in self.items_based_on_tax_rate.items():
            ...
            for rate, items in items_based_on_rate.items():
                ...
                row = b2cs_output.get(
                    (rate, place_of_supply, ecommerce_gstin, inv))
    ...
    def get_row_data_for_invoice(self, invoice, invoice_details, tax_rate, items):
        ...
        for fieldname in self.invoice_fields:
            if self.filters.get("type_of_business") == "CDNR" and fieldname == "invoice_value":
                ...
                row.append(formatdate(
                    invoice_details.get(fieldname), 'dd-MMM-YY'))
        ...
        for invoice, items in iteritems(self.invoice_items):
            if invoice not in self.items_based_on_tax_rate \
                ...
                self.items_based_on_tax_rate.setdefault(
                    invoice, {}).setdefault(0, items.keys())
...
def get_json(filters, report_name, data):
    ...
    fp = "%02d%s" % (getdate(filters["to_date"]).month, getdate(
    ...
    if filters["type_of_business"] == "B2B":
        for item in report_data[:-1]:
            res.setdefault(item["customer_gstin"], {}).setdefault(
...
def get_b2b_json(res, gstin):
    inv_type, out = {"Registered Regular": "R",
                     "Deemed Export": "DE", "URD": "URD", "SEZ": "SEZ"}, []
    for gst_in in res:
        ...
        for number, invoice in iteritems(res[gst_in]):
            ...
            inv_item["pos"] = "%02d" % int(
                invoice[0]["place_of_supply"].split('-')[0])
            inv_item["rchrg"] = invoice[0]["reverse_charge"]
            inv_item["inv_typ"] = inv_type.get(
                invoice[0].get("gst_category", ""), "")
...
def get_b2cs_json(data, gstin):
    ...
    for d in data:
        ...
        if company_state_number == pos:
            tax_details.update(
                {"camt": flt(tax/2.0, 2), "samt": flt(tax/2.0, 2)})
        else:
...
def download_json_file():
    ...
    frappe.response['filename'] = frappe.scrub("{0} {1}".format(
