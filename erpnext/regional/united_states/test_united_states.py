...
class TestUnitedStates(unittest.TestCase):
    ...
    def test_irs_1099_report(self):
        ...
        filters = frappe._dict(
            {"fiscal_year": "_Test Fiscal Year 2016", "company": "_Test Company 1"})
...
def make_payment_entry_to_irs_1099_supplier():
    ...
    frappe.db.sql(
        "delete from `tabGL Entry` where party='_US 1099 Test Supplier'")
    frappe.db.sql(
        "delete from `tabGL Entry` where against='_US 1099 Test Supplier'")
    frappe.db.sql(
        "delete from `tabPayment Entry` where party='_US 1099 Test Supplier'")
