...
def make_custom_fields(update=True):
    ...
    invoice_gst_fields = [
        ...
        dict(fieldname='gst_col_break', fieldtype='Column Break',
    ...
    si_einvoice_fields = [
        ...
        dict(fieldname='ack_no', label='Ack. No.', fieldtype='Data',
             read_only=1, hidden=1, insert_after='irn', no_copy=1, print_hide=1),

        dict(fieldname='ack_date', label='Ack. Date', fieldtype='Data',
             read_only=1, hidden=1, insert_after='ack_no', no_copy=1, print_hide=1),
        ...
        dict(fieldname='signed_einvoice', fieldtype='Code',
             options='JSON', hidden=1, no_copy=1, print_hide=1, read_only=1),

        dict(fieldname='signed_qr_code', fieldtype='Code', options='JSON',
             hidden=1, no_copy=1, print_hide=1, read_only=1),

        dict(fieldname='qrcode_image', label='QRCode', fieldtype='Attach Image',
             hidden=1, no_copy=1, print_hide=1, read_only=1)
    ]
...
def make_fixtures(company=None):
    docs = []
    company = company.name if company else frappe.db.get_value(
...
def set_tax_withholding_category(company):
    ...
    tds_account = frappe.get_value(
        "Account", 'TDS Payable - {0}'.format(abbr), 'name')
    ...
    for d in docs:
        try:
            ...
            if fiscal_year:
                ...
                fy_exist = [k for k in doc.get('rates') if k.get(
