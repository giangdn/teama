...
def validate_einvoice_fields(doc):
    einvoicing_enabled = cint(frappe.db.get_value(
        'E Invoice Settings', 'E Invoice Settings', 'enable'))
    invalid_doctype = doc.doctype != 'Sales Invoice'
    invalid_supply_type = doc.get('gst_category') not in [
        'Registered Regular', 'SEZ', 'Overseas', 'Deemed Export']
    company_transaction = doc.get(
        'billing_address_gstin') == doc.get('company_gstin')
    ...
    if doc.docstatus == 0 and doc._action == 'save':
        if doc.irn:
            frappe.throw(_('You cannot edit the invoice after generating IRN'), title=_(
                'Edit Not Allowed'))
        if len(doc.name) > 16:
        ...
        frappe.throw(
            _('You must generate IRN before submitting the document.'), title=_('Missing IRN'))

    elif doc.irn and doc.docstatus == 2 and doc._action == 'cancel' and not doc.irn_cancelled:
        frappe.throw(_('You must cancel IRN before cancelling the document.'), title=_(
            'Cancel Not Allowed'))
...
def read_json(name):
    file_path = os.path.join(os.path.dirname(
        __file__), '{name}.json'.format(name=name))
...
def get_transaction_details(invoice):
    ...
    if not supply_type:
        rr, sez, overseas, export = bold('Registered Regular'), bold(
            'SEZ'), bold('Overseas'), bold('Deemed Export')
...
def get_party_details(address_name):
    d = frappe.get_all('Address', filters={
                       'name': address_name}, fields=['*'])[0]
...
def get_overseas_address_details(address_name):
    address_title, address_line1, address_line2, city = frappe.db.get_value(
        'Address', address_name, ['address_title',
...
def get_item_list(invoice):
    ...
    for d in invoice.items:
        ...
        item.batch_expiry_date = frappe.db.get_value(
            'Batch', d.batch_no, 'expiry_date') if d.batch_no else None
        item.batch_expiry_date = format_date(
            item.batch_expiry_date, 'dd/mm/yyyy') if item.batch_expiry_date else None
        item.is_service_item = 'N' if frappe.db.get_value(
            'Item', d.item_code, 'is_stock_item') else 'Y'
        item.serial_no = ""
...
def update_item_taxes(invoice, item):
    ...
    gst_accounts_list = [d for accounts in gst_accounts.values()
                         for d in accounts if d]
    ...
    for t in invoice.taxes:
        ...
        item_tax_detail = json.loads(
            t.item_wise_tax_detail).get(item.item_code)
        if t.account_head in gst_accounts_list:
            ...
            if t.account_head in gst_accounts.cess_account:
                ...
                if t.charge_type == 'On Item Quantity':
                    item.cess_nadv_amount += abs(
                        item_tax_amount_after_discount)
                else:
...
def get_invoice_value_details(invoice):
    ...
    invoice_value_details.base_grand_total = abs(
        invoice.base_rounded_total) or abs(invoice.base_grand_total)
    invoice_value_details.grand_total = abs(
        invoice.rounded_total) or abs(invoice.grand_total)

    invoice_value_details = update_invoice_taxes(
        invoice, invoice_value_details)
    ...
    for t in invoice.taxes:
        if t.account_head in gst_accounts_list:
            if t.account_head in gst_accounts.cess_account:
                ...
                invoice_value_details.total_cess_amt += abs(
                    t.base_tax_amount_after_discount_amount)

            for tax_type in ['igst', 'cgst', 'sgst']:
                if t.account_head in gst_accounts[f'{tax_type}_account']:
                    invoice_value_details[f'total_{tax_type}_amt'] += abs(
                        t.base_tax_amount_after_discount_amount)
        else:
            invoice_value_details.total_other_charges += abs(
                t.base_tax_amount_after_discount_amount)
...
def get_return_doc_reference(invoice):
    invoice_date = frappe.db.get_value(
        'Sales Invoice', invoice.return_against, 'posting_date')
    return frappe._dict(dict(
        invoice_name=invoice.return_against, invoice_date=format_date(
...
def get_eway_bill_details(invoice):
    if invoice.is_return:
        frappe.throw(_('E-Way Bill cannot be generated for Credit Notes & Debit Notes'),
                     title=_('E Invoice Validation Failed'))

    mode_of_transport = {'': '', 'Road': '1',
                         'Air': '2', 'Rail': '3', 'Ship': '4'}
...
def validate_mandatory_fields(invoice):
    if not invoice.company_address:
        frappe.throw(_('Company Address is mandatory to fetch company GSTIN details.'), title=_(
            'Missing Fields'))
    if not invoice.customer_address:
        frappe.throw(_('Customer Address is mandatory to fetch customer GSTIN details.'), title=_(
            'Missing Fields'))
...
def make_einvoice(invoice):
    ...
    if invoice.gst_category == 'Overseas':
        ...
        place_of_supply = get_place_of_supply(
            invoice, invoice.doctype) or invoice.billing_address_gstin
...
def validate_einvoice(validations, einvoice, errors=[]):
    for fieldname, field_validation in validations.items():
        ...
        if value_type == 'string':
            ...
            precision = 3 if '.999' in str(
                field_validation.get('maximum')) else 2
            einvoice[fieldname] = flt(
                value, precision) if not is_integer else cint(value)
        ...
        if value_type == 'string' and len(value) > max_length:
            errors.append(
                _('{} should not exceed {} characters').format(label, max_length))
        if value_type == 'number' and (value > maximum or value < minimum):
            errors.append(_('{} {} should be between {} and {}').format(
...
class GSPConnector():
    def __init__(self, doctype=None, docname=None):
        ...
        self.invoice = frappe.get_cached_doc(
            doctype, docname) if doctype and docname else None
    ...
    def get_credentials(self):
        if self.invoice:
            ...
            if not self.e_invoice_settings.enable:
                frappe.throw(_("E-Invoicing is disabled. Please enable it from {} to generate e-invoices.").format(
    ...
    def get_seller_gstin(self):
        gstin = self.invoice.company_gstin or frappe.db.get_value(
            'Address', self.invoice.company_address, 'gstin')
        if not gstin:
            frappe.throw(
    ...
    def fetch_auth_token(self):
        ...
        try:
            ...
            self.e_invoice_settings.token_expiry = add_to_date(
    ...
    def fetch_gstin_details(self, gstin):
        ...
        try:
            ...
            res = self.make_request(
                'get', self.gstin_details_url + params, headers)
            if res.get('success'):
    ...
    def generate_irn(self):
        ...
        try:
            res = self.make_request(
                'post', self.generate_irn_url, headers, data)
            if res.get('success'):
    ...
    def get_irn_details(self, irn):
        ...
        try:
            ...
            res = self.make_request(
                'get', self.irn_details_url + params, headers)
            if res.get('success'):
                return res.get('result')
            else:
                raise RequestFailed
    ...
    def generate_eway_bill(self, **kwargs):
        ...
        try:
            res = self.make_request(
                'post', self.generate_ewaybill_url, headers, data)
            if res.get('success'):
    ...
    def cancel_eway_bill(self, eway_bill, reason, remark):
        ...
        try:
            res = self.make_request(
                'post', self.cancel_ewaybill_url, headers, data)
            if res.get('success'):
