...
def get_conditions(filters):
    ...
    if filters.get("material_request"):
        conditions += " and mr.name = '{0}'".format(
            filters.get("material_request"))

    if filters.get("item_code"):
        conditions += " and mr_item.item_code = '{0}'".format(
...
def update_qty_columns(row_to_update, data_row):
    fields = ["qty", "ordered_qty", "received_qty",
              "qty_to_receive", "qty_to_order"]
    for field in fields:
    ...
    for row in data:
        ...
        if filters.get("group_by_mr"):
            ...
            if not row["material_request"] in material_request_map:
                ...
                mr_row["required_date"] = min(
                    getdate(mr_row["required_date"]), getdate(row["required_date"]))
