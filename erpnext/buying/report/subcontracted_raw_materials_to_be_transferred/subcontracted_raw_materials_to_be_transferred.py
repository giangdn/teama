...
def get_data(data, filters):
    po = get_po(filters)
    po_transferred_qty_map = frappe._dict(
        get_transferred_quantity([v.name for v in po]))
...
def get_transferred_quantity(po_name):
    ...
    stock_entries_detail = get_stock_entry_detail(
        [v.name for v in stock_entries])
        ...
        for details in stock_entries_detail:
            if details.parent == entry.name:
                ...
                if entry.purchase_order not in po_transferred_qty_map:
                    ...
                    po_transferred_qty_map[entry.purchase_order][details.item_code] = po_transferred_qty_map[entry.purchase_order].get(
                        details.item_code, 0) + details.qty
