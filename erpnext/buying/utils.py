        ...
        if last_purchase_details and \
            ...
            if flt(d.conversion_factor):
                last_purchase_rate = flt(
                    d.base_net_rate) / flt(d.conversion_factor)
            # Check if item code is present
            # Conversion factor should not be mandatory for non itemized items
            elif d.item_code:
                frappe.throw(
                    _("UOM Conversion factor is required in row {0}").format(d.idx))

        # update last purchsae rate
        frappe.db.set_value('Item', d.item_code,
                            'last_purchase_rate', flt(last_purchase_rate))
...
def validate_for_items(doc):
    ...
    for d in doc.get("items"):
        ...
        f_lst = {'projected_qty': bin and flt(
            bin[0]['projected_qty']) or 0, 'ordered_qty': 0, 'received_qty': 0}
        ...
        if item.is_stock_item == 1 and d.qty and not d.warehouse and not d.get("delivered_by_supplier"):
            frappe.throw(_("Warehouse is mandatory for stock Item {0} in row {1}").format(
                d.item_code, d.idx))
...
def check_on_hold_or_closed_status(doctype, docname):
    ...
    if status in ("Closed", "On Hold"):
        frappe.throw(_("{0} {1} status is {2}").format(
            doctype, docname, status), frappe.InvalidStatusError)
