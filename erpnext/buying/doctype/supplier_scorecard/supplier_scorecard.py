...
class SupplierScorecard(Document):
            ...
            for c2 in self.standings:
                if c1 != c2:
                    if (c1.max_grade > c2.min_grade and c1.min_grade < c2.max_grade):
                        throw(_('Overlap in scoring between {0} and {1}').format(
                            c1.standing_name, c2.standing_name))
                if c2.min_grade == score:
                    score = c2.max_grade
        if score < 100:
            throw(
                _('Unable to find score starting at {0}. You need to have standing scores covering 0 to 100').format(score))
    ...
    def calculate_total_score(self):
        ...
        if total_max_score > 0:
            self.supplier_score = round(
                100.0 * (total_score / total_max_score), 1)
        else:
        ...
        for standing in self.standings:
            if (not standing.min_grade or (standing.min_grade <= self.supplier_score)) and \
                ...
                for fieldname in ('prevent_pos', 'prevent_rfqs', 'warn_rfqs', 'warn_pos'):
                    ...
                    frappe.db.set_value(
                        "Supplier", self.supplier, fieldname, self.get(fieldname))
...
def make_all_scorecards(docname):
    ...
    while (start_date < todays) and (end_date <= todays):
        ...
        end_date = get_scorecard_date(sc.period, start_date)
    if scp_count > 0:
        frappe.msgprint(_("Created {0} scorecards for {1} between: ").format(
            scp_count, sc.supplier) + str(first_start_date) + " - " + str(last_end_date))
    return scp_count
