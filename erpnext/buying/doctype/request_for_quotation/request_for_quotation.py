...
class RequestforQuotation(BuyingController):
    ...
    def validate_supplier_list(self):
        for d in self.suppliers:
            prevent_rfqs = frappe.db.get_value(
                "Supplier", d.supplier, 'prevent_rfqs')
            if prevent_rfqs:
                standing = frappe.db.get_value(
                    "Supplier Scorecard", d.supplier, 'status')
                frappe.throw(_("RFQs are not allowed for {0} due to a scorecard standing of {1}").format(
                    d.supplier, standing))
            warn_rfqs = frappe.db.get_value(
                "Supplier", d.supplier, 'warn_rfqs')
            if warn_rfqs:
                standing = frappe.db.get_value(
                    "Supplier Scorecard", d.supplier, 'status')
                frappe.msgprint(_("{0} currently has a {1} Supplier Scorecard standing, and RFQs to this supplier should be issued with caution.").format(
                    d.supplier, standing), title=_("Caution"), indicator='orange')

    def update_email_id(self):
        for rfq_supplier in self.suppliers:
            if not rfq_supplier.email_id:
                rfq_supplier.email_id = frappe.db.get_value(
                    "Contact", rfq_supplier.contact, "email_id")

    def validate_email_id(self, args):
        if not args.email_id:
            frappe.throw(_("Row {0}: For Supplier {1}, Email Address is Required to send an email").format(
    ...
    def get_supplier_email_preview(self, supplier):
        ...
        rfq_suppliers = list(
            filter(lambda row: row.supplier == supplier, self.suppliers))
        rfq_supplier = rfq_suppliers[0]

        self.validate_email_id(rfq_supplier)

        message = self.supplier_rfq_mail(
            rfq_supplier, '', self.get_link(), True)
    ...
    def send_to_supplier(self):
        ...
        for rfq_supplier in self.suppliers:
            if rfq_supplier.send_email:
                ...
                update_password_link, contact = self.update_supplier_contact(
                    rfq_supplier, self.get_link())

                self.update_supplier_part_no(rfq_supplier.supplier)
                self.supplier_rfq_mail(
                    rfq_supplier, update_password_link, self.get_link())
    ...
    def get_attachments(self):
        attachments = [d.name for d in get_attachments(
...
def check_portal_enabled(reference_doctype):
    if not frappe.db.get_value('Portal Menu Item',
        ...
        frappe.throw(
            _("The Access to Request for Quotation From Portal is Disabled. To Allow Access, Enable it in Portal Settings."))
    ...
    def postprocess(source, target_doc):
        if for_supplier:
            ...
            args = get_party_details(
                for_supplier, party_type="Supplier", ignore_permissions=True)
...
def create_supplier_quotation(doc):
    ...
    try:
        ...
        sq_doc.save()
        frappe.msgprint(
            _("Supplier Quotation {0} Created").format(sq_doc.name))
        return sq_doc.name
    except Exception:
        return None
...
def get_supplier_tag():
    if not frappe.cache().hget("Supplier", "Tags"):
        ...
        tags = list(set([tag.tag for tag in frappe.get_all(
            "Tag Link", filters=filters, fields=["tag"]) if tag]))
...
def get_rfq_containing_supplier(doctype, txt, searchfield, start, page_len, filters):
    ...
    if filters.get("transaction_date"):
        conditions += "and rfq.transaction_date = '{0}'".format(
            filters.get("transaction_date"))
