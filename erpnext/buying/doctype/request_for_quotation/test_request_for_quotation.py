...
class TestRequestforQuotation(unittest.TestCase):
    ...
    def test_make_supplier_quotation_from_portal(self):
        ...
        supplier_quotation_doc = frappe.get_doc(
            'Supplier Quotation', supplier_quotation_name)

        self.assertEqual(supplier_quotation_doc.supplier,
                         rfq.get('suppliers')[0].supplier)
        self.assertEqual(supplier_quotation_doc.get('items')
                         [0].request_for_quotation, rfq.name)
        self.assertEqual(supplier_quotation_doc.get(
            'items')[0].item_code, "_Test Item")
...
def make_request_for_quotation(**args):
    ...
    supplier_data = args.get("supplier_data") if args.get(
        "supplier_data") else get_supplier_data()
