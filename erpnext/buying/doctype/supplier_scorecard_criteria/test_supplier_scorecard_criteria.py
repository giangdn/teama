...
class TestSupplierScorecardCriteria(unittest.TestCase):
    def test_variables_exist(self):
        ...
        self.assertRaises(frappe.ValidationError,
                          frappe.get_doc(test_bad_criteria[0]).insert)

    def test_formula_validate(self):
        delete_test_scorecards()
        self.assertRaises(frappe.ValidationError,
                          frappe.get_doc(test_bad_criteria[1]).insert)
        self.assertRaises(frappe.ValidationError,
                          frappe.get_doc(test_bad_criteria[2]).insert)
...
test_bad_criteria = [
    {
        "name": "Fake Criteria 1",
                ...
                "formula": "(({fake_variable} / {tot_cost_shipments}) if {tot_cost_shipments} > 0 else 1 )* 100",
    ...
    },
    {
        "name": "Fake Criteria 2",
                "weight": 40.0,
                "doctype": "Supplier Scorecard Criteria",
                # Force 0 divided by 0
                "formula": "(({cost_of_on_time_shipments} / {tot_cost_shipments}))* 100",
    ...
    },
    {
        "name": "Fake Criteria 3",
                "weight": 40.0,
                "doctype": "Supplier Scorecard Criteria",
                # Two variables beside eachother
                "formula": "(({cost_of_on_time_shipments} {cost_of_on_time_shipments} / {tot_cost_shipments}))* 100",
