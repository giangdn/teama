...
class PurchaseOrder(BuyingController):
    ...
    def validate_supplier(self):
        prevent_po = frappe.db.get_value(
            "Supplier", self.supplier, 'prevent_pos')
        if prevent_po:
        ...
        if warn_po:
            standing = frappe.db.get_value(
                "Supplier Scorecard", self.supplier, 'status')
            frappe.msgprint(_("{0} currently has a {1} Supplier Scorecard standing, and Purchase Orders to this supplier should be issued with caution.").format(
                self.supplier, standing), title=_("Caution"), indicator='orange')
        ...
        for d in self.get('items'):
            if d.meta.get_field('material_request') and d.material_request and d.material_request not in check_list:
                ...
                check_on_hold_or_closed_status(
                    'Material Request', d.material_request)

    def update_requested_qty(self):
        material_request_map = {}
        for d in self.get("items"):
            if d.material_request_item:
                material_request_map.setdefault(
                    d.material_request, []).append(d.material_request_item)
        ...
        for mr, mr_item_rows in material_request_map.items():
            if mr and mr_item_rows:
                ...
                if mr_obj.status in ["Stopped", "Cancelled"]:
                    frappe.throw(_("Material Request {0} is cancelled or stopped").format(
                        mr), frappe.InvalidStatusError)
    ...
    def check_modified_date(self):
        mod_db = frappe.db.sql("select modified from `tabPurchase Order` where name = %s",
                               self.name)
        date_diff = frappe.db.sql("select '%s' - '%s' " %
    ...
    def update_receiving_percentage(self):
        ...
        for item in self.items:
            ...
            total_qty += item.qty
        if total_qty:
            self.db_set("per_received", flt(received_qty/total_qty)
...
def item_last_purchase_rate(name, conversion_rate, item_code, conversion_factor=1.0):
    ...
    if last_purchase_details:
        last_purchase_rate = (
            last_purchase_details['base_net_rate'] * (flt(conversion_factor) or 1.0)) / conversion_rate
...
def make_purchase_invoice_from_portal(purchase_order_name):
    doc = get_mapped_purchase_invoice(
        purchase_order_name, ignore_permissions=True)
...
def get_mapped_purchase_invoice(source_name, target_doc=None, ignore_permissions=False):
    ...
    def update_item(obj, target, source_parent):
        ...
        target.qty = target.amount / \
            flt(obj.rate) if (flt(obj.rate) and flt(
                obj.billed_amt)) else flt(obj.qty)
