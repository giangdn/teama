...
class TestPurchaseOrder(unittest.TestCase):
    ...
    def test_update_child_removing_item(self):
        ...
        self.assertRaises(frappe.ValidationError, update_child_qty_rate,
                          'Purchase Order', trans_item, po.name)
    ...
    def test_update_child_with_tax_template(self):
        ...
        self.assertEqual(po.taxes[1].account_head,
    ...
    def test_update_child_uom_conv_factor_change(self):
        po = create_purchase_order(
            item_code="_Test FG Item", is_subcontracted="Yes")
        total_reqd_qty = sum([d.get("required_qty")
                              for d in po.as_dict().get("supplied_items")])
    ...
    def test_make_purchase_invoice_with_terms(self):
        ...
        self.assertEqual(
            getdate(po.payment_schedule[0].due_date), getdate(po.transaction_date))
        self.assertEqual(po.payment_schedule[1].payment_amount, 2500.0)
        self.assertEqual(getdate(po.payment_schedule[1].due_date), add_days(
        ...
        self.assertEqual(
            getdate(pi.payment_schedule[0].due_date), getdate(po.transaction_date))
        self.assertEqual(pi.payment_schedule[1].payment_amount, 2500.0)
        self.assertEqual(getdate(pi.payment_schedule[1].due_date), add_days(
            getdate(po.transaction_date), 30))
    ...
    def test_ordered_qty_for_closing_po(self):
        ...
        self.assertEqual(get_ordered_qty(item_code="_Test Item",
                                         warehouse="_Test Warehouse - _TC"), existing_ordered_qty+1)

        po.update_status("Closed")

        self.assertEqual(get_ordered_qty(item_code="_Test Item",
                                         warehouse="_Test Warehouse - _TC"), existing_ordered_qty)
    ...
    def test_po_for_blocked_supplier_payments_past_date(self):
        ...
        with self.assertRaises(Exception):
            try:
                ...
                po = create_purchase_order()
                get_payment_entry('Purchase Order', po.name,
    ...
    def test_exploded_items_in_subcontracted(self):
        ...
        bom_items = sorted(
            [d.item_code for d in bom.items if not d.get('sourced_by_supplier')])
    ...
    def test_backflush_based_on_stock_entry(self):
        ...
        issued_items = sorted(
            [d.rm_item_code for d in pr.get('supplied_items')])
        ...
        for item in pr.get('supplied_items'):
            self.assertEqual(item.get('required_qty'), (transferred_rm_map[item.get(
                'rm_item_code')].get('qty') / order_qty) * received_qty)
    ...
    def test_backflushed_based_on_for_multiple_batches(self):
        ...
        self.assertTrue(pr1.supplied_items[0].consumed_qty,  flt(
            552.0) - flt(consumed_qty))
    ...
    def test_supplied_qty_against_subcontracted_po(self):
        ...
        make_subcontracted_item(item_code=item_code, raw_materials=[
                                "Sub Contracted Raw Material 4"])
    ...
    def test_po_optional_blanket_order(self):
        ...
        bo = make_blanket_order(
            blanket_order_type="Purchasing", quantity=10, rate=10)

        po = create_purchase_order(
            item_code="_Test Item", qty=5, against_blanket_order=1)
        ...
        po = create_purchase_order(
            item_code="_Test Item", qty=5, against_blanket_order=0)
        po_doc = frappe.get_doc('Purchase Order', po.get('name'))
...
def create_purchase_order(**args):
    ...
    po.currency = args.currency or frappe.get_cached_value(
