...
class TestSalesOrder(unittest.TestCase):
    ...
    def test_make_sales_invoice_with_terms(self):
        ...
        self.assertEqual(si.payment_schedule[1].due_date, add_days(
            so.transaction_date, 30))
    ...
    def test_return_against_sales_order(self):
        ...
        dn1 = create_delivery_note(
            is_return=1, return_against=dn.name, qty=-3, do_not_submit=True)
    ...
    def test_reserved_qty_for_partial_delivery_with_packing_list(self):
        ...
        make_stock_entry(item="_Test Item Home Desktop 100",
                         target="_Test Warehouse - _TC", qty=10, rate=100)
        ...
        self.assertEqual(get_reserved_qty(
            "_Test Item Home Desktop 100"), existing_reserved_qty_item2)
    ...
    def test_reserved_qty_for_over_delivery_with_packing_list(self):
        ...
        # set over-delivery allowance
        frappe.db.set_value('Item', "_Test Product Bundle Item",
                            'over_delivery_receipt_allowance', 50)
    ...
    def test_update_child_removing_item(self):
        ...
        trans_item = json.dumps([
            {"item_code": '_Test Item', "qty": 5, "rate": 1000, "docname": so.get("items")[
                0].name},
    ...
    def test_update_child_with_precision(self):
        ...
        precision = get_field_precision(
            frappe.get_meta("Sales Order Item").get_field("rate"))

        make_property_setter("Sales Order Item", "rate",
                             "precision", 7, "Currency")
    ...
    def test_update_child_perm(self):
        ...
        # add new item
        trans_item = json.dumps(
            [{'item_code': '_Test Item', 'rate': 100, 'qty': 2}])
        self.assertRaises(frappe.ValidationError,
                          update_child_qty_rate, 'Sales Order', trans_item, so.name)
    ...
    def test_update_child_qty_rate_with_workflow(self):
        ...
        so = make_sales_order(item_code="_Test Item",
                              qty=1, rate=150, do_not_submit=1)
        ...
        frappe.set_user("Administrator")
        test_user.remove_roles(
            "Sales User", "Test Junior Approver", "Test Approver")
        test_user2.remove_roles(
            "Sales User", "Test Junior Approver", "Test Approver")
    ...
    def test_update_child_product_bundle(self):
        ...
        if not frappe.db.exists("Item", "_Product Bundle Item"):
            bundle_item = make_item(
                "_Product Bundle Item", {"is_stock_item": 0})
        ...
        added_item = json.dumps(
            [{"item_code": "_Product Bundle Item", "rate": 200, 'qty': 2}])
    ...
    def test_update_child_with_tax_template(self):
        ...
        self.assertEqual(so.taxes[1].account_head,
        ...
        frappe.get_doc("Item Tax Template",
                       "Test Update Items Template").delete()
        frappe.db.set_value("Stock Settings", None,
                            "default_warehouse", old_stock_settings_value)

    def test_warehouse_user(self):
        frappe.permissions.add_user_permission(
            "Warehouse", "_Test Warehouse 1 - _TC", "test@example.com")
        frappe.permissions.add_user_permission(
            "Warehouse", "_Test Warehouse 2 - _TC1", "test2@example.com")
        frappe.permissions.add_user_permission(
            "Company", "_Test Company 1", "test2@example.com")
    ...
    def test_service_type_product_bundle(self):
        ...
        so = make_sales_order(
            item_code="_Test Service Product Bundle", warehouse=None)
    ...
    def test_mix_type_product_bundle(self):
        ...
        self.assertRaises(WarehouseRequired, make_sales_order,
                          item_code="_Test Mix Product Bundle", warehouse="")
    ...
    def test_auto_insert_price(self):
        ...
        make_sales_order(item_code="_Test Item for Auto Price List",
                         selling_price_list="_Test Price List", rate=100)
    ...
    def test_drop_shipping(self):
        ...
        if frappe.db.get_value("Item", "_Test Regular Item", "is_stock_item") == 1:
            make_stock_entry(item="_Test Regular Item",
                             target="_Test Warehouse - _TC", qty=2, rate=100)

        # create so, po and dn
        so = make_sales_order(item_list=so_items, do_not_submit=True)
        so.submit()

        po = make_purchase_order_for_default_supplier(
            so.name, selected_items=[so_items[0]])
        po.submit()
        ...
        self.assertEqual(flt(frappe.db.get_value(
            "Sales Order", so.name, "per_delivered"), 2), 100.00)
        po.load_from_db()
    ...
    def test_drop_shipping_partial_order(self):
        ...
        # make items
        po_item1 = make_item("_Test Item for Drop Shipping 1", {
                             "is_stock_item": 1, "delivered_by_supplier": 1})
        po_item2 = make_item("_Test Item for Drop Shipping 2", {
                             "is_stock_item": 1, "delivered_by_supplier": 1})
        ...
        po1 = make_purchase_order_for_default_supplier(
            so.name, selected_items=[so_items[0]])
        po1.submit()
        ...
        po2 = make_purchase_order_for_default_supplier(
            so.name, selected_items=[so_items[1]])
        po2.submit()
    ...
    def test_reserved_qty_for_closing_so(self):
        ...
        self.assertEqual(get_reserved_qty(item_code="_Test Item",
                                          warehouse="_Test Warehouse - _TC"), existing_reserved_qty+1)

        so.update_status("Closed")

        self.assertEqual(get_reserved_qty(item_code="_Test Item",
                                          warehouse="_Test Warehouse - _TC"), existing_reserved_qty)

    def test_create_so_with_margin(self):
        so = make_sales_order(item_code="_Test Item",
                              qty=1, do_not_submit=True)
        ...
        self.assertEqual(new_so.get("items")[0].rate, flt(
            (price_list_rate*25)/100 + price_list_rate))
    ...
    def test_make_work_order(self):
        ...
        for item in so.get_work_order_items():
            ...
            so_item_name[item.get("sales_order_item")
        ...
        for item in so_item_name:
            wo_qty = frappe.db.sql(
                "select sum(qty) from `tabWork Order` where sales_order=%s and sales_order_item=%s", (so.name, item))
    ...
    def test_serial_no_based_delivery(self):
        ...
        frappe.db.sql(
            """delete from `tabSerial No` where item_code=%s""", (item.item_code))
        ...
        make_stock_entry(item_code=item.item_code,
                         target="_Test Warehouse - _TC", qty=1)
        item_serial_no = frappe.get_doc(
            "Serial No", {"item_code": item.item_code})
        from erpnext.manufacturing.doctype.work_order.work_order import \
            make_stock_entry as make_production_stock_entry
        se = frappe.get_doc(make_production_stock_entry(
            work_order.name, "Manufacture", 1))
        se.submit()
        reserved_serial_no = se.get("items")[2].serial_no
        serial_no_so = frappe.get_value(
            "Serial No", reserved_serial_no, "sales_order")
    ...
    def test_request_for_raw_materials(self):
        ...
        mr = frappe.db.sql(
            """select name from `tabMaterial Request` ORDER BY creation DESC LIMIT 1""", as_dict=1)[0]
    ...
    def test_so_optional_blanket_order(self):
        ...
        bo = make_blanket_order(
            blanket_order_type="Selling", quantity=10, rate=10)

        so = make_sales_order(item_code="_Test Item",
                              qty=5, against_blanket_order=1)
...
def make_sales_order(**args):
    ...
    # no need to test set_warehouse permission since it only affects the client
    so.set_warehouse = ""
...
def make_sales_order_workflow():
    ...
    frappe.get_doc(dict(doctype='Role', role_name='Test Approver')
    ...
    workflow.append('states', dict(state='Approved',
                                   allow_edit='Test Approver', doc_status=1))
