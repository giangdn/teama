...
class SalesOrder(SellingController):
    ...
    def validate_po(self):
        ...
        if self.po_no and self.customer and not self.skip_delivery_note:
            ...
            if so and so[0][0] and not cint(frappe.db.get_single_value("Selling Settings",
                ...
                frappe.msgprint(_("Warning: Sales Order {0} already exists against Customer's Purchase Order {1}").format(
                    so[0][0], self.po_no))
    ...
    def validate_proj_cust(self):
        if self.project and self.customer_name:
            ...
            if not res:
                frappe.throw(_("Customer {0} does not belong to project {1}").format(
    ...
    def update_enquiry_status(self, prevdoc, flag):
        enq = frappe.db.sql(
            "select t2.prevdoc_docname from `tabQuotation` t1, `tabQuotation Item` t2 where t2.parent = t1.name and t1.name=%s", prevdoc)
        if enq:
            frappe.db.sql(
    ...
    def check_nextdoc_docstatus(self):
        ...
        if submit_dn:
            submit_dn = [get_link_to_form("Delivery Note", dn)
                         for dn in submit_dn]
        ...
        if submit_rv:
            submit_rv = [get_link_to_form("Sales Invoice", si)
                         for si in submit_rv]
        ...
        if submit_ms:
            submit_ms = [get_link_to_form(
                "Maintenance Schedule", ms) for ms in submit_ms]
        ...
        if submit_mv:
            submit_mv = [get_link_to_form(
                "Maintenance Visit", mv) for mv in submit_mv]
        ...
        if pro_order:
            pro_order = [get_link_to_form("Work Order", po)
                         for po in pro_order]
        ...
        for item in self.items:
            if item.supplier:
                ...
                if item.ordered_qty > 0.0 and item.supplier != supplier:
                    exc_list.append(
                        _("Row #{0}: Not allowed to change Supplier as Purchase Order already exists").format(item.idx))
    ...
    def get_work_order_items(self, for_raw_material_request=0):
        ...
        product_bundle_parents = [pb.new_item_code for pb in frappe.get_all(
        ...
        for item in self.items:
            if item.ensure_delivery_based_on_produced_serial_no:
                if item.item_code in normal_items:
                    frappe.throw(_("Cannot ensure delivery by Serial No as Item {0} is added with and without Ensure Delivery by Serial No.").format(
                        item.item_code))
                if item.item_code not in reserved_items:
                    if not frappe.get_cached_value("Item", item.item_code, "has_serial_no"):
                        frappe.throw(_("Item {0} has no Serial No. Only serilialized items can have delivery based on Serial No").format(
                            item.item_code))
                    if not frappe.db.exists("BOM", {"item": item.item_code, "is_active": 1}):
                        frappe.throw(_("No active BOM found for item {0}. Delivery by Serial No cannot be ensured").format(
                            item.item_code))
...
def make_sales_invoice(source_name, target_doc=None, ignore_permissions=False):
    ...
    def update_item(source, target, source_parent):
        ...
        target.qty = target.amount / \
            flt(source.rate) if (
                source.rate and source.billed_amt) else source.qty - source.returned_qty
...
def make_purchase_order_for_default_supplier(source_name, selected_items=None, target_doc=None):
    ...
    suppliers = [item.get('supplier') for item in selected_items if item.get(
        'supplier') and item.get('supplier')]
    suppliers = list(set(suppliers))

    items_to_map = [item.get('item_code') for item in selected_items if item.get(
        'item_code') and item.get('item_code')]
...
def create_pick_list(source_name, target_doc=None):
    def update_item_quantity(source, target, source_parent):
        target.qty = flt(source.qty) - flt(source.delivered_qty)
        target.stock_qty = (
            flt(source.qty) - flt(source.delivered_qty)) * flt(source.conversion_factor)
