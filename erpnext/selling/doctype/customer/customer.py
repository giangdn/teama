...
class Customer(TransactionBase):
    ...
    def validate_default_bank_account(self):
        if self.default_bank_account:
            ...
            if not is_company_account:
                frappe.throw(_("{0} is not a company bank account").format(
    ...
    def create_lead_address_contact(self):
        if self.lead_name:
            ...
            lead = frappe.db.get_value("Lead", self.lead_name, [
                                       "organization_lead", "lead_name", "email_id", "phone", "mobile_no", "gender", "salutation"], as_dict=True)

            if not lead.lead_name:
                frappe.throw(
                    _("Please mention the Lead Name in Lead {0}").format(self.lead_name))
    ...
    def validate_name_with_customer_group(self):
        if frappe.db.exists("Customer Group", self.name):
            frappe.throw(
                _("A Customer Group exists with same name please change the Customer name or rename the Customer Group"), frappe.NameError)
    ...
    def validate_credit_limit_on_change(self):
        ...
        for limit in self.credit_limits:
            if limit.company in company_record:
                frappe.throw(_("Credit limit is already defined for the Company {0}").format(
                    limit.company, self.name))
            else:
                company_record.append(limit.company)

            outstanding_amt = get_customer_outstanding(
                self.name, limit.company)
            if flt(limit.credit_limit) < outstanding_amt:
                frappe.throw(_("""New credit limit is less than current outstanding amount for the customer. Credit limit has to be atleast {0}""").format(
                    outstanding_amt))
    ...
    def set_loyalty_program(self):
        ...
        if len(loyalty_program) == 1:
            ...
            frappe.msgprint(
                _("Multiple Loyalty Program found for the Customer. Please select manually."))
...
def make_quotation(source_name, target_doc=None):
    ...
    price_list, currency = frappe.db.get_value("Customer", {'name': source_name}, [
...
def get_loyalty_programs(doc):
    ...
    for loyalty_program in loyalty_programs:
        customer_groups = [d.value for d in get_children(
            "Customer Group", loyalty_program.customer_group)] + [loyalty_program.customer_group]
        customer_territories = [d.value for d in get_children(
            "Territory", loyalty_program.customer_territory)] + [loyalty_program.customer_territory]
...
def check_credit_limit(customer, company, ignore_outstanding_sales_order=False, extra_amount=0):
    ...
    if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
        ...
        if not credit_controller_role or credit_controller_role not in frappe.get_roles():
            ...
            credit_controller_users_formatted = [get_formatted_email(user).replace(
                "<", "(").replace(">", ")") for user in credit_controller_users]
