...
class Analytics(object):
    ...
    def get_periodic_data(self):
        ...
        for d in self.entries:
            ...
            self.entity_periodic_data.setdefault(
                d.entity, frappe._dict()).setdefault(period, 0.0)
    ...
    def get_period(self, posting_date):
        if self.filters.range == 'Weekly':
            period = "Week " + str(posting_date.isocalendar()
                                   [1]) + " " + str(posting_date.year)
        elif self.filters.range == 'Monthly':
            period = str(self.months[posting_date.month - 1]
    ...
    def get_period_date_ranges(self):
        ...
        from_date, to_date = getdate(
            self.filters.from_date), getdate(self.filters.to_date)
        ...
        for dummy in range(1, 53):
            if self.filters.range == "Weekly":
                ...
                period_end_date = add_to_date(
                    from_date, months=increment, days=-1)
    ...
    def get_groups(self):
        ...
        for d in self.group_entries:
            if d.parent:
                self.depth_map.setdefault(
                    d.name, self.depth_map.get(d.parent) + 1)
            else:
    ...
    def get_supplier_parent_child_map(self):
        self.parent_child_map = frappe._dict(frappe.db.sql(
