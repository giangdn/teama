...
def get_columns(filters):
    ...
    return [
        ...
        "{party_value_type}::150".format(
            party_value_type=frappe.unscrub(str(party_type_value))),
        "Address Line 1",
        "Address Line 2",
        "City",
        "State",
...
def get_party_addresses_and_contact(party_type, party, party_group):
    ...
    fetch_party_list = frappe.get_list(party_type, filters=filters, fields=[
                                       "name", party_group], as_list=True)
    ...
    party_details = get_party_details(
        party_type, party_list, "Address", party_details)
    party_details = get_party_details(
        party_type, party_list, "Contact", party_details)
