    ...
    for d in entries:
        if d.stock_qty > 0 or filters.get('show_return_entries', 0):
            data.append([
                ...
                item_details.get(d.item_code, {}).get(
                    "item_group"), item_details.get(d.item_code, {}).get("brand"),
...
def get_conditions(filters, date_field):
    ...
    if filters.get("sales_person"):
        ...
        conditions.append(
            "exists(select name from `tabSales Person` where lft >= {0} and rgt <= {1} and name=st.sales_person)".format(lft, rgt))
