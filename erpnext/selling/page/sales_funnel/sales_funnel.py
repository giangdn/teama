...
def get_funnel_data(from_date, to_date, company):
    ...
    return [
        {"title": _("Active Leads"), "value": active_leads,
         "color": "#B03B46"},
        {"title": _("Opportunities"), "value": opportunities,
         "color": "#F09C00"},
...
def get_opp_by_lead_source(from_date, to_date, company):
    ...
    opportunities = frappe.get_all("Opportunity", filters=[['status', 'in', ['Open', 'Quotation', 'Replied']], ['company', '=', company], [
                                   'transaction_date', 'Between', [from_date, to_date]]], fields=['currency', 'sales_stage', 'opportunity_amount', 'probability', 'source'])
    ...
    if opportunities:
        ...
        df = pd.DataFrame(cp_opportunities).groupby(
            ['source', 'sales_stage'], as_index=False).agg({'compound_amount': 'sum'})
...
def get_pipeline_data(from_date, to_date, company):
    ...
    opportunities = frappe.get_all("Opportunity", filters=[['status', 'in', ['Open', 'Quotation', 'Replied']], ['company', '=', company], [
                                   'transaction_date', 'Between', [from_date, to_date]]], fields=['currency', 'sales_stage', 'opportunity_amount', 'probability'])
    ...
    if opportunities:
        ...
        result['datasets'] = []
        result['datasets'].append({'name': _(
            "Total Amount"), 'values': df['compound_amount'].values(), 'chartType': 'bar'})
