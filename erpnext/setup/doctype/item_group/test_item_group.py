...
class TestItem(unittest.TestCase):
    def test_basic_tree(self, records=None):
        ...
        for item_group in records:
            ...
            self.assertTrue(parent_rgt == (
                parent_lft + 1 + (2 * no_of_children)))
    ...
    def test_move_group_into_another(self):
        # before move
        old_lft, old_rgt = frappe.db.get_value(
            "Item Group", "_Test Item Group C", ["lft", "rgt"])
        ...
        # after move
        new_lft, new_rgt = frappe.db.get_value(
            "Item Group", "_Test Item Group C", ["lft", "rgt"])
    ...
    def print_tree(self):
        import json
        print(json.dumps(frappe.db.sql(
            "select name, lft, rgt from `tabItem Group` order by lft"), indent=1))
    ...
    def test_delete_leaf(self):
        ...
        for item_group in ancestors:
            new_lft, new_rgt = frappe.db.get_value(
                "Item Group", item_group.name, ["lft", "rgt"])
    ...
    def test_delete_group(self):
        ...
        self.assertRaises(NestedSetChildExistsError,
                          frappe.delete_doc, "Item Group", "_Test Item Group B")

    def test_merge_groups(self):
        frappe.rename_doc("Item Group", "_Test Item Group B",
                          "_Test Item Group C", merge=True)
        records_to_test = test_records[2:]
    ...
    def test_merge_leaves(self):
        frappe.rename_doc("Item Group", "_Test Item Group B - 2",
                          "_Test Item Group B - 1", merge=True)
        records_to_test = test_records[2:]
