    ...
    def make_route(self):
        ...
        if not self.route:
            ...
            if self.parent_item_group:
                parent_item_group = frappe.get_doc(
    ...
    def validate_name_with_item(self):
        if frappe.db.exists("Item", self.name):
            frappe.throw(frappe._("An item exists with same name ({0}), please change the item group name or rename the item").format(
                self.name), frappe.NameError)

    def get_context(self, context):
        context.show_search = True
        context.page_length = cint(frappe.db.get_single_value(
            'Products Settings', 'products_per_page')) or 6
        ...
        engine = ProductQuery()
        context.items = engine.query(
            attribute_filters, field_filters, search, start)
...
def get_product_list_for_group(product_group=None, start=0, limit=10, search=None):
    ...
    child_groups = ", ".join([frappe.db.escape(i[0])
    ...
    data = frappe.db.sql(query, {
                         "product_group": product_group, "search": search, "today": nowdate()}, as_dict=1)
...
def get_group_item_count(item_group):
    child_groups = ", ".join(
        ['"' + i[0] + '"' for i in get_child_groups(item_group)])
...
def invalidate_cache_for(doc, item_group=None):
    ...
    for d in get_parent_item_groups(item_group):
        ...
        if item_group_name:
            clear_cache(frappe.db.get_value(
                'Item Group', item_group_name, 'route'))
