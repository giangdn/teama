...
class NamingSeries(Document):
    def get_transactions(self, arg=None):
        ...
        doctypes = list(set(get_doctypes_with_read()
                            ).intersection(set(doctypes)))
        prefixes = ""
        for d in doctypes:
            options = ""
            try:
    ...
    def update_series(self, arg=None):
        ...
        # create series
        map(self.insert_series, [d.split('.')[0]
                                 for d in series_list if d.strip()])
    ...
    def check_duplicate(self):
        ...
        sr = [[frappe.get_meta(p).get_field(
            "naming_series").options, p] for p in parent]
...
def get_default_naming_series(doctype):
    naming_series = frappe.get_meta(doctype).get_field(
        "naming_series").options or ""
    naming_series = naming_series.split("\n")
    out = naming_series[0] or (
        naming_series[1] if len(naming_series) > 1 else None)
