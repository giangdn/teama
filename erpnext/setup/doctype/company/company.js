...
frappe.ui.form.on("Company", {
	...
	refresh: function(frm) {
		...
		if (!frm.doc.__islocal) {
			...
			frm.toggle_enable(
				"default_currency",
				frm.doc.__onload && !frm.doc.__onload.transactions_exist
			);
	...
	onload_post_render: function(frm) {
		if (frm.get_field("delete_company_transactions").$input)
			frm.get_field("delete_company_transactions").$input.addClass(
				"btn-danger"
			);
	},
		...
		frappe.verify_password(function() {
			var d = frappe.prompt(
				...
				function(data) {
					...
					frappe.call({
						method:
							"erpnext.setup.doctype.company.delete_company_transactions.delete_company_transactions",
						args: {
...
cur_frm.cscript.change_abbr = function() {
	...
	dialog.fields_dict.update.$input.click(function() {
		...
		return frappe.call({
			method:
				"erpnext.setup.doctype.company.company.enqueue_replace_abbr",
			args: {
