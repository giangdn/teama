...
class Company(NestedSet):
    ...
    def validate_abbr(self):
        if not self.abbr:
            self.abbr = ''.join([c[0]
                                 for c in self.company_name.split()]).upper()
    ...
    def validate_default_accounts(self):
        ...
        for account in accounts:
            if self.get(account[1]):
                for_company = frappe.db.get_value(
                    "Account", self.get(account[1]), "company")
    ...
    def validate_currency(self):
        ...
        if self.default_currency and self.previous_default_currency and \
            ...
            frappe.throw(
                _("Cannot change company's default currency, because there are existing transactions. Transactions must be cancelled to change the default currency."))
    ...
    def on_update(self):
        ...
        if self.default_currency:
            frappe.db.set_value(
                "Currency", self.default_currency, "enabled", 1)
    ...
    def set_default_accounts(self):
        ...
        if self.update_default_account:
            for default_account in default_accounts:
                self._set_default_account(
                    default_account, default_accounts.get(default_account))
        ...
        if not self.default_payable_account:
            self.db_set("default_payable_account",
                        self.default_payable_account)
        ...
        if not self.default_payroll_payable_account:
            ...
            self.db_set("default_payroll_payable_account",
        ...
        if not self.default_employee_advance_account:
            ...
            self.db_set("default_employee_advance_account",
                        employe_advance_account)
    ...
    def create_default_cost_center(self):
        ...
        frappe.db.set(self, "round_off_cost_center",
                      _("Main") + " - " + self.abbr)
        frappe.db.set(self, "depreciation_cost_center",
                      _("Main") + " - " + self.abbr)
    ...
    def on_trash(self):
        ...
        rec = frappe.db.sql(
            "SELECT name from `tabGL Entry` where company = %s", self.name)
        if not rec:
            ...
            for doctype in ["Account", "Cost Center", "Budget", "Party Account"]:
                frappe.db.sql(
                    "delete from `tab{0}` where company = %s".format(doctype), self.name)

        if not frappe.db.get_value("Stock Ledger Entry", {"company": self.name}):
            frappe.db.sql(
                """delete from `tabWarehouse` where company=%s""", self.name)
        ...
        boms = frappe.db.sql_list(
            "select name from tabBOM where company=%s", self.name)
        if boms:
...
def enqueue_replace_abbr(company, old, new):
    ...
    frappe.enqueue(
        'erpnext.setup.doctype.company.company.replace_abbr', **kwargs)
...
def replace_abbr(company, old, new):
    ...
    def _rename_records(dt):
        ...
        doc = (d for d in frappe.db.sql(
            "select name from `tab%s` where company=%s" % (dt, '%s'), company))
        for d in doc:
            _rename_record(d)
...
def install_country_fixtures(company):
    ...
    path = frappe.get_app_path(
        'erpnext', 'regional', frappe.scrub(company_doc.country))
    if os.path.exists(path.encode("utf-8")):
        try:
            module_name = "erpnext.regional.{0}.setup.setup".format(
                frappe.scrub(company_doc.country))
            frappe.get_attr(module_name)(company_doc, False)
        except Exception as e:
            frappe.log_error()
            frappe.throw(_("Failed to setup defaults for country {0}. Please contact support@erpnext.com").format(
                frappe.bold(company_doc.country)))
...
def update_company_current_month_sales(company):
    ...
    frappe.db.set_value("Company", company,
                        "total_monthly_sales", monthly_total)
...
def get_timeline_data(doctype, name):
    ...
    history = frappe.get_cached_value(
        'Company',  name,  "transactions_annual_history")

    try:
        date_to_value_dict = json.loads(
