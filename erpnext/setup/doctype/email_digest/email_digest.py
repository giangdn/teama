...
class EmailDigest(Document):
    ...
    def get_calendar_events(self):
        ...
        from_date, to_date = get_future_date_for_calendaer_event(
            self.frequency)
        ...
        for key in ("income", "expenses_booked", "income_year_to_date", "expense_year_to_date",
            ...
            if self.get(key):
                ...
                if card:
                    ...
                    if card.last_value:
                        card.diff = int(
                            flt(card.value - card.last_value) / card.last_value * 100)
                        if card.diff < 0:
                        ...
                        card.last_value = self.fmt_money(card.last_value, False if key in (
                            "bank_balance", "credit_balance") else True)

                    if card.billed_value:
                        card.billed = int(
                            flt(card.billed_value) / card.value * 100)
                        card.billed = "% Billed " + str(card.billed)

                    if card.delivered_value:
                        card.delivered = int(
                            flt(card.delivered_value) / card.value * 100)
                        if key == "pending_sales_orders":
                            card.delivered = "% Delivered " + \
                                str(card.delivered)
                        else:
                            card.delivered = "% Received " + \
                                str(card.delivered)

                    if key == "credit_balance":
                        card.value = card.value * -1
                    card.value = self.fmt_money(card.value, False if key in (
                        "bank_balance", "credit_balance") else True)

                    cache.set_value(cache_key, card,
    ...
    def get_income(self):
        ...
        income, past_income, count = self.get_period_amounts(
            self.get_roots("income"), 'income')
    ...
    def get_year_to_date_balance(self, root_type, fieldname):
        ...
        if fieldname == 'income':
            ...
            label = get_link_to_report('Profit and Loss Statement', label=self.meta.get_label(
                root_type + "_year_to_date"), filters=filters)
    ...
    def get_period_amounts(self, accounts, fieldname):
        ...
        for account in accounts:
            ...
            count += get_count_for_period(account, fieldname,
                                          self.future_from_date, self.future_to_date)
    ...
    def get_type_balance(self, fieldname, account_type, root_type=None):
        ...
        for account in accounts:
            ...
            count += get_count_on(account, fieldname, date=self.future_to_date)
            prev_balance += get_balance_on(account,
                                           date=self.past_to_date, in_account_currency=False)
    ...
    def get_summary_of_doc(self, doc_type, fieldname):
        ...
        value = flt(self.get_total_on(
            doc_type, self.future_from_date, self.future_to_date)[0].grand_total)
        count = self.get_total_on(
            doc_type, self.future_from_date, self.future_to_date)[0].count

        last_value = flt(self.get_total_on(
            doc_type, self.past_from_date, self.past_to_date)[0].grand_total)
    ...
    def set_dates(self):
        ...
        # decide from date based on email digest frequency
        if self.frequency == "Daily":
            self.past_from_date = self.past_to_date = self.future_from_date - \
                relativedelta(days=1)

        elif self.frequency == "Weekly":
            self.past_from_date = self.future_from_date - \
    ...
    def get_purchase_orders_items_overdue_list(self):
        ...
        purchase_order_items_overdue_list = frappe.db.sql(
            sql_poi, as_dict=True)
...
def get_incomes_expenses_for_period(account, from_date, to_date):
    ...
    balance_before_from_date = get_balance_on(
        account, date=from_date - timedelta(days=1))
    ...
    if from_date == fy_start_date:
        ...
        last_year_closing_balance = get_balance_on(
            account, date=fy_start_date - timedelta(days=1))
        print(fy_start_date - timedelta(days=1), last_year_closing_balance)
        val = balance_on_to_date + \
            (last_year_closing_balance - balance_before_from_date)
...
def get_count_for_period(account, fieldname, from_date, to_date):
    ...
    count_before_from_date = get_count_on(
        account, fieldname, from_date - timedelta(days=1))
    ...
    if from_date == fy_start_date:
        ...
        count = count_on_to_date + \
            (last_year_closing_count - count_before_from_date)
