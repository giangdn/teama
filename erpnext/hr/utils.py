...
def get_onboarding_details(parent, parenttype):
    return frappe.get_all("Employee Boarding Activity",
                          fields=["activity_name", "role", "user",
                                  "required_for_employee_creation", "description", "task_weight"],
...
def update_employee(employee, details, date=None, cancel=False):
    ...
    for item in details:
        fieldtype = frappe.get_meta("Employee").get_field(
            item.fieldname).fieldtype
...
def validate_dates(doc, from_date, to_date):
    ...
    if getdate(from_date) > getdate(to_date):
        ...
        frappe.throw(
            _("To date can not greater than employee's relieving date"))
...
def validate_overlap(doc, from_date, to_date, company=None):
    ...
    if overlap_doc:
        ...
        if company:
            exists_for = company
        throw_overlap_error(
            doc, exists_for, overlap_doc[0].name, from_date, to_date)
...
def validate_tax_declaration(declarations):
    ...
    for d in declarations:
        if d.exemption_sub_category in subcategories:
            frappe.throw(_("More than one selection for {0} not allowed").format(
...
def get_total_exemption_amount(declarations):
    ...
    for d in declarations:
        ...
        exemptions.get(
            d.exemption_category).total_exemption_amount += flt(sub_category_exemption_amount)

        if category_max_amount and exemptions.get(d.exemption_category).total_exemption_amount > category_max_amount:
            exemptions.get(
                d.exemption_category).total_exemption_amount = category_max_amount
...
def generate_leave_encashment():
    ...
    if frappe.db.get_single_value('HR Settings', 'auto_leave_encashment'):
        leave_type = frappe.get_all('Leave Type', filters={
                                    'allow_encashment': 1}, fields=['name'])
...
def update_previous_leave_allocation(allocation, annual_allocation, e_leave_type):
    divide_by_frequency = {"Yearly": 1,
                           "Half-Yearly": 6, "Quarterly": 4, "Monthly": 12}
    if annual_allocation:
        earned_leaves = flt(annual_allocation) / \
            divide_by_frequency[e_leave_type.earned_leave_frequency]
...
def get_earned_leaves():
    return frappe.get_all("Leave Type",
                          fields=["name", "max_leaves_allowed", "earned_leave_frequency",
                                  "rounding", "based_on_date_of_joining"],
...
def grant_leaves_automatically():
    automatically_allocate_leaves_based_on_leave_policy = frappe.db.get_singles_value(
        "HR Settings", "automatically_allocate_leaves_based_on_leave_policy")
    if automatically_allocate_leaves_based_on_leave_policy:
        lpa = frappe.db.get_all("Leave Policy Assignment", filters={
                                "effective_from": getdate(), "docstatus": 1, "leaves_allocated": 0})
        for assignment in lpa:
            frappe.get_doc("Leave Policy Assignment",
                           assignment.name).grant_leave_alloc_for_employee()
