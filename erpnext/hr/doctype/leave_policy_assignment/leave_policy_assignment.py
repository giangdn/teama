...
class LeavePolicyAssignment(Document):
    ...
    def set_dates(self):
        if self.assignment_based_on == "Leave Period":
            ...
            self.effective_from = frappe.db.get_value(
        ...
        if self.leaves_allocated:
            ...
            for leave_policy_detail in leave_policy.leave_policy_details:
                ...
                leave_allocations[leave_policy_detail.leave_type] = {
                    "name": leave_allocation, "leaves": new_leaves_allocated}
    ...
    def get_new_leaves(self, leave_type, new_leaves_allocated, leave_type_details, date_of_joining):
        ...
        if getdate(date_of_joining) > getdate(self.effective_from):
            ...
            new_leaves_allocated = ceil(
                new_leaves_allocated * remaining_period)
...
def grant_leave_for_multiple_employees(leave_policy_assignments):
    ...
    for assignment in leave_policy_assignments:
        ...
        if len(not_granted):
            msg = _("Leave not Granted for Assignments:") + \
                bold(comma_and(not_granted)) + _(". Please Check documents")
        else:
