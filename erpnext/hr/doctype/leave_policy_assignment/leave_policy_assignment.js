...
frappe.ui.form.on("Leave Policy Assignment", {
			...
			frm.add_custom_button(__("Grant Leave"), function() {
				frappe.call({
					...
					callback: function(r) {
						...
						let msg = frm.events.get_success_message(
							leave_allocations
						);
						frappe.msgprint(msg);
						cur_frm.refresh();
	...
	get_success_message: function(leave_allocations) {
		...
		for (let key in leave_allocations) {
			msg +=
				...
				key +
				"</th><td>" +
				leave_allocations[key]["name"] +
				"</td><td>" +
				leave_allocations[key]["leaves"] +
				"</td></tr>";
		}
		msg += "</table>";
		return msg;
	...
	set_effective_date: function(frm) {
		if (
			frm.doc.assignment_based_on == "Leave Period" &&
			...
			frm.doc.assignment_based_on == "Joining Date" &&
			frm.doc.employee
		) {
