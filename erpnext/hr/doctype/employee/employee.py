...
class EmployeeLeftValidationError(frappe.ValidationError):
    pass
...
class Employee(NestedSet):
    ...
    def validate_status(self):
        if self.status == 'Left':
            reports_to = frappe.db.get_all('Employee',
                                           filters={
                                               'reports_to': self.name, 'status': "Active"},
                                           fields=['name', 'employee_name']
                                           )
            if reports_to:
                link_to_employees = [frappe.utils.get_link_to_form(
                    'Employee', employee.name, label=employee.employee_name) for employee in reports_to]
                message = _("The following employees are currently still reporting to {0}:").format(
                    frappe.bold(self.employee_name))
                message += "<br><br><ul><li>" + \
                    "</li><li>".join(link_to_employees)
                message += "</li></ul><br>"
                message += _("Please make sure the employees above report to another Active employee.")
                throw(message, EmployeeLeftValidationError,
                      _("Cannot Relieve Employee"))
    ...
    def validate_preferred_email(self):
        if self.prefered_contact_email and not self.get(scrub(self.prefered_contact_email)):
            frappe.msgprint(_("Please enter {0}").format(
                self.prefered_contact_email))
    ...
    def validate_onboarding_process(self):
        ...
        if employee_onboarding:
            doc = frappe.get_doc("Employee Onboarding",
...
def send_birthday_reminders():
    ...
    for company, birthday_persons in employees_born_today.items():
        ...
        birthday_person_emails = [get_employee_email(
            doc) for doc in birthday_persons]
        recipients = list(set(employee_emails) - set(birthday_person_emails))

        reminder_text, message = get_birthday_reminder_text_and_message(
            birthday_persons)
        send_birthday_reminder(recipients, reminder_text,
                               birthday_persons, message)
        ...
        if len(birthday_persons) > 1:
            ...
            for person in birthday_persons:
                ...
                send_birthday_reminder(
                    person_email, reminder_text, others, message)
...
def get_birthday_reminder_text_and_message(birthday_persons):
    if len(birthday_persons) == 1:
        ...
        birthday_person_text = _("{} & {}").format(
            birthday_person_text, last_person)
