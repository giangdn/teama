...
class TestExpenseClaim(unittest.TestCase):
    def test_total_expense_claim_for_project(self):
        frappe.db.sql(
            """delete from `tabTask` where project = "_Test Project 1" """)
        frappe.db.sql(
            """delete from `tabProject` where name = "_Test Project 1" """)
        ...
        make_expense_claim(payable_account, 300, 200, company_name,
                           "Travel Expenses - _TC4", project.name, task_name)

        self.assertEqual(frappe.db.get_value(
            "Task", task_name, "total_expense_claim"), 200)
        self.assertEqual(frappe.db.get_value(
            "Project", project.name, "total_expense_claim"), 200)

        expense_claim2 = make_expense_claim(
            payable_account, 600, 500, company_name, "Travel Expenses - _TC4", project.name, task_name)

        self.assertEqual(frappe.db.get_value(
            "Task", task_name, "total_expense_claim"), 700)
        self.assertEqual(frappe.db.get_value(
            "Project", project.name, "total_expense_claim"), 700)
    ...
    def test_expense_claim_status(self):
        ...
        expense_claim = make_expense_claim(
            payable_account, 300, 200, company_name, "Travel Expenses - _TC4")
    ...
    def test_expense_claim_gl_entry(self):
        ...
        expense_claim = make_expense_claim(
            payable_account, 300, 200, company_name, "Travel Expenses - _TC4", do_not_submit=True, taxes=taxes)
    ...
    def test_rejected_expense_claim(self):
        ...
        expense_claim = frappe.get_doc({
            ...
            "expenses":
            [{"expense_type": "Travel", "default_account": "Travel Expenses - _TC4",
                "amount": 300, "sanctioned_amount": 200}]
        })
        expense_claim.submit()
...
def generate_taxes():
    parent_account = frappe.db.get_value('Account',
                                         {'company': company_name,
                                             'is_group': 1, 'account_type': 'Tax'},
                                         'name')
    account = create_account(company=company_name, account_name="CGST",
                             account_type="Tax", parent_account=parent_account)
    return {'taxes': [{
...
def make_expense_claim(payable_account, amount, sanctioned_amount, company, account, project=None, task_name=None, do_not_submit=False, taxes=None):
    ...
    if not employee:
        employee = make_employee(
            "test_employee@expense_claim.com", company=company)
