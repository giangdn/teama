...
frappe.ui.form.on("Expense Claim Detail", {
	expense_type: function(frm, cdt, cdn) {
		...
		return frappe.call({
			method:
				"erpnext.hr.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
			args: {
	...
	if (!doc.__islocal) {
		if (doc.docstatus === 1) {
			...
			if (doc.__onload.make_payment_via_journal_entry) {
				...
				entry_reference_doctype =
					"Journal Entry Account.reference_type";
				entry_reference_name = "Journal Entry.reference_name";
			} else {
				entry_doctype = "Payment Entry";
				entry_reference_doctype =
					"Payment Entry Reference.reference_doctype";
...
erpnext.expense_claim = {
	set_title: function(frm) {
		if (!frm.doc.task) {
			...
			frm.set_value(
				"title",
				frm.doc.employee_name + " for " + frm.doc.task
			);
		}
	}
};
...
frappe.ui.form.on("Expense Claim", {
	setup: function(frm) {
		...
		frm.set_query("expense_approver", function() {
			return {
				query:
					"erpnext.hr.doctype.department_approver.department_approver.get_approvers",
				filters: {
					employee: frm.doc.employee,
	...
	onload: function(frm) {
		if (frm.doc.docstatus == 0) {
			return frappe.call({
				method:
					"erpnext.hr.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype
				},
				callback: function(r) {
	...
	refresh: function(frm) {
		...
		if (
			frm.doc.docstatus === 1 &&
			!cint(frm.doc.is_paid) &&
			cint(frm.doc.grand_total) > 0 &&
			cint(frm.doc.total_amount_reimbursed) <
				cint(frm.doc.total_sanctioned_amount) &&
	...
	get_advances: function(frm) {
		...
		if (frm.doc.employee) {
			return frappe.call({
				method:
					"erpnext.hr.doctype.expense_claim.expense_claim.get_advances",
				args: {
					employee: frm.doc.employee
				},
					...
					if (r.message) {
						$.each(r.message, function(i, d) {
							...
							row.unclaimed_amount =
								flt(d.paid_amount) - flt(d.claimed_amount);
...
frappe.ui.form.on("Expense Claim Advance", {
	employee_advance: function(frm, cdt, cdn) {
		...
		if (!frm.doc.employee) {
			frappe.msgprint(
				__("Select an employee to get the employee advance.")
			);
			frm.doc.advances = [];
			refresh_field("advances");
		} else {
...
frappe.ui.form.on("Expense Taxes and Charges", {
	...
	calculate_total_tax: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		child.total =
			flt(frm.doc.total_sanctioned_amount) + flt(child.tax_amount);
