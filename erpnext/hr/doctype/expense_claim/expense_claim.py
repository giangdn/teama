...
class ExpenseClaim(AccountsController):
    ...
    def set_status(self):
        ...
        paid_amount = flt(self.total_amount_reimbursed) + \
    ...
    def on_submit(self):
        if self.approval_status == "Draft":
            frappe.throw(
                _("""Approval Status must be 'Approved' or 'Rejected'"""))
    ...
    def update_claimed_amount_in_employee_advance(self):
        for d in self.get("advances"):
            frappe.get_doc("Employee Advance",
                           d.employee_advance).update_claimed_amount()
    ...
    def calculate_taxes(self):
        ...
        for tax in self.taxes:
            if tax.rate:
                tax.tax_amount = flt(
                    self.total_sanctioned_amount) * flt(tax.rate/100)
        ...
        self.grand_total = flt(self.total_sanctioned_amount) + \
            flt(self.total_taxes_and_charges) - flt(self.total_advance_amount)
    ...
    def validate_advances(self):
        ...
        for d in self.get("advances"):
            ...
            d.unclaimed_amount = flt(
                ref_doc.paid_amount) - flt(ref_doc.claimed_amount)
        ...
        if self.total_advance_amount:
            ...
            if flt(self.total_advance_amount, precision) > flt(self.total_claimed_amount, precision):
                frappe.throw(
                    _("Total advance amount cannot be greater than total claimed amount"))
        ...
        for expense in self.expenses:
            if not expense.default_account or not validate:
                expense.default_account = get_expense_claim_account(
                    expense.expense_type, self.company)["account"]
