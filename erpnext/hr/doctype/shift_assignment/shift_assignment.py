...
class ShiftAssignment(Document):
    ...
    def throw_overlap_error(self, shift_details):
        ...
        if shift_details.docstatus == 1 and shift_details.status == "Active":
            msg = _("Employee {0} already has Active Shift {1}: {2}").format(frappe.bold(
                self.employee), frappe.bold(self.shift_type), frappe.bold(shift_details.name))
...
def add_assignments(events, start, end, conditions=None):
    ...
    records = frappe.db.sql(
        query, {"start_date": start, "end_date": end}, as_dict=True)
    ...
    for d in records:
        ...
        while daily_event_start <= daily_event_end:
            start_timing = frappe.utils.get_datetime(
                daily_event_start) + shift_timing_map[d.shift_type]['start_time']
            end_timing = frappe.utils.get_datetime(
                daily_event_start) + shift_timing_map[d.shift_type]['end_time']
...
def get_shift_type_timing(shift_types):
    shift_timing_map = {}
    data = frappe.get_all("Shift Type", filters={"name": (
        "IN", shift_types)}, fields=['name', 'start_time', 'end_time'])
...
def get_employee_shift(employee, for_date=nowdate(), consider_default_shift=False, next_shift_direction=None):
    ...
    if shift_type_name:
        holiday_list_name = frappe.db.get_value(
            'Shift Type', shift_type_name, 'holiday_list')
    ...
    if not shift_type_name and next_shift_direction:
        ...
        if consider_default_shift and default_shift:
            ...
            dates = frappe.db.get_all('Shift Assignment',
                                      ['start_date', 'end_date'],
                                      {'employee': employee, 'start_date': (
                                          direction, for_date), 'docstatus': '1', "status": "Active"},
                                      as_list=True,
            ...
            if dates:
                for date in dates:
                    if date[1] and date[1] < for_date:
                        continue
                    shift_details = get_employee_shift(
                        employee, date[0], consider_default_shift, None)
                    if shift_details:
                        shift_type_name = shift_details.shift_type.name
                        for_date = date[0]
                        break
...
def get_employee_shift_timings(employee, for_timestamp=now_datetime(), consider_default_shift=False):
    ...
    if curr_shift:
        next_shift = get_employee_shift(employee, curr_shift.start_datetime.date(
        )+timedelta(days=1), consider_default_shift, 'forward')
    prev_shift = get_employee_shift(employee, for_timestamp.date(
    )+timedelta(days=-1), consider_default_shift, 'reverse')
...
def get_actual_start_end_datetime_of_shift(employee, for_datetime, consider_default_shift=False):
    ...
    if timestamp_index and timestamp_index % 2 == 1:
        shift_details = shift_timings_as_per_timestamp[int(
            (timestamp_index-1)/2)]
