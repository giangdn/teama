...
class HolidayList(Document):
    ...
    def validate_days(self):
        ...
        for day in self.get("holidays"):
            if not (getdate(self.from_date) <= getdate(day.holiday_date) <= getdate(self.to_date)):
                frappe.throw(_("The holiday on {0} is not between From Date and To Date").format(
    ...
    def get_weekly_off_date_list(self, start_date, end_date):
        ...
        reference_date = start_date + \
            relativedelta.relativedelta(weekday=weekday)

        existing_date_list = [getdate(holiday.holiday_date)
...
def get_events(start, end, filters=None):
    ...
    return frappe.get_list('Holiday List',
                           fields=['name', '`tabHoliday`.holiday_date',
                                   '`tabHoliday`.description', '`tabHoliday List`.color'],
                           filters=filters,
                           update={"allDay": 1})
