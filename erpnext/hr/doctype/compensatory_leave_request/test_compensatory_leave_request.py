...
class TestCompensatoryLeaveRequest(unittest.TestCase):
    def setUp(self):
        ...
        frappe.db.sql(''' delete from `tabAttendance` where attendance_date in {0} '''.format(
            (today(), add_days(today(), -1))))  # nosec
        create_leave_period(add_months(today(), -3),
                            add_months(today(), 3), "_Test Company")
        create_holiday_list()
    ...
    def test_leave_balance_on_submit(self):
        ...
        self.assertEqual(get_leave_balance_on(
            employee.name, compensatory_leave_request.leave_type, add_days(today(), 1)), before + 1)
