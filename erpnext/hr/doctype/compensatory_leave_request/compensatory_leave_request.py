...
class CompensatoryLeaveRequest(Document):
    ...
    def validate_attendance(self):
        ...
        if len(attendance) < date_diff(self.work_end_date, self.work_from_date) + 1:
            frappe.throw(
                _("You are not present all day(s) between compensatory leave request days"))

    def validate_holidays(self):
        holidays = get_holidays_for_employee(
            self.employee, self.work_from_date, self.work_end_date)
        if len(holidays) < date_diff(self.work_end_date, self.work_from_date) + 1:
            frappe.throw(
                _("Compensatory leave request days not in valid holidays"))

    def on_submit(self):
        company = frappe.db.get_value("Employee", self.employee, "company")
        date_difference = date_diff(
            self.work_end_date, self.work_from_date) + 1
        if self.half_day:
            date_difference -= 0.5
        ...
        if leave_period:
            ...
            frappe.throw(_("There is no leave period in between {0} and {1}").format(
