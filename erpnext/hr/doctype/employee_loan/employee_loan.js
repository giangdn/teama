...
frappe.ui.form.on("Employee Loan", {
	onload: function(frm) {
		...
		$.each(["payment_account", "employee_loan_account"], function(
			i,
			field
		) {
	...
	refresh: function(frm) {
		if (
			...
			(frm.doc.status == "Sanctioned" ||
				frm.doc.status == "Partially Disbursed")
		) {
	...
	make_jv: function(frm) {
		frappe.call({
			...
			method:
				"erpnext.hr.doctype.employee_loan.employee_loan.make_jv_entry",
			callback: function(r) {
	...
	employee_loan_application: function(frm) {
		if (frm.doc.employee_loan_application) {
			return frappe.call({
				method:
					"erpnext.hr.doctype.employee_loan.employee_loan.get_employee_loan_application",
				args: {
