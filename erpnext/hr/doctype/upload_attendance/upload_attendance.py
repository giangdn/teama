...
def add_header(w):
    status = ", ".join((frappe.get_meta("Attendance").get_field(
        "status").options or "").strip().split("\n"))
    w.writerow(["Notes:"])
    w.writerow(["Please do not change the template headings"])
    w.writerow(["Status should be one of these values: " + status])
    w.writerow(
        ["If you are overwriting existing attendance records, 'ID' column mandatory"])
    ...
    for date in dates:
        for employee in employees:
            ...
            if existing_attendance_records \
                ...
                existing_attendance = existing_attendance_records[tuple(
                    [getdate(date), employee.name])]

            employee_holiday_list = get_holiday_list_for_employee(
                employee.name)
...
def get_holidays_for_employees(employees, from_date, to_date):
    ...
    for employee in employees:
        ...
        holiday = get_holidays_for_employee(
            employee, getdate(from_date), getdate(to_date))
...
def get_active_employees():
    employees = frappe.db.get_all('Employee',
                                  fields=[
                                      'name', 'employee_name', 'date_of_joining', 'company', 'relieving_date'],
                                  filters={
                                      'docstatus': ['<', 2],
                                      'status': 'Active'
                                  }
...
def get_naming_series():
    series = frappe.get_meta("Attendance").get_field(
...
def upload():
    ...
    frappe.enqueue(import_attendances, rows=rows,
                   now=True if len(rows) < 200 else False)
