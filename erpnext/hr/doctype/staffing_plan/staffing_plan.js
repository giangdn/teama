...
frappe.ui.form.on("Staffing Plan Detail", {
	...
	vacancies: function(frm, cdt, cdn) {
		...
		if (child.vacancies < child.current_openings) {
			frappe.throw(
				__("Vacancies cannot be lower than the current openings")
			);
		}
		set_number_of_positions(frm, cdt, cdn);
	},
...
var set_number_of_positions = function(frm, cdt, cdn) {
	...
	frappe.call({
		method:
			"erpnext.hr.doctype.staffing_plan.staffing_plan.get_designation_counts",
		args: {
		...
		callback: function(data) {
			if (data.message) {
				...
				let total_positions =
					cint(data.message.employee_count) + cint(child.vacancies);
