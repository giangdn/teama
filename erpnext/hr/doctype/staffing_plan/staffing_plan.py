...
class StaffingPlan(Document):
    ...
    def set_total_estimated_budget(self):
        ...
        for detail in self.get("staffing_details"):
            ...
            designation_counts = get_designation_counts(
                detail.designation, self.company)
            detail.current_count = designation_counts['employee_count']
            detail.current_openings = designation_counts['job_openings']

            if detail.number_of_positions > 0:
                if detail.vacancies > 0 and detail.estimated_cost_per_position:
                    detail.total_estimated_cost = cint(
                        detail.vacancies) * flt(detail.estimated_cost_per_position)

            self.total_estimated_budget += detail.total_estimated_cost

    def set_number_of_positions(self, detail):
        detail.number_of_positions = cint(
            detail.vacancies) + cint(detail.current_count)
    ...
    def validate_with_parent_plan(self, staffing_plan_detail):
        ...
        parent_company = frappe.db.get_value(
            "Staffing Plan", parent_plan_details[0].name, "company")
...
def get_active_staffing_plan_details(company, designation, from_date=getdate(nowdate()), to_date=getdate(nowdate())):
    ...
    if not staffing_plan:
        parent_company = frappe.get_cached_value(
