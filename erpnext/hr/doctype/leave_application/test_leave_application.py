...
class TestLeaveApplication(unittest.TestCase):
    ...
    def test_optional_leave(self):
        ...
        self.assertEqual(get_leave_balance_on(
            employee.name, leave_type, today), 9)
    ...
    def test_leave_balance_near_allocaton_expiry(self):
        ...
        self.assertEqual(get_leave_balance_on(
            employee.name, leave_type.name, nowdate(), add_days(nowdate(), 8)), 21)
    ...
    def test_earned_leaves_creation(self):
        ...
        frappe.delete_doc_if_exists(
            "Leave Type", 'Test Earned Leave Type', force=1)
        frappe.get_doc(dict(
            leave_type_name=leave_type,
            doctype='Leave Type',
            is_earned_leave=1,
        ...
        while(i < 14):
            ...
            i += 1
        self.assertEqual(get_leave_balance_on(
            employee.name, leave_type, nowdate()), 6)
        ...
        while(i < 6):
            ...
            i += 1
        self.assertEqual(get_leave_balance_on(
            employee.name, leave_type, nowdate()), 9)
    ...
    def test_creation_of_leave_ledger_entry_on_submit(self):
        ...
        self.assertEquals(
            leave_ledger_entry[0].employee, leave_application.employee)
        self.assertEquals(
            leave_ledger_entry[0].leave_type, leave_application.leave_type)
        self.assertEquals(
            leave_ledger_entry[0].leaves, leave_application.total_leave_days * -1)

        # check if leave ledger entry is deleted on cancellation
        leave_application.cancel()
        self.assertFalse(frappe.db.exists("Leave Ledger Entry", {
                         'transaction_name': leave_application.name}))
    ...
    def test_leave_application_creation_after_expiry(self):
        ...
        self.assertEquals(get_leave_balance_on(employee.name, leave_type.name, add_days(
            nowdate(), -85), add_days(nowdate(), -84)), 0)
