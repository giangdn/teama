...
frappe.ui.form.on("Leave Application", {
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.employee) {
			frappe.call({
				method:
					"erpnext.hr.doctype.leave_application.leave_application.get_leave_details",
				async: false,
				args: {
					employee: frm.doc.employee,
	...
	refresh: function(frm) {
		...
		if (!frm.doc.employee && frappe.defaults.get_user_permissions()) {
			...
			if (perm && perm["Employee"]) {
				frm.set_value(
					"employee",
					perm["Employee"].map(perm_doc => perm_doc.doc)[0]
				);
			}
		}
	},
	...
	set_leave_approver: function(frm) {
		if (frm.doc.employee) {
			...
			return frappe.call({
				method:
					"erpnext.hr.doctype.leave_application.leave_application.get_leave_approver",
				args: {
					employee: frm.doc.employee
				},
				callback: function(r) {
