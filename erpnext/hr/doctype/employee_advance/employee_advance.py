...
class EmployeeAdvance(Document):
    ...
    def set_total_advance_paid(self):
        ...
        self.set_status()
        frappe.db.set_value("Employee Advance", self.name,
                            "status", self.status)
...
def get_pending_amount(employee, posting_date):
    employee_due_amount = frappe.get_all("Employee Advance",
                                         filters={"employee": employee, "docstatus": 1, "posting_date": (
                                             "<=", posting_date)},
...
def make_return_entry(employee, company, employee_advance_name, return_amount,  advance_account, currency, exchange_rate, mode_of_payment=None):
    ...
    advance_account_currency = frappe.db.get_value(
        'Account', advance_account, 'account_currency')
...
def get_voucher_type(mode_of_payment=None):
    ...
    if mode_of_payment:
        mode_of_payment_type = frappe.get_cached_value(
