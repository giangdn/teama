...
frappe.ui.form.on("Employee Advance", {
	...
	make_deduction_via_additional_salary: function(frm) {
		frappe.call({
			method:
				"erpnext.hr.doctype.employee_advance.employee_advance.create_return_through_additional_salary",
			args: {
				doc: frm.doc
			},
			callback: function(r) {
	...
	make_payment_entry: function(frm) {
		...
		if (
			...
			method =
				"erpnext.hr.doctype.employee_advance.employee_advance.make_bank_entry";
		}
		return frappe.call({
			method: method,
			args: {
	...
	make_return_entry: function(frm) {
		frappe.call({
			method:
				"erpnext.hr.doctype.employee_advance.employee_advance.make_return_entry",
			args: {
				employee: frm.doc.employee,
				company: frm.doc.company,
				employee_advance_name: frm.doc.name,
				return_amount: flt(
					frm.doc.paid_amount - frm.doc.claimed_amount
				),
	...
	get_pending_amount: function(frm) {
		frappe.call({
			method:
				"erpnext.hr.doctype.employee_advance.employee_advance.get_pending_amount",
			args: {
				employee: frm.doc.employee,
				posting_date: frm.doc.posting_date
			},
	...
	get_employee_currency: function(frm) {
		frappe.call({
			method:
				"erpnext.payroll.doctype.salary_structure_assignment.salary_structure_assignment.get_employee_currency",
			args: {
				employee: frm.doc.employee
			},
			callback: function(r) {
