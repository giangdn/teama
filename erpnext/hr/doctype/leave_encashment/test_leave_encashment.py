...
class TestLeaveEncashment(unittest.TestCase):
    def setUp(self):
        ...
        self.leave_period = create_leave_period(
            add_months(today(), -3), add_months(today(), 3))
    ...
    def test_leave_balance_value_and_amount(self):
        ...
        # assert links
        add_sal = frappe.get_all("Additional Salary", filters={
                                 "ref_docname": leave_encashment.name})[0]
    ...
    def test_creation_of_leave_ledger_entry_on_submit(self):
        ...
        self.assertEquals(
            leave_ledger_entry[0].employee, leave_encashment.employee)
        self.assertEquals(
            leave_ledger_entry[0].leave_type, leave_encashment.leave_type)
        self.assertEquals(
            leave_ledger_entry[0].leaves, leave_encashment.encashable_days * -1)
        ...
        self.assertFalse(frappe.db.exists("Leave Ledger Entry", {
                         'transaction_name': leave_encashment.name}))
