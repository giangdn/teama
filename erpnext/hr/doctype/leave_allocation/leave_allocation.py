...
class LeaveAllocation(Document):
    ...
    def validate_leave_allocation_days(self):
        ...
        max_leaves_allowed = flt(frappe.db.get_value(
            "Leave Type", self.leave_type, "max_leaves_allowed"))
    ...
    def on_submit(self):
        ...
        allocation = get_previous_allocation(
            self.from_date, self.leave_type, self.employee)
    ...
    def validate_lwp(self):
        if frappe.db.get_value("Leave Type", self.leave_type, "is_lwp"):
            frappe.throw(_("Leave Type {0} cannot be allocated since it is leave without pay").format(
                self.leave_type))
    ...
    def validate_new_leaves_allocated_value(self):
        ...
        if flt(self.new_leaves_allocated) % 0.5:
            frappe.throw(
                _("Leaves must be allocated in multiples of 0.5"), ValueMultiplierError)
    ...
    def limit_carry_forward_based_on_max_allowed_leaves(self):
        ...
        if max_leaves_allowed and self.total_leaves_allocated > flt(max_leaves_allowed):
            ...
            self.unused_leaves = max_leaves_allowed - \
    ...
    def validate_total_leaves_allocated(self):
        ...
        if date_difference < self.total_leaves_allocated:
            frappe.throw(
                _("Total allocated leaves are more than days in the period"), OverAllocationError)
...
def validate_carry_forward(leave_type):
    if not frappe.db.get_value("Leave Type", leave_type, "is_carry_forward"):
        frappe.throw(
            _("Leave Type {0} cannot be carry-forwarded").format(leave_type))
