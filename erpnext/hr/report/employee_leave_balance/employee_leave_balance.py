...
def execute(filters=None):
    if filters.to_date <= filters.from_date:
        frappe.throw(
            _('"From date" can not be greater than or equal to "To date"'))
    ...
    for leave_type in leave_types:
        ...
        for employee in active_employees:
            ...
            if (leave_approvers and len(leave_approvers) and user in leave_approvers) or (user in ["Administrator", employee.user_id]) \
                ...
                opening = get_leave_balance_on(employee.name, leave_type, add_days(
                    filters.from_date, -1))  # allocation boundary condition
                ...
                row.closing_balance = (
                    new_allocation + opening - (row.leaves_expired + leaves_taken))
