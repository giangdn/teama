        ...
        for parameter in group_by_parameters:
            ...
            if (att_map_set & emp_map_set):
                ...
                record, emp_att_data = add_data(
                    emp_map[parameter], att_map, filters, holiday_map, conditions, default_holiday_list, leave_list=leave_list)
...
def get_columns(filters):
    ...
    if filters.summarized_view:
        columns += [_("Total Present") + ":Float:120", _("Total Leaves") + ":Float:120",  _("Total Absent") +
                    ":Float:120", _("Total Holidays") + ":Float:120", _("Unmarked Days") + ":Float:120"]
    return columns, days
...
def get_attendance_list(conditions, filters):
    ...
    if not attendance_list:
        msgprint(_("No attendance record found"),
...
def get_conditions(filters):
    ...
    filters["total_days_in_month"] = monthrange(
        cint(filters.year), cint(filters.month))[1]
...
def get_employee_details(group_by, company):
    ...
    if group_by:
        ...
        group_by_parameters = list(set(detail.get(
            group_by, "") for detail in employee_details if detail.get(group_by, "")))
        for parameter in group_by_parameters:
            emp_map[parameter] = {}
