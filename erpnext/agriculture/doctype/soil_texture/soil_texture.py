...
class SoilTexture(Document):
    ...
    def update_soil_edit(self, soil_type):
        self.soil_edit_order[self.soil_types.index(
            soil_type)] = max(self.soil_edit_order)+1
    ...
    def get_soil_type(self):
        ...
        # calculate soil type
        c, sa, si = flt(self.clay_composition), flt(
            self.sand_composition), flt(self.silt_composition)
