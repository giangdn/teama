	...
	refresh: frm => {
		if (!frm.doc.__islocal)
			frm.add_custom_button(__("Reload Linked Analysis"), () =>
				frm.call("reload_linked_analysis")
			);

		frappe.realtime.on("List of Linked Docs", output => {
			let analysis_doctypes = [
				"Soil Texture",
				"Plant Analysis",
				"Soil Analysis"
			];
			let analysis_doctypes_docs = [
				"soil_texture",
				"plant_analysis",
				"soil_analysis"
			];
			...
			output["Location"].forEach(land_doc => {
				analysis_doctypes.forEach(doctype => {
					output[doctype].forEach(analysis_doc => {
						let point_to_be_tested = JSON.parse(
							analysis_doc.location
						).features[0].geometry.coordinates;
						let poly_of_land = JSON.parse(land_doc.location)
							.features[0].geometry.coordinates[0];
						if (is_in_land_unit(point_to_be_tested, poly_of_land)) {
							obj_to_append[
								analysis_doctypes_docs[
									analysis_doctypes.indexOf(doctype)
								]
							].push(analysis_doc.name);
						}
					});
				});
			});
