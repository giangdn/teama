...
class CallLog(Document):
    ...
    def before_insert(self):
        ...
        """
        lead_number = self.get(
            'from') if self.is_incoming_call() else self.get('to')
    ...
    def on_update(self):
        ...
        if _is_call_missed(doc_before_save, self):
            frappe.publish_realtime(
                'call_{id}_missed'.format(id=self.id), self)
            self.trigger_call_popup()
    ...
    def trigger_call_popup(self):
        if self.is_incoming_call():
            scheduled_employees = get_scheduled_employees_for_popup(
                self.medium)
            ...
            if employee_emails and not emails:
                self.add_comment(
                    text=_("No employee was scheduled for call popup"))
...
def add_call_summary(call_log, summary):
    ...
    doc.add_comment('Comment', frappe.bold(
        _('Call Summary')) + '<br><br>' + summary)
